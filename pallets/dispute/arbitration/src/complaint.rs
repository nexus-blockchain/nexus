use crate::pallet::*;
use frame_support::traits::{fungible::MutateHold as FungibleMutateHold, tokens::Precision, Get};
...
impl<T: Config> Pallet<T> {
    pub(crate) fn slash_complaint_deposit(
        ...
        if let Some(deposit_amount) = ComplaintDeposits::<T>::take(complaint_id) {
            let raw_bps = DomainPenaltyRates::<T>::get(domain).unwrap_or_else(|| {
                let type_rate = complaint_type.penalty_rate();
                if type_rate > 0 {
                    type_rate
                } else {
                    T::ComplaintSlashBps::get()
                }
            });
            let slash_bps = raw_bps.min(10_000);
            let slash_amount =
                sp_runtime::Permill::from_parts((slash_bps as u32) * 100).mul_floor(deposit_amount);
            let return_amount = deposit_amount.saturating_sub(slash_amount);

            if !slash_amount.is_zero() {
                if T::Fungible::transfer_on_hold(
                ...
                .is_err()
                {
                    // Slash failed — attempt full release as fallback

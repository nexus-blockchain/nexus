...
impl<T: Config> Pallet<T> {
    pub(crate) fn slash_complaint_deposit(
        ...
        if let Some(deposit_amount) = ComplaintDeposits::<T>::take(complaint_id) {
            let raw_bps = DomainPenaltyRates::<T>::get(domain)
                .unwrap_or_else(|| {
            ...
            if !slash_amount.is_zero() {
                if T::Fungible::transfer_on_hold(
                    ...
                    Self::deposit_event(Event::DepositOperationFailed {
                        complaint_id, operation: 0, amount: slash_amount,
                    });
            ...
            if !return_amount.is_zero() {
                if T::Fungible::release(
                    ...
                    Self::deposit_event(Event::DepositOperationFailed {
                        complaint_id, operation: 1, amount: return_amount,
                    });
                }
            }
        }
    ...
    pub(crate) fn refund_complaint_deposit(complaint_id: u64, complainant: &T::AccountId) {
        if let Some(deposit_amount) = ComplaintDeposits::<T>::take(complaint_id) {
            if T::Fungible::release(
                ...
                Self::deposit_event(Event::DepositOperationFailed {
                    complaint_id, operation: 2, amount: deposit_amount,
                });
            }
        }
    }

...
//! # Dispute escrow pallet.
//! # 争议托管模块。
//!
//! This pallet provides escrowed fund locking, release, refund, dispute marking,
//! and expiry handling for upstream business modules.
//! 本模块为上游业务模块提供托管资金锁定、释放、退款、争议标记与到期处理能力。
...
pub mod pallet {
    ...
    /// Escrow interface for internal pallet-to-pallet use.
    /// 供其他 Pallet 内部调用的托管接口。
    pub trait Escrow<AccountId, Balance> {
        /// Returns the escrow account address.
        /// 返回托管账户地址
        /// Returns the escrow pallet account so external modules can perform deposit operations.
        /// 返回托管 Pallet 的账户地址，用于外部模块进行押金操作
        fn escrow_account() -> AccountId;
        /// Locks funds from the payer into escrow and records the position.
        /// 从付款人转入托管并记录
        ///
        /// ## Safety requirements
        /// ## 安全要求
        /// - the payer must have sufficient balance, otherwise return `Error::Insufficient`
        /// - 必须确保付款人余额充足（不足则返回 `Error::Insufficient`）
        /// - this is for internal pallet calls only; external extrinsics must enforce origin checks
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Escrow pallet `PalletId` used to derive the escrow account.
        /// EscrowPalletId：用于派生托管账户的 `PalletId`
        #[pallet::constant]
        type EscrowPalletId: Get<PalletId>;

        /// Authorized origin for external entrypoints.
        /// 授权外部入口的 Origin（白名单 Origin）
        type AuthorizedOrigin: EnsureOrigin<Self::RuntimeOrigin>;
        /// Admin origin for governance and emergency actions.
        /// 管理员 Origin（治理 / 应急）
        type AdminOrigin: EnsureOrigin<Self::RuntimeOrigin>;
        /// Maximum number of expiring items handled per block as a defensive limit.
        /// 每块最多处理的到期项（防御性限制）
        #[pallet::constant]
        type MaxExpiringPerBlock: Get<u32>;
        /// Maximum number of `release_split` entries, added by the M4 fix to avoid
        /// overweight blocks.
        /// `release_split` 最大分账条目数（M4 修复，用于防止区块超重）
        #[pallet::constant]
        type MaxSplitEntries: Get<u32>;
        /// Expiry policy injected by the runtime.
        /// 到期处理策略，由 runtime 注入
        ...
        /// Escrow state-change observer used to notify business modules.
        /// 托管状态变更观察者（通知业务模块）
    ...
    #[pallet::storage]
    pub type PayerOf<T: Config> = StorageMap<_, Blake2_128Concat, u64, T::AccountId, OptionQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn try_schedule_expiry_at(id: u64, at: BlockNumberFor<T>) -> bool {
            for offset in 0u32..10 {
                ...
                if ExpiringAt::<T>::try_mutate(target, |ids| ids.try_push(id).map_err(|_| ()))
                    .is_ok()
                {
    ...
    impl<T: Config> Escrow<T::AccountId, BalanceOf<T>> for Pallet<T> {
        ...
        fn split_partial(
            ...
            let release_amount =
                sp_runtime::Permill::from_parts((bps as u32) * 100).mul_floor(total);
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn apply_decision_release_all(
            ...
            Self::ensure_auth(origin)?;
                ...
                LockStateOf::<T>::get(id) == STATE_DISPUTED,
                Error::<T>::NotInDispute
            );
            LockStateOf::<T>::insert(id, STATE_LOCKED);
            DisputedAt::<T>::remove(id);
            Self::remove_expiry_schedule(id);
        ...
        pub fn apply_decision_partial_bps(
            ...
            DisputedAt::<T>::remove(id);
            Self::remove_expiry_schedule(id);
            <Self as Escrow<T::AccountId, BalanceOf<T>>>::split_partial(
                id,
                &release_to,
                &refund_to,
                bps,
        ...
        pub fn force_release(origin: OriginFor<T>, id: u64, to: T::AccountId) -> DispatchResult {
            T::AdminOrigin::ensure_origin(origin)?;
            let amount = Locked::<T>::take(id);
            ensure!(!amount.is_zero(), Error::<T>::NoLock);
            let escrow = Self::account();
            ...
            DisputedAt::<T>::remove(id);
            Self::remove_expiry_schedule(id);
            Self::deposit_event(Event::ForceAction {
                id,
                action: 0,
                to: to.clone(),
                amount,
        ...
        pub fn force_refund(origin: OriginFor<T>, id: u64, to: T::AccountId) -> DispatchResult {
            T::AdminOrigin::ensure_origin(origin)?;
            let amount = Locked::<T>::take(id);
            ensure!(!amount.is_zero(), Error::<T>::NoLock);
            let escrow = Self::account();

use crate::mock::*;
use crate::pallet::{self, DisputedAt, Error, ExpiringAt, ExpiryOf, LockStateOf, Locked, PayerOf};
...
fn transfer_from_escrow_blocked_during_dispute() {
    new_test_ext().execute_with(|| {
        assert_ok!(EscrowPallet::lock(RuntimeOrigin::signed(1), 100, 1, 500));
        assert_ok!(EscrowPallet::dispute(
...
fn release_split_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EscrowPallet::release_split(
            RuntimeOrigin::signed(1),
            100,
            entries
        ));
...
fn schedule_and_cancel_expiry() {
    new_test_ext().execute_with(|| {
        assert_ok!(EscrowPallet::lock(RuntimeOrigin::signed(1), 100, 1, 500));
        assert_ok!(EscrowPallet::schedule_expiry(
            RuntimeOrigin::signed(1),
            100,
            10
        ));
...
fn f1_refund_partial_works() {
    new_test_ext().execute_with(|| {
        ...
        // 部分退款 300（trait 调用）
        assert_ok!(<EscrowTrait as Escrow<u64, u128>>::refund_partial(
            100, &1, 300
        ));
...
fn f3_release_partial_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EscrowTrait as Escrow<u64, u128>>::release_partial(
            100, &2, 400
        ));
...
fn f4_set_resolved_clears_timestamp() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EscrowTrait as pallet::Escrow<u64, u128>>::set_resolved(
            100
        ));
...
fn m1_set_disputed_trait_rejects_already_disputed() {
    new_test_ext().execute_with(|| {
        assert_ok!(EscrowPallet::lock(RuntimeOrigin::signed(1), 100, 1, 500));
        assert_ok!(<EscrowTrait as pallet::Escrow<u64, u128>>::set_disputed(
            100
        ));
        assert_noop!(
...
fn m2_split_partial_works_after_resolved() {
    new_test_ext().execute_with(|| {
        ...
        // set_resolved 后可以分账
        assert_ok!(<EscrowTrait as pallet::Escrow<u64, u128>>::split_partial(
            100, &2, &1, 5000
        ));
        assert_eq!(LockStateOf::<Test>::get(100), 3u8);
    });

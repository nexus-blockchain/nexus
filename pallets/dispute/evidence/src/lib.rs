...
// 允许未使用导入，主要用于 trait 方法调用。
#![allow(unused_imports)]

//! # Dispute evidence pallet.
//! # 争议证据模块。
//!
//! This pallet manages on-chain evidence references, content typing, archival summaries,
//! privacy-related metadata, and CID persistence hooks for dispute workflows.
//! 本模块负责争议流程中的链上证据引用、内容类型、归档摘要、隐私相关元数据与 CID 持久化接线。
...
pub mod pallet {
    ...
    use media_utils::{HashHelper, IpfsHelper, MediaError};
    ...
    /// Content type for evidence payloads.
    /// 证据内容类型枚举。
    ///
    /// 函数级中文注释：标识证据的内容类型
    /// - 用于前端渲染和验证
    ...
    /// Evidence record with CID-based content storage.
    /// 共享证据记录结构（CID 化版本）。
    ///
    /// 函数级详细中文注释：
    /// **核心优化**：
    ...
    pub struct Evidence<AccountId, BlockNumber, MaxContentCidLen: Get<u32>, MaxSchemeLen: Get<u32>> {
        /// Unique evidence ID.
        /// 证据唯一 ID
        pub id: u64,
        /// Domain identifier, such as `1=Evidence`, `2=OtcOrder`, `3=General`, etc.
        /// 所属域（如 `1=Evidence`、`2=OtcOrder`、`3=General` 等）
        pub domain: u8,
        /// Target ID, such as the related `subject_id`.
        /// 目标 ID（如 `subject_id`）
        pub target_id: u64,
        /// Evidence owner.
        /// 证据所有者
        ...
        /// Core IPFS content CID.
        /// 核心字段：IPFS 内容 CID
        /// - points to the JSON file stored on IPFS
        /// - 指向 IPFS 上的 JSON 文件
        /// - contains arrays of image / video / document CIDs
        /// - 包含所有图片 / 视频 / 文档的 CID 数组
        /// - stores only a compact CID reference on-chain
        /// - 链上只存储精简 CID 引用
        pub content_cid: BoundedVec<u8, MaxContentCidLen>,

        /// Content type marker.
        /// 内容类型标识
        /// - helps the frontend recognize and render content quickly
        /// - 便于前端快速识别和渲染
        /// - avoids downloading IPFS content just to know its type
        /// - 无需下载 IPFS 内容即可知道类型
        pub content_type: ContentType,

        /// Creation time in block number.
        /// 创建时间（区块号）
        pub created_at: BlockNumber,

        /// Encryption flag.
        /// 加密标识
        /// - `true`: the content referenced by `content_cid` is encrypted
        /// - `true`：`content_cid` 指向的内容已加密
        /// - `false`: the content is public
        /// - `false`：公开内容
        pub is_encrypted: bool,

        /// Optional encryption-scheme descriptor.
        /// 加密方案描述（可选）
        /// - examples: `aes256-gcm`, `xchacha20-poly1305`
        /// - 例如：`aes256-gcm`、`xchacha20-poly1305`
        /// - used to choose the correct algorithm during decryption
        /// - 用于解密时选择正确的算法
    ...
    pub trait Config: frame_system::Config + TypeInfo + core::fmt::Debug {
        ...
        /// Maximum content-CID length. Recommended as 64 bytes to match `MaxCidLen`.
        /// 内容 CID 最大长度（IPFS CID，建议 64 字节，与 `MaxCidLen` 保持一致）
        #[pallet::constant]
        type MaxContentCidLen: Get<u32>;
        /// Maximum encryption-scheme descriptor length. Recommended as 32 bytes.
        /// 加密方案描述最大长度（建议 32 字节）
        #[pallet::constant]
    ...
    /// 用于查询某证据的所有补充证据
    #[pallet::storage]
    pub type EvidenceChildren<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BoundedVec<u64, T::MaxSupplements>, ValueQuery>;
    ...
    #[pallet::storage]
    pub type EvidenceLinkCount<T: Config> = StorageMap<_, Blake2_128Concat, u64, u32, ValueQuery>;
    ...
    /// 每个私密内容的待处理访问请求计数
    #[pallet::storage]
    pub type AccessRequestCount<T: Config> = StorageMap<_, Blake2_128Concat, u64, u32, ValueQuery>;
    ...
    /// 按所有者索引证据（支持"我的证据"查询）
    #[pallet::storage]
    pub type EvidenceByOwner<T: Config> =
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn commit(
            ...
            let content_cid: BoundedVec<u8, T::MaxContentCidLen> = temp_vec
                .try_into()
            ...
            EvidenceCountByNs::<T>::mutate((ns, target_id), |c| {
            ...
            Self::deposit_event(Event::EvidenceDepositReserved {
                id,
                who: who.clone(),
                amount: deposit,
            });
        ...
        pub fn commit_hash(
            ...
            let content_cid: BoundedVec<u8, T::MaxContentCidLen> = temp_vec2
                .try_into()
                .map_err(|_| Error::<T>::InvalidCidFormat)?;
        ...
        pub fn link(origin: OriginFor<T>, domain: u8, target_id: u64, id: u64) -> DispatchResult {
            ...
            let link_cnt = EvidenceLinkCount::<T>::get(id);
                ...
                link_cnt < T::MaxLinksPerEvidence::get(),
                Error::<T>::TooManyLinks
            );
            ensure!(
        ...
        pub fn unlink(origin: OriginFor<T>, domain: u8, target_id: u64, id: u64) -> DispatchResult {
            ...
            let ev_ns = ev.ns.ok_or(Error::<T>::NamespaceMismatch)?;
                ...
                EvidenceStatuses::<T>::get(id) != EvidenceStatus::Sealed,
            ...
            EvidenceLinkCount::<T>::mutate(id, |c| {
        ...
        pub fn store_private_content(
            ...
            T::Currency::reserve(&who, pc_deposit).map_err(|_| Error::<T>::InsufficientDeposit)?;
            ...
            let inner_cid_str =
                core::str::from_utf8(inner_cid_bytes).map_err(|_| Error::<T>::InvalidCidFormat)?;
            IpfsHelper::validate_cid(inner_cid_str).map_err(|_| Error::<T>::InvalidCidFormat)?;
        ...
        pub fn grant_access(
            ...
            PrivateContents::<T>::try_mutate(content_id, |maybe_content| -> DispatchResult {
                ...
                if AccessRequests::<T>::contains_key(content_id, &user) {
                    ...
                    AccessRequestCount::<T>::mutate(content_id, |c| {
                        *c = c.saturating_sub(1);
                    });
                }
        ...
        pub fn revoke_access(
            ...
            PrivateContents::<T>::try_mutate(content_id, |maybe_content| -> DispatchResult {
                ...
                content
                    ...
                    content.encrypted_keys.len() < before_len,
                    Error::<T>::NotFound
                );
                content.updated_at = <frame_system::Pallet<T>>::block_number();
        ...
        pub fn append_evidence(
            ...
            let parent =
                Evidences::<T>::get(parent_id).ok_or(Error::<T>::ParentEvidenceNotFound)?;
            ...
            let parent_status = EvidenceStatuses::<T>::get(parent_id);
                ...
                parent_status != EvidenceStatus::Sealed,
                Error::<T>::EvidenceSealed
            );
            ensure!(
            ...
            EvidenceCountByTarget::<T>::mutate((parent.domain, parent.target_id), |c| {
                *c = c.saturating_add(1);
            });
        ...
        pub fn request_access(origin: OriginFor<T>, content_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            // 验证内容存在
            let content =
                PrivateContents::<T>::get(content_id).ok_or(Error::<T>::PrivateContentNotFound)?;
            ...
            let req_cnt = AccessRequestCount::<T>::get(content_id);
                ...
                req_cnt < T::MaxPendingRequestsPerContent::get(),
                Error::<T>::TooManyAccessRequests
            );
        ...
        pub fn reveal_commitment(
            ...
            let computed =
                Self::compute_evidence_commitment(&ns, ev.target_id, &cid, &salt, version);
            ensure!(computed == commit_hash, Error::<T>::CommitMismatch);

            // 6. CID 格式验证
            let cid_str =
                core::str::from_utf8(cid.as_slice()).map_err(|_| Error::<T>::InvalidCidFormat)?;
            IpfsHelper::validate_cid(cid_str).map_err(|_| Error::<T>::InvalidCidFormat)?;

            // 7. 更新证据：写入真实 content_cid，清除 commit
            let cid_vec: Vec<u8> = cid.clone().into_inner();
            let content_cid: BoundedVec<u8, T::MaxContentCidLen> = cid_vec
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::InvalidCidFormat)?;
        ...
        pub fn force_remove_evidence(
            ...
            if let Some(deposit) = EvidenceDeposits::<T>::take(evidence_id) {
                ...
                Self::deposit_event(Event::EvidenceDepositSlashed {
                    id: evidence_id,
                    who: ev.owner.clone(),
                    amount: deposit.saturating_sub(slashed.1),
                });
            }
        ...
        pub fn withdraw_evidence(origin: OriginFor<T>, evidence_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let ev = Evidences::<T>::get(evidence_id).ok_or(Error::<T>::NotFound)?;
            ensure!(ev.owner == who, Error::<T>::NotAuthorized);
        ...
        pub fn delete_private_content(origin: OriginFor<T>, content_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn revoke_public_key(origin: OriginFor<T>) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn cancel_access_request(origin: OriginFor<T>, content_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
    ...
    impl<T: Config> pallet_crypto_common::PrivateContentProvider<T::AccountId> for Pallet<T> {
        ...
        fn get_encrypted_key(content_id: u64, user: &T::AccountId) -> Option<Vec<u8>> {
            Self::get_encrypted_key_for_user(content_id, user).map(|k| k.into_inner())
        }
        ...
        fn get_decryption_info(
            ...
            Vec<u8>,
            sp_core::H256,
            pallet_crypto_common::EncryptionMethod,
            Vec<u8>,
        )> {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn can_access_private_content(content_id: u64, user: &T::AccountId) -> bool {
            if let Some(content) = PrivateContents::<T>::get(content_id) {
                ...
                if content
                    .access_policy
                    .is_authorized(user, &content.creator, &now)
                {
                    return true;
                }
            ...
            while archived_count < max_count && cursor < max_id {
                if let Some(evidence) = Evidences::<T>::get(cursor) {
                    ...
                    if now.saturating_sub(created_at) >= archive_delay {
                        // 计算内容哈希
                        let content_hash =
                            H256::from(blake2_256(&evidence.content_cid.clone().into_inner()));
                        ...
                        // 🆕 VM2修复: 清理二级索引（防止存储泄漏）
                        EvidenceByTarget::<T>::remove(
                            (evidence.domain, evidence.target_id),
                            cursor,
                        );
        ...
        fn cleanup_expired_access_requests(now: BlockNumberFor<T>, max_per_call: u32) -> u32 {
            ...
            while cursor < max_content_id && cleaned < max_per_call && scanned < scan_limit {
                ...
                for requester in to_remove {
                    ...
                    AccessRequestCount::<T>::mutate(cursor, |c| {
                        *c = c.saturating_sub(1);
                    });
                    Self::deposit_event(Event::AccessRequestExpired {
                        content_id: cursor,
                        requester,
                    });
                    cleaned += 1;
                    if cleaned >= max_per_call {
                        break;
                    }
                }
                cursor = cursor.saturating_add(1);
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_idle(now: BlockNumberFor<T>, remaining_weight: Weight) -> Weight {
            ...
            if remaining_weight.ref_time()
                > scan_overhead
                    .ref_time()
                    .saturating_add(base_weight.ref_time())
            {
            ...
            if remaining.ref_time() > base_weight.ref_time() * 5 {
                ...
                weight_used =
                    weight_used.saturating_add(base_weight.saturating_mul(cleaned as u64));
            }
...
impl<T: pallet::Config> Pallet<T> {
    ...
    fn build_summary(
        ...
        >,
        id: u64,
    ) -> runtime_api::EvidenceSummary<T::AccountId> {
    ...
    pub fn api_get_evidence_detail(
        id: u64,
    ) -> Option<runtime_api::EvidenceDetail<T::AccountId, pallet::BalanceOf<T>>> {
    ...
    pub fn api_get_evidences_by_target(
        domain: u8,
        target_id: u64,
        offset: u32,
        limit: u32,
    ...
    pub fn api_get_evidences_by_ns(
        ns: [u8; 8],
        subject_id: u64,
        offset: u32,
        limit: u32,
    ...
    pub fn api_get_private_content_meta(
        content_id: u64,
    ) -> Option<runtime_api::PrivateContentMeta<T::AccountId>> {
        use sp_runtime::traits::SaturatedConversion;
    ...
    pub fn api_get_decryption_package(
        content_id: u64,
        viewer: &T::AccountId,
    ) -> Option<runtime_api::DecryptionPackage> {
    ...
    pub fn api_get_private_contents_by_subject(
        ns: [u8; 8],
        subject_id: u64,
        offset: u32,
        limit: u32,
    ...
    pub fn api_get_access_requests(
        content_id: u64,
        offset: u32,
        limit: u32,
    ) -> alloc::vec::Vec<runtime_api::AccessRequestEntry<T::AccountId>> {
        use sp_runtime::traits::SaturatedConversion;
        let cap = core::cmp::min(limit, 50);
        let mut out: alloc::vec::Vec<runtime_api::AccessRequestEntry<T::AccountId>> =
            alloc::vec::Vec::new();
        let mut skipped = 0u32;
        let mut collected = 0u32;

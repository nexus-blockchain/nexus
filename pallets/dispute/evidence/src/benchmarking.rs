...
mod benches {
    ...
    fn commit() {
        ...
        let ns = T::EvidenceNsBytes::get();
        let imgs: BoundedVec<BoundedVec<u8, T::MaxCidLen>, T::MaxImg> = BoundedVec::try_from(vec![
            make_cid_bounded::<T::MaxCidLen>(1),
            make_cid_bounded::<T::MaxCidLen>(2),
        ])
        .unwrap();
    ...
    fn unseal_evidence() {
        ...
        Pallet::<T>::seal_evidence(RawOrigin::Signed(caller.clone()).into(), parent_id, None)
            .unwrap();
        #[extrinsic_call]
        _(RawOrigin::Signed(caller), parent_id, None);
    }
    ...
    fn reveal_commitment() {
        ...
        let ns = T::EvidenceNsBytes::get();
        let cid: BoundedVec<u8, T::MaxCidLen> =
            BoundedVec::try_from(Vec::from("bafkreiaaaaaaaaaa000001".as_bytes())).unwrap();
        let salt: BoundedVec<u8, T::MaxMemoLen> =
            BoundedVec::try_from(Vec::from("benchsalt".as_bytes())).unwrap();
        let version = 1u32;
        let evidence_commit = Pallet::<T>::compute_evidence_commitment(
            &ns,
            1,
            cid.as_slice(),
            salt.as_slice(),
    ...
    fn commit_v2() {
        let caller: T::AccountId = whitelisted_caller();
        let ns = T::EvidenceNsBytes::get();
        let content_cid: BoundedVec<u8, T::MaxContentCidLen> =
            BoundedVec::try_from(Vec::from("bafkreiaaaaaaaaaa000001".as_bytes())).unwrap();
        #[extrinsic_call]
        _(
            RawOrigin::Signed(caller),
            ...
            1u64,
            content_cid,
            crate::pallet::ContentType::Document,
        );
    }
    ...
    fn rotate_content_keys() {
        ...
        > = BoundedVec::try_from(vec![(
            caller.clone(),
            BoundedVec::try_from(vec![0xAAu8; 32]).unwrap(),
        )])
        .unwrap();
        #[extrinsic_call]
        _(
    ...
    fn cancel_access_request() {
        ...
        .unwrap();
        Pallet::<T>::request_access(RawOrigin::Signed(requester.clone()).into(), content_id)
            .unwrap();
        #[extrinsic_call]
        _(RawOrigin::Signed(requester), content_id);
    }

...
fn imgs_n(
    indices: &[u32],
) -> BoundedVec<BoundedVec<u8, <Test as Config>::MaxCidLen>, <Test as Config>::MaxImg> {
...
fn vids_n(
    indices: &[u32],
) -> BoundedVec<BoundedVec<u8, <Test as Config>::MaxCidLen>, <Test as Config>::MaxVid> {
    let v: Vec<_> = indices.iter().map(|i| make_cid(*i)).collect();
    BoundedVec::truncate_from(v)
}
...
fn docs_n(
    indices: &[u32],
) -> BoundedVec<BoundedVec<u8, <Test as Config>::MaxCidLen>, <Test as Config>::MaxDoc> {
    let v: Vec<_> = indices.iter().map(|i| make_cid(*i)).collect();
    BoundedVec::truncate_from(v)
}
...
fn commit_sets_content_type_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Evidences::<Test>::get(0).unwrap().content_type,
            ContentType::Image
        );
        // Only videos → Video
        assert_ok!(EvidencePallet::commit(
        ...
        assert_eq!(
            Evidences::<Test>::get(1).unwrap().content_type,
            ContentType::Video
        );
        // Only docs → Document
        assert_ok!(EvidencePallet::commit(
        ...
        assert_eq!(
            Evidences::<Test>::get(2).unwrap().content_type,
        ...
        assert_eq!(
            Evidences::<Test>::get(3).unwrap().content_type,
            ContentType::Mixed
        );
    });
}
...
fn link_by_ns_and_unlink_by_ns_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EvidencePallet::link_by_ns(
            RuntimeOrigin::signed(1),
            NS,
            200,
            0
        ...
        assert_ok!(EvidencePallet::unlink_by_ns(
            RuntimeOrigin::signed(1),
            NS,
            200,
            0
...
fn store_test_private_content() -> u64 {
    ...
    > = BoundedVec::truncate_from(vec![(1u64, BoundedVec::truncate_from(vec![0xFFu8; 32]))]);
...
fn make_encrypted_keys(
    creator: u64,
) -> BoundedVec<
    (u64, BoundedVec<u8, <Test as Config>::MaxKeyLen>),
    <Test as Config>::MaxAuthorizedUsers,
> {
    BoundedVec::truncate_from(vec![(creator, BoundedVec::truncate_from(vec![0xFFu8; 32]))])
}
...
fn store_private_content_works() {
    new_test_ext().execute_with(|| {
        ...
        assert!(PrivateContentBySubject::<Test>::contains_key(
            (NS, 500u64),
            0u64
        ));
...
fn store_private_content_rejects_missing_creator_key() {
    new_test_ext().execute_with(|| {
        ...
        let wrong_keys: BoundedVec<
            (u64, BoundedVec<u8, <Test as Config>::MaxKeyLen>),
            <Test as Config>::MaxAuthorizedUsers,
        > = BoundedVec::truncate_from(vec![(2u64, BoundedVec::truncate_from(vec![0xFFu8; 32]))]);
        assert_noop!(
            EvidencePallet::store_private_content(
                RuntimeOrigin::signed(1),
                NS,
...
fn cancel_access_request_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(AccessRequestCount::<Test>::get(content_id), 1);
        assert_ok!(EvidencePallet::cancel_access_request(
            RuntimeOrigin::signed(2),
            content_id
        ));
        assert!(AccessRequests::<Test>::get(content_id, 2u64).is_none());
...
fn update_access_policy_works() {
    new_test_ext().execute_with(|| {
        let content_id = store_test_private_content();
        let new_policy =
            pallet_crypto_common::AccessPolicy::SharedWith(BoundedVec::truncate_from(vec![
                2u64, 3u64,
            ]));
...
fn private_content_provider_can_access() {
    new_test_ext().execute_with(|| {
        let content_id = store_test_private_content();
        assert!(
            <EvidencePallet as pallet_crypto_common::PrivateContentProvider<u64>>::can_access(
                content_id, &1
            )
        );
        assert!(
...
fn delete_private_content_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EvidencePallet::delete_private_content(
            RuntimeOrigin::signed(1),
            content_id
        ));
...
fn rotate_content_keys_works() {
    new_test_ext().execute_with(|| {
        ...
        let new_keys: BoundedVec<
            (u64, BoundedVec<u8, frame_support::traits::ConstU32<512>>),
            <Test as Config>::MaxAuthorizedUsers,
        > = BoundedVec::truncate_from(vec![(1u64, BoundedVec::truncate_from(vec![0xAAu8; 32]))]);
...
fn seal_and_unseal_evidence_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EvidencePallet::seal_evidence(
            RuntimeOrigin::signed(1),
            0,
            None
        ));
        ...
        assert_ok!(EvidencePallet::unseal_evidence(
            RuntimeOrigin::signed(1),
            0,
            None
        ));
...
fn withdraw_evidence_refunds_deposit_and_cleans_indexes() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EvidencePallet::withdraw_evidence(
            RuntimeOrigin::signed(1),
            0
        ));
...
fn archive_skips_sealed_evidence() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            Evidences::<Test>::get(0).is_some(),
            "sealed evidence must not be archived"
        );
...
fn rotate_content_keys_rejects_missing_creator_key() {
    new_test_ext().execute_with(|| {
        ...
        let new_keys: BoundedVec<
            (u64, BoundedVec<u8, frame_support::traits::ConstU32<512>>),
            <Test as Config>::MaxAuthorizedUsers,
        > = BoundedVec::truncate_from(vec![(2u64, BoundedVec::truncate_from(vec![0xAAu8; 32]))]);
        assert_noop!(
...
fn reveal_rejects_wrong_hash() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EvidencePallet::reveal_commitment(
                RuntimeOrigin::signed(1),
                0,
                wrong_cid,
                salt,

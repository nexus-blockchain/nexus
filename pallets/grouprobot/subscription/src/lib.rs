...
//! # Pallet GroupRobot Subscription — subscription management, escrow balances,
//! # and tier-fee settlement.
//! # Pallet GroupRobot Subscription — 订阅管理、Escrow 余额与层级费用结算。
//!
//! Split out from the consensus pallet and now responsible for:
//! 从 consensus pallet 拆分而来，负责：
//! - subscription CRUD (`subscribe` / `deposit` / `cancel` / `change_tier`)
//! - 订阅 CRUD（`subscribe` / `deposit` / `cancel` / `change_tier`）
//! - escrowed prepaid-balance management
//! - Escrow 预存余额管理
//! - Era-based subscription-fee settlement with cursor pagination
//! - Era 订阅费结算（游标分页）
//! - implementations of `SubscriptionProvider` and `SubscriptionSettler`
...
/// Ad-commitment subscription record.
/// 广告承诺订阅记录。
#[derive(
    Encode,
    Decode,
...
pub mod pallet {
    ...
    use frame_support::traits::{
        Currency, ExistenceRequirement, ReservableCurrency, StorageVersion,
    };
    ...
    pub enum Event<T: Config> {
        ...
        /// 取消订阅时按比例扣除当期费用
        SubscriptionCancelledWithProration {
            bot_id_hash: BotIdHash,
            prorated_fee: BalanceOf<T>,
        ...
        /// 广告承诺订阅已取消
        AdCommitmentCancelled {
            bot_id_hash: BotIdHash,
        },
        /// 广告承诺达标检查通过
        AdCommitmentFulfilled {
            bot_id_hash: BotIdHash,
            delivered: u32,
            committed: u32,
        },
        /// 广告承诺未达标
        AdCommitmentUnderdelivered {
            bot_id_hash: BotIdHash,
            delivered: u32,
            committed: u32,
            consecutive: u8,
        ...
        /// Root 强制取消订阅
        SubscriptionForceCancelled {
            bot_id_hash: BotIdHash,
            escrow_slashed: BalanceOf<T>,
        },
        /// Root 强制暂停订阅
        SubscriptionForceSuspended {
            bot_id_hash: BotIdHash,
        },
        /// Root 强制变更层级
        TierForceChanged {
        ...
        /// 批量清理已取消记录
        BatchCleaned {
            subscriptions_cleaned: u32,
            ad_commitments_cleaned: u32,
        },
        /// Root 强制取消广告承诺
        AdCommitmentForceCancelled {
            bot_id_hash: BotIdHash,
        },
        ...
        EscrowLow {
            bot_id_hash: BotIdHash,
            remaining: BalanceOf<T>,
            fee_per_era: BalanceOf<T>,
        },
        /// settle 跳过非活跃 Bot
        SettleSkippedInactiveBot {
            bot_id_hash: BotIdHash,
        },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn subscribe(
            ...
            let who = ensure_signed(origin)?;
                ...
                T::BotRegistry::bot_operator(&bot_id_hash).is_some(),
                Error::<T>::BotHasNoOperator
            );
        ...
        pub fn deposit_subscription(
            ...
            let sub =
                Subscriptions::<T>::get(&bot_id_hash).ok_or(Error::<T>::SubscriptionNotFound)?;
            ensure!(
        ...
        pub fn cancel_subscription(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let fee_per_era = Subscriptions::<T>::try_mutate(
                &bot_id_hash,
                |maybe_sub| -> Result<BalanceOf<T>, sp_runtime::DispatchError> {
        ...
        pub fn commit_ads(
            ...
            let who = ensure_signed(origin)?;
                ...
                !AdCommitments::<T>::contains_key(&bot_id_hash),
                Error::<T>::AdCommitmentAlreadyExists
            );
        ...
        pub fn cleanup_subscription(
            ...
            let sub =
                ...
                sub.status == SubscriptionStatus::Cancelled,
                Error::<T>::SubscriptionNotTerminal
            );
        ...
        pub fn cleanup_ad_commitment(
            ...
            ensure_signed(origin)?;
            let record =
                AdCommitments::<T>::get(&bot_id_hash).ok_or(Error::<T>::AdCommitmentNotFound)?;
            ensure!(
                record.status == AdCommitmentStatus::Cancelled,
                Error::<T>::AdCommitmentNotTerminal
            );
        ...
        pub fn force_cancel_subscription(
            ...
            Self::deposit_event(Event::SubscriptionForceCancelled {
                bot_id_hash,
                escrow_slashed: escrow,
            });
            Ok(())
        }
        ...
        pub fn withdraw_escrow(
            ...
            let remaining = escrow.saturating_sub(amount);
                ...
                remaining >= sub.fee_per_era,
                Error::<T>::WithdrawWouldUnderfund
            );

            T::Currency::unreserve(&who, amount);
            SubscriptionEscrow::<T>::insert(&bot_id_hash, remaining);
            Self::deposit_event(Event::EscrowWithdrawn {
                bot_id_hash,
                amount,
                remaining,
            });
        ...
        pub fn update_ad_commitment(
            ...
            AdCommitments::<T>::try_mutate(&bot_id_hash, |maybe| -> DispatchResult {
                let record = maybe.as_mut().ok_or(Error::<T>::AdCommitmentNotFound)?;
                    ...
                    matches!(
                        record.status,
                        AdCommitmentStatus::Active | AdCommitmentStatus::Underdelivery
                    ),
                    Error::<T>::AdCommitmentAlreadyCancelled
                );
                ...
                // 至少 ads 数量或社区有变更
                let community_changed =
                    new_community_id_hash.map_or(false, |c| c != record.community_id_hash);
                ensure!(
        ...
        pub fn operator_deposit_subscription(
            ...
            Self::deposit_event(Event::OperatorDeposited {
                bot_id_hash,
                operator: who,
                amount,
            });
        ...
        pub fn force_change_tier(
            ...
            Subscriptions::<T>::try_mutate(&bot_id_hash, |maybe| -> DispatchResult {
                ...
                sub.tier = new_tier;
                sub.fee_per_era = Self::tier_fee(&new_tier);
                Self::deposit_event(Event::TierForceChanged {
                    bot_id_hash,
                    old_tier,
                    new_tier,
                });
        ...
        pub fn pause_subscription(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Subscriptions::<T>::try_mutate(&bot_id_hash, |maybe| -> DispatchResult {
                let sub = maybe.as_mut().ok_or(Error::<T>::SubscriptionNotFound)?;
                ensure!(sub.owner == who, Error::<T>::NotBotOwner);
        ...
        pub fn resume_subscription(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Subscriptions::<T>::try_mutate(&bot_id_hash, |maybe| -> DispatchResult {
                let sub = maybe.as_mut().ok_or(Error::<T>::SubscriptionNotFound)?;
                ensure!(sub.owner == who, Error::<T>::NotBotOwner);
        ...
        pub fn batch_cleanup(
            origin: OriginFor<T>,
            subscription_ids: sp_runtime::BoundedVec<
                BotIdHash,
                frame_support::traits::ConstU32<50>,
            >,
            ad_commitment_ids: sp_runtime::BoundedVec<
                BotIdHash,
                frame_support::traits::ConstU32<50>,
            >,
        ) -> DispatchResult {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn settle_era_subscriptions() -> (BalanceOf<T>, BalanceOf<T>) {
            ...
            let iter = match SubscriptionSettleCursor::<T>::get() {
                Some(cursor) => {
                    Subscriptions::<T>::iter_from(Subscriptions::<T>::hashed_key_for(&cursor))
                }
            ...
            for (bot_hash, sub) in iter {
                ...
                if !T::BotRegistry::is_bot_active(&sub.bot_id_hash) {
                    Self::deposit_event(Event::SettleSkippedInactiveBot {
                    ...
                    Subscriptions::<T>::mutate(&bot_hash_inner, |maybe_sub| {
                        if let Some(s) = maybe_sub {
                            if s.status == SubscriptionStatus::Active {
                                ...
                                Self::deposit_event(Event::FreeTierFallback {
                                    bot_id_hash: bot_hash_inner,
                                });
                            }
                        }
                    });
        ...
        pub fn settle_ad_commitments() {
            ...
            let iter = match AdCommitmentSettleCursor::<T>::get() {
                Some(cursor) => {
                    AdCommitments::<T>::iter_from(AdCommitments::<T>::hashed_key_for(&cursor))
                }

...
fn deposit_subscription_fails_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::deposit_subscription(RuntimeOrigin::signed(OTHER), bot_hash(1), 20,),
...
fn cancel_subscription_fails_already_cancelled() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::cancel_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1),),
...
fn cancel_ad_commitment_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Subscription::cancel_ad_commitment(RuntimeOrigin::signed(OWNER), bot_hash(1),),
...
fn h4_deposit_subscription_rejects_zero_amount() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::deposit_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1), 0,),
...
fn h1_no_event_on_transfer_failure() {
    new_test_ext().execute_with(|| {
        ...
        let free =
            <<Test as crate::pallet::Config>::Currency as CurrencyT<u64>>::free_balance(&OWNER);
        let _ = <<Test as crate::pallet::Config>::Currency as CurrencyT<u64>>::transfer(
            &OWNER,
            &OTHER,
            free,
        ...
        let events: Vec<_> = System::events()
            ...
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::Subscription(Event::SubscriptionFeeCollected { .. })
                )
            })
            .collect();
...
fn m2_r3_cleanup_subscription_rejects_active() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::cleanup_subscription(RuntimeOrigin::signed(OTHER), bot_hash(1),),
...
fn m2_r3_cleanup_ad_commitment_rejects_active() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::cleanup_ad_commitment(RuntimeOrigin::signed(OTHER), bot_hash(1),),
...
fn force_cancel_subscription_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::force_cancel_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1),),
...
fn force_cancel_subscription_fails_already_cancelled() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::force_cancel_subscription(RuntimeOrigin::root(), bot_hash(1),),
            Error::<Test>::SubscriptionAlreadyCancelled
        );
    });
}
...
fn withdraw_escrow_fails_would_underfund() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::withdraw_escrow(RuntimeOrigin::signed(OWNER), bot_hash(1), 45,),
...
fn withdraw_escrow_fails_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::withdraw_escrow(RuntimeOrigin::signed(OTHER), bot_hash(1), 10,),
            Error::<Test>::NotBotOwner
        );
    });
}
...
fn withdraw_escrow_fails_zero_amount() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::withdraw_escrow(RuntimeOrigin::signed(OWNER), bot_hash(1), 0,),
            Error::<Test>::ZeroDepositAmount
        );
    });
}
...
fn update_ad_commitment_fails_same() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::update_ad_commitment(RuntimeOrigin::signed(OWNER), bot_hash(1), 5, None,),
...
fn force_suspend_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::force_suspend_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1),),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn force_suspend_fails_already_cancelled() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::force_suspend_subscription(RuntimeOrigin::root(), bot_hash(1),),
            Error::<Test>::SubscriptionAlreadyCancelled
        );
    });
}
...
fn pause_subscription_fails_not_active() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::pause_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1),),
            Error::<Test>::SubscriptionNotActive
        );
    });
}
...
fn resume_fails_not_paused() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::resume_subscription(RuntimeOrigin::signed(OWNER), bot_hash(1),),
...
fn batch_cleanup_works() {
    new_test_ext().execute_with(|| {
        ...
        let cancelled_ad = AdCommitmentRecord::<Test> {
            owner: OWNER,
...
fn batch_cleanup_fails_empty() {
    new_test_ext().execute_with(|| {
        ...
        let empty_ad: sp_runtime::BoundedVec<_, frame_support::traits::ConstU32<50>> =
            vec![].try_into().unwrap();
        assert_noop!(
            Subscription::batch_cleanup(RuntimeOrigin::signed(OTHER), empty_sub, empty_ad,),
...
fn update_tier_fee_fails_free() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Subscription::update_tier_fee(RuntimeOrigin::root(), SubscriptionTier::Free, 5,),
            Error::<Test>::CannotSubscribeFree
        );
    });
}
...
fn force_cancel_ad_commitment_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Subscription::force_cancel_ad_commitment(RuntimeOrigin::signed(OWNER), bot_hash(1),),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn escrow_low_event_emitted_during_settle() {
    new_test_ext().execute_with(|| {
        ...
        let low_events: Vec<_> = System::events()
            .into_iter()
...
fn settle_skips_inactive_bot() {
    new_test_ext().execute_with(|| {
        ...
        System::reset_events();
        let (_, _) = Subscription::settle_era_subscriptions();

        // 不扣费
        assert_eq!(SubscriptionEscrow::<Test>::get(bot_hash(99)), 50);
        // 发射 SettleSkippedInactiveBot 事件
        let skip_events: Vec<_> = System::events()
            .into_iter()
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::Subscription(Event::SettleSkippedInactiveBot { .. })
                )
            })
            .collect();

...
fn claim_rewards_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Rewards::claim_rewards(
...
fn distribute_and_record_era_works() {
    new_test_ext().execute_with(|| {
        ...
        let distributed = Rewards::distribute_and_record_era(
            0, 1000u128, 800u128, 0u128, 0u128, 100u128, &weights, 2,
        );
        ...
        // No minting — pool balance unchanged (rewards are accrued, not transferred here)
        ...
        assert_eq!(info.inflation_mint, 0);
...
fn m1_r2_distribute_emits_reward_accrued_per_node() {
    new_test_ext().execute_with(|| {
        System::set_block_number(1);
        let weights = vec![(node_id(1), 100u128), (node_id(2), 100u128)];
        Rewards::distribute_and_record_era(
        ...
        assert!(events.contains(&Event::<Test>::RewardAccrued {
            node_id: node_id(1),
            amount: 500
        }));
...
fn m2_r2_rescue_rejects_active_node() {
    new_test_ext().execute_with(|| {
        ...
        NodePendingRewards::<Test>::insert(node_id(1), 500u128);
        assert_noop!(
            Rewards::rescue_stranded_rewards(RuntimeOrigin::root(), node_id(1), OPERATOR),
...
fn m2_r2_rescue_rejects_no_pending() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Rewards::rescue_stranded_rewards(RuntimeOrigin::root(), node_id(99), OPERATOR),
            Error::<Test>::NoPendingRewards
        );
    });
}
...
fn distribute_empty_weights_zero_distributed() {
    new_test_ext().execute_with(|| {
        ...
        let distributed = Rewards::distribute_and_record_era(
            0, 500u128, 400u128, 0u128, 0u128, 50u128, &weights, 0,
        );
        assert_eq!(distributed, 0);
        // No minting — pool balance unchanged
        assert_eq!(Balances::free_balance(REWARD_POOL), pool_before);
...
fn accrue_zero_amount_is_noop() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(NodePendingRewards::<Test>::get(node_id(1)), 0);
        // 零金额不应发出事件
        let reward_events: Vec<_> = System::events()
            .into_iter()
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::Rewards(Event::<Test>::RewardAccrued { .. })
                )
            })
            .collect();
...
fn clear_reward_recipient_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(
            Event::<Test>::RewardRecipientCleared {
                node_id: node_id(1),
            }
            .into(),
        );
...
fn set_reward_recipient_not_operator_fails() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Rewards::set_reward_recipient(RuntimeOrigin::signed(OTHER), node_id(1), Some(OTHER)),
            Error::<Test>::NotOperator
        );
    });
}
...
fn force_slash_pending_rewards_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(NodePendingRewards::<Test>::get(node_id(1)), 300);
        System::assert_last_event(
            Event::<Test>::PendingRewardsSlashed {
                node_id: node_id(1),
                amount: 200,
            }
            .into(),
...
fn force_prune_era_rewards_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_last_event(
            Event::<Test>::EraRewardsForcePruned {
                from_era: 0,
                to_era: 3,
            }
            .into(),
...
fn force_prune_capped_at_max() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Rewards::force_prune_era_rewards(
            RuntimeOrigin::root(),
            total
        ));
...
fn bind_unbind_node_bot_split_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            NodeBotSplitBinding::<Test>::get(node_id(1)),
            Some(bot_hash(10))
        );

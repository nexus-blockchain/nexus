...
//! # Pallet GroupRobot Rewards — node reward accrual, claiming, and Era reward records.
//! # Pallet GroupRobot Rewards — 节点奖励累积、领取与 Era 奖励记录。
//!
//! Split from the consensus pallet and now responsible for:
//! 从 consensus pallet 拆分而来，负责：
//! - pending node rewards (`NodePendingRewards`)
//! - 节点待领取奖励（`NodePendingRewards`）
//! - cumulative claimed rewards (`NodeTotalEarned`)
//! - 节点累计已领取（`NodeTotalEarned`）
//! - Era reward records (`EraRewards`)
//! - Era 奖励记录（`EraRewards`）
//! - owner-side reward splitting (`RewardSplit` + `OwnerPendingRewards`)
//! - Owner 分成（`RewardSplit` + `OwnerPendingRewards`）
//! - implementations of `RewardAccruer` and `EraRewardDistributor`
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type RewardRecipient<T: Config> = StorageMap<_, Blake2_128Concat, NodeId, T::AccountId>;
    ...
    pub enum Event<T: Config> {
        RewardsClaimed {
            ...
            amount: BalanceOf<T>,
            total_earned_after: BalanceOf<T>,
        },
        EraCompleted {
            era: u64,
            total_distributed: BalanceOf<T>,
            era_info: EraRewardInfo<BalanceOf<T>>,
        },
        ...
        /// 节点退出时残留奖励已自动领取
        OrphanRewardsClaimed {
            node_id: NodeId,
            operator: T::AccountId,
            amount: BalanceOf<T>,
        },
        /// M2-R3: 孤儿奖励领取失败 (奖励池不足, 需 Root rescue)
        OrphanRewardClaimFailed {
            node_id: NodeId,
            amount: BalanceOf<T>,
        },
        /// 批量领取完成
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn integrity_test() {
            assert!(T::MaxEraHistory::get() > 0, "MaxEraHistory must be > 0");
            assert!(T::MaxBatchClaim::get() > 0, "MaxBatchClaim must be > 0");
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn claim_rewards(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ...
            let operator =
                T::NodeConsensus::node_operator(&node_id).ok_or(Error::<T>::NodeNotFound)?;
        ...
        pub fn set_reward_split(
            ...
            let who = ensure_signed(origin)?;
            let owner = T::BotRegistry::bot_owner(&bot_id_hash).ok_or(Error::<T>::NotBotOwner)?;
            ...
            Self::deposit_event(Event::RewardSplitSet {
                bot_id_hash,
                owner_bps,
            });
            Ok(())
        ...
        pub fn claim_owner_rewards(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let owner = T::BotRegistry::bot_owner(&bot_id_hash).ok_or(Error::<T>::NotBotOwner)?;
            ensure!(owner == who, Error::<T>::NotBotOwner);
        ...
        pub fn pause_distribution(origin: OriginFor<T>) -> DispatchResult {
            ensure_root(origin)?;
                ...
                !DistributionPaused::<T>::get(),
                Error::<T>::DistributionIsPaused
            );
        ...
        pub fn force_prune_era_rewards(origin: OriginFor<T>, target_era: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            Self::deposit_event(Event::EraRewardsForcePruned {
                from_era: from,
                to_era: cursor,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn distribute_and_record_era(
            ...
            // No minting — rewards are distributed solely from the RewardPool's
            // existing balance (funded by subscription fees, ads income, etc.).
            // 不铸币 — 奖励仅从 RewardPool 已有余额中分配（来源：订阅费、广告收入等）。
        ...
        fn accrue_with_split(node_id: &NodeId, amount: BalanceOf<T>) {
            let split = NodeBotSplitBinding::<T>::get(node_id).and_then(|bot_hash| {
    ...
    impl<T: Config> EraRewardDistributor for Pallet<T> {
        ...
        fn reward_pool_balance() -> u128 {
            Self::reward_pool_balance().unique_saturated_into()
        }
    }

    /// 节点→Bot 分成绑定 (用于 Era 分配时查找 owner split)
    #[pallet::storage]
    pub type NodeBotSplitBinding<T: Config> = StorageMap<_, Blake2_128Concat, NodeId, BotIdHash>;
}

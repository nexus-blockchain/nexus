...
//! # Pallet GroupRobot Registry — bot registration, TEE node management, platform
//! # identity binding, and attestation management.
//! # Pallet GroupRobot Registry — Bot 注册、TEE 节点管理、平台身份绑定与证明管理。
//!
//! This pallet consolidates the on-chain logic previously spread across
//! `pallet-bot-registry` and `attestation.rs`.
//! 整合现有 `pallet-bot-registry` + `attestation.rs` 相关链上逻辑。
//!
//! ## Features
//! ## 功能
//! - bot registration (`bot_id_hash`, `public_key`)
//! - Bot 注册（`bot_id_hash`、`public_key`）
//! - public-key rotation
//! - 公钥更换（密钥轮换）
//! - community binding / unbinding
//! - 社区绑定 / 解绑
//! - user platform-identity binding
//! - 用户平台身份绑定
//! - TEE dual-attestation submission / refresh (`TDX + SGX`)
//! - TEE 双证明提交 / 刷新（`TDX + SGX`）
//! - `MRTD` / `MRENCLAVE` whitelist management
//! - `MRTD` / `MRENCLAVE` 白名单管理
//! - `on_initialize`: automatic downgrade for expired attestations
...
/// Peer endpoint info used for node discovery.
/// Peer 端点信息（用于节点发现）。
#[derive(
    Encode,
    Decode,
...
/// Unified TEE attestation record (V2).
/// TEE 证明记录 V2（三模式统一）。
#[derive(
    Encode,
    Decode,
...
pub mod pallet {
    ...
    pub type OwnerBots<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        T::AccountId,
        BoundedVec<BotIdHash, T::MaxBotsPerOwner>,
        ValueQuery,
    >;
    ...
    #[pallet::storage]
    pub type UserPlatformBindings<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, T::AccountId, Blake2_128Concat, Platform, [u8; 32]>;
    ...
    #[pallet::storage]
    pub type ApprovedApiServerMrtd<T: Config> = StorageMap<_, Blake2_128Concat, [u8; 48], u32>;
    ...
    pub type PeerRegistry<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        BotIdHash,
        BoundedVec<PeerEndpoint<T>, T::MaxPeersPerBot>,
        ValueQuery,
    >;
    ...
    pub type AttestationExpiryQueue<T: Config> = StorageValue<
        _,
        BoundedVec<(BlockNumberFor<T>, BotIdHash, bool), ConstU32<1000>>,
        ValueQuery,
    >;
    ...
    pub type OperatorBots<T: Config> = StorageDoubleMap<
        ...
        Blake2_128Concat,
        Platform,
        BoundedVec<BotIdHash, T::MaxBotsPerOperator>,
        ValueQuery,
    >;
    ...
    #[pallet::storage]
    pub type PlatformAppHashIndex<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, Platform, Blake2_128Concat, [u8; 32], T::AccountId>;
    ...
    pub type PeerEraUptime<T: Config> = StorageDoubleMap<
        ...
        Blake2_128Concat,
        [u8; 32],
        BoundedVec<(u64, u32), T::MaxUptimeEraHistory>,
        ValueQuery,
    >;
    ...
    pub enum Event<T: Config> {
        ...
        BotAssignedToOperator {
            bot_id_hash: BotIdHash,
            operator: T::AccountId,
            platform: Platform,
        },
        BotUnassignedFromOperator {
            bot_id_hash: BotIdHash,
            operator: T::AccountId,
            platform: Platform,
        },
        ...
        UserPlatformBindingUpdated {
            account: T::AccountId,
            platform: Platform,
            old_hash: [u8; 32],
        },
        /// Bot 所有权已转移
        BotOwnershipTransferred {
            bot_id_hash: BotIdHash,
            old_owner: T::AccountId,
            new_owner: T::AccountId,
        },
        /// API Server MRTD 已从白名单撤销
        ApiServerMrtdRevoked {
            mrtd: [u8; 48],
        },
        /// PCK 公钥已撤销
        PckKeyRevoked {
        ...
        fn on_initialize(n: BlockNumberFor<T>) -> Weight {
            ...
            Weight::from_parts(
                reads
                    ...
                    .saturating_add(writes.saturating_mul(25_000_000)),
                reads
                    .saturating_mul(5_000)
                    .saturating_add(writes.saturating_mul(5_000)),
            )
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_bot(
            ...
            let who = ensure_signed(origin)?;
                ...
                !Bots::<T>::contains_key(&bot_id_hash),
                Error::<T>::BotAlreadyRegistered
            );

            OwnerBots::<T>::try_mutate(&who, |bots| -> DispatchResult {
                bots.try_push(bot_id_hash)
                    .map_err(|_| Error::<T>::MaxBotsReached)?;
                Ok(())
            })?;
        ...
        pub fn update_public_key(
            ...
            Self::deposit_event(Event::PublicKeyUpdated {
                bot_id_hash,
                new_key,
            });
            Ok(())
        ...
        pub fn deactivate_bot(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Bots::<T>::try_mutate(&bot_id_hash, |maybe_bot| -> DispatchResult {
                let bot = maybe_bot.as_mut().ok_or(Error::<T>::BotNotFound)?;
                ensure!(bot.owner == who, Error::<T>::NotBotOwner);
        ...
        pub fn bind_community(
            ...
            Self::deposit_event(Event::CommunityBound {
                community_id_hash,
                bot_id_hash,
                platform,
            });
        ...
        pub fn bind_user_platform(
            ...
            if let Some(old_hash) = old {
                Self::deposit_event(Event::UserPlatformBindingUpdated {
                    account: who,
                    platform,
                    old_hash,
                });
        ...
        pub fn refresh_attestation(
            ...
            let has_v1 = Attestations::<T>::contains_key(&bot_id_hash);
                ...
                ApprovedMrtd::<T>::contains_key(&mrtd),
        ...
        pub fn approve_mrtd(origin: OriginFor<T>, mrtd: [u8; 48], version: u32) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                !ApprovedMrtd::<T>::contains_key(&mrtd),
                Error::<T>::MrtdAlreadyApproved
            );
        ...
        pub fn submit_verified_attestation(
            ...
            let report_data =
                &quote[TDX_REPORTDATA_OFFSET..TDX_REPORTDATA_OFFSET + TDX_REPORTDATA_LEN];

            // ── 验证 report_data[0..32] == SHA256(bot.public_key) ──
            let expected_hash = sp_core::hashing::sha2_256(&bot.public_key);
            ensure!(
                report_data[..32] == expected_hash[..],
                Error::<T>::QuoteReportDataMismatch
            );

            // ── P2a: 验证 report_data[32..64] == 链上 nonce (防重放) ──
            let now = frame_system::Pallet::<T>::block_number();
            let (stored_nonce, issued_at) =
                AttestationNonces::<T>::get(&bot_id_hash).ok_or(Error::<T>::NonceMissing)?;
            ...
            let nonce_deadline = issued_at.saturating_add(T::AttestationValidityBlocks::get());
                ...
                report_data[32..64] == stored_nonce[..],
        ...
        pub fn approve_mrenclave(
            ...
            ensure_root(origin)?;
                ...
                !ApprovedMrenclave::<T>::contains_key(&mrenclave),
                Error::<T>::MrenclaveAlreadyApproved
            );
        ...
        #[pallet::weight(Weight::from_parts(20_000_000, 3_000))]
        pub fn revoke_mrtd(origin: OriginFor<T>, mrtd: [u8; 48]) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                ApprovedMrtd::<T>::contains_key(&mrtd),
                Error::<T>::MrtdNotFound
            );
        ...
        #[pallet::weight(Weight::from_parts(20_000_000, 3_000))]
        pub fn revoke_mrenclave(origin: OriginFor<T>, mrenclave: [u8; 32]) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                ApprovedMrenclave::<T>::contains_key(&mrenclave),
        ...
        pub fn register_pck_key(
            ...
            // L2-fix: 防止意外覆盖已注册的 PCK key
                ...
                !RegisteredPckKeys::<T>::contains_key(&platform_id),
                Error::<T>::PckKeyAlreadyRegistered
            );
            let now = frame_system::Pallet::<T>::block_number();
        ...
        pub fn submit_dcap_attestation(
            ...
            let (dcap_result, dcap_level) = if let Some(ref pid) = platform_id {
                ...
                let (pck_key, _) =
                    RegisteredPckKeys::<T>::get(pid).ok_or(Error::<T>::PckKeyNotRegistered)?;
        ...
        pub fn submit_dcap_dual_attestation(
            ...
            let api_result = if let Some(ref pid) = platform_id {
                ...
                dcap::verify_quote_level2(api_quote).map_err(|e| Self::dcap_error_to_dispatch(e))?
            };
        ...
        pub fn submit_sgx_attestation(
            ...
            // 必须已有 TDX 证明
            let mut record =
                Attestations::<T>::get(&bot_id_hash).ok_or(Error::<T>::AttestationNotFound)?;
            ...
            let expected_hash = sp_core::hashing::sha2_256(&bot.public_key);
                ...
                sgx_report_data[..32] == expected_hash[..],
                Error::<T>::QuoteReportDataMismatch
            );
            ...
            let nonce_deadline = issued_at.saturating_add(T::AttestationValidityBlocks::get());
                ...
                sgx_report_data[32..64] == stored_nonce[..],
                Error::<T>::NonceMismatch
            );
            AttestationNonces::<T>::remove(&bot_id_hash);
            ...
            Self::deposit_event(Event::SgxAttestationSubmitted {
                bot_id_hash,
                sgx_dcap_level,
            });
            Ok(())
        ...
        pub fn submit_tee_attestation(
            ...
            let (tee_type, primary_measurement, mrenclave, report_data, dcap_level, quote_verified) =
                match version {
                    4 => Self::verify_tdx_quote_unified(
                        quote,
                        &platform_id,
                        &pck_cert_der,
                        &intermediate_cert_der,
                    )?,
                    3 => Self::verify_sgx_quote_unified(
                        quote,
                        &platform_id,
                        &pck_cert_der,
                        &intermediate_cert_der,
        ...
        pub fn register_peer(
            ...
            PeerRegistry::<T>::try_mutate(&bot_id_hash, |peers| -> DispatchResult {
                ...
                peers
                    .try_push(peer)
                    .map_err(|_| Error::<T>::MaxPeersReached)?;
        ...
        pub fn deregister_peer(
            ...
            PeerRegistry::<T>::try_mutate(&bot_id_hash, |peers| -> DispatchResult {
                let idx = peers
                    .iter()
                    .position(|p| p.public_key == peer_public_key)
        ...
        pub fn register_operator(
            ...
            let who = ensure_signed(origin)?;
                ...
                !Operators::<T>::contains_key(&who, platform),
        ...
        #[pallet::weight(Weight::from_parts(40_000_000, 8_000))]
        pub fn deregister_operator(origin: OriginFor<T>, platform: Platform) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Self::deposit_event(Event::OperatorDeregistered {
                operator: who,
                platform,
            });
            Ok(())
        ...
        pub fn set_operator_sla(
            ...
            Self::deposit_event(Event::OperatorSlaUpdated {
                operator,
                platform,
                sla_level,
            });
        ...
        pub fn assign_bot_to_operator(
            ...
            let op = Operators::<T>::get(&who, platform).ok_or(Error::<T>::OperatorNotFound)?;
                ...
                !BotOperator::<T>::contains_key(&bot_id_hash),
                Error::<T>::BotAlreadyAssigned
            );

            OperatorBots::<T>::try_mutate(&who, platform, |bots| -> DispatchResult {
                bots.try_push(bot_id_hash)
                    .map_err(|_| Error::<T>::MaxBotsPerOperatorReached)?;
                Ok(())
            })?;
            ...
            Self::deposit_event(Event::BotAssignedToOperator {
                bot_id_hash,
                operator: who,
                platform,
            });
        ...
        pub fn unassign_bot_from_operator(
            ...
            let (operator, platform) =
                BotOperator::<T>::get(&bot_id_hash).ok_or(Error::<T>::BotNotAssigned)?;
            ...
            Self::deposit_event(Event::BotUnassignedFromOperator {
                bot_id_hash,
                operator,
                platform,
            });
        ...
        #[pallet::weight(Weight::from_parts(25_000_000, 5_000))]
        pub fn suspend_bot(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            ensure_root(origin)?;
            Bots::<T>::try_mutate(&bot_id_hash, |maybe_bot| -> DispatchResult {
                let bot = maybe_bot.as_mut().ok_or(Error::<T>::BotNotFound)?;
                ensure!(bot.status == BotStatus::Active, Error::<T>::BotNotActive);
        ...
        #[pallet::weight(Weight::from_parts(25_000_000, 5_000))]
        pub fn reactivate_bot(origin: OriginFor<T>, bot_id_hash: BotIdHash) -> DispatchResult {
            ensure_root(origin)?;
            Bots::<T>::try_mutate(&bot_id_hash, |maybe_bot| -> DispatchResult {
                let bot = maybe_bot.as_mut().ok_or(Error::<T>::BotNotFound)?;
                ensure!(
                    bot.status == BotStatus::Suspended,
                    Error::<T>::BotNotSuspended
                );
        ...
        #[pallet::weight(Weight::from_parts(25_000_000, 5_000))]
        pub fn unbind_user_platform(origin: OriginFor<T>, platform: Platform) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ensure!(
        ...
        pub fn transfer_bot_ownership(
            ...
            let who = ensure_signed(origin)?;
            let old_owner = Bots::<T>::try_mutate(
                &bot_id_hash,
                |maybe_bot| -> Result<T::AccountId, DispatchError> {
                    let bot = maybe_bot.as_mut().ok_or(Error::<T>::BotNotFound)?;
                    ensure!(bot.owner == who, Error::<T>::NotBotOwner);
            ...
            Self::deposit_event(Event::BotOwnershipTransferred {
                bot_id_hash,
                old_owner,
                new_owner,
            });
        ...
        #[pallet::weight(Weight::from_parts(20_000_000, 3_000))]
        pub fn revoke_api_server_mrtd(origin: OriginFor<T>, mrtd: [u8; 48]) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                ApprovedApiServerMrtd::<T>::contains_key(&mrtd),
                Error::<T>::ApiServerMrtdNotFound
            );
        ...
        #[pallet::weight(Weight::from_parts(20_000_000, 3_000))]
        pub fn revoke_pck_key(origin: OriginFor<T>, platform_id: [u8; 32]) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                RegisteredPckKeys::<T>::contains_key(&platform_id),
                Error::<T>::PckKeyNotRegistered
            );
        ...
        pub fn update_peer_endpoint(
            ...
            Self::deposit_event(Event::PeerEndpointUpdated {
                bot_id_hash,
                public_key: peer_public_key,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn enqueue_attestation_expiry(
            ...
            AttestationExpiryQueue::<T>::try_mutate(|queue| {
                queue
                    .try_push((expires_at, bot_id_hash, is_v2))
        ...
        fn dcap_error_to_dispatch(e: dcap::DcapError) -> DispatchError {
            match e {
                ...
                dcap::DcapError::BodySignatureInvalid => {
                    Error::<T>::DcapBodySignatureInvalid.into()
                }
                dcap::DcapError::AttestationKeyBindingFailed => {
                    Error::<T>::DcapAkBindingFailed.into()
                }
                dcap::DcapError::QeReportSignatureInvalid => {
                    Error::<T>::DcapQeSignatureInvalid.into()
                }
                ...
                dcap::DcapError::RootCaVerificationFailed => {
                    Error::<T>::DcapRootCaVerificationFailed.into()
                }
                dcap::DcapError::IntermediateCaVerificationFailed => {
                    Error::<T>::DcapIntermediateCaVerificationFailed.into()
                }
            }
        }
        ...
        {
            let (dcap_result, dcap_level) =
                if let (Some(pck), Some(inter)) = (pck_cert_der, intermediate_cert_der) {
                    let result =
        ...
        pub fn operator_info(
            operator: &T::AccountId,
            platform: Platform,
        ) -> Option<OperatorInfo<T>> {
            Operators::<T>::get(operator, platform)
        }

        /// 获取运营商的 Bot 列表 (指定平台)
        pub fn operator_bots(
            operator: &T::AccountId,
            platform: Platform,
        ) -> BoundedVec<BotIdHash, T::MaxBotsPerOperator> {
        ...
        pub fn attestation_info(
            bot_id_hash: &BotIdHash,
        ) -> Option<(u8, bool, BlockNumberFor<T>, Option<TeeType>)> {
            if let Some(v2) = AttestationsV2::<T>::get(bot_id_hash) {
                return Some((
                    v2.dcap_level,
                    v2.quote_verified,
            ...
            Attestations::<T>::get(bot_id_hash)
                .map(|a| (a.dcap_level, a.quote_verified, a.expires_at, None))
        }
    }

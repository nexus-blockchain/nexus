...
pub const INTEL_ROOT_CA_PUBKEY: [u8; 64] = [
    // x (32 bytes)
    0x4f, 0xfa, 0x0f, 0xfd, 0x56, 0x1c, 0xda, 0xd6, 0xc0, 0xf9, 0x8d, 0x30, 0x8c, 0x81, 0x28, 0xc5,
    0xb9, 0x27, 0xa2, 0x73, 0x32, 0xc8, 0xe8, 0xeb, 0x13, 0xf6, 0xbe, 0x42, 0xb5, 0x71, 0xd6, 0x46,
    // y (32 bytes)
    0x6f, 0x53, 0xc6, 0x44, 0xff, 0xc2, 0xff, 0xc1, 0x02, 0x82, 0x20, 0xe4, 0x9a, 0x49, 0x66, 0xcf,
    0x02, 0xf3, 0x2e, 0x2f, 0xb4, 0xd3, 0x49, 0xbb, 0x2c, 0xba, 0xed, 0x28, 0x90, 0x37, 0xa0, 0x2d,
];

/// DER-encoded OID for P-256 curve (prime256v1): 1.2.840.10045.3.1.7
pub const OID_PRIME256V1: [u8; 10] = [0x06, 0x08, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x03, 0x01, 0x07];
...
pub fn verify_p256_ecdsa(
    ...
    let point = EncodedPoint::from_bytes(&sec1[..]).map_err(|_| DcapError::InvalidPublicKey)?;
    let vk = VerifyingKey::from_encoded_point(&point).map_err(|_| DcapError::InvalidPublicKey)?;
...
pub fn extract_tbs_from_cert(cert: &[u8]) -> Result<&[u8], DcapError> {
    ...
    let tbs_total = tbs_hdr
        .checked_add(tbs_len)
        .ok_or(DcapError::CertParsingFailed)?;
    let tbs_end = tbs_start
        .checked_add(tbs_total)
        .ok_or(DcapError::CertParsingFailed)?;
    if cert.len() < tbs_end {
        return Err(DcapError::CertParsingFailed);
    }
...
pub fn extract_ecdsa_sig_from_cert(cert: &[u8]) -> Result<[u8; 64], DcapError> {
    ...
    pos = pos
        .checked_add(tbs_hdr)
        .and_then(|p| p.checked_add(tbs_len))
        .ok_or(DcapError::CertParsingFailed)?;
    ...
    pos = pos
        .checked_add(alg_hdr)
        .and_then(|p| p.checked_add(alg_len))
        .ok_or(DcapError::CertParsingFailed)?;
...
fn copy_der_integer_to_fixed(dest: &mut [u8], src: &[u8]) {
    ...
    let src = if src.len() > 32 && src[0] == 0x00 {
        &src[1..]
    } else {
        src
    };
...
pub fn verify_cert_signature(cert_der: &[u8], issuer_pubkey: &[u8; 64]) -> Result<(), DcapError> {
    ...
    verify_p256_ecdsa(issuer_pubkey, tbs, &sig).map_err(|_| DcapError::CertSignatureInvalid)
}
...
pub fn parse_sgx_quote(raw: &[u8]) -> Result<ParsedSgxQuote<'_>, DcapError> {
    ...
    let mut report_data = [0u8; 64];
    report_data
        .copy_from_slice(&raw[SGX_REPORTDATA_OFFSET..SGX_REPORTDATA_OFFSET + SGX_REPORTDATA_LEN]);
    ...
    Ok(ParsedSgxQuote {
        ...
        qe_report,
        qe_report_signature: &raw
            [SGX_QE_REPORT_SIG_OFFSET..SGX_QE_REPORT_SIG_OFFSET + QE_REPORT_SIG_LEN],
...
pub mod test_utils {
    ...
    impl TestQuoteBuilder {
        ...
        pub fn build(&self) -> Vec<u8> {
            ...
            quote[BODY_SIG_OFFSET..BODY_SIG_OFFSET + BODY_SIG_LEN].copy_from_slice(&body_sig_bytes);
            // AK at offset 700
            ...
            quote
                [QE_REPORT_OFFSET + QE_MRSIGNER_OFFSET..QE_REPORT_OFFSET + QE_MRSIGNER_OFFSET + 32]
            ...
            let qe_report_bytes = &quote[QE_REPORT_OFFSET..QE_REPORT_OFFSET + QE_REPORT_LEN];
    ...
    impl TestSgxQuoteBuilder {
        ...
        pub fn build(&self) -> Vec<u8> {
            ...
            let sig_data_total = BODY_SIG_LEN + AK_LEN + QE_REPORT_LEN + QE_REPORT_SIG_LEN + 2;
            let len_bytes = (sig_data_total as u32).to_le_bytes();
            quote[SGX_SIG_DATA_LEN_OFFSET..SGX_SIG_DATA_LEN_OFFSET + 4].copy_from_slice(&len_bytes);
    ...
    pub fn build_test_cert(subject_vk: &VerifyingKey, issuer_sk: &SigningKey) -> Vec<u8> {
        ...
        tbs_content.extend_from_slice(&[0x06, 0x07, 0x2a, 0x86, 0x48, 0xce, 0x3d, 0x02, 0x01]);
...
mod tests {
    ...
    fn parse_rejects_short_quote() {
        assert_eq!(
            parse_quote(&[0u8; 100]).unwrap_err(),
            DcapError::QuoteTooShort
        );
    }
    ...
    fn cert_chain_valid() {
        ...
        assert_eq!(
            ...
            &expected_intermediate_point.as_bytes()[1..65]
        );
    ...
    fn der_rejects_empty_input() {
        assert_eq!(
            extract_tbs_from_cert(&[]).unwrap_err(),
            DcapError::CertParsingFailed
        );
        assert_eq!(
            extract_p256_pubkey_from_cert(&[]).unwrap_err(),
            DcapError::CertParsingFailed
        );
        assert_eq!(
            extract_ecdsa_sig_from_cert(&[]).unwrap_err(),
            DcapError::CertParsingFailed
        );
    }
    ...
    fn sgx_parse_rejects_short_quote() {
        assert_eq!(
            parse_sgx_quote(&[0u8; 100]).unwrap_err(),
            DcapError::QuoteTooShort
        );
    }
    ...
    fn sgx_parse_rejects_tdx_quote() {
        ...
        assert_eq!(
            parse_sgx_quote(&tdx_quote).unwrap_err(),
            DcapError::InvalidVersion
        );
    }
    ...
    fn tdx_rejects_malicious_sig_data_len() {
        ...
        assert_eq!(
            parse_quote(&quote).unwrap_err(),
            DcapError::InvalidSigDataLen
        );
    }
    ...
    fn sgx_rejects_malicious_sig_data_len() {
        ...
        assert_eq!(
            parse_sgx_quote(&quote).unwrap_err(),
            DcapError::InvalidSigDataLen
        );
    }
    ...
    fn der_rejects_oversized_integer_in_sig() {
        ...
        assert_eq!(
            decode_ecdsa_der_sig(&der).unwrap_err(),
            DcapError::CertParsingFailed
        );
    }
    ...
    fn cert_rejects_oversized_input() {
        ...
        assert_eq!(
            extract_tbs_from_cert(&huge).unwrap_err(),
            DcapError::CertParsingFailed
        );
        assert_eq!(
            extract_p256_pubkey_from_cert(&huge).unwrap_err(),
            DcapError::CertParsingFailed
        );
        assert_eq!(
            extract_ecdsa_sig_from_cert(&huge).unwrap_err(),
            DcapError::CertParsingFailed
        );
    }
    ...
    fn der_rejects_length_exceeding_data() {
        ...
        assert_eq!(
            der_read_element(&data).unwrap_err(),
            DcapError::CertParsingFailed
        );
    }
}

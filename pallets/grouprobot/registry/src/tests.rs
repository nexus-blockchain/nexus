...
fn register_bot_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(GroupRobotRegistry::register_bot(
...
fn deactivate_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::deactivate_bot(
            RuntimeOrigin::signed(OWNER),
            bot_hash(1)
        ));
        assert_eq!(
            Bots::<Test>::get(bot_hash(1)).unwrap().status,
            BotStatus::Deactivated
        );
    });
}
...
fn unbind_community_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::unbind_community(
            RuntimeOrigin::signed(OWNER),
            community_hash(1)
        ));
        assert!(!CommunityBindings::<Test>::contains_key(community_hash(1)));
...
fn bind_user_platform_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            UserPlatformBindings::<Test>::get(OWNER, Platform::Discord),
            Some(pk(42))
        );
    });
}
...
fn submit_attestation_returns_deprecated() {
    new_test_ext().execute_with(|| {
        assert_ok!(GroupRobotRegistry::approve_mrtd(
            RuntimeOrigin::root(),
            mrtd(1),
            1
        ));
...
fn setup_v1_attestation_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::approve_mrenclave(
            RuntimeOrigin::root(),
            mrenclave(1),
            1
        ));
...
fn build_fake_tdx_quote(mrtd_val: &[u8; 48], public_key: &[u8; 32], nonce: &[u8; 32]) -> Vec<u8> {
    ...
    quote[crate::TDX_REPORTDATA_OFFSET + 32..crate::TDX_REPORTDATA_OFFSET + 64]
...
fn v1_attestation_has_quote_verified_false() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            ...
            "V1 attestation must set quote_verified=false"
        );
    });
}
...
fn bot_owner_and_public_key() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotRegistry::bot_public_key(&bot_hash(1)),
            Some(pk(1))
        );
    });
}
...
fn der_parser_handles_malicious_lengths() {
    ...
    assert_eq!(
        extract_tbs_from_cert(&malicious_cert).unwrap_err(),
        DcapError::CertParsingFailed
    );
    assert_eq!(
        extract_ecdsa_sig_from_cert(&malicious_cert).unwrap_err(),
        DcapError::CertParsingFailed
    );
}
...
fn submit_verified_attestation_level_is_1_not_0() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !record.quote_verified,
            "C1: no ECDSA → quote_verified must be false"
        );
...
fn deregister_peer_fails_not_found() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::deregister_peer(RuntimeOrigin::signed(OWNER), bot_hash(1), pk(10),),
...
fn heartbeat_peer_fails_not_found() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::heartbeat_peer(RuntimeOrigin::signed(OWNER), bot_hash(1), pk(10),),
            Error::<Test>::PeerNotFound
        );
    });
}
...
fn peer_survives_if_heartbeat_fresh() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(60);
        assert_noop!(
            GroupRobotRegistry::report_stale_peer(
                RuntimeOrigin::signed(OTHER),
                bot_hash(1),
                pk(10)
            ),
...
fn build_sgx_quote(mrenclave_val: &[u8; 32], public_key: &[u8; 32]) -> (Vec<u8>, [u8; 64]) {
...
fn register_operator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            PlatformAppHashIndex::<Test>::get(Platform::Telegram, app_hash(1)),
            Some(OWNER)
        );
    });
}
...
fn deregister_operator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::deregister_operator(
            RuntimeOrigin::signed(OWNER),
            Platform::Telegram
        ));
...
fn deregister_operator_fails_with_active_bots() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::deregister_operator(
                RuntimeOrigin::signed(OWNER),
                Platform::Telegram
            ),
...
fn set_operator_sla_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::set_operator_sla(
            RuntimeOrigin::root(),
            OWNER,
            Platform::Telegram,
            2
        ...
        assert_eq!(
            Operators::<Test>::get(OWNER, Platform::Telegram)
                .unwrap()
                .sla_level,
            2
        );
...
fn set_operator_sla_rejects_invalid_level() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::set_operator_sla(
                RuntimeOrigin::root(),
                OWNER,
                Platform::Telegram,
                4
...
fn assign_bot_to_operator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            BotOperator::<Test>::get(bot_hash(1)),
            Some((OWNER, Platform::Telegram))
        );
        assert_eq!(
            OperatorBots::<Test>::get(OWNER, Platform::Telegram).len(),
            1
        );
        assert_eq!(
            Operators::<Test>::get(OWNER, Platform::Telegram)
...
fn operator_multi_bot_multi_peer_integration() {
    new_test_ext().execute_with(|| {
        ...
        // 验证 helper functions
        assert_eq!(
            GroupRobotRegistry::bot_operator_account(&bot_hash(1)),
            Some(OWNER)
        );
        assert_eq!(
            GroupRobotRegistry::bot_operator_full(&bot_hash(1)),
            Some((OWNER, tg))
        );
...
fn two_operators_independent_bots() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotRegistry::bot_operator_account(&bot_hash(2)),
            Some(OWNER2)
        );
...
fn cross_platform_operator_isolation() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotRegistry::bot_operator_full(&bot_hash(2)),
            Some((OWNER, Platform::Discord))
        );
        assert_eq!(
            GroupRobotRegistry::operator_bots(&OWNER, Platform::Telegram).len(),
            1
        );
        assert_eq!(
            GroupRobotRegistry::operator_bots(&OWNER, Platform::Discord).len(),
            1
        );
        assert_eq!(
            Operators::<Test>::get(OWNER, Platform::Telegram)
        ...
        assert_eq!(
            Operators::<Test>::get(OWNER, Platform::Discord)
                .unwrap()
                .bot_count,
            1
        );
...
fn heartbeat_increments_uptime_counter() {
    new_test_ext().execute_with(|| {
        ...
        // 初始计数为 0
        assert_eq!(
            GroupRobotRegistry::peer_heartbeat_count(&bot_hash(1), &pk(10)),
            0
        );
...
fn l1_revoke_mrtd_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::revoke_mrtd(
            RuntimeOrigin::root(),
            mrtd(1)
        ));
        assert!(!ApprovedMrtd::<Test>::contains_key(mrtd(1)));
...
fn l1_revoke_mrenclave_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::revoke_mrenclave(
            RuntimeOrigin::root(),
            mrenclave(1)
        ));
        assert!(!ApprovedMrenclave::<Test>::contains_key(mrenclave(1)));
...
fn suspend_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::suspend_bot(
            RuntimeOrigin::root(),
            bot_hash(1)
        ));
        assert_eq!(
...
fn reactivate_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::reactivate_bot(
            RuntimeOrigin::root(),
            bot_hash(1)
        ));
        assert_eq!(
...
fn unbind_user_platform_works() {
    new_test_ext().execute_with(|| {
        ...
        assert!(UserPlatformBindings::<Test>::contains_key(
            OWNER,
            Platform::Discord
        ));
        ...
        System::assert_has_event(RuntimeEvent::GroupRobotRegistry(
            crate::Event::UserPlatformUnbound {
                account: OWNER,
                platform: Platform::Discord,
            },
        ));
...
fn unbind_user_platform_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotRegistry::unbind_user_platform(
                RuntimeOrigin::signed(OWNER),
                Platform::Discord
            ),
...
fn transfer_bot_ownership_fails_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::transfer_bot_ownership(
                RuntimeOrigin::signed(OTHER),
                bot_hash(1),
                OWNER2
            ),
...
fn revoke_api_server_mrtd_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::revoke_api_server_mrtd(
            RuntimeOrigin::root(),
            mrtd(10)
        ));
        assert!(!ApprovedApiServerMrtd::<Test>::contains_key(mrtd(10)));
...
fn revoke_pck_key_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::revoke_pck_key(
            RuntimeOrigin::root(),
            pid
        ));
        assert!(!RegisteredPckKeys::<Test>::contains_key(pid));
...
fn force_deactivate_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::force_deactivate_bot(
            RuntimeOrigin::root(),
            bot_hash(1)
        ));
        assert_eq!(
...
fn suspend_operator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::suspend_operator(
            RuntimeOrigin::root(),
            OWNER,
            Platform::Telegram
        ));
...
fn suspend_operator_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::suspend_operator(
                RuntimeOrigin::signed(OWNER),
                OWNER,
                Platform::Telegram
            ),
...
fn unsuspend_operator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::unsuspend_operator(
            RuntimeOrigin::root(),
            OWNER,
            Platform::Telegram
        ));
        ...
        System::assert_has_event(RuntimeEvent::GroupRobotRegistry(
            crate::Event::OperatorUnsuspended {
                operator: OWNER,
                platform: Platform::Telegram,
            },
        ));
...
fn unsuspend_operator_fails_not_suspended() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::unsuspend_operator(
                RuntimeOrigin::root(),
                OWNER,
                Platform::Telegram
            ),
...
fn update_peer_endpoint_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::update_peer_endpoint(
            ...
            bot_hash(1),
            pk(10),
            endpoint("https://new:9443"),
        ));
        let peers = PeerRegistry::<Test>::get(bot_hash(1));
        assert_eq!(peers[0].endpoint.as_slice(), b"https://new:9443");
        System::assert_has_event(RuntimeEvent::GroupRobotRegistry(
            crate::Event::PeerEndpointUpdated {
                bot_id_hash: bot_hash(1),
                public_key: pk(10),
            },
        ));
...
fn cleanup_deactivated_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        // Anyone can clean up
        assert_ok!(GroupRobotRegistry::cleanup_deactivated_bot(
            RuntimeOrigin::signed(OTHER),
            bot_hash(1)
        ));
        assert!(!Bots::<Test>::contains_key(bot_hash(1)));
        ...
        assert_eq!(OwnerBots::<Test>::get(OWNER).len(), 0);
        System::assert_has_event(RuntimeEvent::GroupRobotRegistry(crate::Event::BotCleaned {
            bot_id_hash: bot_hash(1),
        }));
    });
}
...
fn operator_unassign_bot_works() {
    new_test_ext().execute_with(|| {
        ...
        // Operator unassigns
        assert_ok!(GroupRobotRegistry::operator_unassign_bot(
            RuntimeOrigin::signed(OWNER),
            bot_hash(1)
        ));
        assert!(BotOperator::<Test>::get(bot_hash(1)).is_none());
...
fn force_expire_attestation_v1_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotRegistry::force_expire_attestation(
            RuntimeOrigin::root(),
            bot_hash(1)
        ));
        assert!(!GroupRobotRegistry::is_tee_node(&bot_hash(1)));
        ...
        assert!(matches!(bot.node_type, NodeType::StandardNode));
        System::assert_has_event(RuntimeEvent::GroupRobotRegistry(
            crate::Event::AttestationExpired {
                bot_id_hash: bot_hash(1),
            },
        ));
    });
...
fn force_transfer_bot_ownership_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotRegistry::force_transfer_bot_ownership(
                RuntimeOrigin::signed(OWNER),
                bot_hash(1),
                OWNER2
            ),
...
fn r1_refresh_preserves_dcap_level_v1() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            refreshed.dcap_level, 2,
            "R1-fix: dcap_level must be preserved"
        );
        assert_eq!(
            refreshed.quote_verified, record.quote_verified,
            "R1-fix: quote_verified must be preserved"
        );
    });
}
...
fn r1_refresh_preserves_dcap_level_v2() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            refreshed.dcap_level, 2,
            "R1-fix: V2 dcap_level must be preserved"
        );
        assert_eq!(
            refreshed.tee_type, v2.tee_type,
            "R1-fix: V2 tee_type must be preserved"
        );
    });
}
...
fn query_tee_type_v1_tee_node() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotRegistry::get_tee_type(&bot_hash(1)),
            Some(TeeType::Tdx)
        );
    });
}

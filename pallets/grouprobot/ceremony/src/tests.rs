...
fn record_ceremony_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ActiveCeremony::<Test>::get(bot_pk(1)),
...
fn revoke_ceremony_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotCeremony::revoke_ceremony(
            RuntimeOrigin::root(),
            ceremony_hash(1)
        ));
...
fn force_re_ceremony_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotCeremony::force_re_ceremony(
            RuntimeOrigin::root(),
            ceremony_hash(1)
        ));
...
fn at_risk_not_triggered_when_peer_count_above_k() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert!(!System::events().iter().any(|e| {
            matches!(
                &e.event,
                RuntimeEvent::GroupRobotCeremony(crate::pallet::Event::CeremonyAtRisk { .. })
            )
        }));
    });
}
...
fn m3_r6_cleanup_expired_ceremony() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::GroupRobotCeremony(
            crate::pallet::Event::CeremonyCleaned {
                ceremony_hash: ceremony_hash(1),
            },
        ));
    });
...
fn f7_revoke_by_mrenclave_rejects_non_root() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotCeremony::revoke_by_mrenclave(RuntimeOrigin::signed(OWNER), mrenclave(1)),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn f12_trigger_expiry_rejects_not_expired() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCeremony::trigger_expiry(RuntimeOrigin::signed(OTHER), ceremony_hash(1)),
...
fn f12_trigger_expiry_rejects_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotCeremony::trigger_expiry(RuntimeOrigin::signed(OTHER), ceremony_hash(99)),
            Error::<Test>::CeremonyNotFound
        );
    });
}
...
fn f11_batch_cleanup_rejects_empty() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotCeremony::batch_cleanup_ceremonies(RuntimeOrigin::signed(OTHER), vec![],),
...
fn f11_batch_cleanup_rejects_too_many() {
    new_test_ext().execute_with(|| {
        ...
        let hashes: alloc::vec::Vec<[u8; 32]> = (0..21u8)
            .map(|i| {
                let mut h = [0u8; 32];
                h[0] = i;
                h[31] = 0xFA;
            ...
            .collect();
        assert_noop!(
            GroupRobotCeremony::batch_cleanup_ceremonies(RuntimeOrigin::signed(OTHER), hashes,),
...
fn f2_renew_rejects_non_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCeremony::renew_ceremony(RuntimeOrigin::signed(OTHER), ceremony_hash(1)),
            Error::<Test>::NotBotOwner
        );
    });
}
...
fn f2_renew_rejects_not_active() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCeremony::renew_ceremony(RuntimeOrigin::signed(OWNER), ceremony_hash(1)),
            Error::<Test>::CeremonyNotActive
        );
    });
}
...
fn f13_ceremony_expires_at_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotCeremony::ceremony_expires_at(&bot_pk(1)),
            Some(101)
        );
...
fn f13_ceremony_participant_enclaves_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            GroupRobotCeremony::ceremony_participant_enclaves(&bot_pk(99)),
            None
        );
    });
}

...
//! # Pallet GroupRobot Ceremony — RA-TLS ceremony auditing, enclave whitelists,
//! # and automatic risk detection.
//! # Pallet GroupRobot Ceremony — RA-TLS 仪式审计、Enclave 白名单与自动风险检测。
//!
//! Consolidates the on-chain parts previously split across
//! `pallet-ceremony-audit` and `ceremony.rs`.
//! 整合现有 `pallet-ceremony-audit` + `ceremony.rs` 链上部分。
//!
//! ## Features
//! ## 功能
//! - record ceremonies with enclave-whitelist and Shamir-parameter validation
//! - 记录仪式（验证 Enclave 白名单 + Shamir 参数）
//! - revoke ceremonies
//! - 撤销仪式
//! - force re-ceremony
//! - 强制 re-ceremony
//! - ceremony enclave whitelist management
//! - Ceremony Enclave 白名单管理
//! - `on_initialize`: expiry checks and automatic risk detection
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type Ceremonies<T: Config> = StorageMap<_, Blake2_128Concat, [u8; 32], CeremonyRecord<T>>;

    /// Bot 公钥 → 当前活跃仪式哈希
    #[pallet::storage]
    pub type ActiveCeremony<T: Config> = StorageMap<_, Blake2_128Concat, [u8; 32], [u8; 32]>;
    ...
    pub type CeremonyHistory<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        [u8; 32],
        BoundedVec<[u8; 32], T::MaxCeremonyHistory>,
        ValueQuery,
    >;
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn on_initialize(n: BlockNumberFor<T>) -> Weight {
            ...
            if remaining_budget > 0 {
                ...
                let iter: alloc::boxed::Box<dyn Iterator<Item = ([u8; 32], [u8; 32])>> =
                    match cursor {
                        Some(ref start_key) => {
                            // iter_from 不可用于 StorageMap，使用 iter 并跳过
                            // M1-R5: 使用 < 而非 <=，否则游标位置的仪式会被永久跳过
                            alloc::boxed::Box::new(
                                ActiveCeremony::<T>::iter().skip_while(move |(k, _)| k < start_key),
                            )
                        }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn record_ceremony(
            ...
            if let Some(registered_pk) = T::BotRegistry::bot_public_key(&bot_id_hash) {
                    ...
                    registered_pk == bot_public_key,
                    Error::<T>::BotPublicKeyMismatch
                );
            }

            ensure!(
                !Ceremonies::<T>::contains_key(&ceremony_hash),
                Error::<T>::CeremonyAlreadyExists
            );
            ensure!(k > 0 && k <= n && n <= 254, Error::<T>::InvalidShamirParams);
            ensure!(
                !participant_enclaves.is_empty(),
                Error::<T>::EmptyParticipants
            );
            // H1-fix: 参与者数量必须 >= k (否则无法恢复 secret)
            ensure!(
                participant_enclaves.len() >= k as usize,
                Error::<T>::InsufficientParticipants
            );
            // M2-R6: 参与者数量不能超过 Shamir n 参数（n 份 share 分配给 n 个参与者）
            ensure!(
                participant_enclaves.len() <= n as usize,
                Error::<T>::ParticipantCountExceedsN
            );
            ...
            for i in 0..participant_enclaves.len() {
                for j in (i + 1)..participant_enclaves.len() {
                        ...
                        participant_enclaves[i] != participant_enclaves[j],
                        Error::<T>::DuplicateParticipant
                    );
                }
            }
            ...
            if let Some(old_hash) = old_ceremony_hash {
                Ceremonies::<T>::mutate(&old_hash, |maybe_old| {
                    if let Some(old) = maybe_old {
                        old.status = CeremonyStatus::Superseded {
                            replaced_by: ceremony_hash,
                        };
                    }
                });
            ...
            Self::deposit_event(Event::CeremonyRecorded {
                ceremony_hash,
                bot_public_key,
                k,
                n,
        ...
        pub fn revoke_ceremony(origin: OriginFor<T>, ceremony_hash: [u8; 32]) -> DispatchResult {
            ensure_root(origin)?;
            // L1-R6: 使用共享 helper
        ...
        pub fn approve_ceremony_enclave(
            ...
            let bounded_desc: BoundedVec<u8, ConstU32<128>> = description
                .try_into()
        ...
        pub fn force_re_ceremony(origin: OriginFor<T>, ceremony_hash: [u8; 32]) -> DispatchResult {
            ensure_root(origin)?;
            // L1-R6: 使用共享 helper
            Self::do_revoke(&ceremony_hash)?;
        ...
        pub fn cleanup_ceremony(origin: OriginFor<T>, ceremony_hash: [u8; 32]) -> DispatchResult {
            ensure_signed(origin)?;

            let record =
                Ceremonies::<T>::get(&ceremony_hash).ok_or(Error::<T>::CeremonyNotFound)?;
            ensure!(
                matches!(
                    record.status,
        ...
        pub fn owner_revoke_ceremony(
            ...
            let owner =
                T::BotRegistry::bot_owner(&record.bot_id_hash).ok_or(Error::<T>::BotNotFound)?;
            ensure!(owner == who, Error::<T>::NotBotOwner);

            let bot_public_key = record.bot_public_key;
            Self::do_revoke(&ceremony_hash)?;
            Self::deposit_event(Event::OwnerCeremonyRevoked {
                ceremony_hash,
                bot_public_key,
            });
            Ok(())
        ...
        pub fn revoke_by_mrenclave(origin: OriginFor<T>, mrenclave: [u8; 32]) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn trigger_expiry(origin: OriginFor<T>, ceremony_hash: [u8; 32]) -> DispatchResult {
            ensure_signed(origin)?;
        ...
        pub fn batch_cleanup_ceremonies(
            ...
            let max = T::MaxProcessPerBlock::get() as usize;
                ...
                !ceremony_hashes.is_empty(),
                Error::<T>::NoCeremoniesToCleanup
            );
            ensure!(ceremony_hashes.len() <= max, Error::<T>::TooManyCeremonies);

            // 先验证全部，再清理全部（原子性）
            for hash in &ceremony_hashes {
                let record = Ceremonies::<T>::get(hash).ok_or(Error::<T>::CeremonyNotFound)?;
                ensure!(
                    matches!(
                        record.status,
                        CeremonyStatus::Expired
            ...
            for hash in &ceremony_hashes {
                ...
                Self::deposit_event(Event::CeremonyCleaned {
                    ceremony_hash: *hash,
                });
            }
        ...
        pub fn renew_ceremony(origin: OriginFor<T>, ceremony_hash: [u8; 32]) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            let owner =
                ...
                T::BotRegistry::is_bot_active(&record.bot_id_hash),
                Error::<T>::BotNotActive
            );
            ensure!(
            ...
            Self::deposit_event(Event::CeremonyRenewed {
                ceremony_hash,
                new_expires_at,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_revoke(ceremony_hash: &[u8; 32]) -> DispatchResult {
            ...
            Ceremonies::<T>::mutate(ceremony_hash, |maybe_record| {
                if let Some(r) = maybe_record {
                    r.status = CeremonyStatus::Revoked {
                        revoked_at: now_u64,
                    };
                }
            });
        ...
        pub fn ceremony_health(bot_public_key: &[u8; 32]) -> Option<(BlockNumberFor<T>, u32, u8)> {
            ActiveCeremony::<T>::get(bot_public_key)
                .and_then(|hash| Ceremonies::<T>::get(&hash))
        ...
        /// F13: 获取活跃仪式的过期区块
        pub fn ceremony_expires_at(bot_public_key: &[u8; 32]) -> Option<BlockNumberFor<T>> {
            ActiveCeremony::<T>::get(bot_public_key)
                .and_then(|hash| Ceremonies::<T>::get(&hash))
                .filter(|r| matches!(r.status, CeremonyStatus::Active))
                .map(|r| r.expires_at)

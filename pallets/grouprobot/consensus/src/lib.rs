...
//! # Pallet GroupRobot Consensus — node management, TEE verification, and Era orchestration.
//! # Pallet GroupRobot Consensus — 节点管理、TEE 验证与 Era 编排。
//!
//! Manages node lifecycle and Era orchestration. Subscription and rewards were split
//! into dedicated pallets.
...
//! - 节点注册 + 质押
//! - node exit with cooldown
//! - 节点退出（冷却期）
//! - equivocation reporting and slashing
//! - Equivocation 举报 + Slash
//! - message-sequence deduplication (`ProcessedSequences`)
//! - 消息序列去重（`ProcessedSequences`）
//! - TEE verification and weight calculation
//! - TEE 验证 + 权重计算
//! - `on_era_end` orchestration via `SubscriptionSettler` and `EraRewardDistributor`
...
/// Benchmark helper trait implemented by the runtime for setting up external state.
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type ProcessedSequences<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, BotIdHash, Blake2_128Concat, u64, BlockNumberFor<T>>;

    /// Equivocation 记录: (node_id, sequence) → EquivocationRecord
    #[pallet::storage]
    pub type EquivocationRecords<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, NodeId, Blake2_128Concat, u64, EquivocationRecord<T>>;
    ...
    #[pallet::storage]
    pub type NodeBotBinding<T: Config> = StorageMap<_, Blake2_128Concat, NodeId, BotIdHash>;
    ...
    #[pallet::storage]
    pub type AllowSingleSourceFallback<T: Config> =
    ...
    pub enum Event<T: Config> {
        ...
        /// P9: Slash 百分比已更新
        SlashPercentageUpdated {
            new_pct: u32,
        },
        /// P5: 举报者奖励百分比已更新
        ReporterRewardPctUpdated {
            new_pct: u32,
        },
        /// P10: 节点被治理强制恢复
        NodeForceReinstated {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            ...
            assert!(T::MaxActiveNodes::get() > 0, "MaxActiveNodes must be > 0");
        }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_node(
            ...
            let who = ensure_signed(origin)?;
                ...
                !OperatorNodes::<T>::contains_key(&who),
                Error::<T>::NodeAlreadyRegistered
            );
            ensure!(
                stake >= Self::calculate_min_stake(),
        ...
        pub fn request_exit(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ...
            Nodes::<T>::try_mutate(&node_id, |maybe_node| -> DispatchResult {
                ...
                // H5-fix: 允许 Suspended 节点退出 (slash 后可回收剩余质押)
                    ...
                    !ExitRequests::<T>::contains_key(&node_id),
        ...
        pub fn finalize_exit(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ...
            Self::deposit_event(Event::ExitFinalized {
                node_id,
        ...
        ///
        /// Reporter reward is paid from the slashed amount (not minted),
        /// ensuring no net inflation.
        /// 举报者奖励从罚没金额中支付（非铸币），不产生净增发。
        #[pallet::call_index(4)]
        ...
        pub fn slash_equivocation(
            ...
            let reporter = EquivocationRecords::<T>::try_mutate(
                &node_id,
                sequence,
                |maybe_record| -> Result<T::AccountId, DispatchError> {
            ...
            if let Some(node) = Nodes::<T>::get(&node_id) {
                ...
                let (slash_imbalance, _remaining) =
                    T::Currency::slash_reserved(&node.operator, slash_amount);
                let actual_slashed = slash_imbalance.peek();

                // P5: 举报者奖励 — 从 slash NegativeImbalance 中分拆，不铸造新币
                // Reporter reward split from slash NegativeImbalance — no new tokens minted
                ...
                if reporter_pct > 0 && actual_slashed > BalanceOf::<T>::default() {
                    ...
                    if reward > BalanceOf::<T>::default() {
                        // Split the imbalance: reporter_part + remainder (burned)
                        let (reporter_part, _burned) = slash_imbalance.split(reward);
                        // Resolve reporter_part into reporter's account (credit, not mint)
                        T::Currency::resolve_creating(&reporter, reporter_part);
                        Self::deposit_event(Event::ReporterRewarded {
                            reporter,
                            amount: reward,
                        });
                    } else {
                        // reward rounds to zero — entire slash is burned (imbalance dropped)
                        drop(slash_imbalance);
                    }
                } else {
                    // No reporter reward — entire slash is burned (imbalance dropped)
                    drop(slash_imbalance);
                }
        ...
        pub fn mark_sequence_processed(
            ...
            if ProcessedSequences::<T>::contains_key(&bot_id_hash, sequence) {
                Self::deposit_event(Event::SequenceDuplicate {
                    bot_id_hash,
                    sequence,
                });
                return Ok(());
            ...
            Self::deposit_event(Event::SequenceProcessed {
                bot_id_hash,
                sequence,
            });
            Ok(())
        ...
        pub fn verify_node_tee(
            ...
            // 验证 Bot 活跃且属于该操作者
                ...
                T::BotRegistry::is_bot_active(&bot_id_hash),
                Error::<T>::BotNotRegistered
            );
            let bot_owner =
                T::BotRegistry::bot_owner(&bot_id_hash).ok_or(Error::<T>::BotNotRegistered)?;
            ensure!(bot_owner == who, Error::<T>::BotOwnerMismatch);

            // 验证 Bot 有有效的 TEE 证明 (Registry 的证明记录)
            ensure!(
                T::BotRegistry::is_tee_node(&bot_id_hash),
                Error::<T>::AttestationNotValid
            );
            ensure!(
                T::BotRegistry::is_attestation_fresh(&bot_id_hash),
                Error::<T>::AttestationNotValid
            );
        ...
        pub fn set_tee_reward_params(
            ...
            Self::deposit_event(Event::TeeRewardParamsUpdated {
                tee_multiplier,
                sgx_bonus,
            });
            Ok(())
        ...
        pub fn reinstate_node(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Nodes::<T>::try_mutate(&node_id, |maybe_node| -> DispatchResult {
                let node = maybe_node.as_mut().ok_or(Error::<T>::NodeNotFound)?;
                ensure!(node.operator == who, Error::<T>::NotOperator);
                    ...
                    Error::<T>::NodeNotSuspended
                );
                ensure!(
                    node.stake >= Self::calculate_min_stake(),
                    Error::<T>::InsufficientStake
                );
        ...
        pub fn force_suspend_node(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ensure_root(origin)?;
            Nodes::<T>::try_mutate(&node_id, |maybe_node| -> DispatchResult {
                let node = maybe_node.as_mut().ok_or(Error::<T>::NodeNotFound)?;
                ensure!(node.status == NodeStatus::Active, Error::<T>::NodeNotActive);
        ...
        pub fn force_remove_node(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ensure_root(origin)?;
            let node = Nodes::<T>::get(&node_id).ok_or(Error::<T>::NodeNotFound)?;
            ...
            Self::deposit_event(Event::NodeForceRemoved {
                node_id,
                stake_slashed: actual_slashed,
            });
            Ok(())
        }
        ...
        pub fn unbind_bot(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let node = Nodes::<T>::get(&node_id).ok_or(Error::<T>::NodeNotFound)?;
            ensure!(node.operator == who, Error::<T>::NotOperator);
            ensure!(
                NodeBotBinding::<T>::contains_key(&node_id),
                Error::<T>::NoBotBinding
            );
        ...
        pub fn replace_operator(
            ...
            let who = ensure_signed(origin)?;
                ...
                !OperatorNodes::<T>::contains_key(&new_operator),
                Error::<T>::NewOperatorAlreadyHasNode
            );
        ...
        pub fn set_slash_percentage(origin: OriginFor<T>, new_pct: u32) -> DispatchResult {
            ensure_root(origin)?;
            if new_pct == 0 {
                // 恢复使用 Config 默认值
        ...
        pub fn set_reporter_reward_pct(origin: OriginFor<T>, pct: u32) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn force_reinstate_node(origin: OriginFor<T>, node_id: NodeId) -> DispatchResult {
            ensure_root(origin)?;
            Nodes::<T>::try_mutate(&node_id, |maybe_node| -> DispatchResult {
                let node = maybe_node.as_mut().ok_or(Error::<T>::NodeNotFound)?;
                ensure!(
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn calculate_min_stake() -> BalanceOf<T> {
            T::DepositCalculator::calculate_deposit(T::MinStakeUsd::get(), T::MinStake::get())
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn on_era_end(now: BlockNumberFor<T>) {
            ...
            // 2. 可分配总额 = RewardPool 已有余额（订阅费、广告收入等，不铸币）
            // Distributable pool = existing RewardPool balance (no minting)
            let reward_pool = T::RewardDistributor::reward_pool_balance();
            ...
            // 6. 委托 rewards pallet 分配奖励并记录 Era 信息 (inflation=0, 不铸币)

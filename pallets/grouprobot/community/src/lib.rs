...
//! # Pallet GroupRobot Community — community management, group-rule configuration,
//! # node-admission policy, and action logs.
//! # Pallet GroupRobot Community — 社区管理、群规则配置、节点准入策略与动作日志。
//!
//! Consolidates the on-chain parts of `pallet-bot-group-mgmt`, `group_config.rs`,
//! and `local_processor.rs`.
//! 整合现有 `pallet-bot-group-mgmt`、`group_config.rs`、`local_processor.rs` 链上部分。
//!
//! ## Features
//! ## 功能
//! - community rule configuration (anti-spam, warn limits, welcome messages)
//! - 社区群规则配置（防刷屏、警告限制、欢迎消息）
//! - node-admission policies (`Any`, `TeeOnly`, `TeePreferred`, `MinTee`)
//! - 节点准入策略（`Any`、`TeeOnly`、`TeePreferred`、`MinTee`）
//! - action-log notarization with Ed25519 signature verification
//! - 动作日志存证（Ed25519 签名验证）
//! - batched log submission
//! - 批量提交日志
//! - expired-log cleanup
//! - 过期日志清理
...
/// On-chain community rule configuration.
/// 群规则配置（链上精简版）。
#[derive(
    Encode,
    Decode,
...
/// Local community reputation record.
/// 声誉记录（本地社区声誉）。
#[derive(
    Encode,
    Decode,
...
pub mod pallet {
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            assert!(
                T::MaxLogsPerCommunity::get() > 0,
                "MaxLogsPerCommunity must be > 0"
            );
            assert!(
                T::MaxReputationDelta::get() > 0,
                "MaxReputationDelta must be > 0"
            );
    ...
    pub type ActionLogs<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        CommunityIdHash,
        BoundedVec<ActionLog<T>, T::MaxLogsPerCommunity>,
        ValueQuery,
    >;
    ...
    #[pallet::storage]
    pub type LastSequence<T: Config> = StorageMap<_, Blake2_128Concat, CommunityIdHash, u64>;
    ...
    pub enum Event<T: Config> {
        ...
        /// Bot Owner 删除社区配置
        CommunityConfigDeleted { community_id_hash: CommunityIdHash },
        /// Root 强制删除社区
        CommunityForceRemoved { community_id_hash: CommunityIdHash },
        /// Root 封禁社区
        CommunityBanned { community_id_hash: CommunityIdHash },
        /// Root 解封社区
        CommunityUnbanned { community_id_hash: CommunityIdHash },
        /// Root 强制更新社区配置
        ...
        /// Root 强制重置社区全部声誉
        CommunityReputationForceReset { community_id_hash: CommunityIdHash },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_node_requirement(
            ...
            CommunityConfigs::<T>::try_mutate(&community_id_hash, |maybe_config| {
                let config = maybe_config.as_mut().ok_or(Error::<T>::CommunityNotFound)?;
                    ...
                    config.node_requirement != requirement,
                    Error::<T>::SameNodeRequirement
                );
                config.node_requirement = requirement.clone();
                Ok::<(), DispatchError>(())
            })?;

            Self::deposit_event(Event::NodeRequirementUpdated {
                community_id_hash,
                requirement,
            });
            Ok(())
        ...
        pub fn update_community_config(
            ...
            if let Some(ref ex) = existing {
                    ...
                    Error::<T>::ConfigVersionConflict
                );
                ensure!(
                    ex.status != CommunityStatus::Banned,
                    Error::<T>::CommunityBanned
                );
            } else {
                ensure!(expected_version == 0, Error::<T>::ConfigVersionConflict);
            }

            let node_req = existing
                .as_ref()
                .map(|c| c.node_requirement.clone())
                .unwrap_or_default();
        ...
        pub fn batch_submit_logs(
            ...
            Self::deposit_event(Event::BatchLogsSubmitted {
                community_id_hash,
                count,
            });
            Ok(())
        ...
        pub fn clear_expired_logs(
            ...
            // CM1-fix: 防止 max_age_blocks=0 擦除所有日志
                ...
                max_age_blocks > BlockNumberFor::<T>::default(),
                Error::<T>::InvalidMaxAge
            );
            ...
            if gate.log_retention_days > 0 {
                ...
                let min_retention: u32 =
                    (gate.log_retention_days as u32).saturating_mul(T::BlocksPerDay::get());
                let min_retention_blocks: BlockNumberFor<T> = min_retention.into();
                ensure!(
                    max_age_blocks >= min_retention_blocks,
                    Error::<T>::RetentionPeriodNotExpired
                );
            } else {
            ...
            Self::deposit_event(Event::ExpiredLogsCleared {
                community_id_hash,
                cleared,
            });
            Ok(())
        ...
        pub fn cleanup_expired_cooldowns(
            ...
            // M1-R3: 使用专用错误码替代复用 NoLogsToClear
                ...
                last > BlockNumberFor::<T>::default(),
                Error::<T>::CooldownNotFound
            );
        ...
        pub fn ban_community(
            ...
            CommunityConfigs::<T>::try_mutate(&community_id_hash, |maybe_config| {
                let config = maybe_config.as_mut().ok_or(Error::<T>::CommunityNotFound)?;
                    ...
                    Error::<T>::CommunityAlreadyInStatus
                );
                config.status = CommunityStatus::Banned;
                Ok::<(), DispatchError>(())
            })?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn verify_action_log_signature(
            ...
            let ed_pk = ed25519::Public::from_raw(pk_bytes);
                ...
                ed25519_verify(&ed_sig, &msg, &ed_pk),
        ...
        fn ensure_bot_owner(
            ...
            let owner =
                T::BotRegistry::bot_owner(community_id_hash).ok_or(Error::<T>::NotBotOwner)?;
        ...
        fn do_modify_reputation(
            ...
            Self::ensure_not_banned(&community_id_hash)?;
                ...
                delta <= T::MaxReputationDelta::get(),
                Error::<T>::ReputationDeltaTooLarge
            );
            Self::check_cooldown(&who, &community_id_hash, &user_hash)?;

            let signed_delta = delta as i64;
            let new_score = MemberReputation::<T>::mutate(&community_id_hash, &user_hash, |rec| {
                if is_award {
        ...
        fn ensure_active_bot_owner(
            ...
            Self::ensure_bot_owner(who, community_id_hash)?;
                ...
                T::BotRegistry::is_bot_active(community_id_hash),

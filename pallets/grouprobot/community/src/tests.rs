...
fn update_community_config_cas_conflict() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            CommunityConfigs::<Test>::get(community_hash(1))
                .unwrap()
                .version,
            2
        );
...
fn batch_submit_logs_works() {
    new_test_ext().execute_with(|| {
        ...
        let logs: BoundedVec<_, <Test as crate::Config>::MaxBatchSize> =
...
fn helper_is_community_configured() {
    new_test_ext().execute_with(|| {
        assert!(!GroupRobotCommunity::is_community_configured(
            &community_hash(1)
        ));
...
fn cooldown_enforced() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            MemberReputation::<Test>::get(community_hash(1), user_hash(1)).score,
            10
        );
    });
}
...
fn cooldown_different_target_same_block() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            MemberReputation::<Test>::get(community_hash(1), user_hash(2)).score,
            5
        );
    });
}
...
fn global_reputation_aggregates_communities() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            MemberReputation::<Test>::get(community_hash(2), user_hash(1)).score,
            20
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCommunity::batch_submit_logs(
                RuntimeOrigin::signed(OWNER),
                community_hash(4),
                BoundedVec::try_from(vec![(ActionType::Ban, [1u8; 32], 1, [2u8; 32], sig)])
                    .unwrap(),
            ),
            Error::<Test>::FreeTierNotAllowed
        );
    ...
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCommunity::batch_submit_logs(
                RuntimeOrigin::signed(OWNER),
                community_hash(1),
                BoundedVec::try_from(vec![(ActionType::Kick, [3u8; 32], 5, [4u8; 32], sig1)])
                    .unwrap(),
            ),
            Error::<Test>::SequenceNotMonotonic
        );
...
fn m2_r2_cleanup_expired_cooldowns_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ReputationCooldowns::<Test>::get((&OWNER, &community_hash(1), &user_hash(1))),
            0
        );
    });
}
...
fn m1_r3_cooldown_not_found_error_is_distinct() {
    new_test_ext().execute_with(|| {
        ...
        assert_ne!(result, Err(Error::<Test>::NoLogsToClear.into()),);
    });
}
...
fn m2_r3_batch_respects_configurable_max_size() {
    new_test_ext().execute_with(|| {
        ...
        let target = [0u8; 32];
        let msg_hash = [0u8; 32];
        let logs_vec: Vec<(ActionType, [u8; 32], u64, [u8; 32], [u8; 64])> = (1..=51u64)
            .map(|seq| {
        ...
        assert!(
            BoundedVec::<_, <Test as crate::Config>::MaxBatchSize>::try_from(logs_vec).is_err()
        );
        ...
        let small_logs: BoundedVec<_, <Test as crate::Config>::MaxBatchSize> =
            BoundedVec::try_from(
                (1..=2u64)
                    .map(|seq| {
                        let sig = test_sign(&community_hash(1), &action, &target, seq, &msg_hash);
...
fn ban_community_fails_already_banned() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCommunity::ban_community(RuntimeOrigin::root(), community_hash(1),),
...
fn ban_community_fails_not_root() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotCommunity::ban_community(RuntimeOrigin::signed(OWNER), community_hash(1),),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn ban_community_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            GroupRobotCommunity::ban_community(RuntimeOrigin::root(), community_hash(99),),
            Error::<Test>::CommunityNotFound
        );
    });
}
...
fn unban_community_fails_not_banned() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            GroupRobotCommunity::unban_community(RuntimeOrigin::root(), community_hash(1),),
...
fn community_provider_is_configured() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !<GroupRobotCommunity as CommunityProvider>::is_community_configured(&community_hash(
                1
            ))
        );
        ...
        assert!(
            <GroupRobotCommunity as CommunityProvider>::is_community_configured(&community_hash(1))
        );
    });
}
...
fn community_provider_is_banned() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !<GroupRobotCommunity as CommunityProvider>::is_community_banned(&community_hash(1))
        );
...
fn community_provider_ads_disabled_when_banned() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(GroupRobotCommunity::update_community_config(
            ...
            false,
            true,
            *b"en", // ads_enabled=true
        ));
        assert!(<GroupRobotCommunity as CommunityProvider>::is_ads_enabled(
            &community_hash(1)
        ));
...
fn community_provider_language() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            <GroupRobotCommunity as CommunityProvider>::language(&community_hash(1)),
            *b"en"
        );

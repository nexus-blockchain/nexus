...
| `remove_api_key(endpoint)` | 删除端点的 API Key（保留端点本身） |
| `get_api_key_for_endpoint(endpoint)` | 查询端点是否已配置 API Key |
| `get_configured_api_key_endpoints()` | 获取已配置 Key 的端点列表（不含 Key 值） |
| `bootstrap_api_keys()` | 从 offchain storage 批量引导 API Key（OCW 自动调用） |
...
通过节点 RPC `offchain_localStorageSet` 在运行中的节点上设置、修改、删除 TronGrid API Key，**无需重启节点**，下一个区块 OCW 自动生效。

### 前提条件

节点启动时需要带 `--rpc-methods=unsafe` 参数以启用 offchain RPC：

```bash
./nexus-node --rpc-methods=unsafe ...其他参数...
```

> API Key 配完后可重启去掉 `--rpc-methods=unsafe`，Key 已持久化不会丢失。

### 存储键速查

| 用途 | 明文 Key | Hex Key |
|------|----------|---------|
| Bootstrap 数据 | `ocw_bootstrap_api_keys` | `0x6f63775f626f6f7473747261705f6170695f6b657973` |
| 已应用版本号 | `ocw_bootstrap_applied_ver` | `0x6f63775f626f6f7473747261705f6170706c6965645f766572` |
| 端点配置（含 Key） | `ocw_custom_endpoints` | `0x6f63775f637573746f6d5f656e64706f696e7473` |
...
<端点URL>=<API Key>
# 注释行（以 # 开头，会被跳过）
<端点URL>=              ← 空 value = 删除该端点的 Key
```

- 首行为**版本标识**（任意字符串，如 v1、v2、v3）
- **每次修改必须更改版本号**，否则幂等检查会跳过
- 不需要 Key 的端点**不写该行**即可
- API Key 中可包含 `=` 字符（使用 `split_once` 解析，只按第一个 `=` 分割）

### API Key 申请

| 端点 | 服务商 | 申请地址 |
|------|--------|----------|
| `https://api.trongrid.io` | TronGrid (官方) | https://www.trongrid.io/dashboard |
| `https://api.tronstack.io` | TronStack | https://www.tronstack.io |
| `https://apilist.tronscanapi.com` | TronScan | 通常无需 Key |

> 生产环境建议至少为 TronGrid 配置 API Key。无 Key 时免费配额极低（~100 次/天），交易频繁时会被限流。
...
```bash
VALUE=$(printf 'v1\nhttps://api.trongrid.io=你的TronGrid-API-Key' | xxd -p | tr -d '\n')

curl -s -H "Content-Type: application/json" \
  -d "{\"jsonrpc\":\"2.0\",\"id\":1,\"method\":\"offchain_localStorageSet\",\"params\":[\"PERSISTENT\",\"0x6f63775f626f6f7473747261705f6170695f6b657973\",\"0x${VALUE}\"]}" \
  http://localhost:9944
```
...
```bash
VALUE=$(printf 'v1\nhttps://api.trongrid.io=trongrid-key-xxx\nhttps://api.tronstack.io=tronstack-key-yyy' | xxd -p | tr -d '\n')
...
```bash
# v1 → v2
VALUE=$(printf 'v2\nhttps://api.trongrid.io=新的Key值' | xxd -p | tr -d '\n')
...
```bash
VALUE=$(printf 'v3\nhttps://api.trongrid.io=' | xxd -p | tr -d '\n')
...
```bash
VALUE=$(printf 'v4\nhttps://api.trongrid.io=\nhttps://api.tronstack.io=\nhttps://apilist.tronscanapi.com=' | xxd -p | tr -d '\n')
...
### 6. 硬重置（清空 bootstrap 数据 + 版本记录）

当 bootstrap 数据损坏或想彻底从头来时：

```bash
# 清空 bootstrap 数据
curl -s -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"offchain_localStorageSet","params":["PERSISTENT","0x6f63775f626f6f7473747261705f6170695f6b657973","0x"]}' \
  http://localhost:9944

# 清空已应用版本号（下次设数据后一定会重新应用）
curl -s -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":2,"method":"offchain_localStorageSet","params":["PERSISTENT","0x6f63775f626f6f7473747261705f6170706c6965645f766572","0x"]}' \
  http://localhost:9944
```

### 7. 恢复出厂（重置端点配置为默认值：3 默认端点 + 无 Key）

```bash
curl -s -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"offchain_localStorageSet","params":["PERSISTENT","0x6f63775f637573746f6d5f656e64706f696e7473","0x"]}' \
  http://localhost:9944
```
...
# 读取 bootstrap 原始数据
curl -s -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":1,"method":"offchain_localStorageGet","params":["PERSISTENT","0x6f63775f626f6f7473747261705f6170695f6b657973"]}' \
  http://localhost:9944 | python3 -c "
import sys, json
r = json.load(sys.stdin)
if r.get('result'):
    print(bytes.fromhex(r['result'][2:]).decode('utf-8', errors='replace'))
else:
    print('(empty)')
"
...
# 读取已应用的版本号
curl -s -H "Content-Type: application/json" \
  -d '{"jsonrpc":"2.0","id":2,"method":"offchain_localStorageGet","params":["PERSISTENT","0x6f63775f626f6f7473747261705f6170706c6965645f766572"]}' \
  http://localhost:9944 | python3 -c "
import sys, json
r = json.load(sys.stdin)
if r.get('result'):
    print('Applied version:', bytes.fromhex(r['result'][2:]).decode())
else:
    print('(no version applied yet)')
"
```

### 注意事项

1. **无需重启节点** — 下一个区块 OCW 运行时自动生效
2. **每次修改必须改版本号** — 否则幂等检查跳过（v1 → v2 → v3 ...）
3. **硬重置后** — 需先清版本号再设新数据，否则版本可能冲突
4. **无 Key 也能验证** — 只是 TronGrid 免费配额很低（~100 次/天），超限会返回 403/429
5. **本节点本地存储** — 不会广播到其他节点，每个节点需单独配置
6. 日志中搜索 `"Bootstrap"` 可确认执行结果：`Bootstrap [v1]: 1 key(s) set, 0 key(s) removed`
7. **安全建议** — `--rpc-methods=unsafe` 仅在配置时需要，配完后可重启去掉；RPC 端口不要暴露到公网
...
| `ocw_bootstrap_api_keys` | Bootstrap API Key 预设数据 |
| `ocw_bootstrap_applied_ver` | Bootstrap 已应用版本号 |
...
- **Bootstrap API Key** — 无数据跳过 / 正常应用 / 幂等 / 版本变更重应用 / 非法 UTF-8 / 缺版本行 / 注释空行 / 自动添加端点 / 拒绝非 HTTPS / Key 含等号 / 空白 trim / 空 value 删除 Key
- **remove_api_key** — 删除已有 Key / 删除不存在的 Key
- **get_configured_api_key_endpoints** — 空列表 / 多 Key 列表
...
| 版本 | 日期 | 变更 |
|------|------|------|
| **v0.5.0** | 2026-03-20 | API Key 运维增强: remove_api_key, bootstrap 删除语义(空 value), bootstrap 批量优化(单次写盘), get_configured_api_key_endpoints 诊断, get_api_key_for_endpoint 公开, 15 个新测试 |

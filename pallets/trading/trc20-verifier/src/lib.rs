...
impl VerificationError {
    ...
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::ResponseTooLarge => "HTTP response body exceeds size limit",
        }
    }
}
...
/// 端点 API 类型 — 不同端点使用不同的 URL 路径和响应格式
#[derive(Debug, Clone, PartialEq, Eq, Encode, Decode)]
pub enum EndpointKind {
    /// TronGrid: `/v1/accounts/{addr}/transactions/trc20` 路径
    TronGrid,
    /// TronScan: `/api/token_trc20/transfers` 路径
    TronScan,
}
...
pub const DEFAULT_ENDPOINTS: &[&str] = &[
    ...
    "https://apilist.tronscanapi.com", // TronScan API
];

/// 默认端点的 API 类型映射
pub const DEFAULT_ENDPOINT_KINDS: &[(&str, EndpointKind)] = &[
    ("https://api.trongrid.io", EndpointKind::TronGrid),
    ("https://apilist.tronscanapi.com", EndpointKind::TronScan),
];
...
/// P0-3: HTTP 响应 body 最大大小（字节），防止恶意端点 DoS
const MAX_RESPONSE_BODY_BYTES: usize = 512 * 1024; // 512 KB

/// 速率限制存储键前缀 (H2, P0-5: 改为按端点限流)
const RATE_LIMIT_PREFIX: &[u8] = b"ocw_rate_limit::";
...
/// NEW-6: EndpointConfig 当前版本 (v2: 含 endpoint_kinds)
const ENDPOINT_CONFIG_VERSION: u8 = 2;
...
/// 节点运维人员通过 RPC `offchain_localStorageSet` 预设 API Key 的存储键
///
/// 格式: 首行为版本标识（任意字符串），后续每行 `endpoint=api_key`
/// 例如:
/// ```text
/// v1
/// https://api.trongrid.io=your-api-key-here
/// ```
///
/// 更新 Key 时只需修改首行版本号（如 v1 → v2），OCW 检测到变化后自动重新应用。
///
/// 设置方法 (curl):
/// ```bash
/// # value 为 hex 编码的 UTF-8 字符串
/// curl -X POST http://localhost:9944 -H 'Content-Type: application/json' -d '{
///   "jsonrpc": "2.0", "id": 1,
///   "method": "offchain_localStorageSet",
///   "params": ["PERSISTENT", "0x6f63775f626f6f7473747261705f6170695f6b657973",
///              "<hex-encoded-value>"]
/// }'
/// ```
const BOOTSTRAP_API_KEYS_KEY: &[u8] = b"ocw_bootstrap_api_keys";

/// 记录上次已应用的 bootstrap 版本标识
const BOOTSTRAP_APPLIED_VERSION_KEY: &[u8] = b"ocw_bootstrap_applied_ver";
    ...
    pub fn record_success(&mut self, response_ms: u32) {
        ...
        if self.avg_response_ms == 0 {
            ...
            self.avg_response_ms = ((self.avg_response_ms as u64 * HEALTH_DECAY_FACTOR as u64
                + response_ms as u64 * (100 - HEALTH_DECAY_FACTOR) as u64)
                / 100) as u32;
        }
...
fn save_endpoint_health(endpoint: &str, health: &EndpointHealth) {
    let key = [ENDPOINT_HEALTH_PREFIX, endpoint.as_bytes()].concat();
    sp_io::offchain::local_storage_set(StorageKind::PERSISTENT, &key, &health.encode());
}
...
pub struct EndpointConfig {
    ...
    /// 端点 API 类型映射: (endpoint_url, kind)
    pub endpoint_kinds: Vec<(String, EndpointKind)>,
}
...
impl Default for EndpointConfig {
    fn default() -> Self {
        Self {
            ...
            endpoint_kinds: DEFAULT_ENDPOINT_KINDS
                .iter()
                .map(|(url, kind)| (String::from(*url), kind.clone()))
                .collect(),
        }
    }
}
...
impl Default for VerifierConfig {
    fn default() -> Self {
        Self {
            ...
            enabled: true,
            consensus_enabled: true,                 // P0-4: 生产默认开启共识
            min_consensus_responses: 2,              // 2-of-N
            allow_single_source_fallback: true,      // 单端点不可用时降级为单源验证
...
pub fn save_verifier_config(config: &VerifierConfig) -> Result<(), VerificationError> {
    ...
    sp_io::offchain::local_storage_set(StorageKind::PERSISTENT, VERIFIER_CONFIG_KEY, &versioned);
...
pub fn validate_verifier_config(config: &VerifierConfig) -> Result<(), VerificationError> {
    ...
    if config.consensus_timestamp_tolerance_ms > 0
...
/// 生成 8 字节随机锁令牌（取 random_seed 前 8 字节 → u64）
///
/// P0-2: 替代毫秒时间戳，避免同毫秒碰撞导致误释放
fn generate_lock_token() -> u64 {
    let seed = sp_io::offchain::random_seed();
    u64::from_le_bytes(seed[..8].try_into().unwrap_or([0u8; 8]))
}

/// 尝试获取 OCW 验证锁 (M1, P0-2: 随机令牌防碰撞)
///
...
pub fn try_acquire_verify_lock(lock_id: &[u8]) -> Option<u64> {
    ...
    // 存储格式: (timestamp_ms: u64, random_token: u64)
    let new_value = (now, token).encode();
    ...
    let acquired = match existing {
        Some(ref data) if !data.is_empty() => {
            if let Ok((locked_at, _old_token)) = <(u64, u64)>::decode(&mut &data[..]) {
        ...
        other => sp_io::offchain::local_storage_compare_and_set(
            StorageKind::PERSISTENT,
            &key,
            other,
            &new_value,
...
fn register_cache_key(url_hash: &[u8]) {
    let mut keys: Vec<Vec<u8>> =
        sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, CACHE_KEYS_KEY)
    ...
    if !keys.iter().any(|k| k == url_hash) {
        ...
        sp_io::offchain::local_storage_set(StorageKind::PERSISTENT, CACHE_KEYS_KEY, &keys.encode());
    }
}
...
/// 获取当前端点配置 (NEW-6: 支持版本化存储格式, V2: endpoint_kinds 迁移)
pub fn get_endpoint_config() -> EndpointConfig {
    match sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, CUSTOM_ENDPOINTS_KEY) {
        Some(data) if data.len() >= 2 && data[0] == CONFIG_VERSION_MARKER => {
            let version = data[1];
            // V2: 当前完整格式（含 endpoint_kinds）
            if let Ok(config) = EndpointConfig::decode(&mut &data[2..]) {
                return config;
            }
            // V1: 无 endpoint_kinds 字段 — 解码旧格式后填充
            if version <= 1 {
                #[derive(Decode)]
                struct V1EndpointConfig {
                    ...
                    timeout_ms: u64,
                    timeout_race_ms: u64,
                    priority_boosts: Vec<(String, u32)>,
                }
                if let Ok(v1) = V1EndpointConfig::decode(&mut &data[2..]) {
                    return EndpointConfig {
                        endpoint_kinds: infer_endpoint_kinds(&v1.endpoints),
                        endpoints: v1.endpoints,
                        parallel_mode: v1.parallel_mode,
                        updated_at: v1.updated_at,
                        api_keys: v1.api_keys,
                        timeout_ms: v1.timeout_ms,
                        timeout_race_ms: v1.timeout_race_ms,
                        priority_boosts: v1.priority_boosts,
                    };
                }
            }
        ...
        Some(data) if !data.is_empty() => {
            // Pre-versioned format — try current, then V1 struct
            if let Ok(config) = EndpointConfig::decode(&mut &data[..]) {
                return config;
            }
            #[derive(Decode)]
            struct V1EndpointConfig {
            ...
            if let Ok(v1) = V1EndpointConfig::decode(&mut &data[..]) {
                return EndpointConfig {
                    endpoint_kinds: infer_endpoint_kinds(&v1.endpoints),
                    endpoints: v1.endpoints,
                    parallel_mode: v1.parallel_mode,
...
/// 根据端点 URL 推断 EndpointKind（V1→V2 迁移用）
fn infer_endpoint_kinds(endpoints: &[String]) -> Vec<(String, EndpointKind)> {
    endpoints
        .iter()
        .map(|ep| {
            let kind = DEFAULT_ENDPOINT_KINDS
                .iter()
                .find(|(url, _)| *url == ep.as_str())
                .map(|(_, k)| k.clone())
                .unwrap_or(EndpointKind::TronGrid); // 未知端点默认 TronGrid
            (ep.clone(), kind)
        })
        .collect()
}
...
/// 获取端点的 API 类型
pub fn get_endpoint_kind(endpoint: &str, config: &EndpointConfig) -> EndpointKind {
    config
        .endpoint_kinds
        .iter()
        ...
        .unwrap_or_else(|| {
            // 回退到默认映射
            DEFAULT_ENDPOINT_KINDS
                .iter()
                .find(|(url, _)| *url == endpoint)
                .map(|(_, k)| k.clone())
                .unwrap_or(EndpointKind::TronGrid)
        })
}
...
pub fn save_endpoint_config(config: &EndpointConfig) -> Result<(), VerificationError> {
    ...
    versioned.extend_from_slice(&config.encode());
    sp_io::offchain::local_storage_set(StorageKind::PERSISTENT, CUSTOM_ENDPOINTS_KEY, &versioned);
    Ok(())
}
...
pub fn add_endpoint(endpoint: &str, kind: EndpointKind) -> Result<(), VerificationError> {
    ...
    config.endpoints.push(endpoint_str.clone());
    config.endpoint_kinds.push((endpoint_str, kind));
...
pub fn get_api_key_for_endpoint(endpoint: &str) -> Option<String> {
...
/// 移除端点的 API Key（保留端点本身）
///
/// 场景：端点不再需要 API Key（如降级为免费访问），但仍想保留端点。
/// 返回是否实际移除了 Key。
pub fn remove_api_key(endpoint: &str) -> bool {
    let mut config = get_endpoint_config();
    let ep = String::from(endpoint);
    let before = config.api_keys.len();
    config.api_keys.retain(|(e, _)| e != &ep);
    if config.api_keys.len() < before {
        config.updated_at = current_timestamp_ms();
        let _ = save_endpoint_config(&config);
        log::info!(target: "trc20-verifier", "Removed API key for endpoint: {}", endpoint);
        true
    } else {
        false
    }
...
/// 获取已配置 API Key 的端点列表（不返回 Key 值本身，用于诊断）
pub fn get_configured_api_key_endpoints() -> Vec<String> {
    let config = get_endpoint_config();
    config.api_keys.iter().map(|(e, _)| e.clone()).collect()
}

/// 移除端点 (M1修复: 同时清理关联的 api_keys, priority_boosts, endpoint_kinds)
pub fn remove_endpoint(endpoint: &str) {
    ...
    if let Some(pos) = config.endpoints.iter().position(|e| e == &endpoint_str) {
        ...
        config.endpoint_kinds.retain(|(e, _)| e != &endpoint_str);
        config.updated_at = current_timestamp_ms();
        let _ = save_endpoint_config(&config);
...
/// 从 offchain local storage 引导/更新 API Key 配置
///
/// 节点运维人员通过 `offchain_localStorageSet` RPC 预设 API Key，
/// OCW 运行时自动读取并应用到端点配置中。
///
/// ## 存储格式
/// Key: `ocw_bootstrap_api_keys` (PERSISTENT)
/// Value: UTF-8 文本，首行为版本标识，后续每行 `endpoint=api_key`
///
/// ## 示例
/// ```text
/// v1
/// https://api.trongrid.io=your-trongrid-api-key
/// https://api.tronstack.io=your-tronstack-key
/// # 删除某端点的 Key（保留端点）：value 留空
/// https://apilist.tronscanapi.com=
/// ```
///
/// ## 更新 Key
/// 修改内容并更改首行版本号（如 v1 → v2），下个区块 OCW 自动重新应用。
///
/// ## 删除 Key
/// `endpoint=`（等号后为空）表示删除该端点的 API Key，端点本身保留。
///
/// ## 幂等保护
/// 记录已应用的版本标识，版本未变时跳过，避免每个区块重复解析。
pub fn bootstrap_api_keys() {
    let raw =
        match sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, BOOTSTRAP_API_KEYS_KEY) {
            Some(data) if !data.is_empty() => data,
            _ => return, // 没有预设 Key，正常跳过
        };

    let text = match core::str::from_utf8(&raw) {
        Ok(t) => t,
        Err(_) => {
            log::warn!(target: "trc20-verifier", "Bootstrap API keys: invalid UTF-8, skipping");
            return;
        }
    };
    ...
    let version = match lines.next() {
        ...
        _ => {
            log::warn!(target: "trc20-verifier", "Bootstrap API keys: missing version line");
            return;
        }
    };
    ...
    // 版本未变则跳过
    let applied =
        sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, BOOTSTRAP_APPLIED_VERSION_KEY);
    if let Some(ref prev) = applied {
        if prev == version.as_bytes() {
            return;
        }
    }

    // P3: 批量操作 — 循环外读一次 config，循环内只做内存修改，最后一次性写回
    let mut config = get_endpoint_config();
    ...
    for line in lines {
        ...
        if let Some((endpoint, api_key)) = line.split_once('=') {
            ...
            // P2: 空 value = 删除该端点的 Key
            if api_key.is_empty() {
                let before = config.api_keys.len();
                config.api_keys.retain(|(e, _)| e != endpoint);
                if config.api_keys.len() < before {
                    remove_count += 1;
                    dirty = true;
                    log::info!(target: "trc20-verifier",
                        "Bootstrap: removed API key for {}", endpoint);
                }
                continue;
            }

            // 确保端点已注册（默认端点在 EndpointConfig::default 中）
            let ep = String::from(endpoint);
            if !config.endpoints.contains(&ep) {
                // 先校验再添加（内联 add_endpoint 逻辑以避免中间写盘）
                let mut trial = config.clone();
                trial.endpoints.push(ep.clone());
                if let Err(e) = validate_endpoint_config(&trial) {
                    log::warn!(target: "trc20-verifier",
                        "Bootstrap: cannot add endpoint {}: {} ({})", endpoint, e, e.as_str());
                    continue;
                }
                config.endpoints.push(ep.clone());
                log::info!(target: "trc20-verifier",
                    "Bootstrap: auto-added endpoint {}", endpoint);
            }
            ...
            dirty = true;
            log::info!(target: "trc20-verifier",
                "Bootstrap: applied API key for {}", endpoint);
        }
    }
    ...
    if dirty {
        config.updated_at = current_timestamp_ms();
        if let Err(e) = save_endpoint_config(&config) {
            log::warn!(target: "trc20-verifier",
                "Bootstrap: failed to save endpoint config: {}", e);
            return;
        }
    }
    ...
    sp_io::offchain::local_storage_set(
        StorageKind::PERSISTENT,
        BOOTSTRAP_APPLIED_VERSION_KEY,
        version.as_bytes(),
    );

    log::info!(target: "trc20-verifier",
        "Bootstrap [{}]: {} key(s) set, {} key(s) removed", version, set_count, remove_count);
}

/// 获取按健康评分排序的端点列表 (L2 增强: 支持优先级加成 + 熔断过滤)
///
/// M-6 审计修复: 保证至少返回 1 个端点。如果所有端点均被隔离，
/// 选择隔离时间最早到期的端点作为降级选项，防止完全 DoS。
pub fn get_sorted_endpoints() -> Vec<String> {
    let config = get_endpoint_config();
    let mut healthy: Vec<(String, u32)> = Vec::new();
    let mut best_quarantined: Option<(String, u64)> = None;

    for e in &config.endpoints {
        if is_endpoint_quarantined(e) {
            log::debug!(target: "trc20-verifier", "Skipping quarantined endpoint: {}", e);
            // M-6: 跟踪最早解除隔离的端点作为降级备选
            let key = [QUARANTINE_PREFIX, e.as_bytes()].concat();
            let until = sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, &key)
                .and_then(|d| u64::decode(&mut &d[..]).ok())
                .unwrap_or(u64::MAX);
            match &best_quarantined {
                None => {
                    best_quarantined = Some((e.clone(), until));
                }
        ...
        let boost = config
            ...
            .map(|(_, b)| *b)
            .unwrap_or(0);
        healthy.push((e.clone(), health.score.saturating_add(boost)));
    }

    // M-6: 所有端点被隔离时，降级使用最早到期的端点
    if healthy.is_empty() {
        if let Some((fallback_ep, _)) = best_quarantined {
            log::warn!(target: "trc20-verifier",
                "M-6: All endpoints quarantined, degrading to least-quarantined: {}", fallback_ep);
            return alloc::vec![fallback_ep];
        }
    }

    healthy.sort_by(|a, b| b.1.cmp(&a.1));
    healthy.into_iter().map(|(e, _)| e).collect()
}
...
/// 检查端点级速率限制，通过则更新时间戳
///
/// P0-5: 每个端点独立限流，防止不同订单的 API 调用互相阻塞。
/// 空 endpoint 时使用全局限流（向后兼容）。
fn check_rate_limit(endpoint: &str) -> Result<(), VerificationError> {
    let config = get_verifier_config();
    if config.rate_limit_interval_ms == 0 {
        return Ok(());
    }
    let key = if endpoint.is_empty() {
        [RATE_LIMIT_PREFIX, b"_global_"].concat()
    } else {
        [RATE_LIMIT_PREFIX, endpoint.as_bytes()].concat()
    };
    let now = current_timestamp_ms();
    let last = sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, &key)
        .and_then(|d| u64::decode(&mut &d[..]).ok())
        .unwrap_or(0);
...
/// P1-6: 构造语义化缓存键（不含 min_timestamp 等易变参数）
///
/// 旧实现使用完整 URL 作为缓存键，但 min_timestamp 每次调用都在变，
/// 导致命中率几乎为零。改为 `from:to:contract:fingerprint` 语义键。
fn make_cache_key(from: &str, to: &str, contract: &str, fingerprint: Option<&str>) -> Vec<u8> {
...
/// 获取缓存的响应
fn get_cached_response(cache_key: &[u8]) -> Option<Vec<u8>> {
    let config = get_verifier_config();
    if config.cache_ttl_ms == 0 {
        return None;
    }
    let key = [RESPONSE_CACHE_PREFIX, cache_key].concat();
    ...
    log::debug!(target: "trc20-verifier", "Cache hit (age={}ms)", now.saturating_sub(timestamp));
    // M4: 记录缓存命中
    let mut m = get_verifier_metrics();
...
/// 设置响应缓存
fn set_cached_response(cache_key: &[u8], response: &[u8]) {
    let config = get_verifier_config();
    if config.cache_ttl_ms == 0 {
        return;
    }
...
fn write_audit_log(entry: &AuditLogEntry) {
    let config = get_verifier_config();
    if config.audit_log_retention == 0 {
        return;
    }

    let counter =
        sp_io::offchain::local_storage_get(StorageKind::PERSISTENT, AUDIT_LOG_COUNTER_KEY)
            .and_then(|d| u64::decode(&mut &d[..]).ok())
            .unwrap_or(0);
...
pub fn get_recent_audit_logs(max_count: u32) -> Vec<AuditLogEntry> {
    ...
    let effective =
        (max_count as u64)
            .min(counter)
            .min(if retention > 0 { retention } else { counter });
...
/// 构建 TronScan API 转账查询 URL
///
/// TronScan 使用 `/api/token_trc20/transfers` 路径，参数与 TronGrid `/v1/` 不同。
/// `toAddress` 过滤接收方，`start_timestamp` 替代 `min_timestamp`。
fn build_tronscan_transfer_url(
    to_address: &str,
    contract: &str,
    min_timestamp: u64,
    start_offset: u32,
    ...
    format!(
        "https://apilist.tronscanapi.com/api/token_trc20/transfers?\
         contract_address={}&toAddress={}&start_timestamp={}&limit=50&start={}&confirm=true",
        contract, to_address, min_timestamp, start_offset
    )
}
...
///
/// 返回 (响应体, 使用的端点名称)
/// `cache_key`: 语义化缓存键（P1-6），`None` 时不走缓存。
...
fn fetch_url_with_fallback(
    ...
    if let Some(ck) = cache_key {
        if let Some(cached) = get_cached_response(ck) {
            return Ok((cached, String::from("(cached)")));
        }
    }

    // P0-5: 速率限制移至 per-endpoint 层（fetch_url_with_key / fetch_url_parallel_race）
    ...
    // M1+S5+P1-6: 成功时写入语义缓存
    if let (Ok((ref body, _)), Some(ck)) = (&result, cache_key) {
        if !body.is_empty() && (body[0] == b'{' || body[0] == b'[') {
            set_cached_response(ck, body);
        }
    }
...
/// H3-C: 共识查询参数 — 替代预构建的 URL，允许每个端点按自身 API 构建不同 URL
struct TransferQuery<'a> {
    ...
    /// 最早区块时间戳（毫秒）
    min_timestamp: u64,
    /// TronGrid 分页令牌（仅 TronGrid 使用，TronScan 忽略）
...
fn fetch_url_with_consensus(
    ...
    // P0-5: 速率限制移至 per-endpoint 层（fetch_all_endpoints）

    let config = get_endpoint_config();
    let total_queried = config
        .endpoints
        .iter()
        .filter(|e| !is_endpoint_quarantined(e))
        .count() as u8;

    let all_responses = fetch_all_endpoints(query, &config)?;
    ...
    for (endpoint, body, response_ms) in all_responses {
        // 根据端点类型选择不同的解析器
        let kind = get_endpoint_kind(&endpoint, &config);
        let parse_result = match kind {
            EndpointKind::TronGrid => {
                extract_normalized_transfers(&body, expected_from, expected_to, usdt_contract)
            }
            EndpointKind::TronScan => extract_normalized_transfers_tronscan(
                &body,
                expected_from,
                expected_to,
                usdt_contract,
    ...
    match build_consensus(endpoint_responses, min_responses, tolerance_ms) {
        ConsensusResult::Agreed {
            ...
            // H3-A3+P1-6: 共识成功后缓存结论（语义化键）
            let consensus_cache_key =
                make_cache_key(expected_from, expected_to, usdt_contract, None);
            if !raw_body.is_empty() && (raw_body[0] == b'{' || raw_body[0] == b'[') {
                set_cached_response(&consensus_cache_key, &raw_body);
            }

            let detail = ConsensusDetail {
                consensus_mode: true,
                agreeing_endpoints: agreeing_endpoints
                    .iter()
...
fn fetch_url_parallel_race(
    ...
    let timeout = sp_io::offchain::timestamp().add(Duration::from_millis(config.timeout_race_ms));
    ...
    for endpoint in endpoints.iter() {
        ...
        // P0-5: 按端点限流（限流失败时跳过该端点，不阻塞其他端点）
        if check_rate_limit(endpoint).is_err() {
            log::debug!(target: "trc20-verifier", "Skipping rate-limited endpoint: {}", endpoint);
            continue;
        }
    ...
    for (endpoint, pending) in pending_requests {
        ...
        match pending.try_wait(timeout) {
            Ok(Ok(response)) => {
                ...
                if status_code == 200 {
                    match collect_body_limited(response) {
...
/// 收集所有成功响应用于后续共识比对。
///
/// 根据端点的 EndpointKind 构建不同的 URL:
/// - TronGrid: `/v1/accounts/{addr}/transactions/trc20?...`
/// - TronScan: `/api/token_trc20/transfers?...`
///
/// 返回: 所有成功端点的 (endpoint, body, response_ms) 列表
fn fetch_all_endpoints(
    query: &TransferQuery,
    config: &EndpointConfig,
) -> Result<Vec<(String, Vec<u8>, u32)>, VerificationError> {
    let endpoints = &config.endpoints;
    ...
    for endpoint in endpoints.iter() {
        ...
        let target_url = match kind {
            EndpointKind::TronGrid => {
                // 构建 TronGrid URL 并替换基础域名
                let trongrid_url = if let Some(fp) = query.fingerprint {
                    format!(
                        ...
                        TRONGRID_MAINNET, query.to_address, query.contract, query.min_timestamp, percent_encode_param(fp)
                    )
                } else {
                    format!(
                        "{}/v1/accounts/{}/transactions/trc20?contract_address={}&only_to=true&min_timestamp={}&limit=50&order_by=block_timestamp,desc",
                        TRONGRID_MAINNET, query.to_address, query.contract, query.min_timestamp
                    )
                };
                build_endpoint_url(&trongrid_url, endpoint)
            }
            EndpointKind::TronScan => {
                // TronScan: 不支持 fingerprint 分页，使用 start=0
                build_tronscan_transfer_url(
                    query.to_address,
                    query.contract,
                    query.min_timestamp,
                    0,
    ...
    for (endpoint, pending) in pending_requests {
        match pending.try_wait(timeout) {
            Ok(Ok(response)) => {
                ...
                // 非 200 或 body 过大/空 → 真正失败
                log::warn!(target: "trc20-verifier",
                    "Endpoint {} returned status {} or bad body", endpoint, status_code);
                let mut health = get_endpoint_health(&endpoint);
                health.record_failure();
                save_endpoint_health(&endpoint, &health);
                if health.score < 15 {
...
/// P0-3: 带大小上限的 HTTP 响应体收集
///
/// 逐块读取响应体，超过 `MAX_RESPONSE_BODY_BYTES` 时立即截断并返回错误。
/// 防止恶意端点通过超大响应做内存 DoS。
fn collect_body_limited(response: http::Response) -> Result<Vec<u8>, VerificationError> {
    let body = response.body().collect::<Vec<u8>>();
    if body.len() > MAX_RESPONSE_BODY_BYTES {
        log::warn!(target: "trc20-verifier",
            "P0-3: Response body too large ({} bytes > {} limit), rejecting",
...
/// 发送 HTTP GET 请求 (H1: 支持 API Key, L1: 可配置超时, P0-5: 按端点限流)
fn fetch_url_with_key(
    url: &str,
    endpoint: &str,
    timeout_ms: u64,
    ...
    let timeout = sp_io::offchain::timestamp().add(Duration::from_millis(timeout_ms));
    ...
    let body = collect_body_limited(response)?;
...
pub enum ConsensusResult {
    ...
    /// 所有端点返回不同数据，无法达成共识
    NoConsensus { responses: Vec<EndpointResponse> },
...
pub fn extract_normalized_transfers(
    ...
    let response_str =
        core::str::from_utf8(response).map_err(|_| VerificationError::InvalidUtf8)?;

    let json_value = parse_json(response_str).map_err(|_| VerificationError::InvalidJson)?;
...
/// 从 TronScan 原始响应中提取标准化转账列表（用于共识比对）
///
/// TronScan 响应格式:
/// ```json
/// {
...
///       "tokenInfo": { "tokenId": "TR7NHq..." }
///     }
///   ]
/// }
/// ```
pub fn extract_normalized_transfers_tronscan(
    response: &[u8],
    expected_from: &str,
    expected_to: &str,
    usdt_contract: &str,
    ...
    // TronScan 用 `total` 字段表示结果数量（无 `success` 字段）
    // 校验 total >= 0 作为合法性检查
    match json_obj_get(root_obj, "total") {
        Some(JsonValue::Number(n)) if !n.negative => {}
        _ => return Err(VerificationError::InvalidJson),
    }

    let data_array = match json_obj_get(root_obj, "token_transfers") {
        Some(JsonValue::Array(arr)) => arr,
        _ => return Err(VerificationError::InvalidJson),
    };
    ...
    for entry_value in data_array.iter() {
        ...
        // TronScan 字段: from_address（TronGrid 用 from）
        let from_addr = match json_obj_get_str(entry_obj, "from_address") {
            Some(addr) if addr == expected_from => addr,
            _ => continue,
        };

        // TronScan 字段: to_address（TronGrid 用 to）
        let to_addr = json_obj_get_str(entry_obj, "to_address").unwrap_or_default();
        if !expected_to.is_empty() && to_addr != expected_to {
            continue;
        }

        // 合约地址: 优先用 contractAddress，回退到 tokenInfo.tokenId
        let contract_addr = json_obj_get_str(entry_obj, "contractAddress").or_else(|| {
            json_obj_get(entry_obj, "tokenInfo")
                .and_then(|ti| ti.as_object())
                .and_then(|ti_obj| json_obj_get_str(ti_obj, "tokenId"))
        });
        let contract_addr = match contract_addr {
            Some(addr) if addr == usdt_contract => addr,
            _ => continue,
        ...
        // 金额: TronScan 用 quant（字符串→u64）
        let amount = match json_obj_get_u64(entry_obj, "quant") {
            Some(a) if a > 0 => a,
            _ => continue,
        };

        // 交易哈希: transaction_id（两个 API 同名）
        let tx_hash = json_obj_get_str(entry_obj, "transaction_id")
            .map(|s| s.into_bytes())
            .unwrap_or_default();
        if tx_hash.is_empty() {
        ...
        // 区块时间戳: TronScan 用 block_ts（TronGrid 用 block_timestamp）
        let block_timestamp = json_obj_get_u64(entry_obj, "block_ts").unwrap_or(0);
...
/// - `lock_context`: 并发锁上下文（如 trade_id 字节），追加到锁 key 防止同买卖对多单互斥。
///   空切片时退化为旧行为 `from:to`。
///
/// ## 返回
...
pub fn verify_trc20_by_transfer(
    ...
    let from_str =
        core::str::from_utf8(from_address).map_err(|_| VerificationError::InvalidUtf8)?;
    ...
    // M1+NEW-7+P0-1: OCW 并发锁 — lock_context 防止同买卖对多单互斥
    let lock_id = if lock_context.is_empty() {
        [from_address, b":", to_address].concat()
    } else {
        [from_address, b":", to_address, b":", lock_context].concat()
    };
    ...
    let paging_result: Result<TransferSearchResult, VerificationError> = (|| {
        ...
        loop {
            // 构建 TronGrid URL（非共识模式 fallback 仍用此）
            ...
            // P1-6: 语义化缓存键（不含 min_timestamp）
            let cache_key = make_cache_key(from_str, to_str, &contract, fingerprint.as_deref());
            ...
            let (response, endpoint_used) = if use_consensus {
                ...
                let (body, ep, detail) = fetch_url_with_consensus(
                    &query,
                    from_str,
                    to_str,
                    &contract,
                ...
                fetch_url_with_fallback(&url, Some(&cache_key))?
            };
            ...
            combined
                .matched_transfers
                .retain(|t| !is_tx_hash_used(&t.tx_hash));
            if combined.matched_transfers.is_empty() {
                ...
                combined.amount_status =
                    calculate_amount_status(expected_amount, filtered_total, tolerance_bps);
            ...
            let enough = matches!(
                combined.amount_status,
                AmountStatus::Exact | AmountStatus::Overpaid { .. }
            );
    ...
    let duration_ms = sp_io::offchain::timestamp()
        .unix_millis()
        .saturating_sub(verify_start);
...
fn merge_transfer_results(
    combined: &mut TransferSearchResult,
    page: &TransferSearchResult,
    expected_amount: u64,
    tolerance_bps: u32,
) {
    ...
    for t in &page.matched_transfers {
        let is_dup = !t.tx_hash.is_empty()
            && combined
                .matched_transfers
                .iter()
                .any(|existing| existing.tx_hash == t.tx_hash);
        if !is_dup {
...
fn parse_trc20_transfer_list_paged(
    ...
    result.amount_status =
        calculate_amount_status(expected_amount, total_matched_amount, tolerance_bps);
...
pub fn calculate_amount_status(expected: u64, actual: u64, tolerance_bps: u32) -> AmountStatus {
    ...
    if actual >= min_exact && actual <= max_exact {
        ...
        AmountStatus::Overpaid {
            excess: actual.saturating_sub(expected),
        }
    } else if actual >= severe_threshold {
        AmountStatus::Underpaid {
            shortage: expected.saturating_sub(actual),
        }
    } else if actual > 0 {
...
/// 通过单笔 tx_hash 交叉验证 TRC20 转账
///
/// 查询路径: GET {endpoint}/wallet/gettransactioninfobyid?value={hash}
/// 解析 `log[]` 中的 Transfer event：topics[1]=from, topics[2]=to, data=amount
///
/// ## 参数
/// - `tx_hash`: TRON 交易哈希（十六进制字节）
/// - `expected_to`: 期望收款地址（Base58 TRON 地址字节）
/// - `expected_from`: 可选的期望发款地址（Base58 TRON 地址字节）
///
/// ## 返回
/// - `Ok(TxVerificationResult)`: 验证结果
/// - `Err(VerificationError)`: HTTP 或解析错误
pub fn verify_trc20_by_txhash(
    tx_hash: &[u8],
    expected_to: &[u8],
    expected_from: Option<&[u8]>,
) -> Result<TxVerificationResult, VerificationError> {
    let verifier_config = get_verifier_config();
    if !verifier_config.enabled {
        return Err(VerificationError::VerifierDisabled);
    }

    let hash_str = core::str::from_utf8(tx_hash).map_err(|_| VerificationError::InvalidUtf8)?;
    let to_str = core::str::from_utf8(expected_to).map_err(|_| VerificationError::InvalidUtf8)?;
    let from_str_opt = expected_from
        .map(|f| core::str::from_utf8(f))
        .transpose()
    ...
    let url = format!(
        ...
        TRONGRID_MAINNET, hash_str
    );

    let (body, _endpoint) = fetch_url_with_fallback(&url, None)?;

    if body.is_empty() {
        return Err(VerificationError::EmptyResponse);
    }

    let json_str = core::str::from_utf8(&body).map_err(|_| VerificationError::InvalidUtf8)?;
    let json = parse_json(json_str).map_err(|_| VerificationError::InvalidJson)?;

    parse_transaction_info_response(&json, to_str, from_str_opt, &contract)
}

/// 解析 gettransactioninfobyid JSON 响应
///
/// 响应格式:
/// ```json
/// {
...
///   "log": [
///     {
///       "address": "合约地址(hex,无0x前缀)",
///       "topics": ["Transfer事件签名", "from(32字节hex)", "to(32字节hex)"],
///       "data": "amount(32字节hex)"
///     }
///   ]
/// }
/// ```
fn parse_transaction_info_response(
    json: &JsonValue,
    expected_to: &str,
    expected_from: Option<&str>,
    usdt_contract_base58: &str,
) -> Result<TxVerificationResult, VerificationError> {
    let obj = match json {
    ...
    // 空响应（tx 未上链或不存在）：id 不存在
    let has_id = obj
        .iter()
        .any(|(k, _)| matches!(k.iter().collect::<String>().as_str(), "id"));
    if !has_id {
    ...
    let logs = obj
        .iter()
        .find(|(k, _)| k.iter().collect::<String>() == "log")
    ...
    // 将 expected TRON base58 地址转为小写 hex（20字节，无 0x/41 前缀）
    let expected_to_hex = tron_base58_to_hex_20(expected_to);
    let expected_from_hex = expected_from.map(tron_base58_to_hex_20);

    // 将 USDT 合约 base58 地址转为小写 hex（20字节）
    let contract_hex = tron_base58_to_hex_20(usdt_contract_base58);

    for log_entry in logs.iter() {
        let log_obj = match log_entry {
            JsonValue::Object(o) => o,
            _ => continue,
        };

        // 读取 log.address（合约地址 hex，无前缀）
        let log_address = log_obj
            .iter()
            .find(|(k, _)| k.iter().collect::<String>() == "address")
            .and_then(|(_, v)| match v {
                JsonValue::String(s) => Some(s.iter().collect::<String>()),
                _ => None,
            });

        let log_address = match log_address {
            Some(a) => a.to_lowercase(),
            None => continue,
        };

        // 读取 topics 数组
        let topics: Vec<String> = log_obj
            .iter()
            .find(|(k, _)| k.iter().collect::<String>() == "topics")
            .and_then(|(_, v)| match v {
                JsonValue::Array(arr) => Some(arr),
                _ => None,
            })
        ...
        // 检查 Transfer 事件签名
        if topics[0].to_lowercase() != transfer_sig {
            continue;
        }

        // 读取 data（amount hex）
        let data_hex = log_obj
            .iter()
        ...
        let data_hex = match data_hex {
            Some(d) => d,
            None => continue,
        };

        // topics[1] = from（32字节 hex，左填充0，取后20字节 = 后40字符）
        let from_hex_40 = if topics[1].len() >= 40 {
            topics[1][topics[1].len() - 40..].to_lowercase()
        } else {
            topics[1].to_lowercase()
        };

        // topics[2] = to
        let to_hex_40 = if topics[2].len() >= 40 {
            topics[2][topics[2].len() - 40..].to_lowercase()
        } else {
            topics[2].to_lowercase()
        };

        // 检查合约地址
        let contract_match = if let Some(ref ch) = contract_hex {
            log_address == *ch
        } else {
            false
        };
        ...
        // 检查 to 地址
        let to_match = if let Some(ref th) = expected_to_hex {
            to_hex_40 == *th
        } else {
            false
        };
        ...
        // 检查 from 地址（可选）
        let from_match = match &expected_from_hex {
            Some(Some(fh)) => from_hex_40 == *fh,
            _ => true, // 不检查 from
        };

        // 解析金额（hex → u64）
        let amount = u64::from_str_radix(data_hex.trim_start_matches('0'), 16).unwrap_or(0);

        if contract_match && to_match {
            return Ok(TxVerificationResult {
                found: true,
                amount,
                from_match,
...
/// 将 TRON Base58Check 地址转为小写 20 字节 hex 字符串
///
/// TRON 地址 = 0x41 + 20字节地址 + 4字节校验 → Base58
/// 我们需要去掉 0x41 前缀，取中间 20 字节
///
/// 简化实现：直接用 bs58 解码不可用于 no_std，
/// 改用比对策略 — 将 hex 地址和 topics 中的 hex 直接比较
fn tron_base58_to_hex_20(base58_addr: &str) -> Option<String> {
    // TRON Base58Check 解码需要完整实现
    // 这里用简化路径：如果地址以 'T' 开头且长度 34，
    // 尝试 Base58 解码（manual implementation for no_std）
    if base58_addr.len() != 34 || !base58_addr.starts_with('T') {
        return None;
    }
    ...
    let mut result = [0u8; 25]; // 1 byte prefix + 20 bytes address + 4 bytes checksum
    let mut scratch = [0u32; 25];

    for ch in base58_addr.bytes() {
    ...
    // result[0] should be 0x41 for TRON mainnet
    if result[0] != 0x41 {
        return None;
    }

    // 取 20 字节地址部分（跳过 0x41 前缀）
    let addr_bytes = &result[1..21];
...
mod tests {
    ...
    fn test_parse_transfer_list_overpaid() {
        with_offchain_ext(|| {
            ...
            assert_eq!(
                result.amount_status,
                AmountStatus::Overpaid { excess: 5_000_000 }
            );
        });
    }
    ...
    fn test_calculate_amount_status() {
        assert_eq!(
            calculate_amount_status(1_000_000, 1_000_000, 50),
            AmountStatus::Exact
        );
        assert_eq!(
            calculate_amount_status(1_000_000, 1_004_000, 50),
            AmountStatus::Exact
        ); // within 0.5%
        assert_eq!(
            calculate_amount_status(1_000_000, 1_010_000, 50),
            AmountStatus::Overpaid { excess: 10_000 }
        );
        assert_eq!(
            calculate_amount_status(1_000_000, 800_000, 50),
            AmountStatus::Underpaid { shortage: 200_000 }
        );
        assert_eq!(
            calculate_amount_status(1_000_000, 400_000, 50),
            AmountStatus::SeverelyUnderpaid { shortage: 600_000 }
        );
        assert_eq!(
            calculate_amount_status(1_000_000, 0, 50),
            AmountStatus::Invalid
        );
    }
    ...
    fn l2_calculate_amount_status_expected_zero_returns_invalid() {
        ...
        // 修复前返回 Overpaid
        assert_eq!(
            calculate_amount_status(0, 1_000_000, 50),
            AmountStatus::Invalid
        );
    ...
    fn h8_add_endpoint_rejects_http() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("http://api.example.com", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must use HTTPS"
                ))
            );
        });
    ...
    fn h8_add_endpoint_rejects_short_url() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://x", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint URL length must be 10-256 characters"
                ))
            );
    ...
    fn h8_add_endpoint_rejects_whitespace() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://api.example .com", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint URL must not contain whitespace"
                ))
            );
    ...
    fn l6_hex_to_bytes_handles_0x_prefix() {
        // L6修复: 0x 前缀应被自动去除
        assert_eq!(
            hex_to_bytes("0x1234abcd").unwrap(),
    ...
    fn h2_merge_transfer_results_cumulative_amount_status() {
        ...
        let page1 = TransferSearchResult {
            ...
            amount_status: AmountStatus::SeverelyUnderpaid {
                shortage: 5_000_000,
            },
        ...
        let page2 = TransferSearchResult {
            ...
            amount_status: AmountStatus::Underpaid {
                shortage: 4_000_000,
            },
    ...
    fn m1_remove_endpoint_cleans_api_key_and_priority() {
        with_offchain_ext(|| {
            ...
            add_endpoint(ep, EndpointKind::TronGrid).unwrap();
    ...
    fn h1_validate_config_rejects_invalid_contract_address() {
        ...
        assert!(matches!(
            err,
            VerificationError::InvalidTronAddress(_) | VerificationError::InvalidConfig(_)
        ));
    }
    ...
    fn h3_build_endpoint_url_strip_prefix() {
        ...
        assert_eq!(
            result,
            "https://custom-api.example.com/v1/accounts/TAddr/transactions/trc20"
        );
    }
    ...
    fn h1r3_api_key_exact_match_prevents_prefix_leak() {
        with_offchain_ext(|| {
            ...
            add_endpoint(trusted, EndpointKind::TronGrid).unwrap();
            add_endpoint(malicious, EndpointKind::TronGrid).unwrap();
            set_api_key(trusted, "secret-key-123").unwrap();

            // H1-R3修复: 恶意端点不应获取到信任端点的 API Key
            assert_eq!(
                get_api_key_for_endpoint(trusted),
    ...
    fn m2r3_set_api_key_rejects_nonexistent_endpoint() {
        with_offchain_ext(|| {
            ...
            assert!(matches!(
                result.unwrap_err(),
                VerificationError::InvalidEndpointUrl(_)
            ));
        });
    }
    ...
    fn s3_add_endpoint_rejects_localhost() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://localhost/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
    ...
    fn s3_add_endpoint_rejects_loopback_ip() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://127.0.0.1/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
    ...
    fn s3_add_endpoint_rejects_private_ips() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://10.0.0.1/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
            assert_eq!(
                add_endpoint("https://192.168.1.1/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
            assert_eq!(
                add_endpoint("https://172.16.0.1/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
    ...
    fn s3_add_endpoint_rejects_link_local() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://169.254.1.1/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
    ...
    fn s3_add_endpoint_allows_public_domain() {
        with_offchain_ext(|| {
            assert!(add_endpoint("https://api.trongrid.io", EndpointKind::TronGrid).is_ok());
        });
    }
    ...
    fn s5_cache_rejects_non_json_response() {
        with_offchain_ext(|| {
            ...
            let cache_key = b"TFrom:TTo:contract:test";
            set_cached_response(cache_key, b"<html>error</html>");
            // 缓存已写入（低层 set_cached_response 不过滤），但可以读取
            let cached = get_cached_response(cache_key);
    ...
    fn new1_ssrf_ipv4_mapped_ipv6_loopback() {
        ...
        assert!(is_private_or_loopback_url("https://[::FFFF:127.0.0.1]/api"));
        assert!(is_private_or_loopback_url(
    ...
    fn new1_add_endpoint_rejects_ipv4_mapped_ipv6() {
        with_offchain_ext(|| {
            assert_eq!(
                add_endpoint("https://[::ffff:127.0.0.1]/api", EndpointKind::TronGrid),
                Err(VerificationError::InvalidEndpointUrl(
                    "Endpoint must not target private or loopback addresses"
                ))
            );
    ...
    fn new9_amount_tolerance_configurable() {
        ...
        assert_eq!(
            calculate_amount_status(1_000_000, 1_040_000, 500),
            AmountStatus::Exact
        );

        // 零容差: 精确匹配才算 Exact
        assert_eq!(
            calculate_amount_status(1_000_000, 1_000_000, 0),
            AmountStatus::Exact
        );
        assert_eq!(
    ...
    fn p1_sorted_endpoints_skip_quarantined() {
        with_offchain_ext(|| {
            add_endpoint("https://api.trongrid.io", EndpointKind::TronGrid).unwrap();
            add_endpoint("https://api.tronstack.io", EndpointKind::TronGrid).unwrap();
    ...
    fn p1_verification_error_variants_display() {
        assert_eq!(
            VerificationError::VerifierDisabled.as_str(),
            "Verifier is globally disabled"
        );
        assert_eq!(
            VerificationError::TxHashAlreadyUsed.as_str(),
    ...
    fn h3c_insufficient_responses_error_variant() {
        ...
        assert_eq!(
            err.as_str(),
            "Insufficient endpoint responses for consensus"
        );
        assert_eq!(
            format!("{}", err),
            "Insufficient endpoint responses for consensus"
    ...
    fn h3c_consensus_result_no_consensus() {
        let result = ConsensusResult::NoConsensus { responses: vec![] };
    ...
    fn h3c_extract_normalized_transfers_basic() {
        ...
        let transfers = extract_normalized_transfers(
            &response,
            "TSender1",
            "",
            "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t",
    ...
    fn h3c_extract_normalized_transfers_filters_wrong_contract() {
        let response = make_trongrid_response(&[
            ...
            (
                ...
                5_000_000,
                1700000001000,
                "TFakeContract12345678901234567890",
            ),
        ]);
        let transfers = extract_normalized_transfers(
            &response,
    ...
    fn h3c_transfers_agree_identical() {
        let a = vec![
            make_normalized("tx1", 100, 1000),
            make_normalized("tx2", 200, 2000),
        ];
        let b = vec![
            make_normalized("tx1", 100, 1000),
            make_normalized("tx2", 200, 2000),
    ...
    fn h3c_build_consensus_insufficient_responses() {
        let t = vec![make_normalized("tx1", 1000, 5000)];
        let responses = vec![make_endpoint_response("ep1", t)];
        match build_consensus(responses, 2, 3000) {
    ...
    #[test]
    fn h3c_consensus_config_routing_default_on() {
        // P0-4: 默认 consensus_enabled=true，验证主函数应走共识路径
        let config = VerifierConfig::default();
        assert!(config.consensus_enabled);
    }
    ...
    // ==================== Bootstrap API Keys 测试 ====================

    #[test]
    fn bootstrap_no_data_skips() {
        with_offchain_ext(|| {
            // 无预设数据 → 跳过，EndpointConfig 不变
            bootstrap_api_keys();
            let config = get_endpoint_config();
    ...
    #[test]
    fn bootstrap_normal_v1_applies() {
        with_offchain_ext(|| {
            let data = b"v1\nhttps://api.trongrid.io=my-key-123";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
            );
            bootstrap_api_keys();
            assert_eq!(
                get_api_key_for_endpoint("https://api.trongrid.io"),
                Some("my-key-123".into()),
            );
        });
    }

    #[test]
    fn bootstrap_idempotent_same_version() {
        with_offchain_ext(|| {
            let data = b"v1\nhttps://api.trongrid.io=key-a";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
            ...
            // 手动改 Key，再次 bootstrap 同版本 → 不覆盖
            set_api_key("https://api.trongrid.io", "key-manual").unwrap();
            bootstrap_api_keys();
            assert_eq!(
                get_api_key_for_endpoint("https://api.trongrid.io"),
                Some("key-manual".into()),
            );
        });
    }

    #[test]
    fn bootstrap_version_change_reapplies() {
        with_offchain_ext(|| {
            let data_v1 = b"v1\nhttps://api.trongrid.io=key-v1";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data_v1,
    ...
    #[test]
    fn bootstrap_invalid_utf8_skips() {
        with_offchain_ext(|| {
            // 非法 UTF-8 数据
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
    ...
    #[test]
    fn bootstrap_missing_version_line_skips() {
        with_offchain_ext(|| {
            // 首行为空
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                b"\nhttps://api.trongrid.io=key",
            );
            bootstrap_api_keys();
            let config = get_endpoint_config();
            assert!(config.api_keys.is_empty());
    ...
    #[test]
    fn bootstrap_comment_and_blank_lines_skipped() {
        with_offchain_ext(|| {
            let data = b"v1\n\n# this is a comment\n  \nhttps://api.trongrid.io=actual-key\n# another comment";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
            ...
            bootstrap_api_keys();
            let keys = get_configured_api_key_endpoints();
            assert_eq!(keys.len(), 1);
            assert_eq!(keys[0], "https://api.trongrid.io");
        });
    }

    #[test]
    fn bootstrap_auto_adds_unknown_endpoint() {
        with_offchain_ext(|| {
            let new_ep = "https://api.newprovider.io";
            let data = format!("v1\n{}=new-key", new_ep);
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
            ...
            bootstrap_api_keys();
            let config = get_endpoint_config();
            assert!(config.endpoints.contains(&String::from(new_ep)));
            assert_eq!(get_api_key_for_endpoint(new_ep), Some("new-key".into()));
        });
    }

    #[test]
    fn bootstrap_rejects_non_https_endpoint() {
        with_offchain_ext(|| {
            let data = b"v1\nhttp://evil.com=bad-key\nhttps://api.trongrid.io=good-key";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
            ...
            bootstrap_api_keys();
            // http://evil.com should be rejected, trongrid should succeed
            assert_eq!(get_api_key_for_endpoint("http://evil.com"), None);
            assert_eq!(
                get_api_key_for_endpoint("https://api.trongrid.io"),
                Some("good-key".into()),
            );
    ...
    #[test]
    fn bootstrap_api_key_with_equals_sign() {
        with_offchain_ext(|| {
            // Key 中包含 '=' 字符（如 base64 编码的 Key）
            let data = b"v1\nhttps://api.trongrid.io=abc123==def";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
            ...
            assert_eq!(
                get_api_key_for_endpoint("https://api.trongrid.io"),
                Some("abc123==def".into()),
            );
        });
    }

    #[test]
    fn bootstrap_whitespace_trimmed() {
        with_offchain_ext(|| {
            let data = b"v1\n  https://api.trongrid.io  =  my-key  \n";
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
                BOOTSTRAP_API_KEYS_KEY,
                data,
    ...
    #[test]
    fn bootstrap_delete_key_empty_value() {
        with_offchain_ext(|| {
            // 先设置一个 Key
            set_api_key("https://api.trongrid.io", "old-key").unwrap();
            assert_eq!(
                get_api_key_for_endpoint("https://api.trongrid.io"),
                Some("old-key".into()),
            );

            // bootstrap 带空 value = 删除
            ...
            bootstrap_api_keys();
            assert_eq!(get_api_key_for_endpoint("https://api.trongrid.io"), None);
            // 端点本身应保留
            let config = get_endpoint_config();
            assert!(config
                .endpoints
                .contains(&String::from("https://api.trongrid.io")));
        });
    }
    ...
    #[test]
    fn remove_api_key_existing() {
        with_offchain_ext(|| {
            set_api_key("https://api.trongrid.io", "my-key").unwrap();
            assert!(remove_api_key("https://api.trongrid.io"));
            assert_eq!(get_api_key_for_endpoint("https://api.trongrid.io"), None);
            // 端点仍在
            let config = get_endpoint_config();
            assert!(config
    ...
    #[test]
    fn remove_api_key_nonexistent() {
        with_offchain_ext(|| {
            assert!(!remove_api_key("https://api.trongrid.io"));
        });
    }

    // ==================== get_configured_api_key_endpoints 测试 ====================

    #[test]
    fn get_configured_api_key_endpoints_empty_and_populated() {
        with_offchain_ext(|| {
            assert!(get_configured_api_key_endpoints().is_empty());

            set_api_key("https://api.trongrid.io", "k1").unwrap();
            set_api_key("https://apilist.tronscanapi.com", "k2").unwrap();

            let eps = get_configured_api_key_endpoints();
            assert_eq!(eps.len(), 2);
            assert!(eps.contains(&String::from("https://api.trongrid.io")));
            assert!(eps.contains(&String::from("https://apilist.tronscanapi.com")));
        });
    }
    ...
    #[test]
    fn tronscan_build_transfer_url() {
        let url = build_tronscan_transfer_url(
            "TReceiverAddr",
            "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t",
            1700000000000,
            0,
        );
        assert!(url.starts_with("https://apilist.tronscanapi.com/api/token_trc20/transfers?"));
        assert!(url.contains("contract_address=TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"));
        assert!(url.contains("toAddress=TReceiverAddr"));
        assert!(url.contains("start_timestamp=1700000000000"));
        assert!(url.contains("limit=50"));
        assert!(url.contains("start=0"));
        assert!(url.contains("confirm=true"));
    }

    #[test]
    fn tronscan_build_transfer_url_with_offset() {
        let url = build_tronscan_transfer_url("TAddr", "TContract", 1700000000000, 50);
        assert!(url.contains("start=50"));
    }

    #[test]
    fn tronscan_extract_normalized_transfers_basic() {
        let json = r#"{
            "total": 1,
            "token_transfers": [
                {
        ...
        let result = extract_normalized_transfers_tronscan(
            json.as_bytes(),
            "TSender111",
            "TReceiver222",
            "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t",
        ...
        assert_eq!(result[0].tx_hash, b"abc123def456");
        assert_eq!(result[0].from, "TSender111");
        assert_eq!(result[0].to, "TReceiver222");
        assert_eq!(result[0].amount, 50_000_000);
        assert_eq!(result[0].block_timestamp, 1700000000000);
        assert_eq!(
    ...
    #[test]
    fn tronscan_extract_filters_wrong_from() {
        let json = r#"{
            "total": 1,
            "token_transfers": [
                {
                    "transaction_id": "abc123",
                    "from_address": "TWrongSender",
                    "to_address": "TReceiver222",
                    "quant": "50000000",
                    "block_ts": 1700000000000,
                    "contractAddress": "TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t"
    ...
    #[test]
    fn tronscan_extract_filters_wrong_contract() {
        let json = r#"{
            "total": 1,
            "token_transfers": [
                {
                    ...
                    "quant": "50000000",
                    "block_ts": 1700000000000,
                    "contractAddress": "TWrongContract"
                }
            ]
        }"#;
        let result = extract_normalized_transfers_tronscan(
    ...
    #[test]
    fn tronscan_extract_uses_tokeninfo_fallback() {
        // contractAddress 不存在时，回退到 tokenInfo.tokenId
        let json = r#"{
            "total": 1,
            "token_transfers": [
                {
        ...
        .unwrap();
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].amount, 100_000_000);
    }

    #[test]
    fn tronscan_extract_rejects_invalid_json() {
        let bad = b"not json";
        assert_eq!(
            extract_normalized_transfers_tronscan(bad, "T1", "T2", "TC"),
            Err(VerificationError::InvalidJson)
        );
    }

    #[test]
    fn tronscan_extract_rejects_missing_total() {
        let json = r#"{"token_transfers": []}"#;
        assert_eq!(
            extract_normalized_transfers_tronscan(json.as_bytes(), "T1", "T2", "TC"),
            Err(VerificationError::InvalidJson)
        );
    }

    #[test]
    fn tronscan_extract_rejects_missing_token_transfers() {
        let json = r#"{"total": 0}"#;
        assert_eq!(
            extract_normalized_transfers_tronscan(json.as_bytes(), "T1", "T2", "TC"),
            Err(VerificationError::InvalidJson)
    ...
    #[test]
    fn tronscan_extract_sorted_by_tx_hash() {
        let json = r#"{
            "total": 2,
            "token_transfers": [
                {
                    "transaction_id": "zzz_last",
        ...
        let result =
            extract_normalized_transfers_tronscan(json.as_bytes(), "TSender", "TRecv", "TUSDT")
                .unwrap();
        assert_eq!(result.len(), 2);
        assert_eq!(result[0].tx_hash, b"aaa_first");
        assert_eq!(result[1].tx_hash, b"zzz_last");
    }

    #[test]
    fn cross_source_consensus_trongrid_tronscan_agree() {
        // 同一笔转账的 TronGrid 和 TronScan JSON 格式
        let trongrid_json = r#"{
            "success": true,
            "data": [
                {
            ...
            "token_transfers": [
                {
                    ...
                    "quant": "50000000",
                    "block_ts": 1700000000000,
                    "contractAddress": "TUSDT_CONTRACT"
                }
            ]
        }"#;

        let trongrid_transfers = extract_normalized_transfers(
            trongrid_json.as_bytes(),
            "TSender",
            "TReceiver",
            "TUSDT_CONTRACT",
        )
        .unwrap();
        let tronscan_transfers = extract_normalized_transfers_tronscan(
            tronscan_json.as_bytes(),
            "TSender",
            "TReceiver",
            "TUSDT_CONTRACT",
        )
        ...
        assert_eq!(trongrid_transfers.len(), 1);
        assert_eq!(tronscan_transfers.len(), 1);
        // 标准化后应该完全一致
        assert!(transfers_agree(
            &trongrid_transfers,
            &tronscan_transfers,
            3_000
        ));
    ...
    #[test]
    fn cross_source_consensus_timestamp_tolerance() {
        // TronScan 的 block_ts 可能有微小偏差
        let trongrid_json = r#"{
            "success": true,
            "data": [
                {
        ...
        let tg = extract_normalized_transfers(
            trongrid_json.as_bytes(),
            "TSender",
            "TReceiver",
            "TUSDT_CONTRACT",
        ...
        .unwrap();
        let ts = extract_normalized_transfers_tronscan(
            tronscan_json.as_bytes(),
            "TSender",
            "TReceiver",
            "TUSDT_CONTRACT",
        ...
        // 2s 偏差，在 3s tolerance 内应 agree
        assert!(transfers_agree(&tg, &ts, 3_000));
        // 但 1s tolerance 应 disagree
        assert!(!transfers_agree(&tg, &ts, 1_000));
    }

    #[test]
    fn endpoint_config_v1_migration_fills_kinds() {
        with_offchain_ext(|| {
            // 模拟 V1 格式的 EndpointConfig（无 endpoint_kinds 字段）
            #[derive(Encode)]
            struct V1EndpointConfig {
                endpoints: Vec<String>,
                parallel_mode: bool,
            ...
            let v1 = V1EndpointConfig {
                endpoints: alloc::vec![
                    String::from("https://api.trongrid.io"),
                    String::from("https://apilist.tronscanapi.com"),
                ],
                parallel_mode: true,
                updated_at: 12345,
                api_keys: Vec::new(),
                timeout_ms: 10_000,
                timeout_race_ms: 5_000,
                priority_boosts: Vec::new(),
            };

            // 写入 V1 版本标记的配置
            let mut versioned = alloc::vec![CONFIG_VERSION_MARKER, 1u8];
            versioned.extend_from_slice(&v1.encode());
            sp_io::offchain::local_storage_set(
                StorageKind::PERSISTENT,
            ...
            assert_eq!(config.endpoint_kinds.len(), 2);

            // 检查正确的 kind 映射
            let trongrid_kind = config
                .endpoint_kinds
                .iter()
                .find(|(ep, _)| ep == "https://api.trongrid.io")
                .map(|(_, k)| k.clone());
            assert_eq!(trongrid_kind, Some(EndpointKind::TronGrid));

            let tronscan_kind = config
                .endpoint_kinds
                .iter()
                .find(|(ep, _)| ep == "https://apilist.tronscanapi.com")
                .map(|(_, k)| k.clone());
            assert_eq!(tronscan_kind, Some(EndpointKind::TronScan));
        });
    }

    #[test]
    fn endpoint_config_v1_unknown_endpoint_defaults_to_trongrid() {
        with_offchain_ext(|| {
            #[derive(Encode)]
            struct V1EndpointConfig {
                endpoints: Vec<String>,
            ...
            let v1 = V1EndpointConfig {
                endpoints: alloc::vec![
                    String::from("https://api.trongrid.io"),
                    String::from("https://api.unknown-endpoint.io"),
                ],
                parallel_mode: true,
                updated_at: 0,
                api_keys: Vec::new(),
            ...
            let unknown_kind = config
                .endpoint_kinds
                .iter()
                .find(|(ep, _)| ep == "https://api.unknown-endpoint.io")
                .map(|(_, k)| k.clone());
            // 未知端点默认为 TronGrid
            assert_eq!(unknown_kind, Some(EndpointKind::TronGrid));
        });
    }

    #[test]
    fn default_endpoints_are_trongrid_and_tronscan() {
        assert_eq!(DEFAULT_ENDPOINTS.len(), 2);
        assert_eq!(DEFAULT_ENDPOINTS[0], "https://api.trongrid.io");
        assert_eq!(DEFAULT_ENDPOINTS[1], "https://apilist.tronscanapi.com");

        assert_eq!(DEFAULT_ENDPOINT_KINDS.len(), 2);
        assert_eq!(
            DEFAULT_ENDPOINT_KINDS[0],
            ("https://api.trongrid.io", EndpointKind::TronGrid)
        );
        assert_eq!(
            DEFAULT_ENDPOINT_KINDS[1],
            ("https://apilist.tronscanapi.com", EndpointKind::TronScan)
        );
    }

    #[test]
    fn default_endpoint_config_has_kinds() {
        with_offchain_ext(|| {
            let config = EndpointConfig::default();
            assert_eq!(config.endpoints.len(), 2);
            assert_eq!(config.endpoint_kinds.len(), 2);
            assert!(config
                .endpoint_kinds
                .iter()
                .any(|(ep, k)| ep == "https://api.trongrid.io" && *k == EndpointKind::TronGrid));
            assert!(config
                .endpoint_kinds
                .iter()
                .any(|(ep, k)| ep == "https://apilist.tronscanapi.com"
                    && *k == EndpointKind::TronScan));
        });
    }

    #[test]
    fn get_endpoint_kind_returns_correct_kind() {
        with_offchain_ext(|| {
            let config = get_endpoint_config();
            assert_eq!(
                get_endpoint_kind("https://api.trongrid.io", &config),
                EndpointKind::TronGrid
            );
            assert_eq!(
                get_endpoint_kind("https://apilist.tronscanapi.com", &config),
                EndpointKind::TronScan
            );
            // 未知端点默认 TronGrid
            assert_eq!(
                get_endpoint_kind("https://unknown.example.com", &config),
                EndpointKind::TronGrid
            );
        });
    }
    ...
    #[test]
    fn add_endpoint_with_kind_persisted() {
        with_offchain_ext(|| {
            add_endpoint("https://api.custom-tron.io", EndpointKind::TronScan).unwrap();
            let config = get_endpoint_config();
            assert!(config
                .endpoints
                .contains(&String::from("https://api.custom-tron.io")));
            let kind = config
                .endpoint_kinds
                .iter()
                .find(|(ep, _)| ep == "https://api.custom-tron.io")
                .map(|(_, k)| k.clone());
            assert_eq!(kind, Some(EndpointKind::TronScan));
        });
    }

    #[test]
    fn remove_endpoint_cleans_endpoint_kinds() {
        with_offchain_ext(|| {
            add_endpoint("https://api.custom-tron.io", EndpointKind::TronScan).unwrap();
            remove_endpoint("https://api.custom-tron.io");
            let config = get_endpoint_config();
            assert!(!config
                .endpoints
                .contains(&String::from("https://api.custom-tron.io")));
            assert!(!config
                .endpoint_kinds
                .iter()
                .any(|(ep, _)| ep == "https://api.custom-tron.io"));
        });
    }

    #[test]
    fn allow_single_source_fallback_default_true() {
        let config = VerifierConfig::default();
        assert!(config.allow_single_source_fallback);
    }
}

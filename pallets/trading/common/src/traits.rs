//! # Shared trading traits.
//! # 公共交易 Trait 定义。
//!
//! This module defines shared trading interfaces used across multiple pallets.
//! 本模块定义供多个 pallet 共享的交易相关公共接口。
//!
//! ## Retained traits
//! ## 保留 Trait
//! - `PricingProvider`: NEX/USD rate lookup used by arbitration and storage-service.
//! - `PricingProvider`：NEX/USD 汇率查询，被 arbitration、storage-service 等模块使用。
//! - `DepositCalculator`: unified deposit calculation used by storage-service.
//! - `DepositCalculator`：统一保证金计算，被 storage-service 等模块使用。
...
/// ## Description
/// ## 说明
/// Provides real-time NEX/USD exchange-rate lookup.
/// 提供 NEX/USD 实时汇率查询功能。
///
/// ## Consumers
/// ## 使用者
/// - `pallet-nex-market`: order amount calculation and trade price reporting.
/// - `pallet-nex-market`：订单金额计算、上报成交价。
/// - `pallet-dispute-arbitration`: complaint deposit conversion.
/// - `pallet-dispute-arbitration`：投诉押金换算。
/// - `pallet-storage-service`: operator bond calculation.
/// - `pallet-storage-service`：运营者保证金计算。
///
/// ## Implementer
/// ## 实现者
/// - `TradingPricingProvider` (`runtime/src/configs/mod.rs`): based on pallet-nex-market TWAP.
...
/// Deposit calculation interface.
/// 保证金计算接口。
///
/// ## Description
/// ## 说明
/// Provides a unified way to compute NEX-denominated deposits from USD value.
/// 提供统一的 USD 价值动态计算保证金功能。
/// All modules that require deposits should use this interface.
/// 所有需要保证金的模块都应使用此接口。
///
/// ## Consumers
/// ## 使用者
...
/// - `pallet-dispute-arbitration`: complaint deposit calculation.
/// - `pallet-dispute-arbitration`：投诉押金。
///
/// ## Implementers
/// ## 实现者
/// - Implemented through `DepositCalculatorImpl` in each module.
    ...
    fn calculate_deposit(usd_amount: u64, fallback: Balance) -> Balance {
        ...
        if let Some(rate) = P::get_nex_to_usd_rate() {
            if rate > Balance::zero() {
                ...
                let nex_amount_u128 = match usd_u128
                    .saturating_mul(nex_precision)
...
/// On-chain NEX/USDT price oracle interface.
/// NEX/USDT 链上价格预言机接口。
///
/// ## Description
/// ## 说明
/// Provides price queries based on the pallet-nex-market TWAP accumulator.
/// 基于 pallet-nex-market 的 TWAP 累积器提供价格查询。
/// Other modules use this trait to consume real market prices, including both
/// upward and downward movement.
/// 其他模块通过此 trait 获取真实市场价格（涨跌都反映）。
///
/// ## Consumers
/// ## 使用者
/// - deposit calculation, liquidation engines, fee pricing, and similar flows.
/// - 保证金计算、清算引擎、手续费定价等。
///
/// ## Notes
/// ## 注意
/// - `get_twap` reflects actual market movement and does not add directional protection.
/// - `get_twap` 返回的价格反映真实涨跌，不做方向保护。
/// - Under low volume, TWAP may lag and consumers should check `is_price_stale`.
...
impl PriceOracle for () {
    fn get_twap(_window: TwapWindow) -> Option<u64> {
        None
    }
    fn get_last_trade_price() -> Option<u64> {
        None
    }
    fn is_price_stale(_max_age_blocks: u32) -> bool {
        true
    }
    fn get_trade_count() -> u64 {
        0
...
/// Unified NEX/USDT exchange-rate interface.
/// NEX/USDT 统一兑换比率接口。
///
/// ## Description
/// ## 说明
/// Provides a unified query interface that aggregates TWAP, staleness checks,
/// and confidence scoring.
/// 聚合 TWAP、陈旧检测与置信度评估的统一查询接口。
/// Suitable for modules that need a simple and reliable exchange rate.
/// 适用于需要简单获取可靠汇率的模块（佣金换算、直播打赏定价等）。
///
/// ## Relationship to `PricingProvider` and `PriceOracle`
/// ## 与 `PricingProvider` / `PriceOracle` 的关系
/// - `PricingProvider`: low-level raw rate lookup kept for existing consumers.
/// - `PricingProvider`：底层原始汇率查询（已有多个消费方，保留不变）。
/// - `PriceOracle`: low-level TWAP / last-trade / staleness interface.
/// - `PriceOracle`：底层 TWAP / 成交价 / 陈旧检测接口（已有，保留不变）。
/// - `ExchangeRateProvider`: higher-level wrapper that combines those interfaces.
/// - `ExchangeRateProvider`：高级封装，内部组合上述接口，提供带置信度的汇率。
///
/// ## Confidence levels
/// ## 置信度等级
/// - 90-100: fresh TWAP with strong trade coverage.
/// - 90-100：TWAP 可用且新鲜（高交易量）。
/// - 60-89: last trade price available but not TWAP.
/// - 60-89：LastTradePrice 可用但非 TWAP。
/// - 30-59: only the initial price is available during bootstrap.
/// - 30-59：仅 initial_price 可用（冷启动期）。
/// - 0-29: stale or unavailable pricing.
/// - 0-29：价格过时或不可用。

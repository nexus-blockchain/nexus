...
| **争议仲裁** | 验证超时退款后可提起争议，管理员根据链下证据裁决 |
| **紧急暂停** | 管理员一键暂停/恢复市场 + 队列满自动暂停保护 |
...
```text
AwaitingVerification 超时退款
  │
  ▼
买家认为已付 USDT → dispute_trade (提交 IPFS 证据 CID)
  │
  ▼
管理员审核 → resolve_dispute
  ├─ ReleaseToBuyer → 从国库补偿 NEX 给买家
...
| # | 调用 | 权限 | 参数 | 说明 |
|---|------|------|------|------|
| 0 | `place_sell_order` | 签名 | `nex_amount`, `usdt_price`, `tron_address` | 挂卖单：reserve 锁定 NEX，校验 TRON 地址（Base58Check），校验价格偏离，校验最低金额 |
| 1 | `place_buy_order` | 签名 | `nex_amount`, `usdt_price`, `buyer_tron_address` | 挂买单：预锁保证金，校验 TRON 地址，校验价格偏离，校验最低金额 |
| 2 | `cancel_order` | Owner | `order_id` | 取消订单：退还未成交的锁定 NEX（卖单）或剩余保证金（买单） |
| 3 | `reserve_sell_order` | 签名 | `order_id`, `amount?`, `buyer_tron_address` | 买家吃卖单：校验过期/价格偏离/熔断/暂停，锁保证金或免保证金（L2/L3 检查），创建 UsdtTrade |
| 4 | `accept_buy_order` | 签名 | `order_id`, `amount?`, `tron_address` | 卖家接买单：校验过期/价格偏离/熔断/暂停，锁卖家 NEX，按比例分配预锁保证金，创建 UsdtTrade |
| 5 | `confirm_payment` | 买家 | `trade_id` | 确认已付款：从 AwaitingPayment → AwaitingVerification，加入 OCW 待验证队列，检查队列满自动暂停 |
...
| 22 | `dispute_trade` | 买家/卖家 | `trade_id`, `evidence_cid` | 对 Refunded 交易发起争议，提交 IPFS 证据 CID |
| 25 | `update_order_price` | Owner | `order_id`, `new_price` | 原子修改订单价格（无需 cancel + re-place），买单自动重算保证金差额 |

### OCW 操作（Unsigned）

| # | 调用 | 参数 | 说明 |
|---|------|------|------|
| 7 | `submit_ocw_result` | `trade_id`, `actual_amount`, `tx_hash?` | 正常验证结果：Exact/Overpaid 存储、Underpaid 进补付窗口、SeverelyUnderpaid/Invalid 直接存储 |
| 15 | `auto_confirm_payment` | `trade_id`, `actual_amount`, `tx_hash?` | 预检兜底：一步完成确认 + 验证，少付进补付窗口 |
| 16 | `submit_underpaid_update` | `trade_id`, `new_actual_amount` | 补付窗口内更新累计金额（仅接受递增，达 99.5% 自动升级回 Exact） |

> 三个 unsigned extrinsic 均拒绝 `TransactionSource::External`，仅接受 Local / InBlock 来源，防止外部用户注入伪造结果。`propagate(false)` 阻止 unsigned 交易广播到其他节点。
>
> ValidateUnsigned 层增加安全边界检查：`actual_amount > expected × 10` 时拒绝提交，防恶意强制 Overpaid。tx_hash 在验证阶段即检查重放。
...
| 18 | `force_pause_market` | — | 紧急暂停市场：禁止挂单/吃单/确认付款 |
| 19 | `force_resume_market` | — | 恢复市场交易 |
| 20 | `force_settle_trade` | `trade_id`, `actual_amount`, `resolution` | 管理员强制结算（OCW 宕机时根据链下证据裁决） |
| 21 | `force_cancel_trade` | `trade_id` | 管理员强制取消（退 NEX + 退保证金，不没收） |
| 23 | `resolve_dispute` | `trade_id`, `resolution` | 解决争议：ReleaseToBuyer 从国库补偿 / RefundToSeller 维持原判 |
| 24 | `set_trading_fee` | `fee_bps` | 设置交易手续费率（最大 1000 bps = 10%） |
| 26 | `update_deposit_exchange_rate` | `new_rate` | 更新保证金动态汇率（设为 0 恢复 Config 默认值） |
...
管理员通过 `force_pause_market` / `force_resume_market` 控制。暂停影响范围：
...
当 `PendingUsdtTrades` 队列容量超过 `QueueFullThresholdBps`（默认 80%）时，自动触发市场暂停，防止 OCW 过载。恢复需管理员手动调用 `force_resume_market`。
...
- **`on_idle`**：
  ...
  - 过期订单 GC（每次最多清理 `MaxExpiredOrdersPerBlock` 笔，释放锁定资产）
  - UsedTxHashes TTL 清理（cursor-based 遍历，每次最多清理 10 条过期 tx_hash）
...
> 正常验证和自动确认的值包含 tx_hash 字节（第三个字段），用于防重放。

---

## tx_hash 防重放

同一 TRON 交易哈希不能用于多笔 nex-market 交易的支付证明：

1. `submit_ocw_result` 和 `auto_confirm_payment` 接受可选 `tx_hash` 参数
2. ValidateUnsigned 层和 dispatch 层均检查 `UsedTxHashes` 是否已存在
3. 验证通过后写入 `UsedTxHashes` 并记录区块号
4. `on_idle` 按 `TxHashTtlBlocks`（默认 7 天）TTL 清理过期记录，cursor-based 遍历避免全表扫描
...
> 偏离计算使用 u128 中间值 + saturating cast，防止极端偏离 >655% 时 u16 截断回绕。
...
- 解除：自动到期 或 `lift_circuit_breaker`（MarketAdmin 手动解除，需熔断已到期）
...
| `created_at` | `BlockNumber` | 创建区块 |
| `evidence_cid` | `BoundedVec<u8, 128>` | IPFS 证据哈希 |
...
| `PriceProtectionConfig` | `enabled`, `max_price_deviation`, `circuit_breaker_threshold`, `min_trades_for_twap`, `circuit_breaker_active`, `circuit_breaker_until`, `initial_price` — 价格保护配置 |
| `DisputeResolution` | `ReleaseToBuyer` / `RefundToSeller` — 争议裁决方向 |
...
| `PendingUsdtTrades` | `BoundedVec<u64, MaxPendingTrades>` | OCW 待验证队列（AwaitingVerification） |
| `AwaitingPaymentTrades` | `BoundedVec<u64, MaxAwaitingPaymentTrades>` | 待付款跟踪队列（OCW 预检用） |
| `PendingUnderpaidTrades` | `BoundedVec<u64, MaxUnderpaidTrades>` | 少付补付跟踪队列 |
...
| **价格** | | |
| `BestAsk` / `BestBid` | `Option<u64>` | 最优卖价 / 买价（增量更新 + GC 后全量刷新） |
...
| `TradingFeeBps` | `ValueQuery<u16>` | 交易手续费率（bps） |
...
| `UsedTxHashes` | `Map<TxHash, (u64, BlockNumber)>` | 已使用的 TRON tx_hash（防重放，含 TTL） |
...
| `OrderPriceUpdated` | `order_id`, `old_price`, `new_price` | update_order_price |
...
| `MarketPaused` | — | force_pause_market / 队列满自动暂停 |
| `MarketResumed` | — | force_resume_market |
| `TradeForceSettled` | `trade_id`, `actual_amount`, `resolution` | force_settle_trade |
| `TradeForceCancelled` | `trade_id` | force_cancel_trade |
| `TradeDisputed` | `trade_id`, `initiator`, `evidence_cid` | dispute_trade |
| `DisputeResolved` | `trade_id`, `resolution` | resolve_dispute |
| `TradingFeeUpdated` | `old_fee_bps`, `new_fee_bps` | set_trading_fee |
| `TradingFeeCharged` | `trade_id`, `fee_amount`, `to_treasury` | 结算时手续费扣除 |
| `DepositExchangeRateUpdated` | `old_rate`, `new_rate` | update_deposit_exchange_rate |
...
| `OrderAmountBelowMinimum` | 低于最低挂单/吃单限额（MinOrderNexAmount） |
| `ZeroPrice` | USDT 单价为零 |
...
| `TxHashAlreadyUsed` | TRON 交易哈希已被使用（防重放） |
| `MarketIsPaused` | 市场已暂停，禁止交易操作 |
| `TradeAlreadyDisputed` | 交易已存在争议 |
| `TradeNotDisputable` | 交易状态不可争议（仅 Refunded 可争议） |
| `DisputeNotFound` | 争议不存在 |
| `DisputeAlreadyClosed` | 争议已解决 |
| `FeeTooHigh` | 手续费过高（最大 1000 bps = 10%） |
| `OrderHasActiveTrades` | 订单有活跃交易，不能修改价格 |
...
| entity-order | 订单定价 | `entity_common::PricingProvider` |
...
通过 `NexMarketApi` trait 提供前端查询接口：

| 方法 | 返回 | 说明 |
|------|------|------|
| `get_sell_orders()` | `Vec<OrderInfo>` | 活跃卖单列表（按价格升序） |
| `get_buy_orders()` | `Vec<OrderInfo>` | 活跃买单列表（按价格降序） |
| `get_user_orders(user)` | `Vec<OrderInfo>` | 指定用户的所有订单 |
| `get_user_trades(user)` | `Vec<TradeInfo>` | 用户交易历史 |
| `get_order_trades(order_id)` | `Vec<TradeInfo>` | 订单关联的交易列表 |
| `get_active_trades(user)` | `Vec<TradeInfo>` | 用户活跃交易（待付款/待验证/待补付） |
| `get_order_depth()` | `(Vec<DepthEntry>, Vec<DepthEntry>)` | 订单深度图（asks 升序, bids 降序） |
| `get_best_prices()` | `(Option<u64>, Option<u64>)` | (最优卖价, 最优买价) |
| `get_market_summary()` | `MarketSummary` | 市场摘要（含 best_ask/bid、暂停状态、手续费率、待验证数） |

### API 返回类型

| 类型 | 字段 |
|------|------|
| `OrderInfo` | `order_id`, `side`(0=Sell,1=Buy), `owner`, `nex_amount`, `filled_amount`, `usdt_price`, `status`(0-4), `created_at`, `expires_at` |
| `TradeInfo` | `trade_id`, `order_id`, `seller`, `buyer`, `nex_amount`, `usdt_amount`, `status`(0-4), `created_at` |
...
| `MinOrderNexAmount` | `Balance` | 1 NEX | 最低挂单/吃单 NEX 数量 |
| **USDT 交易** | | | |
| `UsdtTimeout` | `u32` | 12h | 付款超时（区块数） |
| `VerificationGracePeriod` | `u32` | 1h | AwaitingVerification 超时宽限期 |
| `UnderpaidGracePeriod` | `u32` | 2h | 少付补付窗口 |
| **队列** | | | |
| `MaxPendingTrades` | `u32` | 200 | OCW 待验证队列最大容量 |
| `MaxAwaitingPaymentTrades` | `u32` | 200 | 待付款跟踪队列最大容量 |
| `MaxUnderpaidTrades` | `u32` | 100 | 少付补付跟踪队列最大容量 |
| `QueueFullThresholdBps` | `u16` | 8000 (80%) | 队列满自动暂停阈值 |
| **索引** | | | |
...
| **GC & 清理** | | | |
| `MaxExpiredOrdersPerBlock` | `u32` | 20 | on_idle 每次过期订单 GC 最大处理数 |
| `TxHashTtlBlocks` | `u32` | 7d | UsedTxHashes 条目保留时间 |
...
| 场景 | 处理 |
|------|------|
| 订单状态 Filled → 回滚后仍有余量 | 恢复为 Open/PartiallyFilled，重新加入订单簿和用户索引 |
| 回滚时订单已过期 | 标记为 Expired，不加回订单簿（防污染 BestPrice） |
| 回滚时订单已 Cancelled/Expired | 仅回退 filled_amount，不改变状态（防幽灵记录） |
| 重新入簿后 | 刷新最优价格 |
...
| 订单取消/成交 | 仅在影响当前最优价时重扫 | O(1) 或 O(n) |
...
```bash
cargo test -p pallet-nex-market    # 124 个单元测试
```

覆盖范围：
- 卖单/买单创建、取消、过期、改价
- reserve_sell_order / accept_buy_order（含部分成交、最低限额）
- confirm_payment 流程 + 队列满自动暂停
- 完整交易结算（Exact / Overpaid）+ 手续费扣除

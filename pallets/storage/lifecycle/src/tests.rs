...
fn archive_level_u8_roundtrip() {
    assert_eq!(
        ArchiveLevel::from_u8(ArchiveLevel::Active.to_u8()),
        ArchiveLevel::Active
    );
    assert_eq!(
        ArchiveLevel::from_u8(ArchiveLevel::ArchivedL1.to_u8()),
        ArchiveLevel::ArchivedL1
    );
    assert_eq!(
        ArchiveLevel::from_u8(ArchiveLevel::ArchivedL2.to_u8()),
        ArchiveLevel::ArchivedL2
    );
    assert_eq!(
        ArchiveLevel::from_u8(ArchiveLevel::Purged.to_u8()),
        ArchiveLevel::Purged
    );
...
fn d1_on_idle_l1_to_l2() {
    new_test_ext().execute_with(|| {
        ...
        let weight = StorageLifecycle::on_idle(500u64, Weight::from_parts(500_000_000, 50_000));
        assert!(weight.ref_time() > 0);
        let events = System::events();
        assert!(events.iter().any(|e| {
            matches!(&e.event, RuntimeEvent::StorageLifecycle(
...
fn d1_on_idle_l2_to_purge() {
    new_test_ext().execute_with(|| {
        ...
        let _weight = StorageLifecycle::on_idle(1000u64, Weight::from_parts(500_000_000, 50_000));
        let events = System::events();
        assert!(events.iter().any(|e| {
            matches!(&e.event, RuntimeEvent::StorageLifecycle(
...
fn d1_on_idle_skips_when_insufficient_weight() {
    new_test_ext().execute_with(|| {
        set_archivable_ids(vec![1, 2, 3]);
        let weight = StorageLifecycle::on_idle(100u64, Weight::from_parts(1_000, 100));
    ...
    new_test_ext().execute_with(|| {
        ...
        let warning_count = events
            ...
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::StorageLifecycle(pallet::Event::ArchivalWarning { .. })
                )
            })
            .count();
...
fn m2r2_backlog_event_can_fire() {
    new_test_ext().execute_with(|| {
        ...
        let backlog_count = events
            .iter()
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::StorageLifecycle(pallet::Event::ArchivalBacklog { .. })
                )
            })
            .count();
        ...
        assert!(
            backlog_count > 0,
            "ArchivalBacklog event should fire when pending > threshold"
        );
    });
}
...
fn m2r4_force_archive_event_excludes_skipped_ids() {
    new_test_ext().execute_with(|| {
        ...
        let force_event = events.iter().find(|e| {
            matches!(
                e.event,
                RuntimeEvent::StorageLifecycle(pallet::Event::DataForceArchived { .. })
            )
        });

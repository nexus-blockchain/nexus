...
mod benchmarks {
    ...
    fn request_pin_for_subject() {
        ...
        pallet::StorageLayerConfigs::<T>::insert(
    ...
    fn update_operator() {
        ...
        let info = pallet::OperatorInfo::<T> {
            peer_id: BoundedVec::try_from(alloc::vec![1u8; 16]).unwrap(),
            capacity_gib: 100,
            endpoint_hash: T::Hashing::hash(&[0u8]),
            cert_fingerprint: None,
            status: 0,
            registered_at: frame_system::Pallet::<T>::block_number(),
            layer: OperatorLayer::Core,
    ...
    fn cleanup_expired_cids(n: Linear<1, 20>) {
        ...
        for i in 0..n {
            ...
            pallet::PinBilling::<T>::insert(
                cid_hash,
                (frame_system::Pallet::<T>::block_number(), 100u128, 2u8),
            );
            pallet::ExpiredCidQueue::<T>::mutate(|q| {

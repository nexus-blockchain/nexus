...
fn seed_not_started_round<T: Config>(entity_id: u64, creator: &T::AccountId) -> u64 {
    ...
    SaleRounds::<T>::insert(
        ...
        SaleRound {
            ...
            vesting_config: VestingConfig {
                ...
                cliff_duration: Zero::zero(),
                total_duration: Zero::zero(),
    ...
    EntityRounds::<T>::mutate(entity_id, |r| {
        let _ = r.try_push(round_id);
    });
    round_id
}
...
fn seed_active_round<T: Config>(entity_id: u64, creator: &T::AccountId) -> u64 {
    ...
    RoundPaymentOptions::<T>::mutate(round_id, |opts| {
        let _ = opts.try_push(PaymentConfig {
            asset_id: None,
            price: bal::<T>(100),
            min_purchase: bal::<T>(10),
            max_purchase_per_account: bal::<T>(100_000),
            enabled: true,
        });
    });
    ActiveRounds::<T>::mutate(|a| {
        let _ = a.try_push(round_id);
    });
    round_id
}
...
fn seed_subscription<T: Config>(
    ...
    RoundParticipants::<T>::mutate(round_id, |p| {
        let _ = p.try_push(subscriber.clone());
    });
}

/// 插入已 claimed 的认购记录
fn seed_claimed_subscription<T: Config>(
    round_id: u64,
    subscriber: &T::AccountId,
    amount: u128,
    payment: u128,
...
mod benchmarks {
    ...
    fn configure_dutch_auction() {
        ...
        EntityRounds::<T>::mutate(ENTITY_1, |r| {
            let _ = r.try_push(rid);
        });
        #[extrinsic_call]
        _(
            RawOrigin::Signed(caller),
    ...
    fn add_to_whitelist() {
        ...
        let list: BoundedVec<(T::AccountId, Option<BalanceOf<T>>), T::MaxWhitelistSize> =
            vec![(a1, None), (a2, Some(bal::<T>(5000)))]
                .try_into()
                .unwrap();
        #[extrinsic_call]
    ...
    fn start_sale() {
        ...
        setup_entity_for::<T>(ENTITY_1, &caller);
        let rid = seed_not_started_round::<T>(ENTITY_1, &caller);
        RoundPaymentOptions::<T>::mutate(rid, |o| {
            let _ = o.try_push(make_payment::<T>());
        });
        // 更新 payment_options_count 以通过校验
        SaleRounds::<T>::mutate(rid, |r| {
            if let Some(round) = r {
                round.payment_options_count = 1;
            }
        });
        #[extrinsic_call]
        ...
        SaleRounds::<T>::insert(
            ...
            SaleRound {
                ...
                vesting_config: VestingConfig {
                    ...
                    initial_unlock_bps: 5000,
                    cliff_duration: Zero::zero(),
                    total_duration: 100u32.into(),
    ...
    fn reclaim_unclaimed_tokens() {
        ...
        frame_system::Pallet::<T>::set_block_number(
            now.saturating_add(T::RefundGracePeriod::get())
    ...
    fn remove_from_whitelist() {
        ...
        let list: BoundedVec<T::AccountId, T::MaxWhitelistSize> = vec![a1].try_into().unwrap();
        #[extrinsic_call]
        _(RawOrigin::Signed(caller), rid, list);
    }
    ...
    fn extend_sale() {
        ...
        _(
            RawOrigin::Signed(caller),
            rid,
            end.saturating_add(500u32.into()),
        );
    }
    ...
    fn force_batch_refund() {
        ...
        let pa = Pallet::<T>::pallet_account();
        let _ = T::Currency::deposit_creating(&pa, bal::<T>(10_000_000));
        let subs: BoundedVec<T::AccountId, T::MaxBatchRefund> = vec![s1, s2].try_into().unwrap();
        #[extrinsic_call]

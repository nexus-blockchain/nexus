//! # Entity token-sale pallet (pallet-entity-tokensale)
//!
//! ## Overview
//!
//! This pallet implements public token-sale workflows for entity tokens:
//! 本模块实现实体代币公开发售功能：
//! - multiple sale modes (fixed price, Dutch auction, whitelist allocation)
//! - 多种发售模式（固定价格、荷兰拍卖、白名单分配）
//! - multi-round sales
//! - 多轮发售支持
//! - token locking and linear unlocking
//! - 代币锁仓和线性解锁
//! - multi-asset payment support (currently native NEX only)
//! - 多资产支付支持（当前仅支持原生 NEX）
//! - KYC integration
//! - KYC 集成
//! - actual fund transfers for subscriptions, claims, refunds, and withdrawals
//! - 实际资金转账（认购扣款、代币发放、退款）
//!
//! ## Fund flow
//!
//! - `subscribe`: subscriber NEX → pallet escrow account
//! - `subscribe`：认购者 NEX → Pallet 托管账户
//! - `start_sale`: entity token reserve / lock
//! - `start_sale`：Entity 代币 reserve（锁定）
//! - `claim_tokens` / `unlock_tokens`: entity token → subscriber
//! - `claim_tokens` / `unlock_tokens`：Entity 代币 → 认购者
//! - `end_sale`: release unsold tokens
//! - `end_sale`：释放未售代币
//! - `cancel_sale` + `claim_refund`: refund NEX to subscribers + release tokens
//! - `cancel_sale` + `claim_refund`：NEX 退还认购者 + 释放代币
//! - `withdraw_funds`: NEX → Entity derived account
//! - `withdraw_funds`：NEX → Entity 派生账户
//!
//! ## Version history
//!
//! - v0.1.0 (2026-02-03): initial Phase 8 release
//! - v0.1.0 (2026-02-03)：Phase 8 初始版本
//! - v0.2.0 (2026-02-09): audit-driven integration fixes for `EntityProvider`, `Currency`,
//!   `TokenProvider`, KYC, whitelist storage, input validation, transfers, and new
//!   `claim_refund` / `withdraw_funds`
//! - v0.2.0 (2026-02-09)：深度审计修复：`EntityProvider` / `Currency` /
//!   `TokenProvider` / KYC 集成，白名单独立存储，输入校验，实际转账，新增
//!   `claim_refund` / `withdraw_funds`
...
impl<AccountId> KycChecker<AccountId> for NullKycChecker {
    fn kyc_level(_entity_id: u64, _: &AccountId) -> u8 {
        0
    }
}
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        DisclosureProvider, EntityProvider, EntityTokenProvider, TokenSaleStatus,
    };
    use sp_runtime::{
    ...
    /// 发售轮次类型别名
    pub type SaleRoundOf<T> =
        SaleRound<<T as frame_system::Config>::AccountId, BalanceOf<T>, BlockNumberFor<T>>;
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// 资产 ID 类型（多资产支付预留）
        type AssetId: Member + Parameter + Default + Copy + MaxEncodedLen + From<u64> + Into<u64>;
    ...
    #[pallet::storage]
    pub type ActiveRounds<T: Config> =
        StorageValue<_, BoundedVec<u64, T::MaxActiveRounds>, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 发售轮次已取消
        SaleRoundCancelled { round_id: u64 },
        /// 用户已认购（NEX 已转入托管）
        Subscribed {
            round_id: u64,
        ...
        /// 白名单已更新
        WhitelistUpdated { round_id: u64, count: u32 },
        ...
        /// 白名单地址已移除
        WhitelistRemoved { round_id: u64, removed_count: u32 },
        /// 支付选项已移除
        PaymentOptionRemoved { round_id: u64, index: u32 },
        /// 发售时间已延长
        SaleExtended {
            round_id: u64,
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            assert!(
                T::MaxPaymentOptions::get() > 0,
                "MaxPaymentOptions must be > 0"
            );
            assert!(
                T::MaxWhitelistSize::get() > 0,
                "MaxWhitelistSize must be > 0"
            );
            assert!(
                T::MaxRoundsHistory::get() > 0,
                "MaxRoundsHistory must be > 0"
            );
            assert!(
                T::MaxSubscriptionsPerRound::get() > 0,
                "MaxSubscriptionsPerRound must be > 0"
            );
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_sale_round(
            ...
            let owner =
                ...
                owner == who
                    || T::EntityProvider::is_entity_admin(
                        entity_id,
                        &who,
                        pallet_entity_common::AdminPermission::TOKEN_MANAGE
                    ),
            ...
            EntityRounds::<T>::try_mutate(entity_id, |rounds| -> DispatchResult {
                rounds
                    .try_push(round_id)
                    .map_err(|_| Error::<T>::RoundsHistoryFull)?;
                Ok(())
            })?;
        ...
        pub fn add_payment_option(
            ...
            // H3-fix: 当前仅支持原生 NEX（asset_id = None），阻止多资产键不一致
                ...
                max_purchase_per_account >= min_purchase,
                Error::<T>::InvalidPurchaseLimits
            );
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                let round = maybe_round.as_mut().ok_or(Error::<T>::RoundNotFound)?;
                    ...
                    !T::EntityProvider::is_entity_locked(round.entity_id),
                    Error::<T>::EntityLocked
                );
                ensure!(
                ...
                RoundPaymentOptions::<T>::try_mutate(round_id, |options| -> DispatchResult {
                    ...
                    options
                        .try_push(option)
                        .map_err(|_| Error::<T>::PaymentOptionsFull)?;
                    Ok(())
                })?;
        ...
        pub fn set_vesting_config(
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                let round = maybe_round.as_mut().ok_or(Error::<T>::RoundNotFound)?;
                    ...
                    initial_unlock_bps <= 10000,
                    Error::<T>::InvalidVestingConfig
                );
                // H7: total_duration >= cliff_duration
                ensure!(
                    total_duration >= cliff_duration,
                    Error::<T>::InvalidVestingDuration
                );
        ...
        pub fn configure_dutch_auction(
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                let round = maybe_round.as_mut().ok_or(Error::<T>::RoundNotFound)?;
                    ...
                    round.mode == SaleMode::DutchAuction,
                    Error::<T>::InvalidRoundStatus
                );
                // M3: 必须在 NotStarted 状态
                    ...
                    Error::<T>::InvalidDutchAuctionConfig
                );
        ...
        pub fn start_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                ...
                // L2: 检查独立存储中的支付选项数
                    ...
                    round.payment_options_count > 0,
                    Error::<T>::NoPaymentOptions
                );
                ...
                ActiveRounds::<T>::try_mutate(|active| -> DispatchResult {
                    active
                        .try_push(round_id)
                        .map_err(|_| Error::<T>::ActiveRoundsFull)?;
                    Ok(())
                })?;
        ...
        pub fn subscribe(
            ...
            let now = <frame_system::Pallet<T>>::block_number();
                ...
                now >= round.start_block && now <= round.end_block,
                Error::<T>::SaleNotInTimeWindow
            );

            // 验证未重复认购
            ensure!(
                !Subscriptions::<T>::contains_key(round_id, &who),
            ...
            if round.mode == SaleMode::WhitelistAllocation {
                    ...
                    RoundWhitelist::<T>::contains_key(round_id, &who),
            ...
            let payment_option = payment_options
                .iter()
            ...
            // 验证购买限额
                ...
                amount >= payment_option.min_purchase,
                Error::<T>::BelowMinPurchase
            );
            // F5: 白名单个人额度优先于默认限额
            ...
            RoundParticipants::<T>::try_mutate(round_id, |participants| -> DispatchResult {
                participants
                    .try_push(who.clone())
                    .map_err(|_| Error::<T>::ParticipantsFull)?;
                Ok(())
            })?;
        ...
        pub fn end_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                ...
                if !round.remaining_amount.is_zero() {
                    let entity_account = T::EntityProvider::entity_account(round.entity_id);
                    let deficit = T::TokenProvider::unreserve(
                        round.entity_id,
                        &entity_account,
                        round.remaining_amount,
                    );
                    ...
                    if !deficit.is_zero() {
                        log::warn!(
                            "tokensale end_sale: unreserve deficit {:?} for round {}",
                            deficit,
                            round_id
                        );
                    }
        ...
        pub fn claim_tokens(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Subscriptions::<T>::try_mutate(round_id, &who, |maybe_sub| -> DispatchResult {
                ...
                // 计算初始解锁量
                let initial_unlock =
                    Self::calculate_initial_unlock(&round.vesting_config, sub.amount);
        ...
        pub fn unlock_tokens(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn cancel_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                let round = maybe_round.as_mut().ok_or(Error::<T>::RoundNotFound)?;
                    ...
                    matches!(
                        round.status,
                        RoundStatus::NotStarted | RoundStatus::Active | RoundStatus::Paused
                    ),
                    Error::<T>::InvalidRoundStatus
                );

                // 如果已启动（Active/Paused），释放未售 Entity 代币
                if matches!(round.status, RoundStatus::Active | RoundStatus::Paused)
                    && !round.remaining_amount.is_zero()
                {
                    let entity_account = T::EntityProvider::entity_account(round.entity_id);
                    let deficit = T::TokenProvider::unreserve(
                        round.entity_id,
                    ...
                    if !deficit.is_zero() {
                        log::warn!(
                            "tokensale cancel_sale: unreserve deficit {:?} for round {}",
                            deficit,
                            round_id
                        );
                    }
        ...
        pub fn claim_refund(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let round = SaleRounds::<T>::get(round_id).ok_or(Error::<T>::RoundNotFound)?;
            ensure!(
                round.status == RoundStatus::Cancelled,
                Error::<T>::SaleNotCancelled
            );
            ...
            Subscriptions::<T>::try_mutate(round_id, &who, |maybe_sub| -> DispatchResult {
                ...
                SaleRounds::<T>::mutate(round_id, |maybe_round| {
                    if let Some(r) = maybe_round {
                        r.total_refunded_tokens =
                            r.total_refunded_tokens.saturating_add(sub.amount);
                        r.total_refunded_nex =
                            r.total_refunded_nex.saturating_add(sub.payment_amount);
                    }
                });
        ...
        pub fn reclaim_unclaimed_tokens(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                ...
                // 计算未领取的代币和 NEX
                let unclaimed_tokens = round
                    .sold_amount
                    .saturating_sub(round.total_refunded_tokens);
        ...
        pub fn withdraw_funds(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn force_cancel_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                    ...
                    && !round.remaining_amount.is_zero()
                {
                    ...
                    if !deficit.is_zero() {
                        log::warn!(
                            "tokensale force_cancel_sale: unreserve deficit {:?} for round {}",
                            deficit,
                            round_id
                        );
                    }
        ...
        pub fn force_end_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                ...
                if !round.remaining_amount.is_zero() {
                    ...
                    if !deficit.is_zero() {
                        log::warn!(
                            "tokensale force_end_sale: unreserve deficit {:?} for round {}",
                            deficit,
                            round_id
                        );
                    }
        ...
        pub fn force_withdraw_funds(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn update_sale_round(
            ...
            let who = ensure_signed(origin)?;
                ...
                total_supply.is_some()
                    || start_block.is_some()
                    || end_block.is_some()
                    || kyc_required.is_some()
                    || min_kyc_level.is_some(),
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                ...
                // 更新后校验时间窗口一致性
                    ...
                    round.end_block > round.start_block,
                    Error::<T>::InvalidTimeWindow
                );
        ...
        pub fn increase_subscription(
            ...
            let round = SaleRounds::<T>::get(round_id).ok_or(Error::<T>::RoundNotFound)?;
                ...
                round.remaining_amount >= additional_amount,
                Error::<T>::SoldOut
            );
            ...
            // 计算追加支付金额
            let additional_payment =
                Self::calculate_payment_amount(&round, additional_amount, payment_option)?;
        ...
        pub fn remove_from_whitelist(
            ...
            Self::deposit_event(Event::WhitelistRemoved {
                round_id,
                removed_count,
            });
            Ok(())
        ...
        pub fn extend_sale(
            ...
            SaleRounds::<T>::try_mutate(round_id, |maybe_round| -> DispatchResult {
                let round = maybe_round.as_mut().ok_or(Error::<T>::RoundNotFound)?;
                    ...
                    new_end_block > round.end_block,
                    Error::<T>::InvalidExtension
                );

                round.end_block = new_end_block;

                Self::deposit_event(Event::SaleExtended {
                    round_id,
                    new_end_block,
                });
                Ok(())
        ...
        pub fn pause_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn resume_sale(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn cleanup_round(origin: OriginFor<T>, round_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            for subscriber in &subscribers {
                if let Some(mut sub) = Subscriptions::<T>::get(round_id, subscriber) {
                    ...
                    if !deficit.is_zero() {
                        ...
                        if !freed.is_zero() {
                            let _ =
                                T::TokenProvider::reserve(round.entity_id, &entity_account, freed);
                        }
                        log::warn!(
                            "force_batch_refund: unreserve deficit {:?} for subscriber {:?}",
                            deficit,
                            subscriber
                        );
                        continue;
                    ...
                    .is_err()
                    {
                        ...
                        let _ =
                            T::TokenProvider::reserve(round.entity_id, &entity_account, sub.amount);
                        log::warn!(
                            "force_batch_refund: NEX transfer failed for subscriber {:?}",
                            subscriber
                        );
                        continue;
                    }
            ...
            if refunded_count > 0 {
                SaleRounds::<T>::mutate(round_id, |maybe_round| {
                    if let Some(r) = maybe_round.as_mut() {
                        ...
                        r.total_refunded_tokens =
                            r.total_refunded_tokens.saturating_add(total_tokens);
                    }
                });
            }
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 计算荷兰拍卖当前价格
        pub(crate) fn calculate_dutch_price(
            round: &SaleRoundOf<T>,
        ) -> Result<BalanceOf<T>, DispatchError> {
            let start_price = round
                .dutch_start_price
                .ok_or(Error::<T>::InvalidDutchAuctionConfig)?;
            let end_price = round
                .dutch_end_price
                .ok_or(Error::<T>::InvalidDutchAuctionConfig)?;
        ...
        pub(crate) fn calculate_unlockable(
            ...
            // 计算线性/阶梯解锁
            let vesting_duration: u128 = Self::block_to_u128(
                vesting
                    .total_duration
                    .saturating_sub(vesting.cliff_duration),
            );
            ...
            SaleRounds::<T>::mutate(round_id, |maybe_round| {
                if let Some(round) = maybe_round.as_mut() {
                    ...
                    if !round.remaining_amount.is_zero() {
                        ...
                        if !deficit.is_zero() {
                            log::warn!(
                                "tokensale do_auto_end_sale: unreserve deficit {:?} for round {}",
                                deficit,
                                round_id
                            );
                        }

...
fn subscribe_checks_kyc() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::create_sale_round(
            ...
            true,
            2,
            0u128, // kyc_required, min_level=2, soft_cap
        ));
        assert_ok!(EntityTokenSale::add_payment_option(
            RuntimeOrigin::signed(CREATOR),
            0,
...
fn claim_tokens_distributes_entity_tokens() {
    new_test_ext().execute_with(|| {
        ...
        // claim（无锁仓 → 全额解锁）
        assert_ok!(EntityTokenSale::claim_tokens(
            RuntimeOrigin::signed(BUYER),
            round_id
        ));
        let sub = Subscriptions::<Test>::get(round_id, BUYER).unwrap();
...
fn cancel_and_refund_works() {
    new_test_ext().execute_with(|| {
        ...
        // 取消发售
        assert_ok!(EntityTokenSale::cancel_sale(
            RuntimeOrigin::signed(CREATOR),
            round_id
        ));
        let round = SaleRounds::<Test>::get(round_id).unwrap();
        ...
        // 认购者领取退款
        assert_ok!(EntityTokenSale::claim_refund(
            RuntimeOrigin::signed(BUYER),
            round_id
        ));
...
fn withdraw_funds_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::withdraw_funds(
            RuntimeOrigin::signed(CREATOR),
            round_id
        ));
...
fn h1_deep_claim_and_unlock_full_amount_succeeds() {
    new_test_ext().execute_with(|| {
        ...
        // unlock 全部剩余
        frame_system::Pallet::<Test>::set_block_number(111);
        assert_ok!(EntityTokenSale::unlock_tokens(
            RuntimeOrigin::signed(BUYER),
            round_id
        ));
        let sub = Subscriptions::<Test>::get(round_id, BUYER).unwrap();
...
fn m2_deep_dutch_price_clamped_to_end_price() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            price >= end_price,
            "Dutch price {} should be >= end_price {}",
            price,
            end_price
        );
    });
...
fn p0_force_cancel_sale_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::force_cancel_sale(
            RuntimeOrigin::root(),
            round_id
        ));
        let round = SaleRounds::<Test>::get(round_id).unwrap();
...
fn p0_force_end_sale_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::force_end_sale(
            RuntimeOrigin::root(),
            round_id
        ));
        let round = SaleRounds::<Test>::get(round_id).unwrap();
...
fn p1_force_refund_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::force_refund(
            RuntimeOrigin::root(),
            round_id,
            BUYER
        ));
...
fn p1_force_withdraw_funds_works() {
    new_test_ext().execute_with(|| {
        ...
        let entity_before = Balances::free_balance(ENTITY_ACCOUNT);
        assert_ok!(EntityTokenSale::force_withdraw_funds(
            RuntimeOrigin::root(),
            round_id
        ));
        let entity_after = Balances::free_balance(ENTITY_ACCOUNT);
...
fn p2_remove_payment_option_rejects_invalid_index() {
    new_test_ext().execute_with(|| {
        let round_id = setup_round();
        assert_noop!(
            EntityTokenSale::remove_payment_option(RuntimeOrigin::signed(CREATOR), round_id, 0,),
...
fn p2_extend_sale_rejects_shorter() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityTokenSale::extend_sale(RuntimeOrigin::signed(CREATOR), round_id, 50u64.into(),),
...
fn p2_extend_sale_rejects_not_active() {
    new_test_ext().execute_with(|| {
        let round_id = setup_round(); // NotStarted
        assert_noop!(
            EntityTokenSale::extend_sale(RuntimeOrigin::signed(CREATOR), round_id, 200u64.into(),),
            Error::<Test>::InvalidRoundStatus
        );
    });
}
...
fn p3_pause_sale_works() {
    new_test_ext().execute_with(|| {
        let round_id = setup_active_round();
        assert_ok!(EntityTokenSale::pause_sale(
            RuntimeOrigin::signed(CREATOR),
            round_id
        ));
        let round = SaleRounds::<Test>::get(round_id).unwrap();
...
fn p3_resume_sale_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityTokenSale::resume_sale(
            RuntimeOrigin::signed(CREATOR),
            round_id
        ));
        let round = SaleRounds::<Test>::get(round_id).unwrap();
...
fn f7_token_sale_provider_active_round() {
    new_test_ext().execute_with(|| {
        ...
        // 无活跃轮次
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::active_sale_round(ENTITY_ID),
            None
        );
        ...
        // 数据
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::sold_amount(round_id),
            Some(0u128)
        );
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::remaining_amount(round_id),
            Some(1_000_000u128)
        );
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::participants_count(round_id),
            Some(0)
        );
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::sale_entity_id(round_id),
            Some(ENTITY_ID)
        );
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::sale_total_supply(round_id),
            Some(1_000_000u128)
        );
    });
}
...
fn f7_token_sale_provider_missing_round() {
    new_test_ext().execute_with(|| {
        use pallet_entity_common::TokenSaleProvider;
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::sale_round_status(999),
            None
        );
        assert_eq!(
            <EntityTokenSale as TokenSaleProvider<u128>>::sold_amount(999),
            None
        );
    });
}
...
fn f8_cleanup_round_works() {
    new_test_ext().execute_with(|| {
        ...
        // 现在清理
        assert_ok!(EntityTokenSale::cleanup_round(
            RuntimeOrigin::signed(CREATOR),
            round_id
        ));
...
fn h1h2_force_batch_refund_unreserves_entity_tokens() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            reserved_before >= 100,
            "tokens should be reserved after subscribe"
        );
        ...
        assert_eq!(
            reserved_after, 0,
            "all entity tokens should be unreserved after batch refund"
        );
    });
}
...
fn m1_force_batch_refund_updates_total_refunded_tokens() {
    new_test_ext().execute_with(|| {
        ...
        let round = SaleRounds::<Test>::get(round_id).unwrap();
        assert_eq!(
            round.total_refunded_tokens, 300u128,
            "total_refunded_tokens should include both subscribers"
        );
        assert!(
            round.total_refunded_nex > 0,
            "total_refunded_nex should be updated"
        );
    });
}
...
fn h1_force_batch_refund_skips_on_transfer_failure() {
    new_test_ext().execute_with(|| {
        ...
        let pallet_account = EntityTokenSale::pallet_account();
        let pallet_bal =
            <pallet_balances::Pallet<Test> as Currency<u64>>::free_balance(&pallet_account);
        if pallet_bal > 1 {
        ...
        assert!(
            !sub.refunded,
            "subscriber should NOT be marked refunded when NEX transfer fails"
        );
    });
}
...
fn h1r6_claim_refund_works_after_end_sale_soft_cap_failure() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            reserved, 0,
            "all entity tokens should be unreserved after refund"
        );
    });
}
...
fn h1r6_claim_refund_works_after_auto_end_soft_cap_failure() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            "NEX should be fully refunded after auto-cancel"
        );
    });
}
...
fn m2_dutch_price_no_overflow_with_large_values() {
    new_test_ext().execute_with(|| {
        ...
        // 价格应在 start 和 end 之间
        assert!(
            price_u128 >= large_end,
            "price should be >= end_price, got {}",
            price_u128
        );
        assert!(
            price_u128 <= large_start,
            "price should be <= start_price, got {}",
            price_u128
        );
        // 中间点价格应大致在中间
        ...
        assert!(
            price_u128 > expected_mid.saturating_sub(tolerance)
                && price_u128 < expected_mid.saturating_add(tolerance),
            "price at midpoint should be roughly half of start_price, got {} vs expected ~{}",
            price_u128,
            expected_mid
        );
    });
...
fn m1r7_cleanup_round_frees_entity_rounds_slot() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !rounds_after.contains(&round_id),
            "round should be removed from EntityRounds after cleanup"
        );
    });
}
...
fn m1r7_cleanup_allows_new_round_after_slot_freed() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            rounds.len(),
            1,
            "only the new round should be in EntityRounds"
        );
...
fn l1r7_force_batch_refund_rereserves_on_partial_unreserve() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !sub.refunded,
            "subscriber should NOT be refunded when unreserve has deficit"
        );
        ...
        assert_eq!(
            reserved_after, 50,
            "reserved should be restored after re-reserve"
        );
    });
}

...
fn primary_shop_id(entity_id: u64) -> u64 {
    <MockEntityProvider as pallet_entity_common::EntityProvider<u64>>::get_primary_shop_id(
        entity_id,
    )
}
...
fn do_transfer(
    ...
    assert_ok!(Shop::request_transfer_shop(
        RuntimeOrigin::signed(from_owner),
        shop_id,
        to_entity_id
    ));
    assert_ok!(Shop::accept_transfer_shop(
        RuntimeOrigin::signed(to_owner),
        shop_id,
        keep_managers
    ));
}
...
fn close_and_finalize(who: u64, shop_id: u64) {
    ...
    assert_ok!(Shop::finalize_close_shop(
...
#[test]
fn create_shop_fails_initial_fund_below_minimum() {
    new_test_ext().execute_with(|| {
        // min_fund = 5_000_000 * 10^12 / 10^16 = 500
        assert_noop!(
            Shop::create_shop(
                RuntimeOrigin::signed(1),
                1,
            ...
            Error::<Test>::InitialFundBelowMinimum
        );
        // Exact min passes
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
...
#[test]
fn calculate_min_initial_fund_works() {
    new_test_ext().execute_with(|| {
        // MockPricingProvider: price = 10^16
        // min_fund = 5_000_000 * 10^12 / 10^16 = 500
        let min_fund = Shop::calculate_min_initial_fund().unwrap();
        assert_eq!(min_fund, 500);
    });
}
...
fn fund_operating_fails_on_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        // Full close flow
        close_and_finalize(1, shop_id);

        // H2: Funding a closed shop should fail
        assert_noop!(
            Shop::fund_operating(RuntimeOrigin::signed(1), shop_id, 500),
...
fn withdraw_operating_fund_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::withdraw_operating_fund(
            RuntimeOrigin::signed(1),
            1,
            500
        ));
...
fn withdraw_operating_fund_closed_shop_no_minimum() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Shop 2", 1000);
...
fn h3_add_manager_fails_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::add_manager(RuntimeOrigin::signed(1), shop_id, 2),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn h3_remove_manager_fails_closed_shop() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Test Shop", 1000);
        assert_ok!(Shop::add_manager(RuntimeOrigin::signed(1), shop_id, 2));
        assert_ok!(Shop::close_shop(RuntimeOrigin::signed(1), shop_id));

        assert_noop!(
            Shop::remove_manager(RuntimeOrigin::signed(1), shop_id, 2),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn h4_deduct_operating_fund_fails_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Shop as ShopProvider<u64>>::deduct_operating_fund(shop_id, 100),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn h3_trait_pause_rejects_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Shop as ShopProvider<u64>>::pause_shop(shop_id),
            Error::<Test>::ShopNotActive
        );
    });
}
...
fn h3_trait_resume_rejects_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Shop as ShopProvider<u64>>::resume_shop(shop_id),
...
fn m1r3_force_close_shop_rejects_double_close() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::force_close_shop(shop_id));
        assert_eq!(
            Shop::shops(shop_id).unwrap().status,
...
fn closing_grace_period_rejects_early_finalize() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Shop::shops(shop_id).unwrap().status,
            ShopOperatingStatus::Closing
        );
        assert!(Shop::shop_closing_at(shop_id).is_some());

        // Finalize before grace period should fail
        assert_noop!(
            Shop::finalize_close_shop(RuntimeOrigin::signed(1), shop_id),
...
fn closing_allows_fund_deposit() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::fund_operating(RuntimeOrigin::signed(1), shop_id, 500));
    });
}
...
fn finalize_anyone_can_call() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::finalize_close_shop(RuntimeOrigin::signed(5), shop_id));
        assert_eq!(
            Shop::shops(shop_id).unwrap().status,
            ShopOperatingStatus::Closed
        );
...
fn f3_transfer_shop_works() {
    new_test_ext().execute_with(|| {
        ...
        let shop_id = create_non_primary_shop(1, 1, b"Shop B", 500);

        assert_eq!(Shop::shops(shop_id).unwrap().entity_id, 1);

        // request + accept transfer to Entity 2
        do_transfer(1, shop_id, 2, 2, true);

        let shop = Shop::shops(shop_id).unwrap();
        assert_eq!(shop.entity_id, 2);
        assert_eq!(Shop::shop_entity_id(shop_id), Some(2));
    });
}
...
fn f3_transfer_shop_rejects_primary() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::create_primary_shop(
            1,
            b"Primary".to_vec(),
            ShopType::OnlineStore
        ));
...
fn f3_transfer_shop_rejects_same_entity() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Shop", 500);

        assert_noop!(
            Shop::request_transfer_shop(RuntimeOrigin::signed(1), shop_id, 1),
...
fn f3_transfer_shop_rejects_inactive_target() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::request_transfer_shop(RuntimeOrigin::signed(1), shop_id, 3),
            Error::<Test>::EntityNotActive
        );
    });
}
...
fn f3_transfer_shop_rejects_closing() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::request_transfer_shop(RuntimeOrigin::signed(1), shop_id, 2),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn f3_accept_requires_target_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::accept_transfer_shop(RuntimeOrigin::signed(1), shop_id, true),
            Error::<Test>::NotAuthorized
        );
    });
}
...
fn f3_cancel_transfer_by_source() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::cancel_transfer_shop(
            RuntimeOrigin::signed(1),
            shop_id
        ));
        assert!(crate::PendingTransfers::<Test>::get(shop_id).is_none());
    });
}
...
fn f3_transfer_clears_managers_when_requested() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Shop", 500);
        assert_ok!(Shop::add_manager(RuntimeOrigin::signed(1), shop_id, 4));
        assert_eq!(Shop::shops(shop_id).unwrap().managers.len(), 1);

        // Transfer with keep_managers = false
        do_transfer(1, shop_id, 2, 2, false);

        assert_eq!(Shop::shops(shop_id).unwrap().managers.len(), 0);
    });
}
...
fn f3_transfer_rejects_active_orders() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Shop", 500);
        set_shop_has_active_orders(shop_id);
...
fn l4_pause_shop_returns_shop_not_active_for_fund_depleted() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::deduct_operating_fund(1, 950));
        assert_eq!(
            Shop::shops(1).unwrap().status,
            ShopOperatingStatus::FundDepleted
        );
...
fn withdraw_operating_fund_rejects_banned() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::ban_shop(
            RuntimeOrigin::root(),
            1,
            bounded_cid(b"violation")
        ));
        assert_noop!(
...
fn withdraw_operating_fund_rejects_closing() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::close_shop(RuntimeOrigin::signed(1), shop_id));
        assert_noop!(
            Shop::withdraw_operating_fund(RuntimeOrigin::signed(1), shop_id, 100),
            Error::<Test>::ShopAlreadyClosing
        );
    });
}
...
fn set_shop_type_rejects_closed_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::set_shop_type(RuntimeOrigin::signed(1), shop_id, ShopType::Warehouse),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn cancel_close_shop_restores_fund_depleted_when_low_balance() {
    new_test_ext().execute_with(|| {
        ...
        // Deplete funds first
        assert_ok!(<Shop as ShopProvider<u64>>::deduct_operating_fund(1, 450));
        assert_eq!(
            Shop::shops(1).unwrap().status,
            ShopOperatingStatus::FundDepleted
        );
...
fn cancel_close_shop_rejects_non_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Shop::cancel_close_shop(RuntimeOrigin::signed(2), shop_id),
            Error::<Test>::NotAuthorized
        );
    });
}
...
fn m3_transfer_shop_rejects_target_entity_at_limit() {
    new_test_ext().execute_with(|| {
        ...
        // Create a non-primary shop for entity 1
        let shop_id = create_non_primary_shop(1, 1, b"Transfer Shop", 500);
...
fn m3_transfer_shop_works_when_target_has_capacity() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(2),
            2,
            bounded_name(b"Entity2 Shop"),
            ShopType::OnlineStore,
            500,
        ));

        // Entity 1 has a non-primary shop
        let shop_id = create_non_primary_shop(1, 1, b"Transfer Shop", 500);

        // Transfer entity 1's shop to entity 2 — should succeed
        do_transfer(1, shop_id, 2, 2, true);
        assert_eq!(Shop::shops(shop_id).unwrap().entity_id, 2);
    });
}
...
fn l2_update_shop_rating_rejects_invalid_rating() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::update_shop_rating(1, 5));
    });
}
...
fn close_shop_rejects_active_orders() {
    new_test_ext().execute_with(|| {
        ...
        clear_shop_active_orders(shop_id);
        assert_ok!(Shop::close_shop(RuntimeOrigin::signed(1), shop_id));
    });
}
...
fn governance_close_shop_rejects_already_closed() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Shop as ShopProvider<u64>>::governance_close_shop(shop_id),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
fn governance_set_shop_type_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::governance_set_shop_type(
            1,
            ShopType::PhysicalStore
        ));
        assert_eq!(Shop::shops(1).unwrap().shop_type, ShopType::PhysicalStore);
...
fn governance_set_shop_type_rejects_closed() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            <Shop as ShopProvider<u64>>::governance_set_shop_type(shop_id, ShopType::PhysicalStore),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
// ============================================================================
// allocate_from_treasury tests
// ============================================================================

#[test]
fn allocate_from_treasury_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // Fund entity treasury account (account 1001 for entity 1)
        let entity_account = 1001u64;
        Balances::make_free_balance_be(&entity_account, 5000);

        assert_ok!(Shop::allocate_from_treasury(
            RuntimeOrigin::signed(1),
            1,
            1000
        ));
        ...
        let shop_account = Shop::shop_account_id(1);
        assert_eq!(Balances::free_balance(shop_account), 1500); // 500 initial + 1000 allocated
        System::assert_has_event(RuntimeEvent::Shop(crate::Event::TreasuryFundAllocated {
            entity_id: 1,
            shop_id: 1,
            amount: 1000,
            new_balance: 1500,
...
#[test]
fn allocate_from_treasury_fails_not_owner() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // Account 2 is not entity 1's owner
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(2), 1, 1000),
            Error::<Test>::NotAuthorized
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_zero_amount() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 0),
            Error::<Test>::ZeroFundAmount
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_shop_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 999, 1000),
            Error::<Test>::ShopNotFound
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_shop_closed() {
    new_test_ext().execute_with(|| {
        let shop_id = create_non_primary_shop(1, 1, b"Test Shop", 1000);
        close_and_finalize(1, shop_id);
        ...
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), shop_id, 500),
            Error::<Test>::ShopAlreadyClosed
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_shop_banned() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 500),
            Error::<Test>::ShopBanned
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_entity_locked() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 1000),
            Error::<Test>::EntityLocked
        );
    });
}
...
#[test]
fn allocate_from_treasury_fails_entity_not_active() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
...
#[test]
fn allocate_from_treasury_fails_insufficient_treasury() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // Entity treasury account has 0 balance (not funded)
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 1000),
            Error::<Test>::InsufficientTreasuryFund
        );
    });
}

#[test]
fn allocate_from_treasury_fails_wrong_entity() {
    new_test_ext().execute_with(|| {
        // Create shop for entity 1
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
            bounded_name(b"Shop E1"),
            ShopType::OnlineStore,
            500,
        ));
        // Entity 2 owner tries to allocate to entity 1's shop
        let entity2_account = 1002u64;
        Balances::make_free_balance_be(&entity2_account, 5000);

        // Account 2 is entity 2's owner, but shop 1 belongs to entity 1
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(2), 1, 500),
            Error::<Test>::NotAuthorized
        );
    });
}
...
#[test]
fn allocate_from_treasury_restores_fund_depleted() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // Manually set shop status to FundDepleted
        crate::Shops::<Test>::mutate(shop_id, |maybe_shop| {
            if let Some(shop) = maybe_shop {
                shop.status = ShopOperatingStatus::FundDepleted;
            }
        });
        assert_eq!(
            <Shop as ShopProvider<u64>>::shop_own_status(shop_id),
            Some(ShopOperatingStatus::FundDepleted)
        );

        // Fund entity treasury
        let entity_account = 1001u64;
        Balances::make_free_balance_be(&entity_account, 5000);

        // Allocate enough to restore (MinOperatingBalance = 100)
        assert_ok!(Shop::allocate_from_treasury(
            RuntimeOrigin::signed(1),
            shop_id,
            200
        ...
        assert_eq!(
            <Shop as ShopProvider<u64>>::shop_own_status(shop_id),
            Some(ShopOperatingStatus::Active)
        );
        System::assert_has_event(RuntimeEvent::Shop(crate::Event::ShopResumed { shop_id }));
    });
}

#[test]
fn allocate_from_treasury_keeps_entity_alive() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // Fund entity treasury with just enough
        let entity_account = 1001u64;
        Balances::make_free_balance_be(&entity_account, 100);

        // Try to allocate more than available (KeepAlive prevents full drain)
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 100),
...
#[test]
fn governance_allocate_from_treasury_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        assert_ok!(<Shop as ShopProvider<u64>>::governance_allocate_from_treasury(1, 1, 1000));

        let shop_account = Shop::shop_account_id(1);
        assert_eq!(Balances::free_balance(shop_account), 1500);
    });
}

// ============================================================================
// min_treasury_threshold 保护 (allocate_from_treasury)
// ============================================================================

#[test]
fn allocate_from_treasury_blocked_by_min_treasury_threshold() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        let entity_account = 1001u64;
        Balances::make_free_balance_be(&entity_account, 5000);
        // 设置 min_treasury_threshold = 4500
        set_min_treasury_threshold(1, 4500);
        // entity_balance=5000, protected=0, available=5000
        // 但 post_allocate = 5000 - 1000 = 4000 < 4500 → 阻断
        assert_noop!(
            Shop::allocate_from_treasury(RuntimeOrigin::signed(1), 1, 1000),
            Error::<Test>::InsufficientTreasuryFund
        );
...
#[test]
fn allocate_from_treasury_succeeds_above_threshold() {
    new_test_ext().execute_with(|| {
        assert_ok!(Shop::create_shop(
            RuntimeOrigin::signed(1),
            1,
        ...
        // 设置 min_treasury_threshold = 4500
        set_min_treasury_threshold(1, 4500);
        // post_allocate = 5000 - 500 = 4500 >= 4500 → OK
        assert_ok!(Shop::allocate_from_treasury(
            RuntimeOrigin::signed(1),
            1,
            500
        ...
        let shop_account = Shop::shop_account_id(1);
        assert_eq!(Balances::free_balance(shop_account), 1000); // 500 initial + 500 allocated
    });
}

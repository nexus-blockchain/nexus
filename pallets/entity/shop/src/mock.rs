...
thread_local! {
    ...
    static MOCK_MIN_TREASURY_THRESHOLD: RefCell<alloc::collections::BTreeMap<u64, u128>> = RefCell::new(alloc::collections::BTreeMap::new());
}
...
impl pallet_entity_common::ProductProvider<u64> for MockProductProvider {
    fn product_exists(_: u64) -> bool {
        false
    }
    fn is_product_on_sale(_: u64) -> bool {
        false
    }
    fn product_shop_id(_: u64) -> Option<u64> {
        None
    }
    fn product_stock(_: u64) -> Option<u32> {
        None
    }
    fn product_category(_: u64) -> Option<pallet_entity_common::ProductCategory> {
        None
    }
    fn deduct_stock(_: u64, _: u32) -> Result<(), sp_runtime::DispatchError> {
        Ok(())
    }
    fn restore_stock(_: u64, _: u32) -> Result<(), sp_runtime::DispatchError> {
        Ok(())
    }
    fn add_sold_count(_: u64, _: u32) -> Result<(), sp_runtime::DispatchError> {
        Ok(())
    }
}
...
impl pallet_entity_common::OrderProvider<u64, u64> for MockOrderProvider {
    fn order_exists(_: u64) -> bool {
        false
    }
    fn order_buyer(_: u64) -> Option<u64> {
        None
    }
    fn order_seller(_: u64) -> Option<u64> {
        None
    }
    fn order_amount(_: u64) -> Option<u64> {
        None
    }
    fn order_shop_id(_: u64) -> Option<u64> {
        None
    }
    fn is_order_completed(_: u64) -> bool {
        false
    }
    fn is_order_disputed(_: u64) -> bool {
        false
    }
    fn can_dispute(_: u64, _: &u64) -> bool {
        false
    }
...
parameter_types! {
    ...
    pub const MinInitialFundUsdt: u64 = 5_000_000; // 5 USDT（精度 10^6）
}

/// Mock 定价: 价格 = 10^16 → 5 USDT 等值 500 token（适配 u64 Balance 测试规模）
pub struct MockPricingProvider;
impl pallet_entity_common::PricingProvider for MockPricingProvider {
    fn get_nex_usdt_price() -> u64 {
        // 5_000_000 * 10^12 / 10^16 = 500
...
impl pallet_entity_shop::Config for Test {
    ...
    type MinInitialFundUsdt = MinInitialFundUsdt;

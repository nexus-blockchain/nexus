//! # Shop pallet (pallet-entity-shop)
//!
//! ## Overview
//!
//! This pallet manages the lifecycle of business-layer shops, including:
//! 本模块负责 Shop（业务层店铺）的生命周期管理，包括：
//! - shop creation under an Entity
//! - Shop 创建（归属于 Entity）
...
//! - 运营资金管理
//! - shop state management (pause, resume, close)
//! - Shop 状态管理（暂停、恢复、关闭）
//! - optional shop points support
//! - Shop 积分系统（可选）
//!
//! ## Entity-Shop relationship
//!
//! - one Entity can own multiple shops
//! - 一个 Entity 可以拥有多个 Shop
//! - each Entity has exactly one primary shop created automatically with the Entity
//! - 每个 Entity 有且仅有一个 Primary Shop（创建 Entity 时自动创建）
//! - shops inherit the Entity token and governance domain but keep independent
//!   operating funds and business data
//! - Shop 继承 Entity 的代币和治理，但有独立的运营资金和业务数据
//!
//! ## Version history
//!
//! - v0.1.0 (2026-02-05): initial release
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        ...
        FundProtectionQueryPort, OrderProvider, PointsCleanup, PricingProvider, ProductProvider,
        ShopOperatingStatus, ShopProvider, ShopType,
    };
    use pallet_storage_service::{PinTier, StoragePin};
    use sp_runtime::{
    ...
    /// Pending transfer request using a two-sided confirmation flow.
    /// 转让请求（双向确认模式）。
    #[derive(
        Encode,
        Decode,
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Minimum operating balance. A shop pauses below this threshold.
        /// 最低运营余额（低于此值 Shop 暂停）
        #[pallet::constant]
        type MinOperatingBalance: Get<BalanceOf<Self>>;

        /// Warning threshold for operating funds.
        /// 资金预警阈值
        #[pallet::constant]
        type WarningThreshold: Get<BalanceOf<Self>>;

        /// Pricing provider used to convert USDT values into NEX.
        /// 定价提供者（用于计算 USDT 等值 NEX）
        type PricingProvider: pallet_entity_common::PricingProvider;

        /// Minimum initial operating fund in USDT precision 10^6.
        /// `5_000_000` means 5 USDT.
        /// 创建 Shop 最低初始运营资金 USDT 金额（精度 10^6，`5_000_000` = 5 USDT）
        #[pallet::constant]
        type MinInitialFundUsdt: Get<u64>;

        /// Commission-fund guard that checks committed pending and shopping-balance totals.
        /// 佣金资金保护（查询已承诺的 pending + shopping 总额）
        type CommissionFundGuard: pallet_entity_common::CommissionFundGuard;

        /// Shop-closing grace period in blocks.
        /// Shop 关闭宽限期（区块数）
        #[pallet::constant]
        type ShopClosingGracePeriod: Get<BlockNumberFor<Self>>;

        /// Maximum number of shops per entity.
        /// 每个 Entity 最大 Shop 数量
        #[pallet::constant]
        type MaxShopsPerEntity: Get<u32>;

        /// IPFS pin management interface used to persist shop-metadata CIDs.
        /// IPFS Pin 管理接口（用于店铺元数据 CID 持久化）
        type StoragePin: StoragePin<Self::AccountId>;

        /// Product provider used to cascade `unpin` of product CIDs when a shop closes.
        ...
        /// 资金保护配置查询（用于 allocate_from_treasury 检查 min_treasury_threshold）
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        #[cfg(feature = "std")]
        fn integrity_test() {
            assert!(T::MaxManagers::get() >= 1, "MaxManagers must be >= 1");
            assert!(
                T::MaxShopNameLength::get() >= 1,
                "MaxShopNameLength must be >= 1"
            );
            assert!(T::MaxCidLength::get() >= 1, "MaxCidLength must be >= 1");
            assert!(
    ...
    #[pallet::storage]
    pub type ShopStatusBeforeBan<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, ShopOperatingStatus>;

    /// 封禁原因 shop_id -> reason
    #[pallet::storage]
    pub type ShopBanReason<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BoundedVec<u8, T::MaxCidLength>>;

    /// 待确认的转让请求 shop_id -> PendingShopTransfer
    #[pallet::storage]
    pub type PendingTransfers<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, PendingShopTransferOf<T>>;
    ...
    pub enum Event<T: Config> {
        ...
        /// Shop 运营资金充值
        OperatingFundDeposited {
            shop_id: u64,
            amount: BalanceOf<T>,
            new_balance: BalanceOf<T>,
        },
        /// Shop 运营资金扣减
        OperatingFundDeducted {
            shop_id: u64,
            amount: BalanceOf<T>,
            new_balance: BalanceOf<T>,
        },
        ...
        ShopUnbannedByRoot {
            shop_id: u64,
            restored_status: ShopOperatingStatus,
        },
        /// Entity 金库向 Shop 划拨运营资金
        TreasuryFundAllocated {
            entity_id: u64,
            shop_id: u64,
            amount: BalanceOf<T>,
            new_balance: BalanceOf<T>,
    ...
    pub enum Error<T> {
        ...
        /// 无待确认的转让请求
        NoTransferPending,
        /// NEX/USDT 价格不可用（Oracle 离线）
        PriceUnavailable,
        /// 初始运营资金不足（低于 MinInitialFundUsdt 等值 NEX）
        InitialFundBelowMinimum,
        /// Entity 金库可用余额不足（已扣除佣金保护金额）
        InsufficientTreasuryFund,
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_shop(
            ...
            // 最低初始资金校验（MinInitialFundUsdt 等值 NEX）
            let min_fund = Self::calculate_min_initial_fund()?;
            ensure!(
                initial_fund >= min_fund,
                Error::<T>::InitialFundBelowMinimum
            );

            let shop_id = Self::do_create_shop(entity_id, name, shop_type)?;

            // 转移初始资金（Shop 已创建，转账失败会回滚整个 extrinsic）
        ...
        pub fn update_shop(
            ...
            let who = ensure_signed(origin)?;
                ...
                name.is_some()
                    || logo_cid.is_some()
                    || description_cid.is_some()
                    || business_hours_cid.is_some()
                    || policies_cid.is_some(),
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                    ...
                    !T::EntityProvider::is_entity_locked(shop.entity_id),
                    Error::<T>::EntityLocked
                );
                ensure!(
                    T::EntityProvider::is_entity_active(shop.entity_id),
                    Error::<T>::EntityNotActive
                );
                ensure!(
                    !shop.status.is_terminal_or_banned(),
                    Error::<T>::ShopAlreadyClosed
                );

                let owner =
                    T::EntityProvider::entity_owner(shop.entity_id).unwrap_or_else(|| who.clone());
        ...
        pub fn add_manager(
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                let owner = T::EntityProvider::entity_owner(shop.entity_id)
                    ...
                    !shop.managers.contains(&manager),
        ...
        pub fn fund_operating(
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                if shop.status == ShopOperatingStatus::FundDepleted
                    && new_balance >= T::MinOperatingBalance::get()
                {
                    shop.status = ShopOperatingStatus::Active;
                    Self::deposit_event(Event::ShopResumed { shop_id });
                }

                Self::deposit_event(Event::OperatingFundDeposited {
                    shop_id,
                    amount,
                    new_balance,
                });
        ...
        pub fn pause_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn resume_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                let balance = T::Currency::free_balance(&shop_account);
                    ...
                    Error::<T>::InsufficientOperatingFund
                );
        ...
        pub fn close_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                Self::deposit_event(Event::ShopClosing {
                    shop_id,
                    grace_until,
                });
                Ok(())
        ...
        pub fn finalize_close_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                    ...
                    shop.status == ShopOperatingStatus::Closing,
                    Error::<T>::ShopNotClosing
                );

                let closing_at =
                    ShopClosingAt::<T>::get(shop_id).ok_or(Error::<T>::ShopNotFound)?;
                let now = <frame_system::Pallet<T>>::block_number();
                let grace = T::ShopClosingGracePeriod::get();
                ensure!(
                    now >= closing_at.saturating_add(grace),
                    Error::<T>::ClosingGracePeriodNotElapsed
                );
        ...
        pub fn withdraw_operating_fund(
            ...
            ensure!(balance >= amount, Error::<T>::InsufficientOperatingFund);
        ...
        pub fn request_transfer_shop(
            ...
            // 不得有未处理的转让请求
                ...
                !PendingTransfers::<T>::contains_key(shop_id),
            ...
            let owner = T::EntityProvider::entity_owner(from_entity_id)
                ...
                !T::EntityProvider::is_entity_locked(from_entity_id),
                Error::<T>::EntityLocked
            );
                ...
                !shop.status.is_closed_or_closing(),
                Error::<T>::ShopAlreadyClosed
            );
            ensure!(!shop.status.is_banned(), Error::<T>::ShopBanned);
            ...
            // 目标 Entity 必须存在且激活
                ...
                T::EntityProvider::entity_exists(to_entity_id),
                Error::<T>::EntityNotFound
            );
            ensure!(
                T::EntityProvider::is_entity_active(to_entity_id),
                Error::<T>::EntityNotActive
            );
        ...
        pub fn accept_transfer_shop(
            ...
            let pending =
                PendingTransfers::<T>::get(shop_id).ok_or(Error::<T>::NoTransferPending)?;
            ...
            let target_owner = T::EntityProvider::entity_owner(pending.to_entity_id)
                ...
                !T::EntityProvider::is_entity_locked(pending.to_entity_id),
                Error::<T>::EntityLocked
            );

            // 重新验证目标方条件（可能在请求后变化）
            ensure!(
                T::EntityProvider::is_entity_active(pending.to_entity_id),
                Error::<T>::EntityNotActive
            );
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                    ...
                    shop.entity_id == pending.from_entity_id,
                    Error::<T>::InvalidConfig
                );
                ensure!(
                    !shop.status.is_closed_or_closing(),
        ...
        pub fn cancel_transfer_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        /// Entity 金库向 Shop 划拨运营资金（None 模式下 Owner 直接调用）
        #[pallet::call_index(35)]
        #[pallet::weight(T::WeightInfo::allocate_from_treasury())]
        pub fn allocate_from_treasury(
            origin: OriginFor<T>,
            shop_id: u64,
            amount: BalanceOf<T>,
        ) -> DispatchResult {
            ...
            Self::governance_allocate_from_treasury_inner(shop.entity_id, shop_id, amount)?;
            Ok(())
        }
        ...
        pub fn set_primary_shop(
            ...
            let owner =
                ...
                who == owner
                    || T::EntityProvider::is_entity_admin(
                        entity_id,
                        &who,
                        pallet_entity_common::AdminPermission::ENTITY_MANAGE,
                    ),
                Error::<T>::NotAuthorized
            );
            ensure!(
            ...
            let new_shop = Shops::<T>::get(new_primary_shop_id).ok_or(Error::<T>::ShopNotFound)?;
                ...
                !new_shop.status.is_terminal_or_banned(),
                Error::<T>::ShopAlreadyClosed
            );
            ensure!(
        ...
        pub fn force_pause_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn force_close_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn set_shop_type(
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                Self::deposit_event(Event::ShopTypeChanged {
                    shop_id,
                    old_type,
                    new_type: new_shop_type,
                });
                Ok(())
            })
        }
        ...
        #[pallet::weight(T::WeightInfo::cancel_close_shop())]
        pub fn cancel_close_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn resign_manager(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                Self::deposit_event(Event::ManagerResigned {
                    shop_id,
                    manager: who,
                });
                Ok(())
        ...
        pub fn unban_shop(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                ...
                let restored =
                    ShopStatusBeforeBan::<T>::take(shop_id).unwrap_or(ShopOperatingStatus::Paused);
                ShopBanReason::<T>::remove(shop_id);
                shop.status = restored;

                Self::deposit_event(Event::ShopUnbannedByRoot {
                    shop_id,
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 计算创建 Shop 所需最低初始资金（MinInitialFundUsdt 等值 NEX）
        ///
        /// # 算法
        /// 1. 获取 NEX/USDT 价格（精度 10^6）
        /// 2. 计算所需 NEX = USDT 金额 * 10^12 / 价格
        /// 3. 结果取 max(计算值, MinOperatingBalance) 确保不低于运营底线
        pub fn calculate_min_initial_fund() -> Result<BalanceOf<T>, DispatchError> {
            let price = T::PricingProvider::get_nex_usdt_price();
            ensure!(price > 0u64, Error::<T>::PriceUnavailable);

            let usdt_amount = T::MinInitialFundUsdt::get();
            ...
            // 确保不低于 MinOperatingBalance
            Ok(nex_amount.max(T::MinOperatingBalance::get()))
        }
        ...
        pub fn can_manage_shop(shop: &ShopOf<T>, account: &T::AccountId) -> bool {
            ...
            if T::EntityProvider::is_entity_admin(
                shop.entity_id,
                account,
        ...
        /// Entity 金库向 Shop 划拨运营资金（共享内部实现）
        fn governance_allocate_from_treasury_inner(
            entity_id: u64,
            shop_id: u64,
            amount: BalanceOf<T>,
        ) -> DispatchResult {
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> DispatchResult {
                let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                ensure!(shop.entity_id == entity_id, Error::<T>::NotAuthorized);
                ensure!(
                    shop.status != ShopOperatingStatus::Closed,
                    Error::<T>::ShopAlreadyClosed
                );
                ...
                // 佣金资金保护：从 Entity 金库划拨时，不得侵占已承诺给会员的佣金资金
                ...
                let available = entity_balance.saturating_sub(protected);
                ensure!(amount <= available, Error::<T>::InsufficientTreasuryFund);

                // min_treasury_threshold 保护：划拨后金库余额不得低于治理阈值
                ...
                if !min_threshold.is_zero() {
                    let post_allocate = entity_balance.saturating_sub(amount);
                    ensure!(
                        post_allocate >= min_threshold,
                        Error::<T>::InsufficientTreasuryFund
                    );
                }
                ...
                Self::deposit_event(Event::TreasuryFundAllocated {
                    entity_id,
                    shop_id,
                    amount,
                    new_balance,
    ...
    impl<T: Config> ShopProvider<T::AccountId> for Pallet<T> {
        ...
        fn effective_status(shop_id: u64) -> Option<EffectiveShopStatus> {
            ...
            let entity_id = ShopEntity::<T>::get(shop_id)?;
            let entity_status =
                T::EntityProvider::entity_status(entity_id).unwrap_or(EntityStatus::Pending);
        ...
        fn update_shop_stats(
            ...
            let entity_id =
                Shops::<T>::try_mutate(shop_id, |maybe_shop| -> Result<u64, DispatchError> {
                    let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                    shop.total_sales = shop
                        .total_sales
                        .saturating_add(sales_amount.saturated_into());
            ...
            Shops::<T>::try_mutate(shop_id, |maybe_shop| -> Result<(), DispatchError> {
                ...
                match new_rating {
                    Some(nr) => {
                        ...
                        let new_r = nr as u64;
                        shop.rating_total =
                            shop.rating_total.saturating_add(new_r.saturating_mul(100));
                    }
                    None => {
                        // 删除评价：减少 count
        ...
        fn create_primary_shop(
            ...
            let bounded_name: BoundedVec<u8, T::MaxShopNameLength> =
        ...
        fn governance_allocate_from_treasury(
            ...
            let amount_balance: BalanceOf<T> = amount.saturated_into();
            Self::governance_allocate_from_treasury_inner(entity_id, shop_id, amount_balance)
        }
    }
    ...
    impl<T: Config> pallet_entity_common::ShopGovernancePort for Pallet<T> {
        ...
        fn governance_toggle_points(_: u64, _: bool) -> Result<(), DispatchError> {
            Err(DispatchError::Other("not implemented"))
        }
        ...
        fn governance_set_shop_policies(
            ...
            let bounded_cid: BoundedVec<u8, T::MaxCidLength> = policies_cid
                .to_vec()
                .try_into()
                .map_err(|_| DispatchError::Other("CidTooLong"))?;
            ensure!(!bounded_cid.is_empty(), Error::<T>::EmptyCid);

            let shop_ids = T::EntityProvider::entity_shops(entity_id);
            ensure!(!shop_ids.is_empty(), Error::<T>::ShopNotFound);

            for shop_id in shop_ids {
                Shops::<T>::try_mutate(shop_id, |maybe_shop| -> Result<(), DispatchError> {
                    let shop = maybe_shop.as_mut().ok_or(Error::<T>::ShopNotFound)?;
                    shop.policies_cid = Some(bounded_cid.clone());
                    Ok(())
                })?;
                Self::deposit_event(Event::ShopUpdated { shop_id });
            }

//! # Entity member pallet (pallet-entity-member)
//!
//! ## Overview
//!
//! This pallet manages shop-member referral relationships and supports:
//! 本模块实现店铺会员推荐关系管理，支持：
//! - a separate membership system for each shop
//! - 每个店铺独立的会员体系
//! - three-level referral commission relationships
//! - 三级分销推荐返佣
//! - member-level management
//! - 会员等级管理
//! - referral-statistics queries
//! - 推荐统计查询
//!
//! ## Version history
//!
//! - v0.1.0: initial release with basic member referral relationships
...
/// Entity-level KYC checker.
/// Entity 级别的 KYC 检查接口。
///
/// Used for the `KYC_REQUIRED` registration policy and the
/// `KYC_UPGRADE_REQUIRED` upgrade policy.
/// 用于 `KYC_REQUIRED`（注册）和 `KYC_UPGRADE_REQUIRED`（升级）策略。
/// The runtime typically bridges this through
/// `pallet-entity-kyc::can_participate_in_entity`.
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        AdminPermission, EntityProvider, MemberRegistrationPolicy, OnLevelRemoved,
        OnMemberLevelChanged, ShopProvider,
    };
    ...
    pub struct CustomLevel {
        ...
        /// 最低直接推荐人数（0 = 无要求）
        pub min_direct_referrals: u32,
        /// 最低团队总人数（0 = 无要求）
        pub min_team_size: u32,
        /// 最低间接推荐人数（0 = 无要求）
        pub min_indirect_referrals: u32,
    }

    /// Entity member-level system configuration.
    /// 实体会员等级系统配置。
    #[derive(
        Encode,
        Decode,
    ...
    pub enum UpgradeTrigger {
        ...
        /// 累计消费达标（USDT 精度 10^6）
        TotalSpent { threshold: u64 },
        /// 单笔消费达标（USDT 精度 10^6）
        SingleOrder { threshold: u64 },
        /// 推荐人数达标
    ...
    /// 实体升级规则系统类型别名
    pub type EntityUpgradeRuleSystemOf<T> =
        EntityUpgradeRuleSystem<BlockNumberFor<T>, <T as Config>::MaxUpgradeRules>;
    ...
    pub struct EntityMember<AccountId, BlockNumber> {
        ...
        pub total_spent: u64,
        /// 可用于升级的消费额（排除购物余额消费）
        pub upgrade_eligible_spent: u64,
    ...
    /// 实体会员类型别名
    pub type EntityMemberOf<T> =
        EntityMember<<T as frame_system::Config>::AccountId, BlockNumberFor<T>>;
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Shop lookup interface for the Entity-Shop split architecture.
        /// Shop 查询接口（Entity-Shop 分离架构）
        type ShopProvider: pallet_entity_common::ShopProvider<Self::AccountId>;

        /// Maximum direct-referral count.
        /// 最大直接推荐人数
        #[pallet::constant]
        type MaxDirectReferrals: Get<u32>;

        /// Maximum number of custom levels.
        /// 最大自定义等级数量
        #[pallet::constant]
        type MaxCustomLevels: Get<u32>;

        /// Maximum number of upgrade rules.
        /// 最大升级规则数量
        #[pallet::constant]
        type MaxUpgradeRules: Get<u32>;

        /// Maximum number of upgrade-history entries.
        /// 最大升级历史记录数量
        #[pallet::constant]
        type MaxUpgradeHistory: Get<u32>;

        /// Expiry in blocks for pending members after the M6 fix. `0` means no expiry.
        /// M6 修复：待审批会员过期区块数（`0` = 不过期）
        #[pallet::constant]
        type PendingMemberExpiry: Get<BlockNumberFor<Self>>;

        /// KYC checker used by `KYC_REQUIRED` and `KYC_UPGRADE_REQUIRED` policies.
        /// KYC 检查接口（用于 `KYC_REQUIRED` / `KYC_UPGRADE_REQUIRED` 策略）
        type KycChecker: KycChecker<Self::AccountId>;

        /// Member-removal callback used to notify commission plugins to clean up
        /// per-user storage.
        /// P2-14：会员移除回调（通知佣金插件清理 per-user 存储）
        type OnMemberRemoved: pallet_entity_common::OnMemberRemoved<Self::AccountId>;

        /// Member-level change callback used by pool-reward and other dependents to
        /// maintain side counters.
        /// 会员等级变更回调（通知 pool-reward 等依赖方维护侧边计数器）
        type OnMemberLevelChanged: pallet_entity_common::OnMemberLevelChanged<Self::AccountId>;

        /// Member-team change callback used by pool-reward and other dependents to
        /// maintain dynamic-unlock counters.
        /// 会员团队变更回调（通知 pool-reward 等依赖方维护动态解锁计数器）
        type OnMemberTeamChanged: pallet_entity_common::OnMemberTeamChanged<Self::AccountId>;

        /// Level-removal callback used to clean up single-line commission overrides.
        /// 等级删除回调（通知 commission-single-line 清理 override）
        type OnLevelRemoved: pallet_entity_common::OnLevelRemoved;
        ...
        fn on_idle(_n: BlockNumberFor<T>, remaining_weight: Weight) -> Weight {
            ...
            if !expiry.is_zero() {
                ...
                if !remaining_weight.any_lt(min_weight) {
                    ...
                    let mut to_remove: alloc::vec::Vec<(u64, T::AccountId)> =
                        alloc::vec::Vec::new();
            ...
            if processed_id < next_id {
                ...
                if !remaining_weight.any_lt(weight_used) {
                    if let Some(mut update) = PendingTeamSizeUpdates::<T>::get(processed_id) {
                        ...
                        let mut current =
                            EntityMembers::<T>::get(update.entity_id, &update.start_account)
                                ...
                                |maybe_member| {
                                    if let Some(ref mut m) = maybe_member {
                                        if update.delta > 0 {
                                            ...
                                            m.indirect_referrals =
                                                m.indirect_referrals.saturating_add(1);
                                        } else {
                                            m.team_size = m.team_size.saturating_sub(1);
                                            m.indirect_referrals =
                                                m.indirect_referrals.saturating_sub(1);
                                        }
                                    }
                                },
                            );
    ...
    pub type LevelMemberCount<T: Config> =
    ...
    pub type EntityLevelSystems<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, EntityLevelSystemOf<T>>;

    /// 实体升级规则系统 entity_id -> EntityUpgradeRuleSystem
    #[pallet::storage]
    #[pallet::getter(fn entity_upgrade_rules)]
    pub type EntityUpgradeRules<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, EntityUpgradeRuleSystemOf<T>>;
    ...
    pub type MemberOrderCount<T: Config> =
    ...
    pub type EntityMemberPolicy<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, MemberRegistrationPolicy, ValueQuery>;
    ...
    #[pallet::storage]
    pub type PendingTeamSizeUpdates<T: Config> =
        StorageMap<_, Blake2_128Concat, u32, TeamSizeUpdateOf<T>>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 自定义等级更新
        CustomLevelUpdated { shop_id: u64, level_id: u8 },
        /// 自定义等级删除
        CustomLevelRemoved { shop_id: u64, level_id: u8 },
        /// 升级规则系统初始化
        ...
        /// 升级规则更新
        UpgradeRuleUpdated { shop_id: u64, rule_id: u32 },
        /// 升级规则删除
        UpgradeRuleRemoved { shop_id: u64, rule_id: u32 },
        /// 会员通过规则升级
        ...
        /// R6-L1: 自定义等级开关变更
        UseCustomLevelsUpdated { shop_id: u64, use_custom: bool },
        /// R6-L1: 升级模式变更
        UpgradeModeUpdated {
            shop_id: u64,
            upgrade_mode: LevelUpgradeMode,
        ...
        /// R6-L1: 升级规则系统启用/禁用
        UpgradeRuleSystemToggled { shop_id: u64, enabled: bool },
        /// R6-L1: 冲突策略变更
        ...
        /// 批量审批通过
        BatchMembersApproved { entity_id: u64, count: u32 },
        /// 批量审批拒绝
        BatchMembersRejected { entity_id: u64, count: u32 },
        /// 会员被移除
        MemberRemoved {
            entity_id: u64,
            account: T::AccountId,
        ...
        /// 等级系统已重置
        LevelSystemReset { entity_id: u64 },
        /// 升级规则系统已重置
        UpgradeRuleSystemReset { entity_id: u64 },
        /// U1: 会员主动退出
        MemberLeft {
            entity_id: u64,
            account: T::AccountId,
        ...
        /// G1: 治理路径切换升级规则系统开关
        GovernanceUpgradeRuleSystemToggled { entity_id: u64, enabled: bool },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_member(
            ...
            let entity_id = Self::resolve_entity_id(shop_id).ok_or(Error::<T>::ShopNotFound)?;

            // 检查是否已注册（会员统一在 Entity 级别）
            ensure!(
                !EntityMembers::<T>::contains_key(entity_id, &who),
            ...
            // PURCHASE_REQUIRED: 手动注册被拒（必须通过 auto_register 即下单触发）
                ...
                !policy.requires_purchase(),
                Error::<T>::PurchaseRequiredForRegistration
            );

            // KYC_REQUIRED: 注册时需要通过 KYC
            if policy.requires_kyc() {
                ensure!(
                    T::KycChecker::is_kyc_passed(entity_id, &who),
                    Error::<T>::KycNotPassed
                );
            }
            ...
            if policy.requires_referral() {
                    ...
                    referrer.is_some(),
                    Error::<T>::ReferralRequiredForRegistration
                );
            }
            ...
            // 正常注册
            Self::do_register_member(entity_id, &who, referrer.clone())?;
        ...
        pub fn bind_referrer(
            ...
            // 验证是会员
            let member = Self::get_member_by_shop(shop_id, &who).ok_or(Error::<T>::NotMember)?;
            ...
            Self::mutate_member_referral(entity_id, &referrer, &who)?;
        ...
        /// - `min_direct_referrals`: 最低直接推荐人数（0 = 无要求）
        /// - `min_team_size`: 最低团队总人数（0 = 无要求）
        /// - `min_indirect_referrals`: 最低间接推荐人数（0 = 无要求）
        #[pallet::call_index(5)]
        ...
        /// - `min_direct_referrals`: 最低直接推荐人数（可选）
        /// - `min_team_size`: 最低团队总人数（可选）
        /// - `min_indirect_referrals`: 最低间接推荐人数（可选）
        #[pallet::call_index(6)]
        ...
        pub fn update_custom_level(
            ...
            min_direct_referrals: Option<u32>,
            min_team_size: Option<u32>,
            min_indirect_referrals: Option<u32>,
        ) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let entity_id = Self::ensure_shop_owner_or_admin(shop_id, &who)?;
            ensure!(
            ...
            EntityLevelSystems::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                if let Some(v) = min_direct_referrals {
                    level.min_direct_referrals = v;
                }
                if let Some(v) = min_team_size {
                    level.min_team_size = v;
                }
                if let Some(v) = min_indirect_referrals {
                    level.min_indirect_referrals = v;
                }
        ...
        pub fn remove_custom_level(
            ...
            EntityLevelSystems::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                // 只能删除最后一个等级
                    ...
                    level_id > 0
                        && (level_id - 1) as usize == system.levels.len().saturating_sub(1),
        ...
        pub fn manual_set_member_level(
            ...
            if target_level_id > m.custom_level_id {
                ...
                if policy.requires_kyc_for_upgrade() {
                        ...
                        T::KycChecker::is_kyc_passed(entity_id, &member),
                        Error::<T>::KycRequiredForUpgrade
                    );
                }
            }
            ...
            EntityMembers::<T>::mutate(entity_id, &member, |maybe_member| -> DispatchResult {
                ...
                // 维护 LevelMemberCount
                LevelMemberCount::<T>::mutate(entity_id, old_level_id, |c| {
                    *c = c.saturating_sub(1)
                });
                LevelMemberCount::<T>::mutate(entity_id, target_level_id, |c| {
        ...
        pub fn set_use_custom_levels(
            ...
            EntityLevelSystems::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                Self::deposit_event(Event::UseCustomLevelsUpdated {
                    shop_id,
                    use_custom,
                });
        ...
        pub fn set_upgrade_mode(
            ...
            EntityLevelSystems::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                Self::deposit_event(Event::UpgradeModeUpdated {
                    shop_id,
                    upgrade_mode,
                });
        ...
        pub fn add_upgrade_rule(
            ...
            EntityUpgradeRules::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                system
                    .rules
                    .try_push(rule)
                    .map_err(|_| Error::<T>::UpgradeRulesFull)?;
        ...
        pub fn set_conflict_strategy(
            ...
            EntityUpgradeRules::<T>::try_mutate(entity_id, |maybe_system| -> DispatchResult {
                ...
                Self::deposit_event(Event::ConflictStrategyUpdated {
                    shop_id,
        ...
        pub fn approve_member(
            ...
            // 正式注册
            Self::do_register_member(entity_id, &account, referrer)?;
        ...
        pub fn reject_member(
            ...
            Self::deposit_event(Event::MemberRejected { entity_id, account });
        ...
        pub fn cancel_pending_member(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn cleanup_expired_pending(
            ...
            for account in to_remove {
                PendingMembers::<T>::remove(entity_id, &account);
                Self::deposit_event(Event::PendingMemberExpired { entity_id, account });
            }
        ...
        pub fn batch_approve_members(
            ...
            for account in accounts.iter() {
                if let Some((referrer, applied_at)) = PendingMembers::<T>::take(entity_id, account)
                {
                    ...
                    // 尝试注册，失败则静默跳过（如已是会员）
                    if Self::do_register_member(entity_id, account, referrer).is_ok() {
        ...
        pub fn ban_member(
            ...
            EntityMembers::<T>::try_mutate(
                entity_id,
                &account,
                |maybe_member| -> DispatchResult {
        ...
        pub fn unban_member(
            ...
            Self::deposit_event(Event::MemberUnbanned { entity_id, account });
        ...
        pub fn remove_member(
            ...
            Self::deposit_event(Event::MemberRemoved { entity_id, account });
        ...
        pub fn reset_level_system(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let entity_id = Self::ensure_shop_owner_or_admin(shop_id, &who)?;
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
        ...
        pub fn reset_upgrade_rule_system(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            let entity_id = Self::ensure_shop_owner_or_admin(shop_id, &who)?;
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
        ...
        pub fn leave_entity(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let member = EntityMembers::<T>::get(entity_id, &who).ok_or(Error::<T>::NotMember)?;
        ...
        pub fn activate_member(
            ...
            Self::deposit_event(Event::MemberActivated { entity_id, account });
        ...
        pub fn deactivate_member(
            ...
            Self::deposit_event(Event::MemberDeactivated { entity_id, account });
...
impl<T: pallet::Config> MemberProvider<T::AccountId> for pallet::Pallet<T> {
    ...
    fn auto_register(
        ...
        pallet::Pallet::<T>::auto_register_by_entity(entity_id, account, referrer)
    }
    ...
    fn update_spent(
        ...
        pallet::Pallet::<T>::update_spent_by_entity(entity_id, account, amount_usdt, false)
    }
    ...
    fn check_order_upgrade_rules(
        ...
        pallet::Pallet::<T>::check_order_upgrade_rules_by_entity(
            entity_id,
            buyer,
            product_id,
            amount_usdt,
    ...
    fn get_member_total_spent_usdt(entity_id: u64, account: &T::AccountId) -> u64 {
        pallet::EntityMembers::<T>::get(entity_id, account)
            .map(|m| m.total_spent)
            .unwrap_or(0)
    }

    fn get_member_upgrade_eligible_spent_usdt(entity_id: u64, account: &T::AccountId) -> u64 {
        pallet::EntityMembers::<T>::get(entity_id, account)
    ...
    fn add_custom_level(
        ...
        pallet::Pallet::<T>::governance_add_custom_level(
            entity_id,
            name,
            threshold,
            discount_rate,
    ...
    fn update_custom_level(
        ...
        pallet::Pallet::<T>::governance_update_custom_level(
            entity_id,
            level_id,
            name,
            threshold,

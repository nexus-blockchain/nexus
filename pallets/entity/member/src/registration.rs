...
use pallet_entity_common::{
    AdminPermission, EntityProvider, OnMemberLevelChanged, OnMemberRemoved, ShopProvider,
};
...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn ensure_referrer_bound(
        ...
        // 推荐人自己必须有推荐人
        let ref_member =
            EntityMembers::<T>::get(entity_id, ref_account).ok_or(Error::<T>::InvalidReferrer)?;
    ...
    pub(crate) fn do_remove_member(entity_id: u64, account: &T::AccountId) -> DispatchResult {
        let member = EntityMembers::<T>::get(entity_id, account).ok_or(Error::<T>::NotMember)?;
        ...
        LevelMemberCount::<T>::mutate(entity_id, member.custom_level_id, |c| {
    ...
    pub(crate) fn mutate_member_referral(
        ...
        DirectReferrals::<T>::try_mutate(entity_id, ref_account, |referrals| {
            referrals
                ...
                .map_err(|_| Error::<T>::ReferralsFull)
        })?;
        ...
        // 推荐统计变化后，同步重算自定义等级
        Self::recalculate_auto_custom_level_by_entity(entity_id, ref_account)?;
    ...
    pub(crate) fn ensure_shop_owner_or_admin(
        shop_id: u64,
        who: &T::AccountId,
    ) -> Result<u64, DispatchError> {
        let entity_id = T::ShopProvider::shop_entity_id(shop_id).ok_or(Error::<T>::ShopNotFound)?;
        ensure!(
    ...
    pub(crate) fn apply_auto_custom_level_if_needed(
        entity_id: u64,
        account: &T::AccountId,
        member: &mut EntityMemberOf<T>,
    ) {
        let has_active_rule_upgrade = MemberLevelExpiry::<T>::get(entity_id, account)
            .map(|exp| <frame_system::Pallet<T>>::block_number() <= exp)
            .unwrap_or(false);

        let Some(system) = EntityLevelSystems::<T>::get(entity_id) else {
            return;
        };
        ...
        let new_custom_level = Self::calculate_custom_level_full(
            entity_id,
            ...
            && !(has_active_rule_upgrade && new_custom_level < member.custom_level_id)
        {
            ...
            LevelMemberCount::<T>::mutate(entity_id, new_custom_level, |c| {
    ...
    pub(crate) fn recalculate_auto_custom_level_by_entity(
        entity_id: u64,
        account: &T::AccountId,
    ) -> DispatchResult {
        EntityMembers::<T>::mutate(entity_id, account, |maybe_member| -> DispatchResult {
            ...
            Self::apply_auto_custom_level_if_needed(entity_id, account, member);
            Ok(())
        })
    }

    /// 更新会员消费金额（USDT 精度 10^6）
    pub fn update_spent(shop_id: u64, account: &T::AccountId, amount_usdt: u64) -> DispatchResult {
        let entity_id = Self::resolve_entity_id(shop_id).ok_or(Error::<T>::ShopNotFound)?;
        Self::update_spent_by_entity(entity_id, account, amount_usdt, false)
    }

    /// 更新会员消费金额（entity_id 直达，USDT 精度 10^6）
    ///
    /// `is_shopping_balance_payment`: true 表示购物余额支付的订单，
    /// 其金额不计入 `upgrade_eligible_spent`（不用于升级判定）。
    pub fn update_spent_by_entity(
        entity_id: u64,
        account: &T::AccountId,
        amount_usdt: u64,
        is_shopping_balance_payment: bool,
    ) -> DispatchResult {
        ...
        EntityMembers::<T>::mutate(entity_id, account, |maybe_member| -> DispatchResult {
            ...
            if !is_shopping_balance_payment {
                member.upgrade_eligible_spent =
                    member.upgrade_eligible_spent.saturating_add(amount_usdt);
            }
            ...
            if let Some(expires_at) = MemberLevelExpiry::<T>::get(entity_id, account) {
                ...
                if now > expires_at {
                    ...
                    if recalculated != member.custom_level_id {
                        ...
                        // 维护 LevelMemberCount
                        LevelMemberCount::<T>::mutate(entity_id, expired_level_id, |c| {
                            *c = c.saturating_sub(1)
                        });
    ...
    pub fn auto_register(
        ...
        if policy.requires_kyc() {
                ...
                T::KycChecker::is_kyc_passed(entity_id, account),
        ...
        Self::do_register_member(entity_id, account, valid_referrer.clone())?;
    ...
    pub fn get_member_info(
        ...
        let upgrade_history: alloc::vec::Vec<runtime_api::UpgradeRecordInfo> =
            MemberUpgradeHistory::<T>::get(entity_id, account)
                ...
                .map(|r| runtime_api::UpgradeRecordInfo {
                    ...
                    expires_at: r
                        .expires_at
                        .map(|e| sp_runtime::SaturatedConversion::saturated_into(e)),
                })
                .collect();
        ...
        Some(runtime_api::MemberDashboardInfo {
            ...
            banned_at: member
                .banned_at
                .map(|b| sp_runtime::SaturatedConversion::saturated_into(b)),
    ...
    pub fn get_entity_member_overview(entity_id: u64) -> runtime_api::EntityMemberOverview {
            ...
            EntityMembers::<T>::iter_prefix(entity_id)
                ...
                .map(|(account, member)| {
                    ...
                    runtime_api::PaginatedMemberInfo {
                        ...
                        direct_referrals: member.direct_referrals,
                        team_size: member.team_size,
                        joined_at: sp_runtime::SaturatedConversion::saturated_into(
                            member.joined_at,
                        ),
                        is_banned: member.banned_at.is_some(),
                        ban_reason: member.ban_reason.map(|r| r.into_inner()),
    ...
    pub fn get_upline_chain(
        ...
        while let Some(ref curr_account) = current {
            ...
            match EntityMembers::<T>::get(entity_id, curr_account) {
                Some(m) => {
                    let effective_level =
                        Self::get_effective_level_by_entity(entity_id, curr_account);
    ...
    fn build_tree_node(
        ...
        let (children, has_more_children) = if remaining_depth > 0 {
            ...
            let has_more = if remaining_depth == 1 {
                ...
                referrals
                    .iter()
                    .any(|child| DirectReferrals::<T>::get(entity_id, child).len() > 0)
            } else {
                false
            };
        ...
        let page_members: alloc::vec::Vec<runtime_api::GenerationMemberInfo<T::AccountId>> =
            current_layer
                ...
                .filter_map(|child_account| {
                    ...
                    let effective_level =
                        Self::get_effective_level_by_entity(entity_id, &child_account);
                    // referrer 一定存在（能出现在某人的 DirectReferrals 中就有 referrer）
                    let referrer = member
                        .referrer
                        .clone()
                        .unwrap_or_else(|| child_account.clone());

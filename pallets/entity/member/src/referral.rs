...
use crate::pallet::*;
use pallet_entity_common::OnMemberTeamChanged;
...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn is_circular_referral(
        ...
        while let Some(ref curr_account) = current {
            ...
            current = Self::get_member_by_shop(shop_id, curr_account).and_then(|m| m.referrer);
            depth += 1;
        }
    ...
    pub(crate) fn update_team_size_by_entity(entity_id: u64, account: &T::AccountId) {
        use alloc::collections::BTreeSet;
        ...
        while let Some(ref curr_account) = current {
            ...
            let (old_direct, old_team) = EntityMembers::<T>::get(entity_id, curr_account)
                .map(|m| (m.direct_referrals, m.team_size))
                .unwrap_or((0, 0));
            ...
            let (new_direct, new_team) = EntityMembers::<T>::get(entity_id, curr_account)
                .map(|m| (m.direct_referrals, m.team_size))
                .unwrap_or((0, 0));
            T::OnMemberTeamChanged::on_team_changed(
                entity_id,
                curr_account,
                old_direct,
                new_direct,
            ...
            current = EntityMembers::<T>::get(entity_id, curr_account).and_then(|m| m.referrer);
            depth += 1;
        }
        ...
        if current.is_some() && depth >= MAX_SYNC_DEPTH {
            ...
            PendingTeamSizeUpdates::<T>::insert(
                update_id,
                TeamSizeUpdate {
                    entity_id,
    ...
    /// 递减团队人数（会员移除时向上递归更新）
    /// 方案 D: 有界递归 + 溢出异步补偿
    pub(crate) fn decrement_team_size_by_entity(entity_id: u64, account: &T::AccountId) {
        use alloc::collections::BTreeSet;

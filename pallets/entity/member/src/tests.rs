...
fn bind_referrer_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(MemberPallet::bind_referrer(
            RuntimeOrigin::signed(BOB),
            SHOP_1,
            ALICE
        ));
...
fn manual_set_member_level_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(MemberPallet::init_level_system(
            ...
            SHOP_1,
            true,
            LevelUpgradeMode::ManualUpgrade,
        ));
...
fn empty_level_name_fails() {
    new_test_ext().execute_with(|| {
        ...
        let name: BoundedVec<u8, frame_support::traits::ConstU32<32>> = BoundedVec::default();
...
fn member_provider_trait_works() {
    new_test_ext().execute_with(|| {
        ...
        assert!(!<MemberPallet as MemberProvider<u64>>::is_member(
            ENTITY_1, &ALICE
        ));
...
fn h7_apply_upgrade_skips_deleted_level() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(MemberPallet::check_order_upgrade_rules(
            SHOP_1, &ALICE, 0, 150u64
        ));
...
fn h10_stackable_rule_cannot_downgrade() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            MemberPallet::get_member_by_shop(SHOP_1, &ALICE)
                .unwrap()
                .custom_level_id,
            2
        );
        ...
        assert_eq!(
            ...
            "stackable rule must not downgrade"
        );
    });
}
...
fn h12_auto_upgrade_preserves_active_rule_upgrade() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            member.custom_level_id, 2,
            "auto-upgrade must not overwrite active rule upgrade"
        );
        ...
        assert_eq!(
            ...
            "after expiry, auto-upgrade should recalculate to VIP1"
        );
    });
}
...
fn m14_stackable_preserves_permanent_upgrade() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            expiry, None,
            "permanent level must not be demoted to timed by stackable rule"
        );
        ...
        assert_eq!(
            expiry,
            Some(51),
            "first-time stackable upgrade to higher level should set now + duration"
        );
        let member = MemberPallet::entity_members(ENTITY_1, ALICE).unwrap();
        assert_eq!(
            member.custom_level_id, 2,
            "should have been upgraded to VIP2"
        );
    });
}
...
#[test]
fn referral_update_recalculates_auto_custom_level() {
    new_test_ext().execute_with(|| {
        assert_ok!(MemberPallet::init_level_system(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
        ...
        for (name, threshold, min_direct) in [
            (b"L1".to_vec(), 5_000_000u64, 0u32),
            (b"L2".to_vec(), 10_000_000u64, 0u32),
            (b"L3".to_vec(), 15_000_000u64, 0u32),
            (b"L4".to_vec(), 20_000_000u64, 0u32),
            (b"L5".to_vec(), 25_000_000u64, 1u32),
        ] {
            let bounded_name: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                member.total_spent = 25_000_000;
                member.upgrade_eligible_spent = 25_000_000;
                member.custom_level_id = 4;
                member.direct_referrals = 0;
                member.indirect_referrals = 0;
                member.team_size = 0;
            }
        });
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 0, 0);
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 4, 1);
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 5, 0);

        let member = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        assert_eq!(member.custom_level_id, 4, "fixture should start at Lv.4");
        assert_eq!(
            MemberPallet::get_effective_level_by_entity(ENTITY_1, &ALICE),
            4
        );
        let info_before = MemberPallet::get_member_info(ENTITY_1, &ALICE).unwrap();
        assert_eq!(info_before.custom_level_id, 4);
        assert_eq!(info_before.effective_level_id, 4);
        assert_eq!(info_before.direct_referrals, 0);
        ...
        let member = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        assert_eq!(member.direct_referrals, 1);
        assert_eq!(
            member.custom_level_id, 5,
            "meeting the direct referral requirement should immediately upgrade to Lv.5"
        );
        assert_eq!(
            MemberPallet::get_effective_level_by_entity(ENTITY_1, &ALICE),
            5
        );
        assert_eq!(crate::LevelMemberCount::<Test>::get(ENTITY_1, 4), 0);
        assert_eq!(crate::LevelMemberCount::<Test>::get(ENTITY_1, 5), 1);

        let info_after = MemberPallet::get_member_info(ENTITY_1, &ALICE).unwrap();
        assert_eq!(info_after.custom_level_id, 5);
        assert_eq!(info_after.effective_level_id, 5);
        assert_eq!(info_after.direct_referrals, 1);
    });
}

#[test]
fn referral_update_recalculates_auto_custom_level_to_lv7() {
    new_test_ext().execute_with(|| {
        assert_ok!(MemberPallet::init_level_system(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
        ...
        for (name, threshold, min_direct) in [
            ...
            (b"L4".to_vec(), 20_000_000u64, 0u32),
            (b"L5".to_vec(), 25_000_000u64, 1u32),
            (b"L6".to_vec(), 30_000_000u64, 2u32),
            (b"L7".to_vec(), 50_000_000u64, 3u32),
        ] {
            let bounded_name: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
                name.try_into().unwrap();
            assert_ok!(MemberPallet::add_custom_level(
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                member.total_spent = 50_000_000;
                member.upgrade_eligible_spent = 50_000_000;
                member.custom_level_id = 4;
                member.direct_referrals = 0;
                member.indirect_referrals = 0;
                member.team_size = 0;
        ...
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 0, 0);
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 4, 1);
        crate::LevelMemberCount::<Test>::insert(ENTITY_1, 7, 0);
        ...
        let member = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        assert_eq!(member.direct_referrals, 3);
        assert_eq!(
            member.custom_level_id, 7,
            "meeting the 3-direct-referral requirement should immediately upgrade to Lv.7"
        );
        assert_eq!(
            MemberPallet::get_effective_level_by_entity(ENTITY_1, &ALICE),
            7
        ...
        assert_eq!(crate::LevelMemberCount::<Test>::get(ENTITY_1, 4), 0);
        assert_eq!(crate::LevelMemberCount::<Test>::get(ENTITY_1, 7), 1);

        let info_after = MemberPallet::get_member_info(ENTITY_1, &ALICE).unwrap();
        assert_eq!(info_after.custom_level_id, 7);
        assert_eq!(info_after.effective_level_id, 7);
        assert_eq!(info_after.direct_referrals, 3);
    });
}

#[test]
fn calculate_custom_level_full_honors_exact_threshold_with_referral_gate() {
    new_test_ext().execute_with(|| {
        assert_ok!(MemberPallet::init_level_system(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
        ...
        let name1: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
            b"L4".to_vec().try_into().unwrap();
        let name2: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
            b"L5".to_vec().try_into().unwrap();
        ...
        assert_eq!(
            MemberPallet::calculate_custom_level_full(ENTITY_1, 25_000_000u64, 1, 0, 0),
            2,
            "exact threshold plus referral requirement should reach the higher level",
        );
        assert_eq!(
            MemberPallet::calculate_custom_level_full(ENTITY_1, 25_000_000u64, 0, 0, 0),
            1,
            "without the direct referral requirement, member should stay at the lower level",
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                ...
                member.upgrade_eligible_spent = 600;
            }
        });
    ...
    new_test_ext().execute_with(|| {
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                ...
                member.upgrade_eligible_spent = 100;
            }
        });
    ...
    new_test_ext().execute_with(|| {
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                ...
                member.upgrade_eligible_spent = 0;
            }
        });
        ...
        assert_eq!(member.custom_level_id, 1);
        assert_eq!(
            crate::MemberLevelExpiry::<Test>::get(ENTITY_1, ALICE),
            Some(100)
        );
    });
}
...
#[test]
fn get_member_stats_default_returns_direct() {
    new_test_ext().execute_with(|| {
        use crate::MemberProvider;
        ...
        MemberPallet::auto_register_by_entity(ENTITY_1, &BOB, Some(ALICE)).unwrap();
        MemberPallet::auto_register_by_entity(ENTITY_1, &CHARLIE, Some(ALICE)).unwrap();

        // get_member_stats 应返回 direct_referrals
        let stats = <MemberPallet as MemberProvider<u64>>::get_member_stats(ENTITY_1, &ALICE);
        assert_eq!(
            stats.direct_referrals, 2,
            "get_member_stats 应返回 direct_referrals=2"
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        crate::EntityMembers::<Test>::mutate(ENTITY_1, ALICE, |m| {
            if let Some(ref mut member) = m {
                ...
                member.upgrade_eligible_spent = 200;
            }
        });
        ...
        let member = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        assert_eq!(
            member.custom_level_id, 3,
            "VIP3 must persist after old expiry passes"
        );
    });
}
...
fn m9_effective_level_falls_back_when_level_deleted() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            effective, 1,
            "deleted level should fall back to spending-based calculation"
        );
    });
}
...
fn m10_referral_count_trigger_fires_on_registration() {
    new_test_ext().execute_with(|| {
        ...
        let name2: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
            b"VIP2".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_custom_level(
            RuntimeOrigin::signed(OWNER), SHOP_1, name1, 500u64, 0, 0, 0, 0, 0,
        ));
        assert_ok!(MemberPallet::add_custom_level(
            RuntimeOrigin::signed(OWNER), SHOP_1, name2, 1000u64, 0, 0, 0, 0, 0,
        ));
        assert_ok!(MemberPallet::init_upgrade_rule_system(
...
fn m10_referral_trigger_does_not_fire_below_threshold() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !upgraded,
            "should not trigger upgrade below referral threshold"
        );
    });
}
...
fn m10_bind_referrer_triggers_referral_upgrade() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            upgraded_after,
            "M10: bind_referrer should trigger referral upgrade rule"
        );
    });
}
...
fn r6_m1_bind_referrer_updates_all_stats_correctly() {
    new_test_ext().execute_with(|| {
        ...
        // Verify ALICE stats: direct=1, team=1
        let alice = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        ...
        assert_eq!(
            alice.team_size, 2,
            "indirect referral should increment team_size"
        );
        let bob = MemberPallet::get_member_by_shop(SHOP_1, &BOB).unwrap();
...
fn r6_m1_bind_referrer_fails_on_referrals_full() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            ...
            "referrer must not be set on ReferralsFull failure"
        );
    });
}
...
fn r6_l3_conflict_strategy_highest_priority() {
    new_test_ext().execute_with(|| {
        ...
        let rn1: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
            b"LowPri".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_upgrade_rule(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            rn1,
        ...
        let rn2: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
            b"HighPri".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_upgrade_rule(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            rn2,
        ...
        assert_eq!(
            member.custom_level_id, 1,
            "HighestPriority should select priority=10 rule → VIP1"
        );
    });
}
...
fn r6_l3_conflict_strategy_longest_duration() {
    new_test_ext().execute_with(|| {
        ...
        let rn1: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
            b"Short".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_upgrade_rule(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            rn1,
        ...
        assert_eq!(
            member.custom_level_id, 2,
            "LongestDuration should prefer permanent rule"
        );
        assert!(
            MemberLevelExpiry::<Test>::get(ENTITY_1, ALICE).is_none(),
            "permanent rule = no expiry"
        );
    });
}
...
fn r6_l3_conflict_strategy_first_match() {
    new_test_ext().execute_with(|| {
        ...
        let rn1: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
            b"First".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_upgrade_rule(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            rn1,
        ...
        let rn2: BoundedVec<u8, frame_support::traits::ConstU32<64>> =
            b"Second".to_vec().try_into().unwrap();
        assert_ok!(MemberPallet::add_upgrade_rule(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            rn2,
        ...
        assert_eq!(
            member.custom_level_id, 1,
            "FirstMatch should select the first matching rule → VIP1"
        );
    });
}
...
fn m5_get_member_stats_returns_usdt() {
    new_test_ext().execute_with(|| {
        ...
        let stats =
            <MemberPallet as crate::MemberProvider<u64>>::get_member_stats(ENTITY_1, &ALICE);
        assert_eq!(
            stats.spend.total_spent, 1_000_000u128,
            "get_member_stats should return USDT amount"
        );
    });
}
...
#[test]
fn m5_lazy_upgrade_write_through_fixes_stale_member_level() {
    new_test_ext().execute_with(|| {
        assert_ok!(MemberPallet::init_level_system(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
        ...
        for i in 1..=7u8 {
            let name: BoundedVec<u8, frame_support::traits::ConstU32<32>> =
                format!("VIP{}", i).as_bytes().to_vec().try_into().unwrap();
        ...
        EntityMembers::<Test>::mutate(ENTITY_1, &ALICE, |maybe| {
            if let Some(ref mut m) = maybe {
                m.upgrade_eligible_spent = 50_000_000;
                m.direct_referrals = 3;
                m.custom_level_id = 4;
            }
        });
        LevelMemberCount::<Test>::insert(ENTITY_1, 4, 1);
        LevelMemberCount::<Test>::insert(ENTITY_1, 7, 0);

        let level = MemberPallet::get_effective_level_by_entity(ENTITY_1, &ALICE);
        assert_eq!(level, 7);

        let member = MemberPallet::get_member_by_shop(SHOP_1, &ALICE).unwrap();
        assert_eq!(member.custom_level_id, 7);
        assert_eq!(LevelMemberCount::<Test>::get(ENTITY_1, 4), 0);
        assert_eq!(LevelMemberCount::<Test>::get(ENTITY_1, 7), 1);

        let upgraded = System::events().iter().any(|e| {
            matches!(
                &e.event,
                RuntimeEvent::MemberPallet(crate::Event::CustomLevelUpgraded {
                    entity_id: ENTITY_1,
                    account,
                    old_level_id: 4,
                    new_level_id: 7,
        ...
        assert!(
            upgraded,
            "CustomLevelUpgraded event should be emitted on lazy write-through"
        );
    });
}
...
fn m6_cancel_pending_member_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(MemberPallet::cancel_pending_member(
            RuntimeOrigin::signed(ALICE),
            SHOP_1
        ));
        assert!(!PendingMembers::<Test>::contains_key(ENTITY_1, &ALICE));
        ...
        System::assert_has_event(RuntimeEvent::MemberPallet(
            crate::Event::PendingMemberCancelled {
                entity_id: ENTITY_1,
                account: ALICE,
            },
        ));
...
fn m6_cleanup_expired_pending_works() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !PendingMembers::<Test>::contains_key(ENTITY_1, &ALICE),
            "ALICE should be cleaned up"
        );
        assert!(
            PendingMembers::<Test>::contains_key(ENTITY_1, &BOB),
            "BOB should still be pending"
        );

        // Expired event for ALICE
        System::assert_has_event(RuntimeEvent::MemberPallet(
            crate::Event::PendingMemberExpired {
                entity_id: ENTITY_1,
                account: ALICE,
            },
        ));
...
fn batch_approve_members_works() {
    new_test_ext().execute_with(|| {
        ...
        // 验证事件
        System::assert_has_event(RuntimeEvent::MemberPallet(
            crate::Event::BatchMembersApproved {
                entity_id: ENTITY_1,
                count: 3,
            },
        ));
...
fn batch_approve_rejects_over_limit() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            MemberPallet::batch_approve_members(RuntimeOrigin::signed(OWNER), SHOP_1, accounts,),
...
fn batch_reject_members_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::MemberPallet(
            crate::Event::BatchMembersRejected {
                entity_id: ENTITY_1,
                count: 2,
            },
        ));
...
fn batch_reject_rejects_over_limit() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            MemberPallet::batch_reject_members(RuntimeOrigin::signed(OWNER), SHOP_1, accounts,),
            crate::Error::<Test>::BatchLimitExceeded
        );
    });
}
...
fn ban_member_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::MemberPallet(crate::Event::MemberBanned {
            entity_id: ENTITY_1,
            account: ALICE,
            reason: None,
        }));
...
fn unban_member_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::MemberPallet(crate::Event::MemberUnbanned {
            entity_id: ENTITY_1,
            account: ALICE,
        }));
    });
...
fn last_active_at_updated_on_spend() {
    new_test_ext().execute_with(|| {
        ...
        System::set_block_number(100);
        assert_ok!(MemberPallet::update_spent_by_entity(
            ENTITY_1, &ALICE, 1000u64, false
        ));
        assert_eq!(
            <MemberPallet as crate::MemberProvider<u64>>::last_active_at(ENTITY_1, &ALICE),
            100
...
fn kyc_required_blocks_auto_register_by_entity() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            MemberPallet::auto_register_by_entity(ENTITY_1, &ALICE, None),
            Error::<Test>::KycNotPassed
        );

        // 通过 KYC 后 → 成功
        set_kyc_passed(ENTITY_1, ALICE);
        assert_ok!(MemberPallet::auto_register_by_entity(
            ENTITY_1, &ALICE, None
        ));
        assert!(EntityMembers::<Test>::contains_key(ENTITY_1, ALICE));
    });
...
fn kyc_upgrade_required_blocks_auto_upgrade() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(MemberPallet::add_custom_level(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            BoundedVec::try_from(b"Base".to_vec()).unwrap(),
            0,
            0,
            0,
            0,
        ...
        assert_ok!(MemberPallet::add_custom_level(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            BoundedVec::try_from(b"VIP1".to_vec()).unwrap(),
            100,
            500,
            0,
            0,
        ...
        assert_ok!(MemberPallet::apply_upgrade_for_test(
            ENTITY_1, &ALICE, 0, 2, None, false
        ));
        let member = EntityMembers::<Test>::get(ENTITY_1, ALICE).unwrap();
...
fn entity_locked_rejects_set_member_policy() {
    new_test_ext().execute_with(|| {
        set_entity_locked(ENTITY_1);
        assert_noop!(
            MemberPallet::set_member_policy(RuntimeOrigin::signed(OWNER), SHOP_1, 0b0000_0100,),
            Error::<Test>::EntityLocked
        );
    });
}
...
fn member_provider_is_activated_works() {
    new_test_ext().execute_with(|| {
        ...
        let m = EntityMembers::<Test>::get(ENTITY_1, &ALICE).unwrap();
        assert!(!m.activated);
        assert!(!<MemberPallet as crate::MemberProvider<u64>>::is_activated(
            ENTITY_1, &ALICE
        ));
        // 但未被封禁
        assert!(!<MemberPallet as crate::MemberProvider<u64>>::is_banned(
            ENTITY_1, &ALICE
        ));

        // 消费后 → activated=true + MemberActivated 事件
        assert_ok!(<MemberPallet as crate::MemberProvider<u64>>::update_spent(
            ENTITY_1, &ALICE, 100
        ));
        let m = EntityMembers::<Test>::get(ENTITY_1, &ALICE).unwrap();
        assert!(m.activated);
        ...
        System::assert_has_event(RuntimeEvent::MemberPallet(crate::Event::MemberActivated {
            entity_id: ENTITY_1,
            account: ALICE,
        }));
        ...
        let activated_events: Vec<_> = all_events[events_before..]
            .iter()
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::MemberPallet(crate::Event::MemberActivated { .. })
                )
            })
            .collect();
...
fn activate_member_works() {
    new_test_ext().execute_with(|| {
        ...
        // Owner 手动激活
        assert_ok!(MemberPallet::activate_member(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            ALICE
        ));
...
fn deactivate_member_works() {
    new_test_ext().execute_with(|| {
        ...
        // Owner 手动取消激活
        assert_ok!(MemberPallet::deactivate_member(
            RuntimeOrigin::signed(OWNER),
            SHOP_1,
            ALICE
        ));
...
fn h1_apply_upgrade_skips_when_level_system_absent() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(MemberPallet::add_upgrade_rule(
            ...
            SHOP_1,
            rule_name,
            UpgradeTrigger::SingleOrder { threshold: 50 },
            2,
            None,
            1,
            false,
        ...
        System::reset_events();
        assert_ok!(
            <MemberPallet as crate::MemberProvider<u64>>::check_order_upgrade_rules(
        ...
        assert_eq!(
            ...
            "H1: should not upgrade when level system absent"
        );
...
fn d3_on_idle_processes_pending_updates() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            root_after.team_size, 19,
            "异步补偿后 root team_size 应为 19"
        );

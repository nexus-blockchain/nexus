...
use pallet_entity_common::OnMemberLevelChanged;
...
impl<T: Config> Pallet<T> {
    ...
    pub fn check_order_upgrade_rules_by_entity(
        ...
        // 收集匹配的规则
        let mut matched_rules: alloc::vec::Vec<(u32, u8, Option<BlockNumberFor<T>>, u8, bool)> =
            alloc::vec::Vec::new();
        ...
        for rule in system.rules.iter() {
            ...
            let matches = match &rule.trigger {
                UpgradeTrigger::PurchaseProduct { product_id: pid } => *pid == product_id,
                UpgradeTrigger::SingleOrder { threshold } => amount_usdt >= *threshold,
                UpgradeTrigger::TotalSpent { threshold } => {
                    member.upgrade_eligible_spent >= *threshold
                }
                UpgradeTrigger::ReferralCount { count } => member.direct_referrals >= *count,
                UpgradeTrigger::TeamSize { size } => member.team_size >= *size,
                UpgradeTrigger::OrderCount { count } => order_count >= *count,
            };
    ...
    pub(crate) fn resolve_conflict(
        ...
        match strategy {
            ConflictStrategy::HighestLevel => rules.iter().max_by_key(|r| r.1).cloned(),
            ConflictStrategy::HighestPriority => rules.iter().max_by_key(|r| r.3).cloned(),
            ConflictStrategy::LongestDuration => {
                // None = 永久，应视为最长；用 Bounded::max_value 替代 None 参与比较
                use sp_runtime::traits::Bounded;
                rules
                    .iter()
                    .max_by_key(|r| match r.2 {
                        None => <BlockNumberFor<T>>::max_value(),
                        Some(d) => d,
                    })
                    .cloned()
            }
            ConflictStrategy::FirstMatch => rules.first().cloned(),
        }
    }
    ...
    pub(crate) fn apply_upgrade(
        ...
        EntityMembers::<T>::mutate(entity_id, account, |maybe_member| -> DispatchResult {
            ...
            if level_system.use_custom
                && target_level_id > 0
                && ((target_level_id - 1) as usize) >= level_system.levels.len()
            {
                return Ok(());
            }

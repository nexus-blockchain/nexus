...
mod bench {
    ...
    fn add_custom_level() {
        ...
        EntityLevelSystems::<T>::insert(
            BENCH_ENTITY,
            EntityLevelSystem::<<T as Config>::MaxCustomLevels> {
                use_custom: true,
    ...
    fn manual_set_member_level() {
        ...
        let mut levels: BoundedVec<CustomLevel, <T as Config>::MaxCustomLevels> =
            BoundedVec::default();
    ...
    fn add_upgrade_rule() {
        ...
        EntityUpgradeRules::<T>::insert(
            BENCH_ENTITY,
            EntityUpgradeRuleSystem::<
                frame_system::pallet_prelude::BlockNumberFor<T>,
                <T as Config>::MaxUpgradeRules,
            > {
                enabled: true,
    ...
    fn update_upgrade_rule() {
        ...
        let mut rules: BoundedVec<UpgradeRuleOf<T>, <T as Config>::MaxUpgradeRules> =
            BoundedVec::default();
        let _ = rules.try_push(UpgradeRule {
            id: 0,
            name: b"Rule1".to_vec().try_into().unwrap_or_default(),
            trigger: UpgradeTrigger::TotalSpent {
                threshold: 1_000_000,
            },
            target_level_id: 0,
            duration: None,

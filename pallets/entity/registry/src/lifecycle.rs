...
use pallet_entity_common::{
    CommissionFundGuard, EntityStatus, EntityType, GovernanceMode, GovernanceProvider,
    OnEntityStatusChange, ShopProvider, ShopType,
};
...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn do_create_entity(
        ...
        let name_str = core::str::from_utf8(&name).map_err(|_| Error::<T>::InvalidName)?;
            ...
            !name_str.chars().any(|c| c.is_control()),
            Error::<T>::InvalidName
        );
        ...
        if let Some(ref ref_account) = referrer {
                ...
                Self::has_operable_entity(ref_account),
        ...
        UserEntity::<T>::try_mutate(&who, |entities| {
            entities
                .try_push(entity_id)
                .map_err(|_| Error::<T>::MaxEntitiesReached)
        })?;
        NextEntityId::<T>::put(
            entity_id
                .checked_add(1)
        ...
        if primary_shop_id > 0 {
            Entities::<T>::mutate(entity_id, |e| {
                if let Some(e) = e {
                    if e.primary_shop_id == 0
                        && EntityShops::<T>::get(entity_id).contains(&primary_shop_id)
                    {
        ...
        if let Some(ref ref_account) = referrer {
            ...
            EntityReferrerBoundAt::<T>::insert(
                entity_id,
                <frame_system::Pallet<T>>::block_number(),
            );
            // b8: 维护反向索引
            ReferrerEntities::<T>::try_mutate(
                ref_account,
                |entities| -> Result<(), sp_runtime::DispatchError> {
                    entities
                        .try_push(entity_id)
                        .map_err(|_| Error::<T>::ReferrerIndexFull)?;
                    Ok(())
                },
            )?;
    ...
    pub(crate) fn do_update_entity(
        ...
        Entities::<T>::try_mutate(entity_id, |maybe_entity| -> sp_runtime::DispatchResult {
            ...
            // b2: owner 或拥有 ENTITY_MANAGE 权限的 admin 均可更新
                ...
                entity.owner == who
                    || entity.admins.iter().any(|(a, perm)| a == &who
                        && (perm & pallet_entity_common::AdminPermission::ENTITY_MANAGE) != 0),
            ...
            if let Some(n) = name {
                ...
                let n_str = core::str::from_utf8(&n).map_err(|_| Error::<T>::InvalidName)?;
                    ...
                    !n_str.chars().any(|c| c.is_control()),
                    Error::<T>::InvalidName
                );
                let new_name: BoundedVec<u8, T::MaxEntityNameLength> =
                    n.try_into().map_err(|_| Error::<T>::NameTooLong)?;
                // F1: 名称唯一性校验
            ...
            if let Some(c) = logo_cid {
                entity.logo_cid = if c.is_empty() {
                    None
                } else {
                    Some(c.try_into().map_err(|_| Error::<T>::CidTooLong)?)
                };
            }
            if let Some(c) = description_cid {
                entity.description_cid = if c.is_empty() {
                    None
                } else {
                    Some(c.try_into().map_err(|_| Error::<T>::CidTooLong)?)
                };
            ...
            if let Some(uri) = metadata_uri {
                entity.metadata_uri = if uri.is_empty() {
                    None
                } else {
                    Some(uri.try_into().map_err(|_| Error::<T>::CidTooLong)?)
                };
            }
            // F4: 联系方式 CID
            if let Some(c) = contact_cid {
                entity.contact_cid = if c.is_empty() {
                    None
                } else {
                    Some(c.try_into().map_err(|_| Error::<T>::CidTooLong)?)
                };
    ...
    /// 申请关闭实体（需治理审批）
    pub(crate) fn do_request_close_entity(
        who: T::AccountId,
        entity_id: u64,
    ) -> sp_runtime::DispatchResult {
        ...
        let was_active = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<bool, sp_runtime::DispatchError> {
                let entity = maybe_entity.as_mut().ok_or(Error::<T>::EntityNotFound)?;
    ...
    fn do_finalize_close(entity_id: u64, entity: EntityOf<T>) -> BalanceOf<T> {
        ...
        // 佣金资金保护：只退回可用余额，已承诺给会员的佣金资金留在 entity 账户
        let protected: BalanceOf<T> =
            T::CommissionFundGuard::protected_funds(entity_id).saturated_into();
        let transferable = balance.saturating_sub(protected);

        let fund_refunded = if !transferable.is_zero() {
            match T::Currency::transfer(
                ...
                Ok(_) => transferable,
                Err(_) => {
                    Self::deposit_event(Event::FundRefundFailed {
                        entity_id,
                        amount: transferable,
                    });
                    Zero::zero()
        ...
        for sid in EntityShops::<T>::get(entity_id).iter() {
            if T::ShopProvider::force_close_shop(*sid).is_err() {
                Self::deposit_event(Event::ShopCascadeFailed {
                    entity_id,
                    shop_id: *sid,
                });
            }
    ...
    pub(crate) fn do_execute_close_timeout(entity_id: u64) -> sp_runtime::DispatchResult {
        let request_at =
            EntityCloseRequests::<T>::get(entity_id).ok_or(Error::<T>::InvalidEntityStatus)?;
        let now = <frame_system::Pallet<T>>::block_number();
        let timeout = T::CloseRequestTimeout::get();
        ensure!(
            now >= request_at.saturating_add(timeout),
            Error::<T>::CloseRequestNotExpired
        );

...
use pallet_entity_common::{
    AdminPermission, EntityProvider, EntityStatus, EntityType, GovernanceMode,
};
...
fn close_entity_via_timeout(who: u64, entity_id: u64) {
    ...
    assert_ok!(EntityRegistry::execute_close_timeout(
        RuntimeOrigin::signed(who),
        entity_id
    ));
}
...
fn create_entity_works() {
    new_test_ext().execute_with(|| {
        ...
        // Entity ID 从 DefaultNextEntityId (100_000) 开始
        assert_eq!(id, 100_000);

        let entity = Entities::<Test>::get(id).unwrap();
        assert_eq!(entity.owner, ALICE);
        assert_eq!(entity.name.to_vec(), b"Test Entity 100000".to_vec());
        ...
        // NextEntityId
        assert_eq!(NextEntityId::<Test>::get(), 100_001);
    });
}
...
fn create_entity_fails_name_empty() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityRegistry::create_entity(RuntimeOrigin::signed(ALICE), vec![], None, None, None,),
...
#[test]
fn top_up_fund_works_for_banned_entity() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        ...
        // owner 可以对 Banned 实体充值
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
        ));
...
fn donate_to_entity_works_from_anyone() {
    new_test_ext().execute_with(|| {
        ...
        // BOB（非 owner）可以捐赠
        assert_ok!(EntityRegistry::donate_to_entity(
            RuntimeOrigin::signed(BOB),
            id,
            amount
        ));
...
fn reopen_entity_directly_activates() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::reopen_entity(
            RuntimeOrigin::signed(ALICE),
            id
        ));
        assert_eq!(
...
fn remove_admin_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::remove_admin(
            RuntimeOrigin::signed(ALICE),
            id,
            BOB
        ));
...
fn upgrade_entity_type_owner_merchant_to_community() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Entities::<Test>::get(id).unwrap().entity_type,
            EntityType::Community
        );
    });
}
...
fn entity_provider_basic_methods() {
    new_test_ext().execute_with(|| {
        ...
        assert!(<EntityRegistry as EntityProvider<u64>>::is_entity_active(
            id
        ));
        assert_eq!(
...
fn entity_provider_is_entity_admin() {
    new_test_ext().execute_with(|| {
        ...
        assert!(<EntityRegistry as EntityProvider<u64>>::is_entity_admin(
            id,
            &ALICE,
...
fn entity_provider_register_unregister_shop() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityRegistry as EntityProvider<u64>>::register_shop(
            id, 42
        ));
        ...
        assert_ok!(<EntityRegistry as EntityProvider<u64>>::unregister_shop(
            id, 42
        ));
...
fn deduct_operating_fee_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::deduct_operating_fee(
            id,
            fee,
            FeeType::IpfsPin
        ));
...
fn fund_health_levels() {
    new_test_ext().execute_with(|| {
        ...
        // Depleted: 0
        assert_eq!(EntityRegistry::get_fund_health(0u128), FundHealth::Depleted,);
    });
}
...
fn bind_entity_referrer_fails_already_bound() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityRegistry::bind_entity_referrer(RuntimeOrigin::signed(ALICE), alice_id, CHARLIE,),
...
fn bind_entity_referrer_fails_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityRegistry::bind_entity_referrer(RuntimeOrigin::signed(BOB), alice_id, BOB,),
            Error::<Test>::NotEntityOwner
        );
    });
}
...
fn bind_entity_referrer_fails_self_referral() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityRegistry::bind_entity_referrer(RuntimeOrigin::signed(ALICE), alice_id, ALICE,),
            Error::<Test>::SelfReferral
        );
    });
}
...
fn bind_entity_referrer_fails_invalid_referrer() {
    new_test_ext().execute_with(|| {
        ...
        // BOB has no entity
        assert_noop!(
            EntityRegistry::bind_entity_referrer(RuntimeOrigin::signed(ALICE), alice_id, BOB,),
            Error::<Test>::InvalidReferrer
        );
    });
}
...
fn referral_count_increments_correctly() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            EntityReferrer::<Test>::get(EntityRegistry::next_entity_id() - 1),
            Some(ALICE)
        );
...
fn m2_set_governance_mode_rejects_banned_entity() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            <EntityRegistry as EntityProvider<u64>>::set_governance_mode(
                id,
                GovernanceMode::FullDAO
            )
            .is_err()
...
fn m2_reopen_entity_fails_when_owner_at_capacity() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Entities::<Test>::get(id1).unwrap().status,
            EntityStatus::Closed
        );
...
fn m3_deduct_fee_no_spurious_suspend_event_for_already_suspended() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !has_suspend_event,
            "Should not emit EntitySuspendedLowFund for already-suspended entity"
        );
    });
}
...
fn m1_ban_confiscate_success_reports_true() {
    new_test_ext().execute_with(|| {
        ...
        if let RuntimeEvent::EntityRegistry(crate::Event::EntityBanned {
            fund_confiscated, ..
        }) = &ban_event.unwrap().event
        {
            assert!(
                *fund_confiscated,
                "fund_confiscated should be true when confiscation succeeds"
            );
        }
    });
}
...
fn m1_ban_no_confiscate_reports_false() {
    new_test_ext().execute_with(|| {
        ...
        }) = &ban_event.unwrap().event
        {
            assert!(
                !*fund_confiscated,
                "fund_confiscated should be false when not confiscating"
            );
        }
    });
}
...
fn m1_governance_only_change_does_not_emit_type_upgraded() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !has_type_event,
            "Should NOT emit EntityTypeUpgraded when only governance changes"
        );
...
fn m1_type_change_still_emits_type_upgraded() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            has_type_event,
            "Should emit EntityTypeUpgraded when type actually changes"
        );
    });
}
...
fn unban_entity_works() {
    new_test_ext().execute_with(|| {
        ...
        // owner 通过 top_up_fund 给 Banned 实体充值，以满足 unban 资金要求
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
fn unban_entity_directly_activates() {
    new_test_ext().execute_with(|| {
        ...
        // ban with refund: funds returned to owner. Treasury is empty.
        // owner 通过 top_up_fund 给 Banned 实体充值
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
fn cancel_close_request_works_active() {
    new_test_ext().execute_with(|| {
        ...
        // Cancel → Active
        assert_ok!(EntityRegistry::cancel_close_request(
            RuntimeOrigin::signed(ALICE),
            id
        ));
        assert_eq!(
...
fn b2_admin_can_update_entity_with_entity_manage() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::add_admin(
            ...
            id,
            BOB,
            AdminPermission::ENTITY_MANAGE,
        ));
        // BOB (admin with ENTITY_MANAGE) can update
        assert_ok!(EntityRegistry::update_entity(
            RuntimeOrigin::signed(BOB),
            id,
            Some(b"Updated by admin".to_vec()),
            None,
            None,
            None,
            None,
        ...
        assert_eq!(
            Entities::<Test>::get(id).unwrap().name.to_vec(),
            b"Updated by admin".to_vec()
        );
    });
}
...
fn b4_self_pause_entity_works() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        assert_ok!(EntityRegistry::self_pause_entity(
            RuntimeOrigin::signed(ALICE),
            id
        ));
        assert_eq!(
...
fn b4_self_resume_entity_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::self_resume_entity(
            RuntimeOrigin::signed(ALICE),
            id
        ));
        assert_eq!(
...
fn b5_force_transfer_ownership_works() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        assert_ok!(EntityRegistry::force_transfer_ownership(
            RuntimeOrigin::root(),
            id,
            BOB
        ));
...
fn b6_suspend_entity_with_reason() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::suspend_entity(
            RuntimeOrigin::root(),
            id,
            Some(b"Suspicious activity".to_vec()),
        ));
        assert_eq!(
            Entities::<Test>::get(id).unwrap().status,
            EntityStatus::Suspended
...
fn b7_reject_close_request_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::reject_close_request(
            RuntimeOrigin::root(),
            id
        ));
        assert_eq!(
...
fn f3_execute_close_timeout_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            balance_before + fund_in_treasury
        );
        // Name freed
...
fn f4_update_contact_cid_works() {
    new_test_ext().execute_with(|| {
        ...
        let entity = Entities::<Test>::get(id).unwrap();
        assert_eq!(
            entity.contact_cid.unwrap().to_vec(),
            b"ipfs://QmContact123".to_vec()
        );
...
fn f7_suspend_stores_reason() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityRegistry::suspend_entity(
            RuntimeOrigin::root(),
            id,
            Some(b"violation of TOS".to_vec()),
        ));
        assert_eq!(
...
fn m2_unban_lifecycle_owner_paused_clean() {
    new_test_ext().execute_with(|| {
        ...
        // owner 通过 top_up_fund 给 Banned 实体充值 + unban → Active（直接激活）
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
fn h1_unban_restores_user_entity_index() {
    new_test_ext().execute_with(|| {
        ...
        // owner 通过 top_up_fund 充值，Unban should restore UserEntity
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
fn m1_unban_restores_name_index() {
    new_test_ext().execute_with(|| {
        ...
        // owner 通过 top_up_fund 充值 + unban should restore name index（直接激活）
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
fn h1_r11_trait_resume_clears_owner_paused_and_reasons() {
    new_test_ext().execute_with(|| {
        ...
        // 治理也标记暂停 + 添加原因
        GovernanceSuspended::<Test>::insert(id, true);
        SuspensionReasons::<Test>::insert(
            id,
            frame_support::BoundedVec::<u8, sp_runtime::traits::ConstU32<256>>::try_from(
...
fn m2_r12_ban_entity_cleans_sales_and_shops() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !EntityShops::<Test>::get(id).is_empty(),
            "EntityShops should be preserved for unban recovery"
        );
...
fn l1_r12_close_cleans_sales_and_shops() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !EntityShops::<Test>::get(id).is_empty(),
            "EntityShops should be preserved for reopen recovery"
        );
        assert!(EntityShops::<Test>::get(id).contains(&100));
        assert_eq!(EntitySales::<Test>::get(id).total_orders, 0);
    });
...
fn p0_ban_preserves_shops_for_unban_recovery() {
    new_test_ext().execute_with(|| {
        ...
        // owner 通过 top_up_fund 充值以满足 unban 资金要求
        assert_ok!(EntityRegistry::top_up_fund(
            RuntimeOrigin::signed(ALICE),
            id,
            EXPECTED_INITIAL_FUND
...
// ==================== Close Guards: 关闭前置依赖校验 ====================

#[test]
fn request_close_blocked_by_active_proposals() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        set_active_proposals(id);
        assert_noop!(
            EntityRegistry::request_close_entity(RuntimeOrigin::signed(ALICE), id),
            Error::<Test>::HasActiveProposals
        );
    });
}

#[test]
fn request_close_blocked_by_active_orders() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // 注册一个 Shop 到 Entity
        assert_ok!(<EntityRegistry as EntityProvider<u64>>::register_shop(
...
#[test]
fn request_close_blocked_by_active_disputes() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        set_active_disputes(id);
        assert_noop!(
            EntityRegistry::request_close_entity(RuntimeOrigin::signed(ALICE), id),
            Error::<Test>::HasActiveDisputes
        );
    });
}

#[test]
fn request_close_blocked_by_active_sale() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        set_active_sale(id);
        assert_noop!(
            EntityRegistry::request_close_entity(RuntimeOrigin::signed(ALICE), id),
            Error::<Test>::HasActiveTokenSale
        );
    });
}

#[test]
fn request_close_blocked_by_active_market() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        set_active_market(id);
        assert_noop!(
            EntityRegistry::request_close_entity(RuntimeOrigin::signed(ALICE), id),
            Error::<Test>::HasActiveMarket
        );
    });
}

// ==================== CRITICAL-2 审计修复: 未结清佣金资金阻止关闭 ====================

#[test]
fn critical2_request_close_blocked_by_uncommitted_commission_funds() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // 模拟存在未结清佣金资金（pending commissions / shopping balances / pending refunds）
        set_uncommitted_funds(id);
        assert_noop!(
            EntityRegistry::request_close_entity(RuntimeOrigin::signed(ALICE), id),
            Error::<Test>::HasUncommittedCommissionFunds
        );
    });
}

#[test]
fn critical2_request_close_succeeds_when_no_uncommitted_funds() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // 无未结清佣金资金 → 允许关闭
        assert_ok!(EntityRegistry::request_close_entity(
...
#[test]
fn critical2_execute_close_timeout_blocked_by_uncommitted_funds() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // request_close 成功（此时无未结清资金）
        assert_ok!(EntityRegistry::request_close_entity(
            RuntimeOrigin::signed(ALICE),
            id
        ));
...
#[test]
fn critical2_request_close_succeeds_after_clearing_uncommitted_funds() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // 设置未结清资金 → 拒绝
        set_uncommitted_funds(id);
...
#[test]
fn execute_close_timeout_blocked_by_active_orders() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // request_close 成功（此时无活跃订单）
        assert_ok!(EntityRegistry::request_close_entity(
            RuntimeOrigin::signed(ALICE),
            id
        ));
...
#[test]
fn close_succeeds_after_dependencies_cleared() {
    new_test_ext().execute_with(|| {
        let id = create_default_entity(ALICE);
        // 设置阻塞：活跃提案
        set_active_proposals(id);
...
fn force_rebind_referrer_works() {
    new_test_ext().execute_with(|| {
        ...
        // 治理强制重绑到 CHARLIE
        assert_ok!(EntityRegistry::force_rebind_referrer(
            RuntimeOrigin::root(),
            id,
            CHARLIE
        ));

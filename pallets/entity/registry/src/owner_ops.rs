...
use pallet_entity_common::{
    AdminPermission, EntityStatus, EntityType, GovernanceMode, GovernanceProvider,
...
impl<T: Config> Pallet<T> {
        ...
        // b2: owner 或拥有 ENTITY_MANAGE 权限的 admin 均可充值
            ...
            entity.owner == who
                || entity
                    .admins
                    .iter()
                    .any(|(a, perm)| a == &who && (perm & AdminPermission::ENTITY_MANAGE) != 0),
    ...
    pub(crate) fn do_upgrade_entity_type(
        ...
        let (old_type, old_mode) = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<(EntityType, GovernanceMode), sp_runtime::DispatchError> {
                ...
                if new_type == EntityType::DAO {
                        ...
                        new_governance != GovernanceMode::None,
                        Error::<T>::DAORequiresGovernance
                    );
                }
    ...
    pub(crate) fn do_bind_entity_referrer(
        ...
        // 不能已有推荐人
            ...
            !EntityReferrer::<T>::contains_key(entity_id),
            Error::<T>::ReferrerAlreadyBound
        );

        // 验证推荐人（允许 Suspended/Pending 等非终态实体的 owner 作为推荐人）
        ensure!(referrer != who, Error::<T>::SelfReferral);
        ensure!(
            Self::has_operable_entity(&referrer),
    ...
    pub(crate) fn do_cancel_close_request(
        who: T::AccountId,
        entity_id: u64,
    ) -> sp_runtime::DispatchResult {
    ...
    /// Owner 主动暂停实体
    pub(crate) fn do_self_pause_entity(
        who: T::AccountId,
        entity_id: u64,
    ) -> sp_runtime::DispatchResult {
        let entity = Entities::<T>::get(entity_id).ok_or(Error::<T>::EntityNotFound)?;
    ...
    /// Owner 恢复主动暂停的实体
    pub(crate) fn do_self_resume_entity(
        who: T::AccountId,
        entity_id: u64,
    ) -> sp_runtime::DispatchResult {
        let entity = Entities::<T>::get(entity_id).ok_or(Error::<T>::EntityNotFound)?;

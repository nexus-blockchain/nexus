//! # Entity registry pallet (pallet-entity-registry)
//!
//! ## Overview
//!
//! This pallet manages the entity lifecycle, including:
//! 本模块负责实体的生命周期管理，包括：
//! - entity creation (moves operating funds into a derived account and activates on payment)
//! - 实体创建（转入运营资金到派生账户，付费即激活）
//! - entity information updates
//! - 实体信息更新
//! - operating-fund management (top-up, spending, and health monitoring)
//! - 运营资金管理（充值、消费、健康监控）
//! - entity state management (pause, resume, request closure)
//! - 实体状态管理（暂停、恢复、申请关闭）
//! - governance actions (ban, unban, pause, resume, verify)
//! - 治理操作（封禁、解禁、暂停、恢复、认证）
//!
//! ## Operating-fund mechanism
//!
//! - creating an entity transfers 50 USDT worth of NEX into a derived account
//! - 创建实体时转入 50 USDT 等值 NEX 到派生账户
//! - funds can pay for IPFS pinning, storage rent, and other operating costs
//! - 资金可用于支付 IPFS Pin、存储租金等运营费用
//! - funds are not withdrawable and are only returned after governance closure
//! - 资金不可提取，仅治理关闭后退还
//! - the entity auto-pauses when balance drops below the minimum
//! - 低于最低余额时实体自动暂停
//!
//! ## Version history
//!
...
//! - v0.2.0 (2026-02-01): introduced the derived operating-account model
//! - v0.2.0 (2026-02-01)：实现运营资金派生账户机制
//! - v0.3.0 (2026-02-03): refactored to Entity with multiple entity types and governance modes
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        EntityProvider, EntityStatus, EntityType, GovernanceMode, PricingProvider,
    };
    ...
    /// Entity data at the organization layer in the Entity-Shop split architecture.
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Pricing provider used to convert USDT-denominated deposits into NEX.
        /// 定价提供者（用于计算 USDT 等值 NEX 押金）
        type PricingProvider: PricingProvider;

        /// Initial operating-fund amount in USDT precision 10^6.
        /// `50_000_000` means 50 USDT.
        /// 初始运营资金 USDT 金额（精度 10^6，`50_000_000` = 50 USDT）
        #[pallet::constant]
        ...
        /// CRITICAL-2 审计修复: 佣金关闭前置检查（pending commissions + shopping balances + pending refunds）
        type CommissionCloseGuard: pallet_entity_common::CommissionCloseGuard;

        /// 佣金资金保护：Entity 账户转出时，排除已承诺给会员的佣金资金
        type CommissionFundGuard: pallet_entity_common::CommissionFundGuard;

        /// 佣金资金明细查询：供 `get_entity_funds` Runtime API 返回 protected 分项
        type CommissionFundDetail: pallet_entity_common::CommissionFundBreakdown;

        /// 资金保护配置查询：供 `get_entity_funds` Runtime API 返回治理配置 + 当日支出
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn integrity_test() {
            assert!(
                T::MaxEntityNameLength::get() > 0,
                "MaxEntityNameLength must be > 0"
            );
            assert!(T::MaxCidLength::get() > 0, "MaxCidLength must be > 0");
            assert!(T::MaxAdmins::get() > 0, "MaxAdmins must be > 0");
            assert!(
                T::MaxEntitiesPerUser::get() > 0,
                "MaxEntitiesPerUser must be > 0"
            );
            assert!(
                T::MaxShopsPerEntity::get() > 0,
                "MaxShopsPerEntity must be > 0"
            );
            assert!(
                T::MaxReferralsPerReferrer::get() > 0,
                "MaxReferralsPerReferrer must be > 0"
            );
            assert!(
    ...
    #[pallet::type_value]
    pub fn DefaultNextEntityId() -> u64 {
        100_000
    }
    ...
    /// Entity 推荐人绑定时间 (entity_id → block_number)
    /// 用于推荐人佣金保护期计算
    #[pallet::storage]
    #[pallet::getter(fn entity_referrer_bound_at)]
    pub type EntityReferrerBoundAt<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BlockNumberFor<T>>;
    ...
    pub type EntitySales<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, EntitySalesData<BalanceOf<T>>, ValueQuery>;
    ...
    #[pallet::storage]
    pub type EntityNameIndex<T: Config> =
        StorageMap<_, Blake2_128Concat, BoundedVec<u8, T::MaxEntityNameLength>, u64>;

    /// 治理暂停原因（entity_id -> reason）
    #[pallet::storage]
    #[pallet::getter(fn suspension_reason)]
    pub type SuspensionReasons<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BoundedVec<u8, ConstU32<256>>>;

    /// Entity 级支付通道配置（entity_id -> PaymentConfig）
    #[pallet::storage]
    pub type EntityPaymentConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, pallet_entity_common::PaymentConfig, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// Shop 已添加到 Entity
        ShopAddedToEntity { entity_id: u64, shop_id: u64 },
        /// 实体已更新
        ...
        /// 管理员已移除
        AdminRemoved { entity_id: u64, admin: T::AccountId },
        /// 管理员权限已更新
        ...
        /// Shop 级联操作失败（需人工干预）
        ShopCascadeFailed { entity_id: u64, shop_id: u64 },
        /// Entity 推荐人已绑定
        EntityReferrerBound {
            entity_id: u64,
        ...
        /// Shop 已从 Entity 移除
        ShopRemovedFromEntity { entity_id: u64, shop_id: u64 },
        /// 实体已解除封禁（Banned → Active，直接激活）
        EntityUnbanned { entity_id: u64 },
        /// 实体认证已撤销
        EntityUnverified { entity_id: u64 },
        /// 关闭申请已撤销（PendingClose → Active）
        CloseRequestCancelled { entity_id: u64 },
        /// 管理员主动辞职
        AdminResigned { entity_id: u64, admin: T::AccountId },
        // ========== Phase 5 新增事件 ==========
        /// Owner 主动暂停实体
        EntityOwnerPaused { entity_id: u64 },
        /// Owner 主动恢复实体
        EntityOwnerResumed { entity_id: u64 },
        /// 治理强制转移所有权
        ...
        /// 治理拒绝关闭申请
        CloseRequestRejected { entity_id: u64 },
        // ========== Phase 6 新增事件 ==========
        /// 治理暂停实体（附原因）
    ...
    pub enum Error<T> {
        ...
        /// 存在活跃市场交易，不允许关闭
        HasActiveMarket,
        /// CRITICAL-2 审计修复: 存在未结清佣金资金（pending/shopping/refund），不允许关闭
        HasUncommittedCommissionFunds,
        /// 支付通道配置无效（至少启用一个通道）
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_payment_config(
            ...
            let is_admin = <Self as EntityProvider<T::AccountId>>::is_entity_admin(
                entity_id,
                &who,
            ...
            // 至少启用一个通道
                ...
                native_enabled || token_enabled,
                Error::<T>::InvalidPaymentConfig
            );

            EntityPaymentConfigs::<T>::insert(
                entity_id,

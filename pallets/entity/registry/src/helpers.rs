...
use pallet_entity_common::{
    AdminPermission, CommissionCloseGuard, CommissionFundBreakdown, CommissionFundGuard,
    DisputeQueryProvider, EntityStatus, EntityType, FundProtectionQueryPort, GovernanceMode,
    GovernanceProvider, MarketProvider, OnEntityStatusChange, OrderProvider, PricingProvider,
    TokenSaleProvider,
};
...
impl<T: Config> Pallet<T> {
    ...
    pub fn normalize_entity_name(
        name: &[u8],
    ) -> Result<BoundedVec<u8, T::MaxEntityNameLength>, sp_runtime::DispatchError> {
    ...
    pub fn api_get_entity(
        entity_id: u64,
    ) -> Option<crate::runtime_api::EntityInfo<T::AccountId, BalanceOf<T>>> {
    ...
    pub fn api_get_entity_fund_info(
        entity_id: u64,
    ) -> Option<crate::runtime_api::EntityFundInfo<BalanceOf<T>>> {
    ...
    /// 查询 Entity 资金全景（供 Runtime API 使用）
    ///
    /// 聚合金库余额、protected 明细、保护规则、健康度，前端一次 RPC 获取完整视图。
    pub fn api_get_entity_funds(
        entity_id: u64,
    ) -> Option<crate::runtime_api::EntityFundsView<BalanceOf<T>>> {
        ...
        // 2. Protected 明细
        let (pending, shopping, unallocated, refund) =
            T::CommissionFundDetail::protected_funds_breakdown(entity_id);

        let pending_bal: BalanceOf<T> = pending.saturated_into();
        let shopping_bal: BalanceOf<T> = shopping.saturated_into();
        let unallocated_bal: BalanceOf<T> = unallocated.saturated_into();
        let refund_bal: BalanceOf<T> = refund.saturated_into();

        let protected_total = pending_bal
            .saturating_add(shopping_bal)
            .saturating_add(unallocated_bal)
            .saturating_add(refund_bal);

        // 3. 可动资金
        let available = treasury_balance.saturating_sub(protected_total);

        // 4. 资金保护规则
        let protection_config = T::FundProtectionQuery::fund_protection_status(entity_id).map(
            |(min_thresh, max_single, max_daily, daily_spent)| {
                let min_threshold: BalanceOf<T> = min_thresh.saturated_into();
                let max_single_spend: BalanceOf<T> = max_single.saturated_into();
                let max_daily_spend: BalanceOf<T> = max_daily.saturated_into();
                let daily_spent_bal: BalanceOf<T> = daily_spent.saturated_into();

                let daily_remaining = if max_daily_spend.is_zero() {
                    BalanceOf::<T>::zero()
                } else {
                    max_daily_spend.saturating_sub(daily_spent_bal)
                };

                crate::runtime_api::FundProtectionRules {
                    min_treasury_threshold: min_threshold,
                    max_single_spend: max_single_spend,
                    max_daily_spend: max_daily_spend,
                    daily_spent: daily_spent_bal,
                    daily_remaining,
                }
            },
        );
        ...
        // 5. 健康度
        let health_level = Self::get_fund_health(treasury_balance);
        let below_threshold = protection_config
            .as_ref()
            .map(|c| {
                !c.min_treasury_threshold.is_zero() && treasury_balance < c.min_treasury_threshold
            })
            .unwrap_or(false);
        let below_min_operating = available < T::MinOperatingBalance::get();

        Some(crate::runtime_api::EntityFundsView {
            treasury_balance,
            protected_total,
            available,
            protected: crate::runtime_api::ProtectedFundsBreakdown {
                pending_commission: pending_bal,
                shopping_balance: shopping_bal,
                unallocated_pool: unallocated_bal,
                pending_refund: refund_bal,
            },
            protection_config,
            health: crate::runtime_api::FundHealthStatus {
                level: health_level.encode()[0],
                min_operating: T::MinOperatingBalance::get(),
                warning_threshold: T::FundWarningThreshold::get(),
                below_threshold,
                below_min_operating,
    ...
    pub fn deduct_operating_fee(
        ...
        ensure!(available >= fee, Error::<T>::InsufficientOperatingFund);
        ...
        Entities::<T>::get(entity_id)
            .map(|entity| {
                ...
                entity
                    .admins
                    .iter()
                    .any(|(a, perm)| a == who && (perm & required) == required)
            })
            .unwrap_or(false)
    }
    ...
    pub fn ensure_permission(
        ...
        required: u32,
            ...
            Self::has_permission(entity_id, who, required),
            Error::<T>::NotAdmin
        );
        Ok(())
    }
    ...
    pub fn has_active_entity(account: &T::AccountId) -> bool {
        UserEntity::<T>::get(account).iter().any(|&eid| {
            Entities::<T>::get(eid)
    ...
    pub(crate) fn ensure_no_active_dependencies(entity_id: u64) -> sp_runtime::DispatchResult {
        ...
        // CRITICAL-2 审计修复: 存在未结清佣金资金时禁止关闭
        // 检查项: 待提佣金 (NEX/Token)、购物余额 (NEX/Token)、待重试退款
        ensure!(
            !T::CommissionCloseGuard::has_uncommitted_funds(entity_id),
            Error::<T>::HasUncommittedCommissionFunds
        );
        Ok(())
    }
    ...
    pub fn api_get_close_request_info(
        entity_id: u64,
    ) -> Option<crate::runtime_api::CloseRequestInfo<BlockNumberFor<T>>> {

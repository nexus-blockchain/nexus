...
use pallet_entity_common::{
    CommissionFundGuard, EntityProvider, EntityStatus, EntityType, GovernanceMode,
    GovernanceProvider, OnEntityStatusChange,
};
...
impl<T: Config> EntityProvider<T::AccountId> for Pallet<T> {
    ...
    fn register_shop(entity_id: u64, shop_id: u64) -> Result<(), sp_runtime::DispatchError> {
        ...
        // 添加到 EntityShops 列表
        EntityShops::<T>::try_mutate(
            entity_id,
            |shops| -> Result<(), sp_runtime::DispatchError> {
                // H5 修复: 区分重复注册和容量已满两种错误
                ensure!(!shops.contains(&shop_id), Error::<T>::ShopAlreadyRegistered);
                shops
                    .try_push(shop_id)
                    .map_err(|_| Error::<T>::ShopLimitReached)?;
                Ok(())
            },
        )?;
    ...
    fn unregister_shop(entity_id: u64, shop_id: u64) -> Result<(), sp_runtime::DispatchError> {
        ...
        let new_first = EntityShops::<T>::try_mutate(
            entity_id,
            |shops| -> Result<Option<u64>, sp_runtime::DispatchError> {
                let pos = shops
                    .iter()
                    .position(|&id| id == shop_id)
    ...
    fn entity_metadata_cid(entity_id: u64) -> Option<sp_std::vec::Vec<u8>> {
        Entities::<T>::get(entity_id).and_then(|e| e.metadata_uri.map(|c| c.into_inner()))
    }
    ...
    fn set_governance_mode(
        ...
        let old_mode = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<GovernanceMode, sp_runtime::DispatchError> {
...
impl<T: Config> EntityFunding<T::AccountId, BalanceOf<T>> for Pallet<T> {
    fn try_charge_entity(
        ...
        match T::Currency::transfer(&treasury, dest, amount, ExistenceRequirement::AllowDeath) {
            Ok(_) => Ok(true),
            Err(_) => Ok(false),
        }
    }

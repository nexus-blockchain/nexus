...
use pallet_entity_common::{
    CommissionCloseGuard, CommissionFundBreakdown, CommissionFundGuard, DisputeQueryProvider,
    EffectiveShopStatus, FundProtectionQueryPort, GovernanceMode, GovernanceProvider,
    MarketProvider, OrderProvider, PricingProvider, ShopOperatingStatus, ShopProvider, ShopType,
    TokenSaleProvider,
};
...
impl ShopProvider<u64> for MockShopProvider {
    ...
    fn shop_owner(_shop_id: u64) -> Option<u64> {
        None
    }
...
thread_local! {
    ...
    static UNCOMMITTED_FUNDS: RefCell<BTreeSet<u64>> = RefCell::new(BTreeSet::new());
    static PROTECTED_FUNDS: RefCell<sp_std::collections::btree_map::BTreeMap<u64, u128>> = RefCell::new(sp_std::collections::btree_map::BTreeMap::new());
}
...
impl OrderProvider<u64, u128> for MockOrderProvider {
    ...
    fn order_buyer(_order_id: u64) -> Option<u64> {
        None
    }
...
impl TokenSaleProvider<u128> for MockTokenSaleProvider {
    ...
    fn sale_round_status(_round_id: u64) -> Option<pallet_entity_common::TokenSaleStatus> {
        None
    }
    fn sold_amount(_round_id: u64) -> Option<u128> {
        None
    }
    fn remaining_amount(_round_id: u64) -> Option<u128> {
        None
    }
...
impl DisputeQueryProvider<u64> for MockDisputeQueryProvider {
    ...
    fn dispute_resolution(_dispute_id: u64) -> Option<pallet_entity_common::DisputeResolution> {
        None
    }
    fn active_dispute_count(_domain: u8, _account: &u64) -> u32 {
        0
    }
...
impl MarketProvider<u64, u128> for MockMarketProvider {
    ...
    fn trading_volume_24h(_entity_id: u64) -> u128 {
        0
    }
    fn best_bid(_entity_id: u64) -> Option<u128> {
        None
    }
    fn best_ask(_entity_id: u64) -> Option<u128> {
        None
    }
}

// ==================== Mock CommissionCloseGuard ====================

pub struct MockCommissionCloseGuard;
impl CommissionCloseGuard for MockCommissionCloseGuard {
    fn has_uncommitted_funds(entity_id: u64) -> bool {
        UNCOMMITTED_FUNDS.with(|m| m.borrow().contains(&entity_id))
    }
}

pub fn set_uncommitted_funds(entity_id: u64) {
    UNCOMMITTED_FUNDS.with(|m| m.borrow_mut().insert(entity_id));
}

#[allow(dead_code)]
pub fn clear_uncommitted_funds(entity_id: u64) {
    UNCOMMITTED_FUNDS.with(|m| m.borrow_mut().remove(&entity_id));
}

// ==================== Mock CommissionFundGuard ====================

pub struct MockCommissionFundGuard;
impl CommissionFundGuard for MockCommissionFundGuard {
    fn protected_funds(entity_id: u64) -> u128 {
        PROTECTED_FUNDS.with(|m| m.borrow().get(&entity_id).copied().unwrap_or(0))
    }
}

pub fn set_protected_funds(entity_id: u64, amount: u128) {
    PROTECTED_FUNDS.with(|m| m.borrow_mut().insert(entity_id, amount));
}

#[allow(dead_code)]
pub fn clear_protected_funds(entity_id: u64) {
    PROTECTED_FUNDS.with(|m| m.borrow_mut().remove(&entity_id));
}

// ==================== Mock CommissionFundBreakdown ====================

pub struct MockCommissionFundBreakdown;
impl CommissionFundBreakdown for MockCommissionFundBreakdown {
    fn protected_funds_breakdown(entity_id: u64) -> (u128, u128, u128, u128) {
        let total = PROTECTED_FUNDS.with(|m| m.borrow().get(&entity_id).copied().unwrap_or(0));
        // 简单 mock: 全部归入 pending_commission
        (total, 0, 0, 0)
    }
}

// ==================== Mock FundProtectionQueryPort ====================

pub struct MockFundProtectionQuery;
impl FundProtectionQueryPort for MockFundProtectionQuery {
...
impl pallet_entity_registry::Config for Test {
    ...
    type CommissionCloseGuard = MockCommissionCloseGuard;
    type CommissionFundGuard = MockCommissionFundGuard;
    type CommissionFundDetail = MockCommissionFundBreakdown;

...
use pallet_entity_common::{CommissionFundGuard, EntityStatus, OnEntityStatusChange, ShopProvider};
use sp_runtime::traits::{ConstU32, Saturating, Zero};
...
impl<T: Config> Pallet<T> {
    ...
    /// 暂停实体（治理，可附原因）
    pub(crate) fn do_suspend_entity(
        entity_id: u64,
        ...
        // L1: 截断超长原因（而非静默丢弃）
        let bounded_reason: Option<BoundedVec<u8, ConstU32<256>>> =
            reason.filter(|r| !r.is_empty()).map(|mut r| {
                r.truncate(256);
    ...
    pub(crate) fn do_ban_entity(
        ...
        // 仅允许 ban Active/Suspended/PendingClose 实体
            ...
            matches!(
                entity.status,
                EntityStatus::Active | EntityStatus::Suspended | EntityStatus::PendingClose
            ),
            Error::<T>::InvalidEntityStatus
        );
        ...
        // 佣金资金保护：只转走可用余额，已承诺给会员的佣金资金留在 entity 账户
        ...
        if !transferable.is_zero() {
            if confiscate_fund {
                ...
                .is_ok()
                {
                    Self::deposit_event(Event::FundConfiscated {
                        entity_id,
                        amount: transferable,
                    });
    ...
    pub(crate) fn do_unban_entity(entity_id: u64) -> sp_runtime::DispatchResult {
        let owner = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<T::AccountId, sp_runtime::DispatchError> {
                let entity = maybe_entity.as_mut().ok_or(Error::<T>::EntityNotFound)?;
                ensure!(
                    entity.status == EntityStatus::Banned,
                    Error::<T>::InvalidEntityStatus
                );
    ...
    pub(crate) fn do_force_transfer_ownership(
        ...
        let old_owner = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<T::AccountId, sp_runtime::DispatchError> {
                let entity = maybe_entity.as_mut().ok_or(Error::<T>::EntityNotFound)?;
    ...
    pub(crate) fn do_reject_close_request(entity_id: u64) -> sp_runtime::DispatchResult {
        ...
        Entities::<T>::try_mutate(entity_id, |maybe_entity| -> sp_runtime::DispatchResult {
            ...
            entity.status = if restore_to_active {
                EntityStatus::Active
            } else {
    ...
    pub(crate) fn do_force_rebind_referrer(
        ...
        EntityReferrerBoundAt::<T>::insert(entity_id, <frame_system::Pallet<T>>::block_number());

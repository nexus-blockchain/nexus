...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn do_add_admin(
        ...
        permissions: u32,
            ...
            AdminPermission::is_valid(permissions),
            Error::<T>::InvalidPermissions
        );
        ...
        Entities::<T>::try_mutate(entity_id, |maybe_entity| -> sp_runtime::DispatchResult {
            ...
            // 检查是否已是管理员
                ...
                !entity.admins.iter().any(|(a, _)| a == &new_admin),
                Error::<T>::AdminAlreadyExists
            );
            ...
            entity
                .admins
                .try_push((new_admin.clone(), permissions))
    ...
    pub(crate) fn do_remove_admin(
        ...
        Self::deposit_event(Event::AdminRemoved { entity_id, admin });
        Ok(())
    }
    ...
    pub(crate) fn do_update_admin_permissions(
        ...
        new_permissions: u32,
            ...
            AdminPermission::is_valid(new_permissions),
            Error::<T>::InvalidPermissions
        );

        let old_permissions = Entities::<T>::try_mutate(
            entity_id,
            |maybe_entity| -> Result<u32, sp_runtime::DispatchError> {
                let entity = maybe_entity.as_mut().ok_or(Error::<T>::EntityNotFound)?;
                ensure!(entity.owner == who, Error::<T>::NotEntityOwner);
                ensure!(
                    !T::GovernanceProvider::is_governance_locked(entity_id),
        ...
        let old_owner = Entities::<T>::try_mutate(
            ...
            |maybe_entity| -> Result<T::AccountId, sp_runtime::DispatchError> {
                ...
                // PendingClose: 转移后 approve_close 退款给新 owner 而非关闭申请人
                    ...
                    !matches!(
                        entity.status,
                        EntityStatus::Banned | EntityStatus::Closed | EntityStatus::PendingClose
                    ),

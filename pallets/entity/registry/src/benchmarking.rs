...
fn seed_entity<T: Config>(entity_id: u64) -> T::AccountId {
    ...
    let name: BoundedVec<u8, T::MaxEntityNameLength> = name_bytes.try_into().expect("name fits");
    ...
    // 写入 UserEntity 索引
    let _ = UserEntity::<T>::try_mutate(&owner, |entities| entities.try_push(entity_id));
...
fn seed_entity_with_one_admin<T: Config>(entity_id: u64) -> (T::AccountId, T::AccountId) {
    ...
    Entities::<T>::mutate(entity_id, |e| {
        if let Some(e) = e {
            let _ = e
                .admins
                .try_push((admin.clone(), AdminPermission::ALL_DEFINED));
        }
    });
    (owner, admin)
}
...
mod benches {
    ...
    fn resume_entity() {
        ...
        SuspensionReasons::<T>::insert(
            entity_id,
            BoundedVec::<u8, sp_runtime::traits::ConstU32<256>>::try_from(b"test".to_vec())
                .unwrap(),
        );

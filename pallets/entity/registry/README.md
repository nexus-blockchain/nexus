...
- **管理员体系** — owner + admins 两层权限，admin 绑定权限位掩码（11 种细粒度权限）
...
| `approve_close_entity` / `ban_entity` | Shop 强制关闭 | **物理写入**（终态，不可逆） |
...
| 15 | `reopen_entity(entity_id)` | owner | 重开（Closed → Pending，重新缴纳押金） |
...
| 18 | `unban_entity(entity_id)` | governance | 解除封禁（Banned → Pending） |
...
Active ◄─────────────────────────── approve_entity (Pending → Active)
  ...
  │    │ (owner 申请)      (治理审批 / 超时自动关闭，退还资金)
  │    ├── cancel_close_request (owner 撤销)
  │    └── reject_close_request (治理拒绝)
  │
  └──► Banned ──► unban_entity (治理) ──► Pending ──► approve_entity
...
| | 路径 A: 新建 | 路径 B: 重开 |
|---|---|---|
| **入口** | `create_entity` | `reopen_entity` → `approve_entity` |
| **初始状态** | 直接 `Active` | `Pending` → 治理审批 |
| **信任锚点** | 押金 | 押金 + 治理审批 |
| **Shop** | 自动创建 | `approve_entity` 恢复已有 Shop |
...
- **approve 资金检查** — `approve_entity` 检查金库余额 ≥ `MinOperatingBalance`，防止 unban 后零资金激活
...
| Runtime API 实现接线 | 📌 待接 | runtime_api.rs 已声明 8 个新 API + helpers.rs 已实现，需在 runtime/src/lib.rs 的 `impl_runtime_apis!` 中接线 |

use crate::{
    mock::*,
    pallet::{TransferBlacklist, TransferWhitelist},
    Error, Event,
};
...
fn mint_tokens_fails_exceeds_max_supply() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(OWNER), SHOP_ID, USER_A, 501,),
...
fn transfer_blocked_by_locked_tokens() {
    new_test_ext().execute_with(|| {
        ...
        // 尝试转 300 → 可用只有 200
        assert_noop!(
            EntityToken::transfer_tokens(RuntimeOrigin::signed(USER_A), SHOP_ID, USER_B, 300,),
...
fn transfer_blocked_by_reserved_tokens() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityToken as EntityTokenProvider<u64, u128>>::reserve(
            SHOP_ID, &USER_A, 800
        ));
        // 尝试转 300 → 可用只有 200
        assert_noop!(
...
fn distribute_dividend_fails_too_many_recipients() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            BoundedVec::<_, <Test as crate::Config>::MaxDividendRecipients>::try_from(recipients)
                .is_err()
        );
    });
}
...
fn reserve_unreserve_repatriate_works() {
    new_test_ext().execute_with(|| {
        ...
        // 预留 600
        assert_ok!(<Provider as EntityTokenProvider<u64, u128>>::reserve(
            SHOP_ID, &USER_A, 600
        ));
        ...
        assert!(
            <Provider as EntityTokenProvider<u64, u128>>::reserve(SHOP_ID, &USER_A, 500).is_err()
        );

        // 解锁 200
        let unreserved =
            <Provider as EntityTokenProvider<u64, u128>>::unreserve(SHOP_ID, &USER_A, 200);
        assert_eq!(unreserved, 200);
        assert_eq!(EntityToken::reserved_tokens(SHOP_ID, &USER_A), 400);

        // repatriate_reserved 300 → USER_B
        let repatriated = <Provider as EntityTokenProvider<u64, u128>>::repatriate_reserved(
...
fn whitelist_input_length_limited() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::add_to_whitelist(RuntimeOrigin::signed(OWNER), SHOP_ID, too_many,),
...
fn m1_transfer_tokens_rejects_zero_amount() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::transfer_tokens(RuntimeOrigin::signed(USER_A), SHOP_ID, USER_B, 0,),
            Error::<Test>::ZeroAmount
        );
    });
}
...
fn m3_mint_tokens_rejects_inactive_entity() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(OWNER), SHOP_ID, USER_A, 100,),
            Error::<Test>::EntityNotActive
        );
    });
}
...
fn h2r3_claim_dividend_succeeds_despite_later_mint() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            50,
            BoundedVec::try_from(vec![(USER_A, 50)]).unwrap(),
        ));
...
fn m1r4_distribute_multiple_times_respects_max_supply_with_pending() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            150,
            BoundedVec::try_from(vec![(USER_A, 150)]).unwrap(),
        ));
        ...
        assert_noop!(
            EntityToken::distribute_dividend(
                ...
                SHOP_ID,
                60,
                BoundedVec::try_from(vec![(USER_B, 60)]).unwrap(),
            ),
            Error::<Test>::ExceedsMaxSupply
        );
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            50,
            BoundedVec::try_from(vec![(USER_B, 50)]).unwrap(),
        ));
...
fn m1r4_claim_reduces_total_pending_dividends() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            150,
            BoundedVec::try_from(vec![(USER_A, 100), (USER_B, 50)]).unwrap(),
        ));
        assert_eq!(EntityToken::total_pending_dividends(SHOP_ID), 150);
        ...
        assert_noop!(
            EntityToken::distribute_dividend(
                ...
                SHOP_ID,
                1,
                BoundedVec::try_from(vec![(USER_A, 1)]).unwrap(),
            ),
            Error::<Test>::ExceedsMaxSupply
        );
    });
...
fn m1r4_set_max_supply_accounts_for_pending_dividends() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::set_max_supply(RuntimeOrigin::signed(OWNER), SHOP_ID, 140,),
            Error::<Test>::ExceedsMaxSupply
        );
...
fn m1r6_mint_tokens_respects_pending_dividends_in_max_supply() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(OWNER), SHOP_ID, USER_A, 201,),
            Error::<Test>::ExceedsMaxSupply
        );
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(OWNER), SHOP_ID, USER_A, 1,),
            Error::<Test>::ExceedsMaxSupply
        );
    });
}
...
fn m1r6_reward_on_purchase_skips_when_pending_fills_capacity() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            95,
            BoundedVec::try_from(vec![(USER_A, 95)]).unwrap(),
        ));
...
fn p0_non_admin_rejected_for_all_owner_extrinsics() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(NON_ADMIN), SHOP_ID, USER_A, 100,),
            Error::<Test>::NotAuthorized
        );
        assert_noop!(
            EntityToken::set_max_supply(RuntimeOrigin::signed(NON_ADMIN), SHOP_ID, 999,),
            Error::<Test>::NotAuthorized
        );
        assert_noop!(
            EntityToken::add_to_whitelist(RuntimeOrigin::signed(NON_ADMIN), SHOP_ID, vec![USER_A],),
            Error::<Test>::NotAuthorized
        );
    });
}
...
fn p0_wrong_permission_rejected() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::mint_tokens(RuntimeOrigin::signed(ADMIN), SHOP_ID, USER_A, 100,),
            Error::<Test>::NotAuthorized
        );
    });
}
...
fn p1_force_freeze_transfers_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::force_freeze_transfers(
            RuntimeOrigin::root(),
            SHOP_ID
        ));
...
fn p1_force_freeze_transfers_allows_claim_dividend() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::distribute_dividend(
            ...
            SHOP_ID,
            500,
            BoundedVec::try_from(vec![(USER_A, 500)]).unwrap(),
        ));
...
fn p1_force_unfreeze_transfers_works() {
    new_test_ext().execute_with(|| {
        ...
        // 解冻
        assert_ok!(EntityToken::force_unfreeze_transfers(
            RuntimeOrigin::root(),
            SHOP_ID
        ));
...
fn p2_force_burn_rejects_insufficient_balance() {
    new_test_ext().execute_with(|| {
        ...
        // 尝试销毁超过余额的数量
        assert!(EntityToken::force_burn(RuntimeOrigin::root(), SHOP_ID, USER_A, 200,).is_err());
    });
}
...
fn p3_global_token_pause_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(EntityToken::set_global_token_pause(
            RuntimeOrigin::root(),
            true
        ));
...
fn force_enable_token_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityToken::force_enable_token(
            RuntimeOrigin::root(),
            SHOP_ID
        ));
        assert!(EntityToken::is_token_enabled(SHOP_ID));
...
fn trait_available_balance_works() {
    new_test_ext().execute_with(|| {
        ...
        let avail =
            <EntityToken as EntityTokenProvider<u64, u128>>::available_balance(SHOP_ID, &USER_A);
        assert_eq!(avail, 600);
    });
}
...
fn trait_transfer_checks_global_pause() {
    new_test_ext().execute_with(|| {
        ...
        assert!(<EntityToken as EntityTokenProvider<u64, u128>>::transfer(
...
fn trait_transfer_checks_available_balance() {
    new_test_ext().execute_with(|| {
        ...
        // 可用 100，转 200 应失败
        assert!(<EntityToken as EntityTokenProvider<u64, u128>>::transfer(
            SHOP_ID, &USER_A, &USER_B, 200
        )
        .is_err());
        // 可用 100，转 100 应成功
        assert_ok!(<EntityToken as EntityTokenProvider<u64, u128>>::transfer(
            SHOP_ID, &USER_A, &USER_B, 100
        ));
        assert_eq!(EntityToken::token_balance(SHOP_ID, &USER_B), 100);
    });
...
fn m1_force_transfer_cleans_storage_on_zero_balance() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityToken as pallet_entity_common::EntityTokenProvider<
            u64,
            u128,
        >>::reserve(SHOP_ID, &USER_A, 300));
        // 配置分红并分发
        assert_ok!(EntityToken::change_token_type(
            RuntimeOrigin::signed(OWNER),
            SHOP_ID,
...
fn l3r3_trait_reserve_rejects_zero_amount() {
    ...
    new_test_ext().execute_with(|| {
        ...
        assert!(
            <EntityToken as EntityTokenProvider<u64, u128>>::reserve(SHOP_ID, &USER_A, 0).is_err()
        );
        // 非零应成功
        assert_ok!(<EntityToken as EntityTokenProvider<u64, u128>>::reserve(
            SHOP_ID, &USER_A, 500
...
fn r4_transfer_from_rejects_insufficient_allowance() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityToken::transfer_from(RuntimeOrigin::signed(USER_B), SHOP_ID, USER_A, 4, 200,),

//! # Entity token pallet (pallet-entity-token)
//!
//! ## Overview
//!
//! This pallet acts as a bridge on top of `pallet-assets` and provides token features
//! for each entity:
//! 本模块作为 `pallet-assets` 的桥接层，为每个实体提供通证功能：
//! - entity token creation and configuration
//! - 实体通证创建和配置
//! - multiple token types (points, governance, equity, membership, etc.)
//! - 多种通证类型（积分、治理、股权、会员等）
//! - shopping and participation rewards
//! - 购物 / 参与奖励
//! - points / token redemption
//! - 积分 / 通证兑换
//! - token transfers
//! - 通证转让
...
//!
//! ```text
//! pallet-entity-token (bridge layer)
//! pallet-entity-token（桥接层）
//!         │
//!         │ fungibles::* traits
//!         ▼
//! pallet-assets (underlying asset pallet)
...
//! - v0.2.0 (2026-02-03): Phase 2 expansion with multiple token types and dividends
...
pub mod pallet {
    ...
    use frame_support::{
        pallet_prelude::*,
        traits::fungibles::{metadata::Mutate as MetadataMutate, Create, Inspect, Mutate},
        BoundedVec,
    };
    use frame_system::pallet_prelude::*;
    use pallet_entity_common::{
        AdminPermission, DisclosureProvider, DividendConfig, EntityProvider, TokenType,
    ...
    /// Lock entry introduced by audit fix T-M1 to avoid accidentally extending unlock time
    /// when merged.
    ...
    /// 配置类型别名
    pub type EntityTokenConfigOf<T> =
        EntityTokenConfig<<T as Config>::AssetBalance, BlockNumberFor<T>>;
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Asset creation and mutation interface.
        /// 资产创建接口
        ...
        /// Shop-token ID offset to avoid collisions with other assets.
        ...
        /// Maximum address count for transfer whitelist / blacklist.
        /// 白名单 / 黑名单最大地址数
        #[pallet::constant]
        type MaxTransferListSize: Get<u32>;

        /// Maximum number of recipients in a single dividend distribution.
        /// 分红单次最大接收人数
        #[pallet::constant]
        type MaxDividendRecipients: Get<u32>;

        /// Optional KYC lookup interface.
        /// KYC 查询接口（可选）
        type KycProvider: KycLevelProvider<Self::AccountId>;

        /// Optional member lookup interface.
        /// 成员查询接口（可选）
        type MemberProvider: EntityMemberProvider<Self::AccountId>;

        /// Disclosure lookup interface used to enforce insider-trading restrictions
    ...
    /// KYC-level lookup trait. The KYC level is scoped per entity.
    ...
    impl<AccountId> KycLevelProvider<AccountId> for NullKycProvider {
        fn get_kyc_level(_entity_id: u64, _account: &AccountId) -> u8 {
            0
        }
        fn meets_kyc_requirement(_entity_id: u64, _account: &AccountId, min_level: u8) -> bool {
            min_level == 0
        }
    }
    ...
    impl<AccountId> EntityMemberProvider<AccountId> for NullMemberProvider {
        fn is_member(_entity_id: u64, _account: &AccountId) -> bool {
            true
        }
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_shop_token(
            ...
            // 检查代币是否已存在
                ...
                !EntityTokenConfigs::<T>::contains_key(entity_id),
            ...
            // 转换名称和符号
            let name_bounded: BoundedVec<u8, T::MaxTokenNameLength> = name
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::NameTooLong)?;
            let symbol_bounded: BoundedVec<u8, T::MaxTokenSymbolLength> = symbol
                .clone()
                .try_into()
                .map_err(|_| Error::<T>::SymbolTooLong)?;

            // R4: 资产 ID 溢出保护
            T::ShopTokenOffset::get()
                .checked_add(entity_id)
                .ok_or(Error::<T>::AssetIdOverflow)?;
        ...
        pub fn update_token_config(
            ...
            EntityTokenConfigs::<T>::try_mutate(entity_id, |maybe_config| -> DispatchResult {
                ...
                if !config.max_redeem_per_order.is_zero()
                    && config.min_redeem > config.max_redeem_per_order
                {
        ...
        pub fn mint_tokens(
            ...
            // 检查代币是否启用
            let config =
                EntityTokenConfigs::<T>::get(entity_id).ok_or(Error::<T>::TokenNotEnabled)?;
        ...
        pub fn transfer_tokens(
            ...
            // P1: 实体转账冻结检查
                ...
                !TransfersFrozen::<T>::contains_key(entity_id),
                Error::<T>::TokenTransfersFrozen
            );
        ...
        pub fn configure_dividend(
            ...
            EntityTokenConfigs::<T>::try_mutate(entity_id, |maybe_config| -> DispatchResult {
                ...
                // 检查通证类型是否支持分红
                    ...
                    config.token_type.has_dividend_rights(),
                    Error::<T>::TokenTypeNotSupported
                );
        ...
        pub fn distribute_dividend(
            ...
            // H1: 通证必须启用
                ...
                config.dividend_config.enabled,
                Error::<T>::DividendNotEnabled
            );
            // M6: 检查通证类型是否支持分红
            ensure!(
                config.token_type.has_dividend_rights(),
            ...
            if !last.is_zero() {
                    ...
                    now >= last + min_period,
                    Error::<T>::DividendPeriodNotReached
                );
            }
            ...
            if !config.max_supply.is_zero() {
                ...
                let existing_pending = TotalPendingDividends::<T>::get(entity_id);
                    ...
                    current_supply
                        .saturating_add(existing_pending)
            ...
            EntityTokenConfigs::<T>::mutate(entity_id, |maybe_config| {
                if let Some(config) = maybe_config {
                    ...
                    // L5: 更新累计分红金额
                    config.dividend_config.accumulated = config
                        .dividend_config
                        .accumulated
                        .saturating_add(total_amount);
                }
        ...
        pub fn claim_dividend(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn lock_tokens(
            ...
            LockedTokens::<T>::try_mutate(entity_id, &who, |entries| -> DispatchResult {
                entries
                    .try_push(LockEntry { amount, unlock_at })
        ...
        pub fn unlock_tokens(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            LockedTokens::<T>::try_mutate_exists(
                entity_id,
                &who,
                |maybe_entries| -> DispatchResult {
        ...
        pub fn change_token_type(
            ...
            let old_type = EntityTokenConfigs::<T>::try_mutate(
                entity_id,
                |maybe_config| -> Result<TokenType, DispatchError> {
                    let config = maybe_config.as_mut().ok_or(Error::<T>::TokenNotEnabled)?;
        ...
        pub fn add_to_whitelist(
            ...
            // M7: 限制输入列表长度
                ...
                accounts.len() <= T::MaxTransferListSize::get() as usize,
                Error::<T>::TransferListFull
            );
        ...
        pub fn force_disable_token(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn force_freeze_transfers(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            // 幂等性检查
                ...
                !TransfersFrozen::<T>::contains_key(entity_id),
                Error::<T>::TransfersAlreadyFrozen
            );
        ...
        pub fn force_unfreeze_transfers(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;

            ensure!(
                TransfersFrozen::<T>::contains_key(entity_id),
                Error::<T>::TransfersNotFrozen
            );
        ...
        pub fn set_global_token_pause(origin: OriginFor<T>, paused: bool) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn force_enable_token(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn force_cancel_pending_dividends(
            ...
            for account in accounts.iter() {
                ...
                !total_cancelled.is_zero(),
                Error::<T>::NoPendingDividendsToCancel
            );
        ...
        pub fn transfer_from(
            ...
            // 检查并扣减授权额度
            TokenApprovals::<T>::try_mutate(
                (entity_id, &owner, &spender),
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn total_locked_amount(entity_id: u64, who: &T::AccountId) -> T::AssetBalance {
            ...
            LockedTokens::<T>::get(entity_id, who)
                ...
                .fold(T::AssetBalance::zero(), |acc, e| {
                    acc.saturating_add(e.amount)
                })
        }

        /// R4: 统一可用余额检查（balance - locked - reserved >= required）
        pub(crate) fn ensure_available_balance(
            entity_id: u64,
            who: &T::AccountId,
            required: T::AssetBalance,
        ) -> DispatchResult {
            let asset_id = Self::entity_to_asset_id(entity_id);
        ...
        pub(crate) fn check_transfer_restriction(
            ...
            match config.transfer_restriction {
                ...
                TransferRestrictionMode::Whitelist => {
                        ...
                        TransferWhitelist::<T>::contains_key(entity_id, from),
                        Error::<T>::SenderNotInWhitelist
                    );
                    ensure!(
                        TransferWhitelist::<T>::contains_key(entity_id, to),
                        Error::<T>::ReceiverNotInWhitelist
                    );
                    Ok(())
                }
                ...
                TransferRestrictionMode::Blacklist => {
                        ...
                        !TransferBlacklist::<T>::contains_key(entity_id, from),
                        Error::<T>::SenderInBlacklist
                    );
                    ensure!(
                        !TransferBlacklist::<T>::contains_key(entity_id, to),
                        Error::<T>::ReceiverInBlacklist
                    );
                    Ok(())
                }
                ...
                TransferRestrictionMode::KycRequired => {
                        ...
                        T::KycProvider::meets_kyc_requirement(
                            entity_id,
                            from,
                            config.min_receiver_kyc
                        ),
        ...
        pub fn reward_on_purchase(
            ...
            let reward =
                purchase_amount.saturating_mul(config.reward_rate.into()) / 10000u32.into();
        ...
        pub fn redeem_for_discount(
            ...
            let config =
                ...
                tokens_to_use >= config.min_redeem,
                Error::<T>::BelowMinRedeem
            );
            ensure!(
                config.max_redeem_per_order.is_zero()
                    || tokens_to_use <= config.max_redeem_per_order,
            ...
            let discount =
                tokens_to_use.saturating_mul(config.exchange_rate.into()) / 10000u32.into();
...
impl<T: Config> Pallet<T> {
    ...
    pub fn get_lock_entries(
        ...
        pallet::LockEntry<T::AssetBalance, frame_system::pallet_prelude::BlockNumberFor<T>>,
    > {
...
use sp_runtime::traits::{Saturating as _Saturating, Zero as _Zero};
...
impl<T: Config> EntityTokenProvider<T::AccountId, T::AssetBalance> for Pallet<T> {
    ...
    fn unreserve(entity_id: u64, who: &T::AccountId, amount: T::AssetBalance) -> T::AssetBalance {
    ...
    fn governance_set_max_supply(
        entity_id: u64,
        new_max_supply: T::AssetBalance,
    ) -> Result<(), sp_runtime::DispatchError> {
        EntityTokenConfigs::<T>::try_mutate(
            entity_id,
    ...
    fn governance_set_token_type(
        entity_id: u64,
        new_type: TokenType,
    ) -> Result<(), sp_runtime::DispatchError> {
        let old_type = EntityTokenConfigs::<T>::try_mutate(
            entity_id,
            |maybe_config| -> Result<TokenType, sp_runtime::DispatchError> {
                let config = maybe_config.as_mut().ok_or(Error::<T>::TokenNotEnabled)?;
                let old = config.token_type;

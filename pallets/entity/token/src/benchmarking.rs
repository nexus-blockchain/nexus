...
use frame_support::traits::fungibles::{metadata::Mutate as MetadataMutate, Create, Mutate};
...
fn seed_token<T: Config>(entity_id: u64, owner: &T::AccountId) {
    ...
    let name: BoundedVec<u8, T::MaxTokenNameLength> = b"BenchToken".to_vec().try_into().unwrap();
    let symbol: BoundedVec<u8, T::MaxTokenSymbolLength> = b"BT".to_vec().try_into().unwrap();
    ...
    let _ = T::Assets::set(asset_id, owner, b"BenchToken".to_vec(), b"BT".to_vec(), 18);
}
...
mod benchmarks {
    ...
    fn transfer_from() {
        ...
        // 设置授权额度
        TokenApprovals::<T>::insert(
            (ENTITY_1, &owner, &spender),
            T::AssetBalance::from(20_000u128),
        );
        #[extrinsic_call]
        _(
            RawOrigin::Signed(spender),
            ENTITY_1,
            owner,
            to,
            10_000u128.into(),

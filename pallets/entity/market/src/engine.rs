...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn calculate_total_next(
        token_amount: u128,
        price: BalanceOf<T>,
    ...
    pub(crate) fn do_cross_match(
        ...
        for counter_order in counter_orders {
            ...
            if counter_order.maker == *taker {
                continue;
            }

            let available = counter_order
                .token_amount
                .saturating_sub(counter_order.filled_amount);
            ...
            T::TokenProvider::repatriate_reserved(entity_id, token_from, token_to, fill_amount)?;
    ...
    pub(crate) fn do_market_buy(
        ...
        for order in sell_orders.iter_mut() {
            ...
            T::Currency::transfer(buyer, &order.maker, cost, ExistenceRequirement::KeepAlive)?;

            // Token: maker → buyer（从 maker 的 reserved 转出）
            T::TokenProvider::repatriate_reserved(entity_id, &order.maker, buyer, fill_amount)?;
    ...
    pub(crate) fn do_market_sell(
        ...
        for order in buy_orders.iter_mut() {
            ...
            T::TokenProvider::repatriate_reserved(entity_id, seller, &order.maker, fill_amount)?;

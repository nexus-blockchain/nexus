...
impl<T: Config> Pallet<T> {
    ...
    fn order_to_info(
        order: &TradeOrder<T>,
    ) -> runtime_api::OrderInfo<T::AccountId, BalanceOf<T>, T::TokenBalance> {
    ...
    fn trade_to_info(
        trade: &TradeRecord<T>,
    ) -> runtime_api::TradeInfo<T::AccountId, BalanceOf<T>, T::TokenBalance> {
    ...
    pub fn api_get_order(
        order_id: u64,
    ) -> Option<runtime_api::OrderInfo<T::AccountId, BalanceOf<T>, T::TokenBalance>> {
        Orders::<T>::get(order_id).as_ref().map(Self::order_to_info)
    }

    pub fn api_get_order_book_depth(
        entity_id: u64,
        max_depth: u32,
    ) -> runtime_api::OrderBookDepthInfo<BalanceOf<T>, T::TokenBalance> {
        ...
        runtime_api::OrderBookDepthInfo {
            ...
            asks: depth
                .asks
                .into_iter()
                .map(|l| runtime_api::PriceLevelInfo {
                    price: l.price,
    ...
    pub fn api_get_market_summary(
        entity_id: u64,
    ) -> runtime_api::MarketSummaryInfo<BalanceOf<T>, T::TokenBalance> {
    ...
    pub fn api_get_order_book_snapshot(
        entity_id: u64,
    ) -> (
        Vec<(BalanceOf<T>, T::TokenBalance)>,
        Vec<(BalanceOf<T>, T::TokenBalance)>,
    ) {

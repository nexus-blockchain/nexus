...
fn seed_sell_order<T: Config>(
    ...
    UserOrders::<T>::mutate(maker, |orders| {
        let _ = orders.try_push(order_id);
    });
    // 更新统计
...
/// 创建一个已过期的卖单
fn seed_expired_sell_order<T: Config>(
    entity_id: u64,
    maker: &T::AccountId,
    price: u128,
    amount: u128,
...
mod benchmarks {
    ...
    fn place_sell_order() {
        ...
        _(
            ...
            ENTITY_1,
            T::TokenBalance::from(1000u128),
            BalanceOf::<T>::from(100u128),
        );
    }
    ...
    fn modify_order() {
        ...
        _(
            RawOrigin::Signed(caller),
            order_id,
            BalanceOf::<T>::from(110u128),
            T::TokenBalance::from(900u128),
        );
    }

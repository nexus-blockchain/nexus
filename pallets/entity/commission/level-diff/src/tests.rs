...
fn plugin_works_with_level_diff_mode_enabled() {
    new_test_ext().execute_with(|| {
        ...
        let (outputs, remaining) = <pallet::Pallet<Test> as pallet_commission_common::CommissionPlugin<u64, Balance>>::calculate(
            entity_id, &50, 10000, 10000, modes, false, 1, 0,
        );
...
fn set_level_rates_rejects_invalid_rate() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            <pallet::Pallet<Test> as LevelDiffPlanWriter>::set_level_rates(1, vec![10001], 5)
                .is_err()
        );
        assert_ok!(
            <pallet::Pallet<Test> as LevelDiffPlanWriter>::set_level_rates(
                1,
                vec![100, 200, 300],
                5
            )
...
fn max_depth_limits_traversal() {
    new_test_ext().execute_with(|| {
        ...
        let level_rates =
            frame_support::BoundedVec::try_from(vec![300u16, 600, 900, 1200]).unwrap();
        assert_ok!(pallet::Pallet::<Test>::force_set_level_diff_config(
            RuntimeOrigin::root(),
            entity_id,
            level_rates,
...
fn clear_config_removes_config() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<pallet::Pallet<Test> as LevelDiffPlanWriter>::clear_config(
            entity_id
        ));
...
fn m1_set_level_rates_trait_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "LevelDiffConfigUpdated event should be emitted via trait path"
        );
    });
}
...
fn m2r4_token_plugin_basic_calculation() {
    new_test_ext().execute_with(|| {
        ...
        let (outputs, remaining) = <pallet::Pallet<Test> as pallet_commission_common::TokenCommissionPlugin<u64, u128>>::calculate_token(
            entity_id, &50, 10000u128, 10000u128, modes, false, 1, 0,
        );
...
fn m2r4_token_plugin_skips_when_mode_not_enabled() {
    new_test_ext().execute_with(|| {
        let modes = CommissionModes(CommissionModes::DIRECT_REWARD);
        let (outputs, remaining) = <pallet::Pallet<Test> as pallet_commission_common::TokenCommissionPlugin<u64, u128>>::calculate_token(
            1, &50, 10000u128, 10000u128, modes, false, 1, 0,
        );
        assert!(outputs.is_empty());
...
fn x3_clear_rejects_non_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CommissionLevelDiff::clear_level_diff_config(RuntimeOrigin::signed(NON_OWNER), 1,),
            Error::<Test>::NotEntityOwnerOrAdmin
        );
    });
}
...
fn x3_clear_rejects_absent_config() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            CommissionLevelDiff::clear_level_diff_config(RuntimeOrigin::signed(OWNER), 1,),
...
fn x4_force_clear_rejects_non_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CommissionLevelDiff::force_clear_level_diff_config(RuntimeOrigin::signed(OWNER), 1,),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn x4_force_clear_rejects_absent() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            CommissionLevelDiff::force_clear_level_diff_config(RuntimeOrigin::root(), 1,),
            Error::<Test>::ConfigNotFound
        );
    });
}
...
fn x6_locked_entity_rejects_set() {
    new_test_ext().execute_with(|| {
        ...
        let rates = frame_support::BoundedVec::try_from(vec![300u16]).unwrap();
        assert_noop!(
            CommissionLevelDiff::set_level_diff_config(RuntimeOrigin::signed(OWNER), 1, rates, 5,),
...
fn x10_trait_clear_no_phantom_event_when_absent() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !found,
            "No LevelDiffConfigCleared event should be emitted when config absent"
        );
    });
}
...
fn x10_trait_clear_emits_event_when_present() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "LevelDiffConfigCleared should be emitted when config exists"
        );
    });
}
...
fn f3_rejects_non_monotonic_rates_extrinsic() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CommissionLevelDiff::set_level_diff_config(RuntimeOrigin::signed(OWNER), 1, rates, 10,),
...
fn f3_rejects_non_monotonic_rates_force() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            CommissionLevelDiff::force_set_level_diff_config(RuntimeOrigin::root(), 1, rates, 10,),
            Error::<Test>::RatesNotMonotonic
        );
    });
}
    ...
    new_test_ext().execute_with(|| {
        ...
        let detail_events: Vec<_> = events
            ...
            .filter(|e| {
                matches!(
                    e.event,
                    RuntimeEvent::CommissionLevelDiff(
                        pallet::Event::LevelDiffCommissionDetail { .. }
                    )
                )
            })
            .collect();
...
fn f6_trait_set_rejects_nonexistent_entity() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <pallet::Pallet<Test> as LevelDiffPlanWriter>::set_level_rates(999, vec![100, 200], 5);
        assert!(result.is_err());
...
fn f6_trait_set_works_for_existing_entity() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            <pallet::Pallet<Test> as LevelDiffPlanWriter>::set_level_rates(1, vec![100, 200], 5)
        );
        assert!(pallet::CustomLevelDiffConfigs::<Test>::get(1).is_some());
    });
}
...
fn f8_config_updated_event_contains_levels_count() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "LevelDiffConfigUpdated should include levels_count=3"
        );
    });
}
...
fn f8_trait_path_event_contains_levels_count() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "Trait path should emit LevelDiffConfigUpdated with levels_count=4"
        );
    });
}
...
fn l1r7_force_set_rejects_nonexistent_entity() {
    new_test_ext().execute_with(|| {
        let rates = frame_support::BoundedVec::try_from(vec![300u16]).unwrap();
        assert_noop!(
            CommissionLevelDiff::force_set_level_diff_config(RuntimeOrigin::root(), 999, rates, 5,),
            Error::<Test>::EntityNotFound
        );
    });
}
...
fn l1r7_force_clear_rejects_nonexistent_entity() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            CommissionLevelDiff::force_clear_level_diff_config(RuntimeOrigin::root(), 999,),
            Error::<Test>::EntityNotFound
        );
    });
}
...
#[test]
fn set_level_diff_config_rejects_max_rate_exceeding_budget_cap() {
    new_test_ext().execute_with(|| {
        use frame_support::BoundedVec;
        set_budget_cap(1, 2000); // cap = 20%
                                 // level_rates = [1000, 2001] → max = 2001 > cap
        let rates: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![1000, 2001].try_into().unwrap();
        assert_noop!(
            CommissionLevelDiff::set_level_diff_config(RuntimeOrigin::signed(OWNER), 1, rates, 5),
            crate::pallet::Error::<Test>::MaxRateExceedsBudgetCap
        );
        // max = 2000 == cap should pass
        let rates_ok: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![1000, 2000].try_into().unwrap();
        assert_ok!(CommissionLevelDiff::set_level_diff_config(
            RuntimeOrigin::signed(OWNER),
            1,
            rates_ok,
...
#[test]
fn update_level_diff_config_rejects_max_rate_exceeding_budget_cap() {
    new_test_ext().execute_with(|| {
        use frame_support::BoundedVec;
        // set initial config without cap
        let rates: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![1000, 3000].try_into().unwrap();
        assert_ok!(CommissionLevelDiff::set_level_diff_config(
            RuntimeOrigin::signed(OWNER),
            1,
            rates,
        ...
        // now set cap and try to update with exceeding rates
        set_budget_cap(1, 2000);
        let bad_rates: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![1000, 2001].try_into().unwrap();
        assert_noop!(
            CommissionLevelDiff::update_level_diff_config(
                RuntimeOrigin::signed(OWNER),
...
#[test]
fn force_set_level_diff_config_rejects_max_rate_exceeding_budget_cap() {
    new_test_ext().execute_with(|| {
        use frame_support::BoundedVec;
        set_budget_cap(1, 1500);
        let rates: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![500, 1501].try_into().unwrap();
        assert_noop!(
            CommissionLevelDiff::force_set_level_diff_config(RuntimeOrigin::root(), 1, rates, 5),
            crate::pallet::Error::<Test>::MaxRateExceedsBudgetCap
        );
    });
}
...
fn budget_cap_zero_means_no_limit() {
    new_test_ext().execute_with(|| {
        ...
        let rates: BoundedVec<u16, <Test as crate::Config>::MaxCustomLevels> =
            vec![5000, 10000].try_into().unwrap();
        assert_ok!(CommissionLevelDiff::set_level_diff_config(
            RuntimeOrigin::signed(OWNER),
            1,
            rates,
...
fn plan_writer_rejects_max_rate_exceeding_budget_cap() {
    new_test_ext().execute_with(|| {
        ...
        set_budget_cap(1, 2000);
        assert!(
            <crate::pallet::Pallet<Test> as LevelDiffPlanWriter>::set_level_rates(
                1,
                vec![1000, 2001],
                5
            )

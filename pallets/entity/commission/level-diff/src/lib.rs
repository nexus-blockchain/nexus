...
pub mod pallet {
    ...
    pub type CustomLevelDiffConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, CustomLevelDiffConfigOf<T>>;
    ...
    pub enum Event<T: Config> {
        LevelDiffConfigUpdated {
            entity_id: u64,
            levels_count: u32,
        },
        LevelDiffConfigCleared {
            entity_id: u64,
        },
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn clear_level_diff_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            // F1: Entity 活跃状态检查
            ensure!(
                ...
                CustomLevelDiffConfigs::<T>::contains_key(entity_id),
        ...
        pub fn update_level_diff_config(
            ...
            let existing =
                CustomLevelDiffConfigs::<T>::get(entity_id).ok_or(Error::<T>::ConfigNotFound)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_set_config(
            ...
            for w in level_rates.windows(2) {
                ...
                max_depth > 0 && max_depth <= 20,
                Error::<T>::InvalidMaxDepth
            );

            // 预算上限校验: 极差模式最大佣金 = 最高等级 rate（cap=0 表示无上限）
            let cap = T::BudgetCapProvider::level_diff_cap(entity_id);
            if cap > 0 {
                let max_rate = level_rates.iter().copied().max().unwrap_or(0);
            ...
            Self::deposit_event(Event::LevelDiffConfigUpdated {
                entity_id,
                levels_count,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
            ...
            T::AccountId: Ord,
        {
            ...
            while let Some(ref referrer) = current_referrer {
                ...
                let referrer_rate = config
                    .as_ref()
                    .and_then(|c| c.level_rates.get(level_id as usize).copied())
                    .unwrap_or_else(|| {
                        T::MemberProvider::get_level_commission_bonus(entity_id, level_id)
                    });

                if referrer_rate > prev_rate {
                    let diff_rate = referrer_rate - prev_rate;
                    let commission =
                        order_amount.saturating_mul(diff_rate.into()) / 10000u32.into();
        ...
        T::AccountId: Ord,
    {
        ...
        while let Some(ref referrer) = current_referrer {
            ...
            if referrer_rate > prev_rate {
                let diff_rate = referrer_rate - prev_rate;
                let commission =
                    order_amount.saturating_mul(TB::from(diff_rate as u32)) / TB::from(10000u32);
                let actual = commission.min(*remaining);
...
impl<T: pallet::Config> pallet_commission_common::LevelDiffPlanWriter for pallet::Pallet<T> {
    fn set_level_rates(
        ...
        frame_support::ensure!(
            !level_rates.is_empty(),
            sp_runtime::DispatchError::Other("EmptyLevelRates")
        );
        for rate in level_rates.iter() {
            frame_support::ensure!(
                *rate <= 10000,
        ...
        for w in level_rates.windows(2) {
            frame_support::ensure!(
                w[1] >= w[0],
                sp_runtime::DispatchError::Other("RatesNotMonotonic")
            );
        }
        frame_support::ensure!(
            max_depth > 0 && max_depth <= 20,
        ...
        // 预算上限校验
        let cap = <T::BudgetCapProvider as pallet_commission_common::PluginBudgetCapProvider>::level_diff_cap(entity_id);
        if cap > 0 {
            let max_rate = level_rates.iter().copied().max().unwrap_or(0);
            frame_support::ensure!(
                max_rate <= cap,
        ...
        let bounded_rates: frame_support::BoundedVec<u16, T::MaxCustomLevels> = level_rates
            .try_into()
            .map_err(|_| sp_runtime::DispatchError::Other("TooManyLevels"))?;
        // F8: 事件包含 levels_count
        let levels_count = bounded_rates.len() as u32;
        pallet::CustomLevelDiffConfigs::<T>::insert(
            entity_id,
            pallet::CustomLevelDiffConfig {
                level_rates: bounded_rates,
                max_depth,
            },
        );
        // M1 审计修复: trait 路径也发出事件
        pallet::Pallet::<T>::deposit_event(pallet::Event::LevelDiffConfigUpdated {
            entity_id,
            levels_count,
        });
        Ok(())
...
// ============================================================================
// LevelDiffQueryProvider 实现
// ============================================================================

impl<T: pallet::Config> pallet_commission_common::LevelDiffQueryProvider for pallet::Pallet<T> {
    fn chain_depth(entity_id: u64) -> u16 {
        pallet::CustomLevelDiffConfigs::<T>::get(entity_id)

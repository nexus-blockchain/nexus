...
| `OWNER_REWARD` | `0b100_0000_0000` | `0x400` | 创建人收益 |
...
| `set_owner_reward_rate(entity_id, rate)` | 设置创建人收益比例（bps） |
...
| v0.5.0 | 2026-03 | 新增 `OWNER_REWARD` 模式位 / `OwnerReward` 类型 / `set_owner_reward_rate`；新增 `SingleLinePlanWriter` / `ParticipationGuard` trait；`MultiLevelPlanWriter` 新增 `set_multi_level_full`；`MemberProvider` 改为从 `pallet-entity-common` 重新导出 |

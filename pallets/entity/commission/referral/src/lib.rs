...
pub mod pallet {
    ...
    pub struct ReferrerGuardConfig {
        /// 推荐人最低升级/奖励有效消费（u128，来自 MemberProvider 显式 eligible spend，0=无限制）
    ...
    impl<Balance: Default> Default for CommissionCapConfig<Balance> {
        fn default() -> Self {
            Self {
                max_per_order: Balance::default(),
                max_total_earned: Balance::default(),
            }
        }
    }
    ...
    pub type ReferralConfigs<T: Config> = StorageMap<_, Blake2_128Concat, u64, ReferralConfigOf<T>>;

    /// F1: 推荐人激活条件 entity_id -> ReferrerGuardConfig
    #[pallet::storage]
    pub type ReferrerGuardConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, ReferrerGuardConfig>;

    /// F2: 返佣上限配置 entity_id -> CommissionCapConfig
    #[pallet::storage]
    pub type CommissionCapConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, CommissionCapConfig<BalanceOf<T>>>;
    ...
    #[pallet::storage]
    pub type ConfigEffectiveAfter<T: Config> =
    ...
    pub enum Event<T: Config> {
        ...
        ReferralConfigCleared {
            entity_id: u64,
        },
        /// F3: 配置生效时间设置
        ConfigEffectiveAfterSet {
            entity_id: u64,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_direct_reward_config(
            ...
            Self::ensure_rate_within_cap(entity_id, rate)?;
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::DirectReward,
            });
            Ok(())
        }
        ...
        pub fn set_fixed_amount_config(
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::FixedAmount,
            });
            Ok(())
        }
        ...
        pub fn set_first_order_config(
            ...
            ReferralConfigs::<T>::mutate(entity_id, |maybe| {
                let config = maybe.get_or_insert_with(ReferralConfig::default);
                config.first_order = FirstOrderConfig {
                    amount,
                    rate,
                    use_amount,
                };
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::FirstOrder,
            });
            Ok(())
        }
        ...
        pub fn set_repeat_purchase_config(
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::RepeatPurchase,
            });
            Ok(())
        }
        ...
        pub fn clear_referral_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
                ...
                ReferralConfigs::<T>::contains_key(entity_id),
        ...
        pub fn force_clear_referral_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn set_referrer_guard_config(
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::ReferrerGuard,
            });
            Ok(())
        }
        ...
        pub fn set_commission_cap_config(
            ...
            Self::deposit_event(Event::ReferralConfigUpdated {
                entity_id,
                mode: ReferralConfigMode::CommissionCap,
            });
            Ok(())
        }
        ...
        pub fn set_config_effective_after(
            ...
            Self::deposit_event(Event::ConfigEffectiveAfterSet {
                entity_id,
                block_number,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 校验费率不超过插件预算上限（cap=0 表示无上限，跳过校验）
        fn ensure_rate_within_cap(entity_id: u64, rate: u16) -> DispatchResult {
            let cap = T::BudgetCapProvider::referral_cap(entity_id);
            if cap > 0 {
                ensure!(rate <= cap, Error::<T>::RateExceedsBudgetCap);
            }
            Ok(())
        }

        /// F1/F6: 验证推荐人资格（封禁/冻结/激活条件）
        pub(crate) fn is_referrer_qualified(
            ...
            if let Some(guard) = ReferrerGuardConfigs::<T>::get(entity_id) {
                if guard.min_referrer_spent > 0 {
                    let eligible_spent = T::MemberProvider::get_member_upgrade_eligible_spent_usdt(
                        entity_id, referrer,
                    ) as u128;
                    if eligible_spent < guard.min_referrer_spent {
                        return false;
                    }
                }
                if guard.min_referrer_orders > 0 {
                    let orders = T::MemberProvider::completed_order_count(entity_id, referrer);
                    if orders < guard.min_referrer_orders {
                        return false;
                    }
                }
            }
        ...
        /// F2: 更新推荐人累计获佣
        fn track_referrer_earned(entity_id: u64, referrer: &T::AccountId, amount: BalanceOf<T>) {
            if !amount.is_zero() {
        ...
        pub fn process_direct_reward(
            ...
            if let Some(referrer) = T::MemberProvider::get_referrer(entity_id, buyer) {
                if !Self::is_referrer_qualified(entity_id, buyer, &referrer) {
                    return;
                }
        ...
        pub fn process_first_order(
            ...
            if config.use_amount && config.amount.is_zero() {
                return;
            }
            if !config.use_amount && config.rate == 0 {
                return;
            }
            if let Some(referrer) = T::MemberProvider::get_referrer(entity_id, buyer) {
                if !Self::is_referrer_qualified(entity_id, buyer, &referrer) {
        ...
        pub fn process_repeat_purchase(
            ...
            if config.rate == 0 || buyer_order_count < config.min_orders {
                return;
            }
            if let Some(referrer) = T::MemberProvider::get_referrer(entity_id, buyer) {
                if !Self::is_referrer_qualified(entity_id, buyer, &referrer) {
    ...
    for pallet::Pallet<T>
{
    fn calculate(
        ...
        if max_rate < 10000u16 && !outputs.is_empty() {
            let max_amount =
                order_amount.saturating_mul((max_rate as u32).into()) / 10000u32.into();
            ...
            for output in outputs.iter_mut() {
                ...
                if output.amount > space {
                    ...
                    pallet::ReferrerTotalEarned::<T>::mutate(
                        entity_id,
                        &output.beneficiary,
                        |earned| {
...
impl<T: pallet::Config> pallet::Pallet<T> {
        ...
        TB: sp_runtime::traits::AtLeast32BitUnsigned + Copy,
    {
        ...
        if let Some(referrer) = T::MemberProvider::get_referrer(entity_id, buyer) {
            ...
            let commission =
                order_amount.saturating_mul(TB::from(config.rate as u32)) / TB::from(10000u32);
            let actual = commission.min(*remaining);
            if !actual.is_zero() {
                *remaining = remaining.saturating_sub(actual);
                outputs.push(pallet_commission_common::CommissionOutput {
    ...
    T::AccountId: Ord,
{
    fn calculate_token(
        ...
        if max_rate < 10000u16 && !outputs.is_empty() {
            let max_amount =
                order_amount.saturating_mul(TB::from(max_rate as u32)) / TB::from(10000u32);
...
impl<T: pallet::Config> pallet_commission_common::ReferralPlanWriter<pallet::BalanceOf<T>>
    for pallet::Pallet<T>
{
    fn set_direct_rate(entity_id: u64, rate: u16) -> Result<(), sp_runtime::DispatchError> {
        ...
        // 预算上限校验
        let cap = <T::BudgetCapProvider as pallet_commission_common::PluginBudgetCapProvider>::referral_cap(entity_id);
        if cap > 0 {
            frame_support::ensure!(
                rate <= cap,
                sp_runtime::DispatchError::Other("RateExceedsBudgetCap")
            );
        }
        ...
        pallet::Pallet::<T>::deposit_event(pallet::Event::ReferralConfigUpdated {
            entity_id,
    ...
    fn set_first_order(
        ...
        pallet::ReferralConfigs::<T>::mutate(entity_id, |maybe| {
            let config = maybe.get_or_insert_with(pallet::ReferralConfig::default);
            config.first_order = pallet::FirstOrderConfig {
                amount,
                rate,
                use_amount,
            };
    ...
    fn set_referrer_guard(
        ...
        pallet::ReferrerGuardConfigs::<T>::insert(
            entity_id,
    ...
    fn set_commission_cap(
        ...
        pallet::CommissionCapConfigs::<T>::insert(
            entity_id,
...
impl<T: pallet::Config> pallet_commission_common::ReferralQueryProvider<T::AccountId, BalanceOf<T>>
    for pallet::Pallet<T>
{

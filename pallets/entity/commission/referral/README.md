...
清除配置时（`clear_referral_config` / `force_clear` / `ReferralPlanWriter::clear_config`），`do_clear_all_config` 统一删除上述全部 5 项存储，包括通过 `clear_prefix` 批量删除 `ReferrerTotalEarned` 的所有条目，确保无孤立数据残留。
...
- **共享 F1/F3/F6/F7/F8**：推荐人资格、生效时间、冻结检查、首单判定、全局率上限均与 NEX 路径一致
...
| 函数 | 可见性 | 说明 |
|------|--------|------|
| `do_clear_all_config` | `pub(crate)` | 统一清除 5 项存储（主配置 + 4 项附属 + `ReferrerTotalEarned` 前缀清除） |
| `ensure_owner_or_admin` | private | 验证 Entity Owner 或 Admin(`COMMISSION_MANAGE`) 权限 |
| `is_referrer_qualified` | `pub(crate)` | 综合验证推荐人资格（X1 + F1 + F6） |
...
- F1 激活条件、F2 上限封顶、F3 生效时间、F6 冻结、F7 首单、F8 全局上限、F10 事件粒度
- Entity 锁定/未激活拒绝

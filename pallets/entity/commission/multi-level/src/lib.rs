...
//! - 每层独立激活条件（直推人数 / 团队规模 / USDT 累计消费 / 会员等级，AND 逻辑）
//! - 总佣金上限由 `CoreCommissionConfig.plugin_caps.multi_level_cap` 统一控制
...
pub mod pallet {
    ...
    use pallet_commission_common::{
        CommissionOutput, CommissionType, MemberProvider, PluginBudgetCapProvider,
    };
    ...
    /// 多级分销层级配置
    ///
    /// 每层包含佣金比率和四维激活条件（AND 逻辑，值为 0 的条件自动跳过）。
    #[derive(
        Encode,
        Decode,
    ...
    pub struct MultiLevelTier {
        ...
        /// L3: rate=0 时该层不分发佣金，但仍消耗推荐链中一个位置（referrer 向上移动一步）。
        /// 这意味着 rate=0 不是"跳过这层让后续层获益"，而是"占位空层，推荐链继续前进"。
        /// 若需完全禁用某层且不消耗推荐链位置，应使用 remove_tier 删除该层。
        pub rate: u16,
        ...
        /// 最低会员等级 ID，0 = 无要求
        pub required_level_id: u8,
    }

    /// 多级分销配置
    ///
    /// 佣金总额上限由 `CoreCommissionConfig.plugin_caps.multi_level_cap` 统一控制，
    /// 不再在插件内部维护独立的 `max_total_rate`。
    ...
    // F6: 个人多级佣金统计数据
    // L7-FIX: total_orders → commission_receipt_count（实际语义为"获得佣金的次数"，非"下单数"）
    #[derive(
        Encode,
        Decode,
        codec::DecodeWithMemTracking,
    ...
    pub struct MultiLevelPayoutRecord<AccountId> {
        /// 触发分佣的买家
        pub buyer: AccountId,
        /// 关联订单 ID（审计追踪链: OrderCommissionRecords ↔ MemberMultiLevelPayouts）
        pub order_id: u64,
        /// 分佣金额 (NEX, u128)
        pub amount: u128,
        /// 推荐链层级 (1 = L1 直推, 2 = L2 ...)
    ...
    pub struct MemberMultiLevelSummary {
        /// 累计获得佣金 (NEX)
        pub total_earned: u128,
        /// 总分佣笔数
        pub total_payout_count: u32,
        /// 最后一次分佣区块
        pub last_payout_block: u32,
    }
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// 插件预算上限查询（读取 CoreCommissionConfig.plugin_caps.multi_level_cap）
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            assert!(T::MaxMultiLevels::get() >= 1, "MaxMultiLevels must be >= 1");
            assert!(
                T::MaxMultiLevels::get() <= 1000,
                "MaxMultiLevels must be <= 1000 to prevent excessive computation"
            );
            assert!(
            ...
            assert!(
                T::MaxPayoutRecords::get() > 0,
                "MaxPayoutRecords must be > 0 to avoid FIFO panic on empty BoundedVec"
            );
        }
    }
    ...
    pub type MultiLevelConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, MultiLevelConfigOf<T>>;

    // F10: 全局暂停开关
    #[pallet::storage]
    pub type GlobalPaused<T: Config> = StorageMap<_, Blake2_128Concat, u64, bool, ValueQuery>;
    ...
    // F13: Entity 级多级佣金统计
    #[pallet::storage]
    pub type EntityMultiLevelStats<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, EntityStatsData, ValueQuery>;
    ...
    #[pallet::storage]
    pub type ConfigChangeLogs<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, u64, Blake2_128Concat, u32, ConfigChangeEntry<T>>;

    // F1: 待生效配置
    #[pallet::storage]
    pub type PendingConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, PendingConfigEntry<T>>;

    // R10-#14: 待生效配置自动应用队列（entity_id 列表，供 on_initialize 遍历）
    #[pallet::storage]
    pub type PendingConfigQueue<T: Config> =
        StorageValue<_, BoundedVec<u64, ConstU32<100>>, ValueQuery>;

    /// 会员的多级分佣记录 (FIFO, 最多保留最近 MaxPayoutRecords 条)
    #[pallet::storage]
    pub type MemberMultiLevelPayouts<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat,
        u64,
        Blake2_128Concat,
        T::AccountId,
        BoundedVec<MultiLevelPayoutRecord<T::AccountId>, T::MaxPayoutRecords>,
        ValueQuery,
    >;

    /// 会员多级分佣汇总统计
    #[pallet::storage]
    pub type MemberMultiLevelSummaryStats<T: Config> = StorageDoubleMap<
        _,
        Blake2_128Concat,
        u64,
        Blake2_128Concat,
        T::AccountId,
        MemberMultiLevelSummary,
        ValueQuery,
    >;
    ...
    pub enum Event<T: Config> {
        MultiLevelConfigUpdated {
            entity_id: u64,
        },
        MultiLevelConfigCleared {
            entity_id: u64,
        },
        /// 单层配置已更新
        TierUpdated {
        ...
        // F4: rates 总和警告（提示管理员注意，佣金可能被 plugin_budget_cap 截断）
        ...
        // F7: 详细配置变更事件
        ConfigDetailedChange {
            entity_id: u64,
            old_levels_count: u32,
            new_levels_count: u32,
        },
        // F1: 待生效配置已调度
        PendingConfigScheduled {
            entity_id: u64,
            effective_at: u32,
        },
        // F1: 待生效配置已应用
        ...
        // R10-#7: 佣金分发统计更新
        MultiLevelCommissionDistributed {
            entity_id: u64,
            total_amount: u128,
            beneficiary_count: u32,
        },
        // R10-#4: Entity 存储已清理
        EntityStorageCleaned {
            entity_id: u64,
        },
    }
    ...
    pub enum Error<T> {
        ...
        // R10-#14: 待生效配置队列已满
        PendingQueueFull,
        /// 只能从最高层（末尾）开始删除，不允许删除中间层
        CanOnlyRemoveLastTier,
        // R10-#6: 多级分销未暂停（force_resume 时）
        MultiLevelNotPaused,
        /// 各层级费率总和超过插件预算上限 (plugin_caps.multi_level_cap)
        RatesSumExceedsBudgetCap,
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_multi_level_config(
            ...
            Self::validate_config(entity_id, &levels)?;

            let old_summary = Self::read_old_config_summary(entity_id);
            let new_config = MultiLevelConfig { levels };
            Self::do_insert_config(
                entity_id,
                new_config,
                &who,
                ConfigChangeType::SetConfig,
                old_summary,
            );
            Ok(())
        }
        ...
        pub fn clear_multi_level_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
                ...
                MultiLevelConfigs::<T>::contains_key(entity_id),
        ...
        pub fn force_set_multi_level_config(
            ...
            ensure_root(origin)?;
            // V6-FIX: 确保 entity 存在，避免为幽灵 entity 写入配置
            ensure!(
                T::EntityProvider::entity_owner(entity_id).is_some(),
        ...
        pub fn update_multi_level_params(
            ...
            Self::ensure_owner_or_admin(entity_id, &who)?;
                ...
                tier_index.is_some() && tier_update.is_some(),
            ...
            MultiLevelConfigs::<T>::try_mutate(entity_id, |maybe_config| -> DispatchResult {
                ...
                Self::ensure_within_budget_cap(entity_id, config)?;
        ...
        /// F4: 在末尾追加新层级（Owner/Admin）
        ///
        /// AUDIT-FIX(BUG-3): 仅允许追加到末尾（index == levels.len()），
        /// 与 remove_tier 的 CanOnlyRemoveLastTier 保护对称，
        /// 防止中间插入导致推荐链-层级映射错位。
        ...
        pub fn add_tier(
            ...
            MultiLevelConfigs::<T>::try_mutate(entity_id, |maybe_config| -> DispatchResult {
                ...
                // AUDIT-FIX(BUG-3): 仅允许末尾追加，中间插入会导致层级-推荐链错位
                ensure!(idx == config.levels.len(), Error::<T>::TierIndexOutOfBounds);
                ...
                Self::ensure_within_budget_cap(entity_id, config)?;
                Self::check_rates_sum_warning(entity_id, config);
                Self::deposit_event(Event::TierInserted {
                    entity_id,
                    tier_index: index,
                });
        ...
        ///
        /// 只允许删除最高层（末尾层），防止中间层删除导致推荐链错位。
        /// 移除后 levels 不可为空（至少保留 1 层，否则应使用 clear）。
        // R10-#2: 移除后触发 rates_sum 警告检查
        #[pallet::call_index(6)]
        #[pallet::weight(T::WeightInfo::remove_tier())]
        pub fn remove_tier(origin: OriginFor<T>, entity_id: u64, index: u32) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
            ...
            MultiLevelConfigs::<T>::try_mutate(entity_id, |maybe_config| -> DispatchResult {
                ...
                let idx = index as usize;
                    ...
                    idx == config.levels.len() - 1,
                    Error::<T>::CanOnlyRemoveLastTier
                );
                ...
                Self::check_rates_sum_warning(entity_id, config);
                Self::deposit_event(Event::TierRemoved {
                    entity_id,
                    tier_index: index,
                });
        ...
        pub fn pause_multi_level(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            // 审计修复: 锁定后 Owner 不可暂停多级分销
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
                Error::<T>::EntityLocked
            );
        ...
        pub fn resume_multi_level(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            // 审计修复: 锁定后 Owner 不可恢复多级分销
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
                Error::<T>::EntityLocked
            );
        ...
        pub fn schedule_config_change(
            ...
            Self::ensure_owner_or_admin(entity_id, &who)?;
                ...
                !PendingConfigs::<T>::contains_key(entity_id),
            ...
            PendingConfigs::<T>::insert(
                entity_id,
                PendingConfigEntry {
                    config: MultiLevelConfig { levels },
                    effective_at,
            ...
            PendingConfigQueue::<T>::try_mutate(|q| -> DispatchResult {
                q.try_push(entity_id)
                    .map_err(|_| Error::<T>::PendingQueueFull)?;
                Ok(())
            })?;

            Self::deposit_event(Event::PendingConfigScheduled {
                entity_id,
                effective_at,
            });
        ...
        /// F1: 应用待生效的配置变更（Owner/Admin，必须到达生效区块且实体未锁定）
        ///
        /// V1-FIX: 添加 Owner/Admin 权限检查（防止任意用户 front-run 触发配置生效）。
        /// 审计日志记录原调度者 `scheduled_by`，而非当前执行者。
        ...
        pub fn apply_pending_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ...
            let current_block: u32 = <frame_system::Pallet<T>>::block_number()
                ...
                current_block >= pending.effective_at,
            ...
            // V1-FIX: 记录原调度者，而非执行者
            Self::record_change_log(
                entity_id,
                &pending.scheduled_by,
                ConfigChangeType::PendingApplied,
            );
            Ok(())
        }
        ...
        pub fn cancel_pending_config(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            // 审计修复: 锁定后 Owner 不可取消待生效配置（DAO 提案 schedule 的不应被 Owner 取消）
            ensure!(
                !T::EntityProvider::is_entity_locked(entity_id),
                Error::<T>::EntityLocked
            );
        ...
        pub fn force_pause_multi_level(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            // V6-FIX: 确保 entity 存在
            ensure!(
                T::EntityProvider::entity_owner(entity_id).is_some(),
        ...
        pub fn force_resume_multi_level(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            // V6-FIX: 确保 entity 存在
            ensure!(
                T::EntityProvider::entity_owner(entity_id).is_some(),
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn validate_tier(tier: &MultiLevelTier) -> DispatchResult {
                ...
                tier.required_team_size <= 1_000_000,
                Error::<T>::InvalidTeamSize
            );
            ensure!(
                tier.required_spent <= 1_000_000_000_000_000_000u128,
                Error::<T>::InvalidSpent
            );
            Ok(())
        }

        /// 校验 levels 参数合法性 + 费率总和不超过插件预算上限
        pub fn validate_config(
            ...
            // 校验 rates_sum ≤ multi_level_cap（0 = 无上限，跳过校验）
            let cap = T::BudgetCapProvider::multi_level_cap(entity_id);
            if cap > 0 {
                let rates_sum: u32 = levels.iter().map(|t| t.rate as u32).sum();
                ensure!(
                    rates_sum <= cap as u32,
                    Error::<T>::RatesSumExceedsBudgetCap
                );
            }
            Ok(())
        }

        // R2/R3-FIX: 读取旧配置摘要 (levels_count)
        pub fn read_old_config_summary(entity_id: u64) -> u32 {
        ...
        // R2/R3-FIX: 写入新配置 + 发出事件 + 审计日志（set_multi_level_config / force_set / PlanWriter / do_apply 共用）
        pub fn do_insert_config(
            entity_id: u64,
            new_config: MultiLevelConfigOf<T>,
            who: &T::AccountId,
            change_type: ConfigChangeType,
            old_levels_count: u32,
        ) {
            Self::check_rates_sum_warning(entity_id, &new_config);
            let new_levels_count = new_config.levels.len() as u32;
            ...
            Self::record_change_log(entity_id, who, change_type);
        }

        /// 多级分销佣金计算
        ///
        /// 逐层遍历推荐链，每层执行：
        /// 1. rate = 0 → 跳过（占位层），向上移动 referrer（注意：消耗推荐链位置）
        /// 2. 无推荐人 → 终止
        /// 3. 循环检测（BTreeSet）→ 命中则终止
        /// 4. 激活条件不满足 → 跳过该层，继续下一层（同样消耗推荐链位置）
        /// 5. 计算佣金 = order_amount × rate / 10000，取 min(commission, remaining)
        /// 6. remaining 耗尽时终止（预算由 CoreCommissionConfig.plugin_caps.multi_level_cap 控制）
            ...
            B: sp_runtime::traits::AtLeast32BitUnsigned + Copy,
        {
            ...
            for (level_idx, tier) in config.levels.iter().enumerate() {
                if tier.rate == 0 {
                    // AUDIT-FIX(BUG-4): 无 referrer 时提前终止，避免空转
                    let Some(ref r) = current_referrer else { break };
                    visited.insert(r.clone());
                    current_referrer = T::MemberProvider::get_referrer(entity_id, r);
                    continue;
                }

                let Some(ref referrer) = current_referrer else {
                    break;
                };
                ...
                // L1-FIX: 预取统计数据，避免 check_tier_activation 内部重复查询
                let needs_stats = tier.required_directs > 0 || tier.required_team_size > 0;
                let (directs, team_size) = if needs_stats {
                    let stats = T::MemberProvider::get_member_stats(entity_id, referrer);
                    (stats.direct_referrals, stats.team_size)
                } else {
                    (0, 0)
                };
                let spent_usdt: u128 = if tier.required_spent > 0 {
                    T::MemberProvider::get_member_upgrade_eligible_spent_usdt(entity_id, referrer)
                        .into()
                } else {
                    0
                };
                let level_id: u8 = if tier.required_level_id > 0 {
                    T::MemberProvider::custom_level_id(entity_id, referrer)
                } else {
                    0
                };

                if !Self::check_tier_activation_cached(
                    tier, directs, team_size, spent_usdt, level_id,
                ) {
                    current_referrer = T::MemberProvider::get_referrer(entity_id, referrer);
                    continue;
                }
                ...
                let commission =
                    order_amount.saturating_mul(B::from(tier.rate as u32)) / B::from(10000u32);
        ...
        /// 四维激活条件检查（AND 逻辑，值为 0 自动跳过）
        pub fn check_tier_activation(
            entity_id: u64,
            account: &T::AccountId,
            tier: &MultiLevelTier,
        ) -> bool {
            if tier.required_directs == 0
                && tier.required_team_size == 0
                && tier.required_spent == 0
                && tier.required_level_id == 0
            {
                return true;
            }
            if tier.required_directs > 0 || tier.required_team_size > 0 {
                let stats = T::MemberProvider::get_member_stats(entity_id, account);
                let direct_referrals = stats.direct_referrals;
                let team_size = stats.team_size;
            ...
            if tier.required_spent > 0 {
                let spent_usdt: u128 =
                    T::MemberProvider::get_member_upgrade_eligible_spent_usdt(entity_id, account)
                        .into();
                if spent_usdt < tier.required_spent {
                    return false;
                }
            }
            if tier.required_level_id > 0 {
                let level_id = T::MemberProvider::custom_level_id(entity_id, account);
                if level_id < tier.required_level_id {
                    return false;
                }
            }
            true
        ...
        // L1-FIX: 使用预取数据的激活条件检查（process_multi_level 内部使用，避免重复存储读取）
        fn check_tier_activation_cached(
            tier: &MultiLevelTier,
            directs: u32,
            team_size: u32,
            spent_usdt: u128,
            ...
            if tier.required_directs > 0 && directs < tier.required_directs {
                return false;
            }
            if tier.required_team_size > 0 && team_size < tier.required_team_size {
                return false;
            }
            if tier.required_spent > 0 && spent_usdt < tier.required_spent {
                return false;
            }
            if tier.required_level_id > 0 && level_id < tier.required_level_id {
                return false;
            }
            true
        }
        ...
        pub fn get_activation_status(
            ...
            config
                .levels
                .iter()
                .map(|tier| Self::check_tier_activation(entity_id, account, tier))
                .collect()
        }
        ...
        pub fn record_change_log(
            ...
            // V4-FIX: wrapping_add 替代 saturating_add，防止 u32::MAX 饱和后
            // 所有后续日志覆盖同一 slot（slot = u32::MAX % 1000 = 295）
            ConfigChangeLogCount::<T>::insert(entity_id, count.wrapping_add(1));
        }
        ...
        pub fn check_rates_sum_warning(entity_id: u64, config: &MultiLevelConfigOf<T>) {
            ...
            if rates_sum > 10000 {
                Self::deposit_event(Event::RatesSumWarning {
                    entity_id,
                    rates_sum,
                });
            }
        ...
        /// 校验修改后的 rates_sum 不超过 plugin_caps.multi_level_cap
        ...
        pub fn get_activation_progress(
            entity_id: u64,
            account: &T::AccountId,
        ) -> Vec<ActivationProgress> {
            let config = match MultiLevelConfigs::<T>::get(entity_id) {
                Some(c) => c,
                None => return Vec::new(),
            };
            ...
            let stats = T::MemberProvider::get_member_stats(entity_id, account);
            let directs = stats.direct_referrals;
            let team_size = stats.team_size;
            let spent_usdt: u128 =
                T::MemberProvider::get_member_upgrade_eligible_spent_usdt(entity_id, account)
                    .into();
            // BUG-2 修复: 查询会员自定义等级，纳入激活判定
            let level_id = T::MemberProvider::custom_level_id(entity_id, account);
            ...
            config
                ...
                .map(|(idx, tier)| {
                    let activated = {
                        let directs_ok =
                            tier.required_directs == 0 || directs >= tier.required_directs;
                        let team_ok =
                            tier.required_team_size == 0 || team_size >= tier.required_team_size;
                        let spent_ok =
                            tier.required_spent == 0 || spent_usdt >= tier.required_spent;
                        let level_ok =
                            tier.required_level_id == 0 || level_id >= tier.required_level_id;
                        directs_ok && team_ok && spent_ok && level_ok
                    };
                    ActivationProgress {
                        level: (idx as u16) + 1,
                        activated,
                        ...
                        level_id_required: tier.required_level_id,
                    }
                })
                .collect()
        }
        ...
        ///
        /// 使用 order_amount 作为 remaining（预览时无 plugin_budget_cap 约束）。
        pub fn preview_commission(
            entity_id: u64,
            buyer: &T::AccountId,
            order_amount: u128,
        ) -> Vec<(T::AccountId, u128, u16)>
        where
            T::AccountId: Ord,
        {
            ...
            outputs
                .into_iter()
                .map(|o| (o.beneficiary, o.amount, o.level))
                .collect()
        }
        ...
        // V2-FIX: 新增 count_order 参数，NEX 管道传 true，Token 管道传 false，
        //         防止同一订单 total_orders 被 NEX+Token 双管道重复计入。
        pub fn update_stats(
            entity_id: u64,
            outputs: &[CommissionOutput<T::AccountId, u128>],
            count_order: bool,
        ) {
            if outputs.is_empty() {
                return;
            ...
            for output in outputs.iter() {
                MemberMultiLevelStats::<T>::mutate(entity_id, &output.beneficiary, |stats| {
                    ...
                    if count_order {
                        stats.commission_receipt_count =
                            stats.commission_receipt_count.saturating_add(1);
                    }
            ...
            EntityMultiLevelStats::<T>::mutate(entity_id, |stats| {
                ...
                stats.total_distribution_entries = stats
                    .total_distribution_entries
        ...
        /// 记录分佣到会员历史 (FIFO + 汇总), 仅 NEX 管道调用
        pub(crate) fn record_payouts(
            entity_id: u64,
            buyer: &T::AccountId,
            order_id: u64,
            outputs: &[CommissionOutput<T::AccountId, u128>],
            ...
            for o in outputs.iter() {
                let record = MultiLevelPayoutRecord {
                    buyer: buyer.clone(),
                    order_id,
                    amount: o.amount,
                    level: o.level,
                ...
                // 追加到 BoundedVec (FIFO: 满了就移除最旧一条)
                MemberMultiLevelPayouts::<T>::mutate(entity_id, &o.beneficiary, |records| {
                    if records.is_full() {
                        records.remove(0);
                    }
                ...
                // 更新汇总
                MemberMultiLevelSummaryStats::<T>::mutate(entity_id, &o.beneficiary, |summary| {
                    summary.total_earned = summary.total_earned.saturating_add(o.amount);
                    summary.total_payout_count = summary.total_payout_count.saturating_add(1);
                    summary.last_payout_block = block;
                });
            }
        }
        ...
        // V4-FIX: 正确处理 count wrapping_add 回绕至 0 的场景
        pub fn get_recent_change_logs(entity_id: u64, limit: u32) -> Vec<ConfigChangeEntry<T>> {
            ...
            // count == 0 可能是"从未写入"或"wrapping_add 回绕"。
            // 最后写入的 slot = (count - 1) % MAX；检查该 slot 是否有数据来区分。
            if total_count == 0 {
                let last_slot = u32::MAX % Self::MAX_CONFIG_CHANGE_LOGS;
                if !ConfigChangeLogs::<T>::contains_key(entity_id, last_slot) {
                    return Vec::new();
                }
            }

            // 环形缓冲区中有效条目数：若 count < MAX 则为 count，否则为 MAX
            // count == 0 且回绕时，缓冲区一定是满的（已写过 u32::MAX+1 条）
            let effective_count = if total_count == 0 {
                Self::MAX_CONFIG_CHANGE_LOGS
            } else {
                total_count.min(Self::MAX_CONFIG_CHANGE_LOGS)
            };
            let fetch_count = limit.min(effective_count);
            let mut result = Vec::with_capacity(fetch_count as usize);

            for i in 0..fetch_count {
                // 使用 wrapping_sub 与 record_change_log 的 wrapping_add 对称
                let idx = total_count.wrapping_sub(1).wrapping_sub(i);
        ...
        pub fn cleanup_entity_storage(entity_id: u64) {
            ...
            let _ = MemberMultiLevelPayouts::<T>::clear_prefix(entity_id, u32::MAX, None);
            let _ = MemberMultiLevelSummaryStats::<T>::clear_prefix(entity_id, u32::MAX, None);
            Self::remove_from_pending_queue(entity_id);
        }
        ...
        fn do_apply_pending(entity_id: u64, pending: &PendingConfigEntry<T>) -> DispatchResult {
            let old_levels_count = Self::read_old_config_summary(entity_id);
        ...
        // AUDIT-FIX(漏洞-9): 跳过未就绪的 entity 继续检查后面的，
        // 防止队列头部被 locked/paused 的 entity 卡死。
        // MAX_AUTO_APPLY 限制成功应用数，MAX_CHECK_PER_BLOCK 限制总检查数。
        fn auto_apply_pending_configs(n: BlockNumberFor<T>) -> Weight {
            const MAX_AUTO_APPLY: u32 = 5;
            const MAX_CHECK_PER_BLOCK: u32 = 20;
            ...
            for &entity_id in queue.iter() {
                ...
                if checked >= MAX_CHECK_PER_BLOCK {
                    break;
                }
                checked += 1;

                if let Some(pending) = PendingConfigs::<T>::get(entity_id) {
                    // V3-FIX: 暂停状态下不自动应用配置，保留在队列中等待恢复后再应用
                    // AUDIT-FIX(漏洞-9): 未就绪时跳过（不 break），继续检查下一个
                        ...
                        && !GlobalPaused::<T>::get(entity_id)
                    {
                        if Self::do_apply_pending(entity_id, &pending).is_ok() {
                            ...
                            Self::record_change_log(
                                entity_id,
                                &auto_who,
                                ConfigChangeType::PendingAutoApplied,
                            );
            ...
            let write_overhead = if applied_ids.is_empty() {
                0u64
            } else {
                10_000_000u64
            };
...
// ============================================================================
// R1-FIX: 泛型 do_calculate — NEX / Token 共用核心逻辑
// ============================================================================

impl<T: pallet::Config> pallet::Pallet<T> {
    /// 多级分销插件核心计算（NEX / Token 共用）
    ///
    /// `count_order`: NEX 管道传 true（计入订单数），Token 管道传 false（仅累加金额）。
    fn do_calculate<B>(
        entity_id: u64,
        buyer: &T::AccountId,
        order_amount: B,
        ...
        T::AccountId: Ord,
    {
        ...
        // BUG-1 修复: 在插件内部调用 update_stats
        // V2-FIX: count_order 控制是否计入订单数
        if !outputs.is_empty() {
            let stats_outputs: alloc::vec::Vec<
                pallet_commission_common::CommissionOutput<T::AccountId, u128>,
            > = outputs
                .iter()
                .map(|o| pallet_commission_common::CommissionOutput {
                    beneficiary: o.beneficiary.clone(),
                    amount: o.amount.try_into().unwrap_or(u128::MAX),
                    commission_type: o.commission_type,
                    level: o.level,
                })
                .collect();
            Self::update_stats(entity_id, &stats_outputs, count_order);

            // 记录分佣历史 (仅 NEX 管道)
            if count_order {
                Self::record_payouts(entity_id, buyer, order_id, &stats_outputs);
            }
        }
...
impl<T: pallet::Config, B> pallet_commission_common::CommissionPlugin<T::AccountId, B>
    for pallet::Pallet<T>
where
    B: sp_runtime::traits::AtLeast32BitUnsigned + Copy + Default + core::fmt::Debug,
    T::AccountId: Ord,
...
impl<T: pallet::Config> pallet_commission_common::MultiLevelPlanWriter for pallet::Pallet<T> {
    fn set_multi_level(
        ...
        let bounded: frame_support::BoundedVec<pallet::MultiLevelTier, T::MaxMultiLevels> =
            level_rates
                .into_iter()
                .map(|rate| pallet::MultiLevelTier {
                    rate,
                    required_directs: 0,
        ...
        Self::validate_config(entity_id, &bounded)?;

        let old_levels_count = Self::read_old_config_summary(entity_id);
        let new_config = pallet::MultiLevelConfig { levels: bounded };
        let gov_who = T::EntityProvider::entity_account(entity_id);
        Self::do_insert_config(
            entity_id,
            new_config,
            &gov_who,
            pallet::ConfigChangeType::GovernanceSet,
            old_levels_count,
        );
        Ok(())
    }
    ...
    fn set_multi_level_full(
        entity_id: u64,
        tiers: alloc::vec::Vec<(u16, u32, u32, u128, u8)>,
    ) -> Result<(), sp_runtime::DispatchError> {
        frame_support::ensure!(
            !T::EntityProvider::is_entity_locked(entity_id),
            sp_runtime::DispatchError::Other("EntityLocked")
    ...
    fn clear_multi_level_config(entity_id: u64) -> Result<(), sp_runtime::DispatchError> {
        ...
        if pallet::MultiLevelConfigs::<T>::contains_key(entity_id) {
            ...
            Self::record_change_log(
                entity_id,
                &gov_who,
                pallet::ConfigChangeType::GovernanceClear,
            );
        }
        Ok(())
    }
    ...
    T::AccountId: Ord,
{
    fn activation_progress(
        entity_id: u64,
        account: &T::AccountId,
    ) -> alloc::vec::Vec<pallet_commission_common::MultiLevelActivationInfo> {
        Self::get_activation_progress(entity_id, account)
            ...
            .map(|p| pallet_commission_common::MultiLevelActivationInfo {
                ...
                level_id_current: p.level_id_current,
                level_id_required: p.level_id_required,
            })
            .collect()
    }
    ...
    fn member_stats(
        entity_id: u64,
        account: &T::AccountId,
    ) -> Option<pallet_commission_common::MultiLevelMemberStats> {
        let stats = MemberMultiLevelStats::<T>::get(entity_id, account);
        if stats.total_earned == 0 && stats.commission_receipt_count == 0 {
            return None;
        }
        Some(pallet_commission_common::MultiLevelMemberStats {
            total_earned: stats.total_earned,
            total_orders: stats.commission_receipt_count,
...
// ============================================================================
// PluginStatsRollback — 订单取消时回滚 MultiLevel 内部统计
// ============================================================================

impl<T: pallet::Config> pallet_commission_common::PluginStatsRollback<T::AccountId>
    for pallet::Pallet<T>
{
    fn rollback_stats(
        entity_id: u64,
        cancelled_outputs: &[(
            T::AccountId,
            u128,
            pallet_commission_common::CommissionType,
            u16,
        )],
        count_order: bool,
    ) {
        ...
        // 仅处理属于 MultiLevel 的记录
        let ml_outputs: alloc::vec::Vec<&(T::AccountId, u128, CommissionType, u16)> =
            cancelled_outputs
                .iter()
                .filter(|(_, _, ct, _)| *ct == CommissionType::MultiLevel)
                .collect();

        if ml_outputs.is_empty() {
            return;
        }
        ...
        let block: u32 = <frame_system::Pallet<T>>::block_number()
            .try_into()
            .unwrap_or(0u32);
        let mut total_rolled_back: u128 = 0;

        for (beneficiary, amount, _, _) in ml_outputs.iter() {
            pallet::MemberMultiLevelStats::<T>::mutate(entity_id, beneficiary, |stats| {
                stats.total_earned = stats.total_earned.saturating_sub(*amount);
                if count_order {
                    stats.commission_receipt_count =
                        stats.commission_receipt_count.saturating_sub(1);
                }
                stats.last_commission_block = block;
            });
            total_rolled_back = total_rolled_back.saturating_add(*amount);
        }

        pallet::EntityMultiLevelStats::<T>::mutate(entity_id, |stats| {
            stats.total_distributed = stats.total_distributed.saturating_sub(total_rolled_back);
            if count_order {
                stats.order_count = stats.order_count.saturating_sub(1);
            }
            stats.total_distribution_entries = stats
                .total_distribution_entries
                .saturating_sub(ml_outputs.len() as u32);
        });
    }
}

...
thread_local! {
    ...
    // 插件预算上限 (entity_id → multi_level_cap bps, 0 = 无上限)
...
pub fn ban_member(entity_id: u64, account: u64) {
    BANNED_MEMBERS.with(|b| {
        b.borrow_mut().insert((entity_id, account), true);
    });
}

pub fn set_entity_owner(entity_id: u64, owner: u64) {
    ENTITY_OWNERS.with(|o| {
        o.borrow_mut().insert(entity_id, owner);
    });
}

pub fn set_entity_admin(entity_id: u64, admin: u64, perm: u32) {
    ENTITY_ADMINS.with(|a| {
        a.borrow_mut().insert((entity_id, admin), perm);
    });
}

pub fn set_non_member(entity_id: u64, account: u64) {
    NON_MEMBERS.with(|n| {
        n.borrow_mut().insert((entity_id, account), true);
    });
}

pub fn set_entity_inactive(entity_id: u64) {
    INACTIVE_ENTITIES.with(|e| {
        e.borrow_mut().insert(entity_id, true);
    });
}

pub fn set_entity_locked(entity_id: u64) {
    ENTITY_LOCKED.with(|l| {
        l.borrow_mut().insert(entity_id, true);
    });
}

pub fn freeze_member(entity_id: u64, account: u64) {
    FROZEN_MEMBERS.with(|f| {
        f.borrow_mut().insert((entity_id, account), true);
    });
}

pub fn set_budget_cap(entity_id: u64, cap_bps: u16) {
    BUDGET_CAPS.with(|b| {
        b.borrow_mut().insert(entity_id, cap_bps);
    });
}
...
impl pallet_commission_common::MemberProvider<u64> for MockMemberProvider {
    ...
    fn member_count_by_level(_: u64, _: u8) -> u32 {
        0
    }
...
parameter_types! {
    pub const MaxMultiLevels: u32 = 1000;

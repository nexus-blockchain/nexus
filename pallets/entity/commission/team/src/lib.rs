...
//! 对每个上级查询其团队统计（team_size, spend stats），
...
pub mod pallet {
    ...
    pub enum SalesThresholdMode {
        ...
        TotalSpent = 0,
        /// 使用升级/奖励有效消费（USDT 累计，精度 10^6）
        UpgradeEligibleSpent = 1,
    }
    ...
    pub struct TeamPerformanceConfig<Balance, MaxTiers: Get<u32>> {
        ...
        /// 门槛数据源模式（TotalSpent=总消费, UpgradeEligibleSpent=升级/奖励有效消费）
    ...
    impl<Balance: Default, MaxTiers: Get<u32>> Default for TeamPerformanceConfig<Balance, MaxTiers> {
        fn default() -> Self {
            Self {
                ...
                threshold_mode: SalesThresholdMode::TotalSpent,
            }
        }
    }
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        #[cfg(feature = "std")]
        fn integrity_test() {
            assert!(T::MaxTeamTiers::get() > 0, "MaxTeamTiers must be > 0");
    ...
    pub type TeamPerformanceConfigs<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, TeamPerformanceConfigOf<T>>;

    /// F2: 团队业绩返佣启用状态（配置存在时默认 true）
    #[pallet::storage]
    pub type TeamPerformanceEnabled<T: Config> =
    ...
    pub enum Event<T: Config> {
        ...
        TeamPerformanceConfigCleared {
            entity_id: u64,
        },
        /// F2: 团队业绩返佣已暂停
        TeamPerformancePaused {
            entity_id: u64,
        },
        /// F2: 团队业绩返佣已恢复
        TeamPerformanceResumed {
            entity_id: u64,
        },
        /// F3: 档位已添加
        TeamTierAdded {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn set_team_performance_config(
            ...
            Self::ensure_rates_within_cap(entity_id, &tiers)?;
        ...
        pub fn clear_team_performance_config(
            ...
            Self::ensure_owner_or_admin(entity_id, &who)?;
                ...
                TeamPerformanceConfigs::<T>::contains_key(entity_id),
        ...
        pub fn update_team_performance_params(
            ...
            let (tier_count, depth_val, stacking_val, mode_val) =
                TeamPerformanceConfigs::<T>::try_mutate(
                    entity_id,
                    |maybe| -> Result<(u32, u8, bool, SalesThresholdMode), DispatchError> {
                        ...
                        Ok((
                            config.tiers.len() as u32,
                            config.max_depth,
                            config.allow_stacking,
        ...
        pub fn pause_team_performance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            ensure!(
                T::EntityProvider::is_entity_active(entity_id),
                ...
                TeamPerformanceEnabled::<T>::get(entity_id),
                Error::<T>::TeamPerformanceIsPaused
            );
        ...
        pub fn resume_team_performance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            Self::ensure_owner_or_admin(entity_id, &who)?;
            ensure!(
                T::EntityProvider::is_entity_active(entity_id),
                ...
                !TeamPerformanceEnabled::<T>::get(entity_id),
                Error::<T>::TeamPerformanceNotPaused
            );
        ...
        pub fn add_tier(
            ...
            // 校验新档位费率不超过插件预算上限
            let cap = T::BudgetCapProvider::team_cap(entity_id);
            if cap > 0 {
                ensure!(tier.rate <= cap, Error::<T>::MaxRateExceedsBudgetCap);
            }
            ...
            TeamPerformanceConfigs::<T>::try_mutate(entity_id, |maybe| -> DispatchResult {
                ...
                let insert_pos = config
                    .tiers
                    .iter()
                    .position(|t| t.sales_threshold >= tier.sales_threshold);
                if let Some(pos) = insert_pos {
                    ensure!(
                        config.tiers[pos].sales_threshold != tier.sales_threshold,
                        Error::<T>::TiersNotAscending
                    );
                    config
                        .tiers
                        .try_insert(pos, tier)
                        .map_err(|_| Error::<T>::TierLimitReached)?;
                    Self::deposit_event(Event::TeamTierAdded {
                        entity_id,
                        tier_index: pos as u32,
                    });
                } else {
                    config
                        .tiers
                    ...
                    Self::deposit_event(Event::TeamTierAdded {
                        entity_id,
                        tier_index: (config.tiers.len() - 1) as u32,
                    });
                }
                Ok(())
            })?;
        ...
        pub fn update_tier(
            ...
            TeamPerformanceConfigs::<T>::try_mutate(entity_id, |maybe| -> DispatchResult {
                ...
                if let Some(r) = rate {
                    ...
                    let cap = T::BudgetCapProvider::team_cap(entity_id);
                    if cap > 0 {
                        ensure!(r <= cap, Error::<T>::MaxRateExceedsBudgetCap);
                    }
            ...
            Self::deposit_event(Event::TeamTierUpdated {
                entity_id,
                tier_index,
            });
            Ok(())
        ...
        pub fn remove_tier(
            ...
            Self::deposit_event(Event::TeamTierRemoved {
                entity_id,
                tier_index,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 校验最大档位费率不超过插件预算上限（cap=0 表示无上限，跳过校验）
        fn ensure_rates_within_cap(
            entity_id: u64,
            tiers: &[TeamPerformanceTier<BalanceOf<T>>],
        ) -> DispatchResult {
            let cap = T::BudgetCapProvider::team_cap(entity_id);
            if cap > 0 {
                let max_rate = tiers.iter().map(|t| t.rate).max().unwrap_or(0);
                ensure!(max_rate <= cap, Error::<T>::MaxRateExceedsBudgetCap);
            }
            Ok(())
        }
        ...
        pub fn get_matched_tier_for_account(
            ...
            let threshold_spent = match config.threshold_mode {
                SalesThresholdMode::TotalSpent => stats.spend.total_spent,
                SalesThresholdMode::UpgradeEligibleSpent => stats.spend.upgrade_eligible_spent,
            };

            let (tier_idx, rate) =
                Self::match_tier_with_index(&config.tiers, stats.team_size, threshold_spent)?;
            let next_idx = (tier_idx as usize) + 1;
            let (next_threshold, next_team_size) = if next_idx < config.tiers.len() {
                (
                    Some(config.tiers[next_idx].sales_threshold),
                    Some(config.tiers[next_idx].min_team_size),
                )
            } else {
                (None, None)
            };
            ...
            T::AccountId: Ord,
        {
            ...
            while let Some(ref ancestor) = current {
                ...
                // 查询团队统计：(direct_referrals, team_size, spend stats)
                let stats = T::MemberProvider::get_member_stats(entity_id, ancestor);
                let threshold_spent = match config.threshold_mode {
                    SalesThresholdMode::TotalSpent => stats.spend.total_spent,
                    SalesThresholdMode::UpgradeEligibleSpent => stats.spend.upgrade_eligible_spent,
                };

                if let Some((tier_idx, rate)) =
                    Self::match_tier_with_index(&config.tiers, stats.team_size, threshold_spent)
                {
                    if rate > 0 {
                        let commission = order_amount.saturating_mul(rate.into()) / 10000u32.into();
...
impl<T: pallet::Config> pallet::Pallet<T> {
    ...
    ///
    /// 阶梯匹配逻辑与 NEX 版完全一致（基于 MemberProvider 的 team_size / spend stats）。
    /// 仅佣金金额计算使用泛型 TB。
        ...
        T::AccountId: Ord,
    {
        ...
        while let Some(ref ancestor) = current {
            ...
            let stats = T::MemberProvider::get_member_stats(entity_id, ancestor);
            let threshold_spent = match config.threshold_mode {
                pallet::SalesThresholdMode::TotalSpent => stats.spend.total_spent,
                pallet::SalesThresholdMode::UpgradeEligibleSpent => {
                    stats.spend.upgrade_eligible_spent
                }
            };
                ...
                Self::match_tier_with_index(&config.tiers, stats.team_size, threshold_spent)
            {
                if rate > 0 {
                    let commission =
                        order_amount.saturating_mul(TB::from(rate as u32)) / TB::from(10000u32);
                    let actual = commission.min(*remaining);
                    ...
                    if !actual.is_zero() {
                        *remaining = remaining.saturating_sub(actual);
                        outputs.push(pallet_commission_common::CommissionOutput {
                            ...
                            amount: actual,
                            commission_type:
                                pallet_commission_common::CommissionType::TeamPerformance,
                            level: depth as u16,
                        });
    ...
    for pallet::Pallet<T>
{
    fn set_team_config(
        ...
        frame_support::ensure!(
            !tiers.is_empty(),
            sp_runtime::DispatchError::Other("EmptyTiers")
        );
        frame_support::ensure!(
            max_depth > 0 && max_depth <= 30,
        ...
        // 预算上限校验
        let cap =
            <T::BudgetCapProvider as pallet_commission_common::PluginBudgetCapProvider>::team_cap(
                entity_id,
            );
        if cap > 0 {
            let max_rate = tiers.iter().map(|t| t.2).max().unwrap_or(0);
            frame_support::ensure!(
                max_rate <= cap,
                sp_runtime::DispatchError::Other("MaxRateExceedsBudgetCap")
            );
        }
        // 校验阶梯门槛严格递增
        for window in tiers.windows(2) {
        ...
        > = tiers
            .into_iter()
            .map(
                |(threshold, min_team_size, rate)| pallet::TeamPerformanceTier {
        ...
        let mode = match threshold_mode {
            0 => pallet::SalesThresholdMode::TotalSpent,
            1 => pallet::SalesThresholdMode::UpgradeEligibleSpent,
    ...
    fn clear_config(entity_id: u64) -> Result<(), sp_runtime::DispatchError> {
        if pallet::TeamPerformanceConfigs::<T>::contains_key(entity_id) {
            ...
            pallet::Pallet::<T>::deposit_event(pallet::Event::TeamPerformanceConfigCleared {
                entity_id,
            });
        }
        Ok(())
...
impl<T: Config> pallet_commission_common::TeamQueryProvider<T::AccountId, BalanceOf<T>>
    for Pallet<T>
where
    BalanceOf<T>: Into<u128>,
{
    fn matched_tier(
        entity_id: u64,
        account: &T::AccountId,
    ) -> Option<pallet_commission_common::TeamTierInfo<BalanceOf<T>>> {
    ...
    fn chain_depth(entity_id: u64) -> u16 {
        TeamPerformanceConfigs::<T>::get(entity_id)
            .map(|c| c.max_depth as u16)
            .unwrap_or(0)
    }
}
...
mod tests {
    ...
    impl pallet_commission_common::MemberProvider<u64> for MockMemberProvider {
        ...
        fn get_member_stats(entity_id: u64, account: &u64) -> pallet_entity_common::MemberStats {
            MEMBER_STATS.with(|s| {
    ...
    fn set_config_rejects_invalid_rate() {
        new_test_ext().execute_with(|| {
            set_entity_owner(1, 100);
            let tiers = vec![pallet::TeamPerformanceTier {
                sales_threshold: 1000,
                min_team_size: 0,
                rate: 10001,
            }];
    ...
    fn stacking_mode_rewards_multiple() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::force_set_team_performance_config(
                ...
                10,
                true,
                pallet::SalesThresholdMode::TotalSpent, // allow_stacking
            ));
    ...
    fn max_depth_limits_traversal() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::force_set_team_performance_config(
                ...
                2,
                false,
                pallet::SalesThresholdMode::TotalSpent, // max_depth=2
            ));
    ...
    fn plan_writer_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(
                <pallet::Pallet<Test> as TeamPlanWriter<Balance>>::set_team_config(
                    1,
        ...
        new_test_ext().execute_with(|| {
            ...
            System::assert_has_event(RuntimeEvent::CommissionTeam(
                pallet::Event::TeamPerformanceConfigUpdated {
                    ...
                    max_depth: 5,
                    allow_stacking: false,
                    threshold_mode: pallet::SalesThresholdMode::TotalSpent,
                },
            ));
    ...
    fn force_clear_config_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::force_clear_team_performance_config(
                RuntimeOrigin::root(),
                1
            ));
            assert!(pallet::TeamPerformanceConfigs::<Test>::get(1).is_none());
    ...
    fn update_params_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::update_team_performance_params(
                ...
                None,
                Some(true),
                Some(pallet::SalesThresholdMode::UpgradeEligibleSpent),
            ));
            let config = pallet::TeamPerformanceConfigs::<Test>::get(1).unwrap();
            assert_eq!(config.max_depth, 15);
            assert!(config.allow_stacking);
        ...
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                System::events()
                    ...
                    .filter(|e| matches!(
                        e.event,
                        RuntimeEvent::CommissionTeam(
                            pallet::Event::TeamPerformanceConfigCleared { .. }
                        )
                    ))
                    .count(),
                0
    ...
    fn update_params_emits_updated_event() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::set_team_performance_config(
                RuntimeOrigin::signed(100), 1, tiers.try_into().unwrap(), 5, false, pallet::SalesThresholdMode::TotalSpent,
            ));
            assert_ok!(CommissionTeam::update_team_performance_params(
    ...
    fn f2_pause_and_resume_works() {
        new_test_ext().execute_with(|| {
            ...
            // 暂停
            assert_ok!(CommissionTeam::pause_team_performance(
                RuntimeOrigin::signed(100),
                1
            ));
            assert!(!pallet::TeamPerformanceEnabled::<Test>::get(1));
            ...
            // 恢复
            assert_ok!(CommissionTeam::resume_team_performance(
                RuntimeOrigin::signed(100),
                1
            ));
            assert!(pallet::TeamPerformanceEnabled::<Test>::get(1));
    ...
    fn f3_add_tier_works() {
        new_test_ext().execute_with(|| {
            ...
            // 插入中间档位
            let new_tier = pallet::TeamPerformanceTier {
                sales_threshold: 3000,
                min_team_size: 10,
                rate: 200,
            };
            ...
            System::assert_has_event(RuntimeEvent::CommissionTeam(pallet::Event::TeamTierAdded {
                entity_id: 1,
                tier_index: 1,
            }));
        });
    ...
    fn f3_add_tier_rejects_duplicate_threshold() {
        new_test_ext().execute_with(|| {
            ...
            let dup_tier = pallet::TeamPerformanceTier {
                sales_threshold: 1000,
                min_team_size: 10,
                rate: 200,
            };
    ...
    fn f3_remove_tier_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_ok!(CommissionTeam::remove_tier(
                RuntimeOrigin::signed(100),
                1,
                0
            ));
        ...
        new_test_ext().execute_with(|| {
            ...
            System::assert_has_event(RuntimeEvent::CommissionTeam(
                pallet::Event::TeamPerformanceConfigUpdated {
                    ...
                    max_depth: 5,
                    allow_stacking: true,
                    threshold_mode: pallet::SalesThresholdMode::UpgradeEligibleSpent,
                },
            ));
        });
    }

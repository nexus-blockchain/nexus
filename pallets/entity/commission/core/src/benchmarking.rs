...
use pallet_commission_common::{
    CommissionModes, CommissionStatus, WithdrawalMode, WithdrawalTierConfig,
};
...
fn setup_entity<T: Config>(eid: u64, caller: &T::AccountId) {
    ...
    WithdrawalConfigs::<T>::insert(
        eid,
        EntityWithdrawalConfig::<T::MaxCustomLevels> {
...
mod benchmarks {
    ...
    fn withdraw_commission() {
        ...
        #[extrinsic_call]
        _(RawOrigin::Signed(c), 1u64, None, None);
    }
    ...
    #[benchmark]
    fn set_referrer_payout_disabled() {
        let c: T::AccountId = whitelisted_caller();
        setup_entity::<T>(1, &c);
        #[extrinsic_call]
        _(RawOrigin::Signed(c), 1u64, true);

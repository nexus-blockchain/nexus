...
pub struct CoreCommissionConfig {
    ...
    pub owner_reward_rate: u16,        // 创建人收益比例（基点，上限 5000）
...
pub enum CommissionType {
    ...
    SingleLineDownline, EntityReferral, PoolReward, OwnerReward,
}
```
...
| 14 | `set_owner_reward_rate` | Owner/Admin | 设置创建人收益比例（上限 5000 bps）；R8: None+Locked 仅允许降低 |
...
| `set_owner_reward_rate` | 设置创建人收益比例（<= 5000） |
...
返回 Entity 账户中已承诺的佣金资金总额，供 Registry/Shop/Loyalty 在转出资金时保护：
...
| R8 | `set_owner_reward_rate` -- 无币实体（None+Locked）单调递减豁免 |

...
| core | `pallet-commission-core` | 核心调度引擎：配置管理、`process_commission` / `process_token_commission` 分发、提现系统（NEX + Token 独立）、偿付安全、创建人收益 |
...
订单完成
│
├── NEX 管线 (process_commission)
│   ├── 池 A: 平台费 (platform_fee)
│   │   ├── 招商推荐人 → platform_fee × ReferrerShareBps
│   │   └── 国库 → 剩余部分
│   │
│   └── 池 B: available_pool (= 卖家货款 × max_commission_rate)
│       ├── 创建人收益 → available_pool × creator_reward_rate （CREATOR_REWARD 启用时）
...
│
└── Token 管线 (process_token_commission)
...
购物余额仅可用于订单抵扣（`use_shopping_balance`），不可直接提取为 NEX。
...
| 子模式 | 配置 | 计算方式 |
|--------|------|----------|
| DirectReward | `rate: u16` | `order_amount × rate / 10000` |
...
- 每笔只取差额：`(当前节点等级率 - 已达到的最高率) × order_amount / 10000`
...
| **配置校验** | `integrity_test` 在运行时启动时校验常量合法性（如 `MaxMultiLevels ∈ [1, 100]`） |

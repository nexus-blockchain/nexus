//! # Entity review pallet (pallet-entity-review)
//!
//! ## Overview
//!
//! This pallet manages order reviews, including:
//! 本模块负责订单评价管理，包括：
//! - submitting reviews after order completion
//! - 订单完成后提交评价
...
pub mod pallet {
    ...
    use pallet_entity_common::{AdminPermission, EntityProvider, OrderProvider, ShopProvider};
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            ...
            assert!(
                T::MaxReviewsPerUser::get() > 0,
                "MaxReviewsPerUser must be > 0"
            );
            assert!(
                T::MaxProductReviews::get() > 0,
                "MaxProductReviews must be > 0"
            );
        }
    }
    ...
    pub type UserReviews<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        T::AccountId,
        BoundedVec<u64, T::MaxReviewsPerUser>,
        ValueQuery,
    >;
    ...
    pub type EntityReviewDisabled<T: Config> =
    ...
    pub type ProductReviews<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BoundedVec<u64, T::MaxProductReviews>, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 店铺评分更新失败（评价仍已记录）
        ShopRatingUpdateFailed { order_id: u64, shop_id: u64 },
        /// Entity 评价配置已更新
        ReviewConfigUpdated { entity_id: u64, enabled: bool },
        /// M4: 评价已被 Root 移除
        ReviewRemoved {
            order_id: u64,
            reviewer: T::AccountId,
        ...
        /// F3: 商品评价已索引
        ProductReviewIndexed { product_id: u64, order_id: u64 },
        /// F6: 评价已修改
        ReviewEdited {
            order_id: u64,
            reviewer: T::AccountId,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_review(
            ...
            let buyer = T::OrderProvider::order_buyer(order_id).ok_or(Error::<T>::OrderNotFound)?;
                ...
                T::OrderProvider::is_order_completed(order_id),
                Error::<T>::OrderNotCompleted
            );
            ensure!(
                !T::OrderProvider::is_order_disputed(order_id),
                Error::<T>::OrderDisputed
            );
            ensure!(
            ...
            if let Some(eid) = entity_id {
                    ...
                    !EntityReviewDisabled::<T>::contains_key(eid),
                    Error::<T>::ReviewsDisabledForEntity
                );
            }
            ...
            UserReviews::<T>::try_mutate(&who, |reviews| {
                reviews
                    .try_push(order_id)
                    .map_err(|_| Error::<T>::UserReviewLimitReached)
            })?;
            ...
            if let Some(pid) = product_id {
                let indexed = ProductReviews::<T>::try_mutate(pid, |reviews| {
                    reviews
                        .try_push(order_id)
                        .map_err(|_| Error::<T>::ProductReviewsFull)
                });
                if indexed.is_ok() {
                    ...
                    log::warn!(
                        "ProductReviews full for product {}, order {} not indexed",
                        pid,
                        order_id
                    );
                }
        ...
        pub fn remove_review(origin: OriginFor<T>, order_id: u64) -> DispatchResult {
            ensure_root(origin)?;

            let review = Reviews::<T>::take(order_id).ok_or(Error::<T>::ReviewNotFound)?;
        ...
        pub fn reply_to_review(
            ...
            // 不能重复回复
                ...
                !ReviewReplies::<T>::contains_key(order_id),
                Error::<T>::AlreadyReplied
            );
            ...
            let shop_id =
                T::OrderProvider::order_shop_id(order_id).ok_or(Error::<T>::NotShopEntityAdmin)?;
            let entity_id =
                T::ShopProvider::shop_entity_id(shop_id).ok_or(Error::<T>::NotShopEntityAdmin)?;
        ...
        pub fn edit_review(
            ...
            // 评价必须存在
            let mut review = Reviews::<T>::get(order_id).ok_or(Error::<T>::ReviewNotFound)?;
            ...
            if edit_window > 0 {
                let created_u64: u64 = review.created_at.try_into().unwrap_or(u64::MAX);
                let now_u64: u64 = <frame_system::Pallet<T>>::block_number()
                    .try_into()
                    .unwrap_or(u64::MAX);
                ensure!(
            ...
            if old_rating != new_rating {
                ...
                if let Some(sid) = shop_id {
                    if let Err(e) =
                        T::ShopProvider::revert_shop_rating(sid, old_rating, Some(new_rating))
                    {
                        log::warn!(
                            "revert_shop_rating failed for shop {} order {}: {:?}",
                            sid,

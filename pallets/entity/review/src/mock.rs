...
use pallet_entity_common::{
    EffectiveShopStatus, EntityProvider, EntityStatus, OrderProvider, ShopOperatingStatus,
    ShopProvider, ShopType,
};
...
impl EntityProvider<u64> for MockEntityProvider {
    ...
    fn entity_status(entity_id: u64) -> Option<EntityStatus> {
        ENTITIES.with(|e| {
            e.borrow().get(&entity_id).map(|(_, active)| {
                if *active {
                    EntityStatus::Active
                } else {

...
| # | 调用 | Weight | 权限 | 说明 |
|---|------|--------|------|------|
| 0 | `create_product(shop_id, name_cid, images_cid, detail_cid, price, stock, category)` | 250M / 12k | 店主 | 创建商品，price>0，name_cid 非空，从店铺账户扣押金（KeepAlive） |
| 1 | `update_product(product_id, name_cid?, images_cid?, detail_cid?, price?, stock?, category?)` | 150M / 8k | 店主 | 更新商品（所有字段可选，补货可恢复 SoldOut→OnSale，需 Shop 激活） |
...
| 4 | `delete_product(product_id)` | 200M / 10k | 店主 | 删除商品并退还押金（状态须为 Draft/OffShelf，押金记录必须存在） |
...
impl ProductProvider<AccountId, Balance> for Pallet<T> {
    ...
    fn deduct_stock(product_id: u64, quantity: u32) -> DispatchResult;   // 须 OnSale，同步统计
    fn restore_stock(product_id: u64, quantity: u32) -> DispatchResult;  // 拒绝 Draft，同步统计
...
| 操作 | 调用方 | 前置条件 |
|------|--------|----------|
| `create_product` | 店主（signed） | Shop 激活 + 商品数未满 + price > 0 + name_cid 非空 + CID 合法 + 运营资金充足 |
| `update_product` | 店主（signed） | price > 0（若更新）+ name_cid 非空（若更新）+ 补货需 Shop 激活 |
| `publish_product` | 店主（signed） | Shop 激活 + 状态为 Draft/OffShelf |
| `unpublish_product` | 店主（signed） | 状态为 OnSale/SoldOut |
| `delete_product` | 店主（signed） | 状态为 Draft/OffShelf + 押金记录存在 |
| `deduct_stock` | 系统（trait 调用） | 状态须为 OnSale（v0.4.0），由 `pallet-entity-transaction` 下单时调用 |
| `restore_stock` | 系统（trait 调用） | 状态不可为 Draft（v0.4.0），由 `pallet-entity-transaction` 取消/退款/超时时调用 |

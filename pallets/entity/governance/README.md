...
**version** `0.13.0` · **storage_version** `2` · **extrinsics** `16` · **tests** `248`
...
- **执行语义分层** — proposal 按 `OnChain / OffChainApproval / Unsupported` 分类；仅真实链上状态变更进入 `Executed`
...
Voting ─→ Passed ─→ Executed
     │         ├─→ ApprovedOffchain (DAO 已批准，等待链下执行)
...
- **OnChain**：提案执行后会产生可验证的链上状态变化，终态为 `Executed`
- **OffChainApproval**：提案仅代表 DAO 批准，需要链下人工或外部流程落实，终态为 `ApprovedOffchain`
- **Unsupported**：当前未实现或未接线的 proposal，创建阶段直接返回 `ProposalTypeNotSupported`
...
| `TokenBlacklistManage` | 黑名单添加/移除 | 链下审批（ApprovedOffchain） |
...
| `OwnerRewardRateChange` | 创建者分成比率 | 链上 `CommissionProvider::set_owner_reward_rate` |
...
| 类型 | 说明 | 执行方式 |
|------|------|---------|
| `ReferrerGuardChange` | 推荐人资格门槛 | 链上 `CommissionGovernance -> ReferralWriter::set_referrer_guard` |
| `CommissionCapChange` | 返佣上限（单笔+累计） | 链上 `CommissionGovernance -> ReferralWriter::set_commission_cap` |
...
| `KycProviderAuthorize` | 授权 KYC 提供者 | 链上 `KycGovernance::governance_authorize_kyc_provider` |
| `KycProviderDeauthorize` | 取消 KYC 提供者授权 | 链上 `KycGovernance::governance_deauthorize_kyc_provider` |
...
| `ShopPoliciesChange` | 店铺政策（退换货/物流） | 链上 `ShopGovernance::governance_set_shop_policies`（当前按 entity 批量更新其下所有 shop） |
...
| 方式 | 数量 | 说明 |
|------|------|------|
| 链上执行 | 53 | 通过 Provider / Governance Port / Writer trait 直接写入链上状态 |
| 链下审批 | 32 | 执行后进入 `ApprovedOffchain`，并发出 `ProposalExecutionNote` 供链下流程处理 |
| 当前不支持 | 0 | 首批治理面中已无被创建阶段拒绝的 proposal；后续新增未实现能力仍建议 fail-closed |
| 仅记录 | 3 | 社区类提案，不需要执行 |
...
| `ProposalExecuted` | `proposal_id` | 提案完成链上执行 |
| `ProposalApprovedOffchain` | `proposal_id` | 提案已批准，等待链下执行 |
...
| `ProposalExecutionNote` | `proposal_id, note` | 链下审批/执行备注 |

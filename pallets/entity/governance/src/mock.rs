...
use pallet_entity_common::{
    DisclosureLevel, DisclosureProvider, EntityProvider, EntityStatus, EntityTokenProvider,
    GovernanceMode, ProductCategory, ProductProvider, ShopProvider, ShopType, TokenType,
};
use sp_core::H256;
use sp_runtime::{
...
impl EntityProvider<u64> for MockEntityProvider {
    ...
    fn is_entity_active(entity_id: u64) -> bool {
        ENTITY_ACTIVE.with(|e| {
            *e.borrow()
                .get(&entity_id)
                .unwrap_or(&(entity_id == 1 || entity_id == 2))
        })
    }
...
thread_local! {
    ...
    // MockPricingProvider 状态
    static MOCK_NEX_PRICE: RefCell<u64> = RefCell::new(1_000_000); // 默认 1 USDT = 1 NEX
    static MOCK_PRICE_STALE: RefCell<bool> = RefCell::new(false);
    // MockTreasuryPort 状态
    static MOCK_TREASURY_BALANCE: RefCell<HashMap<u64, u128>> = RefCell::new(HashMap::new());
    // Mock KYC governance 状态
    static MOCK_KYC_PROVIDERS: RefCell<HashMap<u64, bool>> = RefCell::new(HashMap::new());
    static MOCK_KYC_AUTHORIZED: RefCell<HashMap<(u64, u64), bool>> = RefCell::new(HashMap::new());
    static MOCK_POOL_REWARD_CONFIGS: RefCell<HashMap<u64, (Vec<(u8, pallet_entity_common::PoolRewardLevelClaimRule)>, u32)>> = RefCell::new(HashMap::new());
    static MOCK_POOL_REWARD_TOKEN_ENABLED: RefCell<HashMap<u64, bool>> = RefCell::new(HashMap::new());
}
...
/// 设置 MockPricingProvider 的 NEX/USDT 价格
pub fn set_mock_nex_price(price: u64) {
    MOCK_NEX_PRICE.with(|p| *p.borrow_mut() = price);
}

/// 设置 MockPricingProvider 的价格过期状态
pub fn set_mock_price_stale(stale: bool) {
    MOCK_PRICE_STALE.with(|s| *s.borrow_mut() = stale);
}

/// 设置 MockTreasuryPort 的金库余额
pub fn set_mock_treasury_balance(entity_id: u64, balance: u128) {
    MOCK_TREASURY_BALANCE.with(|b| b.borrow_mut().insert(entity_id, balance));
}

/// 设置 MockKycGovernance 的 provider 注册状态
pub fn set_mock_kyc_provider(provider: u64, exists: bool) {
    MOCK_KYC_PROVIDERS.with(|p| {
        if exists {
            p.borrow_mut().insert(provider, true);
        } else {
            p.borrow_mut().remove(&provider);
        }
    });
}

/// 设置 MockKycGovernance 的 provider 授权状态
pub fn set_mock_kyc_authorized(entity_id: u64, provider: u64, authorized: bool) {
    MOCK_KYC_AUTHORIZED.with(|a| {
        if authorized {
            a.borrow_mut().insert((entity_id, provider), true);
        } else {
            a.borrow_mut().remove(&(entity_id, provider));
        }
    });
}

pub fn is_mock_kyc_authorized(entity_id: u64, provider: u64) -> bool {
    MOCK_KYC_AUTHORIZED.with(|a| *a.borrow().get(&(entity_id, provider)).unwrap_or(&false))
}

pub fn get_mock_pool_reward_config(
    entity_id: u64,
) -> Option<(
    Vec<(u8, pallet_entity_common::PoolRewardLevelClaimRule)>,
    u32,
)> {
    MOCK_POOL_REWARD_CONFIGS.with(|c| c.borrow().get(&entity_id).cloned())
}

pub fn get_mock_pool_reward_token_enabled(entity_id: u64) -> bool {
    MOCK_POOL_REWARD_TOKEN_ENABLED.with(|m| *m.borrow().get(&entity_id).unwrap_or(&false))
}
...
impl pallet_entity_common::PricingProvider for MockPricingProvider {
    fn get_nex_usdt_price() -> u64 {
        MOCK_NEX_PRICE.with(|p| *p.borrow())
    }
    fn is_price_stale() -> bool {
        MOCK_PRICE_STALE.with(|s| *s.borrow())
    }
}

// ==================== MockTreasuryPort ====================

pub struct MockTreasuryPort;
impl pallet_entity_common::EntityTreasuryPort for MockTreasuryPort {
    fn treasury_balance(entity_id: u64) -> u128 {
        MOCK_TREASURY_BALANCE.with(|b| *b.borrow().get(&entity_id).unwrap_or(&0))
    }
}
...
impl EntityTokenProvider<u64, u128> for MockTokenProvider {
    ...
    fn transfer(_: u64, _: &u64, _: &u64, _: u128) -> Result<(), DispatchError> {
        Ok(())
    }
    ...
    fn repatriate_reserved(_: u64, _: &u64, _: &u64, _: u128) -> Result<u128, DispatchError> {
        Ok(0)
    }
...
impl DisclosureProvider<u64> for MockDisclosureProvider {
    ...
    fn is_insider(_entity_id: u64, _account: &u64) -> bool {
        false
    }
    fn can_insider_trade(_entity_id: u64, _account: &u64) -> bool {
        true
    }
...
impl ProductProvider<u64> for MockProductProvider {
    ...
    fn product_category(_: u64) -> Option<ProductCategory> {
        Some(ProductCategory::Physical)
    }
    fn deduct_stock(_: u64, _: u32) -> Result<(), DispatchError> {
        Ok(())
    }
    fn restore_stock(_: u64, _: u32) -> Result<(), DispatchError> {
        Ok(())
    }
    fn add_sold_count(_: u64, _: u32) -> Result<(), DispatchError> {
        Ok(())
    }
...
pub struct MockKycGovernance;
impl pallet_entity_common::KycGovernancePort<u64> for MockKycGovernance {
    fn governance_set_kyc_requirement(
        ...
        if min_level > 4 {
            return Err(DispatchError::Other("InvalidKycLevel"));
        }
        Ok(())
    }
    ...
    fn governance_authorize_kyc_provider(
        ...
        if !MockEntityProvider::entity_exists(entity_id) {
            return Err(DispatchError::Other("EntityNotFound"));
        }
        if !MockEntityProvider::is_entity_active(entity_id) {
            return Err(DispatchError::Other("EntityNotActive"));
        }
        let exists = MOCK_KYC_PROVIDERS.with(|p| p.borrow().contains_key(&provider));
        if !exists {
            return Err(DispatchError::Other("ProviderNotFound"));
        }
        let already = is_mock_kyc_authorized(entity_id, provider);
        if already {
            return Err(DispatchError::Other("ProviderAlreadyAuthorized"));
        }
        set_mock_kyc_authorized(entity_id, provider, true);
        Ok(())
    }
    ...
    fn governance_deauthorize_kyc_provider(
        ...
        let already = is_mock_kyc_authorized(entity_id, provider);
        if !already {
            return Err(DispatchError::Other("ProviderNotAuthorized"));
        }
        set_mock_kyc_authorized(entity_id, provider, false);
        Ok(())
    }
}

pub struct MockPoolRewardGovernance;
impl pallet_entity_common::CommissionGovernancePort<u128> for MockPoolRewardGovernance {
    fn governance_set_withdrawal_cooldown(_: u64, _: u32, _: u32) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_set_token_withdrawal(_: u64, _: bool) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_set_withdrawal_pause(_: u64, _: bool) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_set_referrer_guard(_: u64, _: u128, _: u32) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_set_commission_cap(_: u64, _: u128, _: u128) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_pause_multi_level(_: u64) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_resume_multi_level(_: u64) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_pause_team_performance(_: u64) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_resume_team_performance(_: u64) -> Result<(), DispatchError> {
        Ok(())
    }
    fn governance_set_pool_reward_config(
        entity_id: u64,
        level_rules: Vec<(u8, pallet_entity_common::PoolRewardLevelClaimRule)>,
        round_duration: u32,
    ) -> Result<(), DispatchError> {
        MOCK_POOL_REWARD_CONFIGS.with(|c| {
            c.borrow_mut()
                .insert(entity_id, (level_rules, round_duration))
        });
        Ok(())
    }
    fn governance_clear_pool_reward_config(entity_id: u64) -> Result<(), DispatchError> {
        MOCK_POOL_REWARD_CONFIGS.with(|c| c.borrow_mut().remove(&entity_id));
        Ok(())
    }
    fn governance_set_pool_reward_token_enabled(
        ...
        MOCK_POOL_REWARD_TOKEN_ENABLED.with(|m| m.borrow_mut().insert(entity_id, enabled));
        Ok(())
    }
}
...
impl pallet_entity_governance::Config for Test {
    ...
    type CommissionGovernance = MockPoolRewardGovernance;
    type SingleLineGovernance = ();
    type KycGovernance = MockKycGovernance;
    type ShopGovernance = ();
    type TokenGovernance = ();
    // Phase 4.3: 资金保护
    type TreasuryPort = MockTreasuryPort;

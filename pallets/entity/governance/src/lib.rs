//! # Entity governance pallet (pallet-entity-governance)
//!
//! ## Overview
//!
//! This pallet implements entity token-governance workflows:
//! 本模块实现实体通证治理功能：
//! - support for multiple governance modes
//! - 多种治理模式支持
//! - proposal creation and management
//! - 提案创建与管理
//! - token-weighted voting
//! - 代币加权投票
//! - tiered governance thresholds
//! - 分层治理阈值
//! - proposal execution
//! - 提案执行
//!
//! ## Governance modes
//!
//! - **None**: no governance, admins keep full control and proposals are disallowed
//! - **None**：无治理（管理员全权控制，禁止创建提案）
//! - **FullDAO**: token voting with optional admin veto as an emergency brake
//! - **FullDAO**：完全 DAO（代币投票，可选管理员否决权作为紧急制动）
//!
//! ## Version history
//!
//! - v0.1.0 (2026-01-31): initial release
//! - v0.1.0 (2026-01-31)：初始版本
//! - v0.2.0 (2026-02-03): Phase 5 governance-mode enhancements
...
pub mod pallet {
    ...
    use pallet_entity_commission::{
        CommissionProvider, MemberProvider, MultiLevelPlanWriter, PoolRewardPlanWriter,
        TeamPlanWriter,
    };
    use pallet_entity_common::{
        CommissionGovernancePort,
        DisclosureProvider,
        EntityProvider,
        EntityTokenProvider,
        // Phase 4.3: 资金查询
        ...
        MarketGovernancePort,
        MemberRegistrationPolicy,
        PoolRewardCapBehavior,
        PoolRewardLevelClaimRule,
        PricingProvider,
        ProductProvider,
        ShopGovernancePort,
        ShopProvider,
        SingleLineGovernancePort,
        TokenGovernancePort,
    };
    ...
    pub struct FundProtectionConfig<Balance: Default> {
        /// Minimum treasury-balance threshold. `0` disables the alert.
        /// 最低金库余额阈值（低于此值触发预警，`0` = 禁用）
        pub min_treasury_threshold: Balance,
        /// Maximum spend per transaction. `0` means unlimited.
        /// 单笔最大支出限额（超出触发预警，`0` = 不限）
        pub max_single_spend: Balance,
        /// Maximum total daily spend. `0` means unlimited.
        /// 每日最大支出限额（`0` = 不限）
    ...
    /// Proposal domain grouping.
    /// 提案领域分组。
    ///
    /// Phase 4.1 groups 87 proposal variants by business domain for frontend display,
    /// query filtering, and permission policy use.
    ...
    /// Proposal type based on token voting.
    /// 提案类型（纯代币投票）。
    #[derive(
        Encode,
        Decode,
    ...
    pub enum ProposalType<AccountId, Balance> {
        ...
        /// 新商品上架
        ProductListing {
            product_cid: BoundedVec<u8, ConstU32<64>>,
        },
        /// 商品下架
        ...
        /// 修改店铺名称
        ShopNameChange {
            new_name: BoundedVec<u8, ConstU32<64>>,
        },
        /// 修改店铺描述
        ...
        TokenMint {
            amount: Balance,
            recipient_cid: BoundedVec<u8, ConstU32<64>>,
        },
        /// 销毁代币（从金库）
        TokenBurn { amount: Balance },
        /// 空投分发
        AirdropDistribution {
            airdrop_cid: BoundedVec<u8, ConstU32<64>>,
        ...
        /// Entity 金库向 Shop 划拨运营资金（链上自动执行）
        TreasuryAllocateToShop { shop_id: u64, amount: Balance },
        /// 手续费调整
        FeeAdjustment { new_fee_rate: u16 },
        /// 收益分配比例调整
        RevenueShare {
            owner_share: u16,
            token_holder_share: u16,
        },
        /// 退款政策调整
        RefundPolicy {
            policy_cid: BoundedVec<u8, ConstU32<64>>,
        },
        ...
        MultiLevelChange {
            /// 各层级配置: Vec<(rate_bps, required_directs, required_team_size, required_spent_usdt, required_level_id)>
            tiers: BoundedVec<(u16, u32, u32, u128, u8), ConstU32<15>>,
        },
        ...
        /// 推荐人免除阈值变更（全局治理，多级分销层数超过阈值的 Entity 推荐人不获得招商提成）
        ...
        /// 社区活动
        CommunityEvent {
            event_cid: BoundedVec<u8, ConstU32<64>>,
        },
        /// 规则建议
        RuleSuggestion {
            suggestion_cid: BoundedVec<u8, ConstU32<64>>,
        },
        /// 通用提案（自定义内容）
        General {
        ...
        SingleLineConfigChange {
            ...
            /// 覆盖模式: 0=Bidirectional, 1=BuyerOnly, 2=BeneficiaryOnly
            reach_mode: u8,
        },
        /// 单线收益暂停
        SingleLinePause,
        ...
        // ==================== R10: 沉淀池奖励类 ====================
        /// 沉淀池奖励完整规则配置变更
        PoolRewardConfigChange {
            level_rules: BoundedVec<(u8, PoolRewardLevelClaimRule), ConstU32<10>>,
            round_duration: u32,
        },
        /// 清除沉淀池奖励配置
        PoolRewardClear,
        /// 沉淀池 Token 池开关
        PoolRewardTokenPoolToggle { enabled: bool },
        ...
        /// 代币黑名单管理
        TokenBlacklistManage {
            account_cid: BoundedVec<u8, ConstU32<64>>,
            add: bool,
        },
        ...
        /// 创建者分成比率变更
        OwnerRewardRateChange { new_rate: u16 },
        /// 提现冷却期变更（区块数）
        WithdrawalCooldownChange {
            nex_cooldown: u32,
            token_cooldown: u32,
        },
        /// 代币提现配置变更
        ...
        // ==================== R10: KYC 管理类 ====================
        /// KYC 等级要求变更
        KycRequirementChange {
            min_level: u8,
            mandatory: bool,
            grace_period: u32,
        },
        /// 授权 KYC 提供者
        KycProviderAuthorize { provider: AccountId },
        /// 取消 KYC 提供者授权
        KycProviderDeauthorize { provider: AccountId },
        ...
        /// 店铺政策变更（退换货/物流政策）
        ShopPoliciesChange {
            policies_cid: BoundedVec<u8, ConstU32<256>>,
        },
        /// 店铺关闭（不可逆）
    ...
    impl<AccountId, Balance> ProposalType<AccountId, Balance> {
        fn execution_mode(&self) -> ProposalExecutionMode {
            match self {
                Self::ProductListing { .. }
                | Self::Promotion { .. }
                | Self::ShopNameChange { .. }
                ...
                | Self::RuleSuggestion { .. }
                | Self::General { .. }
                | Self::TokenBlacklistManage { .. } => ProposalExecutionMode::OffChainApproval,

                _ => ProposalExecutionMode::OnChain,
            }
        }
        ...
        pub fn domain(&self) -> ProposalDomain {
            match self {
                ...
                | Self::TeamPerformanceResume
                | Self::ReferrerExemptThresholdChange { .. }
                | Self::PoolRewardConfigChange { .. }
                | Self::PoolRewardClear
                | Self::PoolRewardTokenPoolToggle { .. } => ProposalDomain::Commission,
                ...
                | Self::UpgradeRuleToggle { .. } => ProposalDomain::Member,
                ...
                // DAO 紧急权限
                Self::EmergencyPauseToggle { .. } | Self::BatchCancelToggle { .. } => {
                    ProposalDomain::Emergency
                }
    ...
    pub struct Proposal<T: Config> {
        ...
        /// 提案类型
        pub proposal_type: ProposalType<T::AccountId, BalanceOf<T>>,
        /// 提案标题
    ...
    /// 投票记录类型别名
    pub type VoteRecordOf<T> =
        VoteRecord<<T as frame_system::Config>::AccountId, BalanceOf<T>, BlockNumberFor<T>>;
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// 返佣服务接口（治理调用）
        type CommissionProvider: pallet_entity_commission::CommissionProvider<
            Self::AccountId,
            Self::Balance,
        >;
        ...
        /// KYC 治理执行接口
        type KycGovernance: KycGovernancePort<Self::AccountId>;
        ...
        /// NEX/USDT 定价接口（用于 min_treasury_threshold 最低值校验）
        type PricingProvider: pallet_entity_common::PricingProvider;

        /// min_treasury_threshold 最低值（USDT 精度 10^6，50_000_000 = 50 USDT）
        #[pallet::constant]
        type MinTreasuryThresholdUsdt: Get<u64>;
    ...
    /// C2: 存储版本声明（v2 — KYC provider proposal payload schema changed to AccountId）
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn on_idle(now: BlockNumberFor<T>, mut remaining_weight: Weight) -> Weight {
            ...
            let per_proposal_base = T::DbWeight::get()
                .reads_writes(4, 4)
                .saturating_add(Weight::from_parts(30_000_000, 3_000));
            let per_voter_weight = T::DbWeight::get()
                .reads_writes(5, 5)
                .saturating_add(Weight::from_parts(5_000_000, 1_000));

            let min_weight = base_read
                .saturating_add(cursor_weight)
                .saturating_add(per_scan_read);
            ...
            remaining_weight = remaining_weight
                .saturating_sub(base_read)
                .saturating_sub(cursor_weight);
    ...
    pub enum Event<T: Config> {
        ...
        /// 提案已批准，等待链下执行
        ProposalApprovedOffchain { proposal_id: ProposalId },
        /// 提案已取消
        ...
        /// 治理配置已锁定（不可再修改）
        GovernanceConfigLocked { entity_id: u64 },
        ...
        /// F6: 治理已暂停
        GovernancePausedEvent { entity_id: u64 },
        /// F6: 治理已恢复
        GovernanceResumedEvent { entity_id: u64 },
        /// F6: 批量取消提案
        ...
        ProposalPartialCleaned { proposal_id: ProposalId },
        /// F5: 提案执行失败（链上 provider 返回错误）
        ProposalExecutionFailed { proposal_id: ProposalId },
        /// P2: 紧急恢复 — 治理被强制解锁
        GovernanceForceUnlocked { entity_id: u64 },
        /// P2: 紧急恢复 — 治理暂停被强制解除
        GovernanceForceResumed { entity_id: u64 },
        /// Phase 4.1: 资金保护配置已更新
    ...
    pub enum Error<T> {
        ...
        /// P2: 当前账户无紧急恢复权限
        NotEmergencyAdmin,
        /// min_treasury_threshold 低于最低值（50 USDT 等值 NEX）
        ThresholdTooLow,
        /// 支出后金库余额低于 min_treasury_threshold
        TreasuryBelowThreshold,
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_proposal(
            ...
            // F6: 治理暂停检查
                ...
                !GovernancePaused::<T>::get(entity_id),
                Error::<T>::GovernanceIsPaused
            );

            // H1: 检查治理模式，None 模式不允许创建提案
            let gov_config = GovernanceConfigs::<T>::get(entity_id);
            let effective_mode = gov_config
                .as_ref()
                .map(|c| c.mode)
                .unwrap_or(GovernanceMode::None);
            ensure!(
                effective_mode != GovernanceMode::None,
            ...
            Self::validate_proposal_type(entity_id, &proposal_type)?;
                ...
                proposal_type.execution_mode() != ProposalExecutionMode::Unsupported,
                Error::<T>::ProposalTypeNotSupported
            );
            ...
            let effective_threshold: u16 = gov_config
                .as_ref()
                .filter(|c| c.proposal_threshold > 0)
                .map(|c| c.proposal_threshold)
                .unwrap_or(T::MinProposalThreshold::get());
            let min_threshold =
                total_supply.saturating_mul(effective_threshold.into()) / 10000u128.into();
            ensure!(
                balance >= min_threshold,
                Error::<T>::InsufficientTokensForProposal
            );
            ...
            // 转换标题和描述
            let title_bounded: BoundedVec<u8, T::MaxTitleLength> = title
                .clone()
                .try_into()
            ...
            let effective_voting_period = gov_config
                .as_ref()
        ...
        pub fn vote(
            ...
            // F5: 实体活跃检查
                ...
                T::EntityProvider::is_entity_active(proposal.entity_id),
                Error::<T>::EntityNotActive
            );

            // F6: 治理暂停检查
            ensure!(
                !GovernancePaused::<T>::get(proposal.entity_id),
                Error::<T>::GovernanceIsPaused
            );
            ...
            let token_type = T::TokenProvider::get_token_type(proposal.entity_id);
                ...
                token_type.has_voting_power(),
                Error::<T>::TokenTypeNoVotingPower
            );
            ...
            let delegated_power =
                Self::calculate_delegated_power(proposal.entity_id, proposal_id, &who);
            ...
            for delegator in delegators.iter() {
                ...
                GovernanceLockCount::<T>::mutate(entity_id, delegator, |c| {
        ...
        pub fn finalize_voting(origin: OriginFor<T>, proposal_id: ProposalId) -> DispatchResult {
        ...
        pub fn execute_proposal(origin: OriginFor<T>, proposal_id: ProposalId) -> DispatchResult {
            ...
            // 验证执行时间
            let now = <frame_system::Pallet<T>>::block_number();
            let exec_time = proposal
                .execution_time
                .ok_or(Error::<T>::ExecutionTimeNotReached)?;
            ensure!(now >= exec_time, Error::<T>::ExecutionTimeNotReached);

            // H3+M5: 执行过期检查——使用快照的执行延迟的 2 倍作为窗口
            let execution_window = proposal
                .snapshot_execution_delay
                .saturating_mul(2u32.into());
            ...
            let execution_mode = proposal.proposal_type.execution_mode();
            ensure!(
                execution_mode != ProposalExecutionMode::Unsupported,
                Error::<T>::ProposalTypeNotSupported
            );
            ...
            match Self::do_execute_proposal(&proposal) {
                Ok(()) => match execution_mode {
                    ProposalExecutionMode::OnChain => {
                        proposal.status = ProposalStatus::Executed;
                        Proposals::<T>::insert(proposal_id, proposal);
                        Self::deposit_event(Event::ProposalExecuted { proposal_id });
                    }
                    ProposalExecutionMode::OffChainApproval => {
                        proposal.status = ProposalStatus::ApprovedOffchain;
                        Proposals::<T>::insert(proposal_id, proposal);
                        Self::deposit_event(Event::ProposalApprovedOffchain { proposal_id });
                    }
                    ProposalExecutionMode::Unsupported => unreachable!(),
                },
                Err(_) => {
        ...
        pub fn cancel_proposal(origin: OriginFor<T>, proposal_id: ProposalId) -> DispatchResult {
            ...
            if is_owner && !is_proposer {
                ...
                if let Some(ref cfg) = gov_config {
                        ...
                        cfg.mode != GovernanceMode::FullDAO,
                        Error::<T>::GovernanceModeNotAllowed
                    );
                }
            }
        ...
        pub fn configure_governance(
            ...
            // S4: 实体必须活跃
                ...
                !GovernanceLocked::<T>::get(entity_id),
                Error::<T>::GovernanceConfigIsLocked
            );

            if mode == GovernanceMode::FullDAO {
                ensure!(
                    T::TokenProvider::is_token_enabled(entity_id),
                    Error::<T>::TokenNotEnabledForDAO
                );
            }
            ...
            if let Some(t) = proposal_threshold {
                // BUG-2 修复: 0 = 使用全局默认，>0 必须 >= MinProposalThreshold
                    ...
                    t == 0 || t >= T::MinProposalThreshold::get(),
                    Error::<T>::ProposalThresholdTooLow
                );
                ensure!(t <= 10000, Error::<T>::InvalidParameter);
            }
            if let Some(period) = voting_period {
                ensure!(
                    period >= T::MinVotingPeriod::get(),
                    Error::<T>::VotingPeriodTooShort
                );
                ensure!(
                    period <= T::MaxVotingPeriod::get(),
                    Error::<T>::VotingPeriodTooLong
                );
            }
            if let Some(delay) = execution_delay {
                ensure!(
                    delay >= T::MinExecutionDelay::get(),
                    Error::<T>::ExecutionDelayTooShort
                );
                ensure!(
                    delay <= T::MaxExecutionDelay::get(),
            ...
            Self::deposit_event(Event::GovernanceConfigUpdated { entity_id, mode });
            Ok(())
        }
        ...
        pub fn lock_governance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            let owner =
                ...
                !GovernanceLocked::<T>::get(entity_id),
                Error::<T>::GovernanceAlreadyLocked
            );
        ...
        pub fn cleanup_proposal(origin: OriginFor<T>, proposal_id: ProposalId) -> DispatchResult {
            ...
            let proposal = Proposals::<T>::get(proposal_id).ok_or(Error::<T>::ProposalNotFound)?;
                ...
                matches!(
                    ...
                    ProposalStatus::Executed
                        | ProposalStatus::ApprovedOffchain
        ...
        pub fn undelegate_vote(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let delegate =
                VoteDelegation::<T>::take(entity_id, &who).ok_or(Error::<T>::NotDelegated)?;
        ...
        pub fn veto_proposal(origin: OriginFor<T>, proposal_id: ProposalId) -> DispatchResult {
            ...
            let config = GovernanceConfigs::<T>::get(proposal.entity_id).unwrap_or_default();
        ...
        pub fn change_vote(
            ...
            let old_record =
                VoteRecords::<T>::get(proposal_id, &who).ok_or(Error::<T>::NotVoted)?;
            ...
            match old_vote {
                ...
                VoteType::Abstain => {
                    proposal.abstain_votes = proposal.abstain_votes.saturating_sub(weight)
                }
            }
        ...
        pub fn pause_governance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        /// F6: 恢复治理
        ///
        /// R8: resume 始终允许（包括 FullDAO 锁定后 EmergencyPause 已关闭的场景），
        /// 因为 resume 是恢复操作——不允许 resume 会导致治理永久卡死在 paused 状态。
        /// None 模式锁定后无治理可 pause，GovernancePaused 不会为 true，自然不可达。
        #[pallet::call_index(16)]
        #[pallet::weight(T::WeightInfo::resume_governance())]
        pub fn resume_governance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn batch_cancel_proposals(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        #[pallet::weight(T::WeightInfo::configure_governance())]
        pub fn force_unlock_governance(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn validate_proposal_type(
            entity_id: u64,
            pt: &ProposalType<T::AccountId, BalanceOf<T>>,
        ) -> DispatchResult {
            match pt {
                // ==================== 商品管理类 — F2 前置校验 ====================
                ProposalType::PriceChange {
                    product_id,
                    new_usdt_price,
                } => {
                ...
                ProposalType::InventoryAdjustment {
                    product_id,
                    new_inventory,
                } => {
                    ensure!(
                        *new_inventory <= u32::MAX as u64,
                ...
                // ==================== 代币经济类 ====================
                ProposalType::TokenConfigChange {
                    reward_rate,
                    exchange_rate,
                } => {
                    if let Some(r) = reward_rate {
                        ensure!(*r <= 10000, Error::<T>::InvalidParameter);
                    }
                    if let Some(e) = exchange_rate {
                        ensure!(*e <= 10000, Error::<T>::InvalidParameter);
                    }
                }
                ...
                ProposalType::TreasuryAllocateToShop { shop_id, amount } => {
                    ensure!(!amount.is_zero(), Error::<T>::InvalidParameter);
                    let entity_shops = T::EntityProvider::entity_shops(entity_id);
                    ensure!(entity_shops.contains(shop_id), Error::<T>::ShopNotFound);
                }
                ...
                ProposalType::ProposalThresholdChange { new_threshold } => {
                    // S3: 增加最小值校验
                        ...
                        *new_threshold >= T::MinProposalThreshold::get(),
                        Error::<T>::InvalidParameter
                    );
                ...
                ProposalType::CommissionModesChange { modes } => {
                        ...
                        *modes & !0b0000_0011_1111_1111u16 == 0,
                        Error::<T>::InvalidParameter
                    );
                }
                ...
                ProposalType::MultiLevelChange { ref tiers } => {
                    ensure!(!tiers.is_empty(), Error::<T>::InvalidParameter);
                    for &(rate, _, _, _, _) in tiers.iter() {
                        ensure!(rate <= 10000u16, Error::<T>::InvalidParameter);
                    }
                }
                ...
                // ==================== 团队业绩类 ====================
                ProposalType::TeamPerformanceChange {
                    ref tiers,
                    max_depth,
                    threshold_mode,
                    ..
                ...
                // ==================== DAO 可控紧急权限 — F3 前置条件 ====================
                ProposalType::EmergencyPauseToggle { .. }
                | ProposalType::BatchCancelToggle { .. } => {
                    let locked = GovernanceLocked::<T>::get(entity_id);
                    let mode = GovernanceConfigs::<T>::get(entity_id)
                        .map(|c| c.mode)
                        .unwrap_or(GovernanceMode::None);
                    ensure!(
                        locked && mode == GovernanceMode::FullDAO,
                        Error::<T>::GovernanceModeNotAllowed
                    );
                }
                ...
                ProposalType::SingleLineConfigChange {
                    ...
                    ensure!(*reach_mode <= 2, Error::<T>::InvalidParameter);
                }
                ProposalType::PoolRewardConfigChange {
                    level_rules,
                    round_duration,
                } => {
                    ensure!(!level_rules.is_empty(), Error::<T>::InvalidParameter);
                    ensure!(*round_duration > 0, Error::<T>::InvalidParameter);
                    for (_, rule) in level_rules.iter() {
                        ensure!(rule.base_cap_percent <= 10000, Error::<T>::InvalidParameter);
                        match &rule.cap_behavior {
                            PoolRewardCapBehavior::Fixed => {}
                            PoolRewardCapBehavior::UnlockByTeam {
                                direct_per_unlock,
                                team_per_unlock,
                                unlock_percent,
                            } => {
                                ensure!(*direct_per_unlock > 0, Error::<T>::InvalidParameter);
                                ensure!(*team_per_unlock > 0, Error::<T>::InvalidParameter);
                                ensure!(*unlock_percent > 0, Error::<T>::InvalidParameter);
                                ensure!(*unlock_percent <= 10000, Error::<T>::InvalidParameter);
                            }
                        }
                    }
                }
                ...
                ProposalType::TransferRestrictionChange {
                    restriction,
                    min_receiver_kyc,
                } => {
                ...
                ProposalType::OwnerRewardRateChange { new_rate } => {
                    ensure!(*new_rate <= 10000, Error::<T>::InvalidParameter);
                }

                // ==================== R10: 推荐人配置类 ====================
                ProposalType::CommissionCapChange {
                    max_per_order,
                    max_total_earned,
                } => {
                    ensure!(
                        !max_per_order.is_zero() || !max_total_earned.is_zero(),
                        Error::<T>::InvalidParameter
                    );
                }
                ProposalType::ReferrerExemptThresholdChange { threshold } => {
                    // threshold = 0 合法（表示禁用规则）；上限 1000（MaxMultiLevels）
                ...
                ProposalType::MemberPolicyChange { policy } => {
                        ...
                        MemberRegistrationPolicy(*policy).is_valid(),
                        Error::<T>::InvalidParameter
                    );
                }
                ...
                // ==================== R10: 店铺扩展类 ====================
                ProposalType::PointsConfigChange {
                    reward_rate,
                    exchange_rate,
                    ..
                } => {
                ...
                // ==================== R10: 商品扩展类 ====================
                ProposalType::ProductVisibilityChange {
                    product_id,
                    visibility,
                } => {
                    Self::validate_product_belongs_to_entity(entity_id, *product_id)?;
                ...
                ProposalType::MarketConfigChange {
                    min_order_amount,
                    order_ttl,
                } => {
                ...
                ProposalType::WithdrawalCooldownChange {
                    nex_cooldown,
                    token_cooldown,
                } => {
                    ensure!(
                        *nex_cooldown > 0 || *token_cooldown > 0,
                        Error::<T>::InvalidParameter
                    );
                }

                // ==================== Phase 4.3: 资金保护类参数校验 ====================
                ProposalType::FundProtectionChange {
                    min_treasury_threshold,
                    max_single_spend,
                    max_daily_spend,
                } => {
                    // 至少设置一项规则
                    ensure!(
                        !min_treasury_threshold.is_zero()
                            || !max_single_spend.is_zero()
                            || !max_daily_spend.is_zero(),
                        Error::<T>::InvalidParameter
                    );
                    // max_daily_spend 必须 >= max_single_spend（如果都设置了）
                    if !max_daily_spend.is_zero() && !max_single_spend.is_zero() {
                        ensure!(
                            *max_daily_spend >= *max_single_spend,
                            Error::<T>::InvalidParameter
                        );
                    }
                    // min_treasury_threshold 非零时必须 >= 50 USDT 等值 NEX
                    if !min_treasury_threshold.is_zero() {
                        if let Some(floor) = Self::min_treasury_threshold_floor() {
                            ensure!(
                                *min_treasury_threshold >= floor,
                                Error::<T>::ThresholdTooLow
                            );
                        }
                        // 价格不可用时放行（不因 oracle 故障阻塞治理）
                    }
                }
        ...
        fn validate_product_belongs_to_entity(entity_id: u64, product_id: u64) -> DispatchResult {
            ...
            let entity_shops = T::EntityProvider::entity_shops(entity_id);
                ...
                entity_shops.contains(&product_shop),
                Error::<T>::InvalidParameter
            );
            Ok(())
        }
        ...
        fn calculate_delegated_power(
            ...
            for delegator in delegators.iter() {
                ...
                {
                    total =
                        total.saturating_add(Self::calculate_voting_power(entity_id, delegator));
                }
            }
            total
        }
        ...
        pub fn calculate_voting_power(entity_id: u64, holder: &T::AccountId) -> BalanceOf<T> {
            ...
            let multiplier: u128 = match FirstHoldTime::<T>::get(entity_id, holder) {
                Some(first_hold) => {
                    ...
                    let bonus = holding_blocks.saturating_mul(bonus_range) / full_period;
        ...
        fn compute_finalize_result(
            ...
            let quorum_threshold: BalanceOf<T> = proposal
            ...
            let new_status = if total_votes < quorum_threshold {
                ...
                let pass_threshold: BalanceOf<T> =
                    decisive_votes.saturating_mul(proposal.snapshot_pass.into()) / 100u128.into();

                if proposal.yes_votes > pass_threshold {
                    proposal.execution_time =
                        Some(now.saturating_add(proposal.snapshot_execution_delay));
                    ProposalStatus::Passed
                } else {
                    ProposalStatus::Failed
                }
        ...
        fn do_execute_proposal(proposal: &ProposalOf<T>) -> DispatchResult {
            ...
            match &proposal.proposal_type {
                ...
                ProposalType::TreasurySpend {
                    ...
                    // Phase 4.3: 资金保护检查（min_treasury_threshold 阻断）
                    Self::validate_fund_protection(entity_id, *amount)?;
                    Self::deposit_event(Event::ProposalExecutionNote {
                        proposal_id: proposal.id,
                ...
                ProposalType::TreasuryAllocateToShop { shop_id, amount } => {
                    Self::validate_fund_protection(entity_id, *amount)?;
                    T::ShopProvider::governance_allocate_from_treasury(
                        entity_id,
                        *shop_id,
                        (*amount).into(),
                    )?;
                    Self::record_daily_spend(entity_id, *amount);
                    Ok(())
                }
                ProposalType::FeeAdjustment { new_fee_rate } => {
                    Self::deposit_event(Event::ProposalExecutionNote {
                ...
                ProposalType::VotingPeriodChange { new_period_blocks } => {
                    ...
                    GovernanceConfigs::<T>::mutate(entity_id, |maybe_config| {
                        let config =
                            maybe_config.get_or_insert_with(GovernanceConfigOf::<T>::default);
                        config.voting_period = period;
                    });
                    Ok(())
                }
                ...
                ProposalType::MultiLevelChange { ref tiers } => {
                    T::MultiLevelWriter::set_multi_level_full(entity_id, tiers.to_vec())
                }
                ProposalType::LevelDiffChange { ref level_rates } => {
                    T::CommissionProvider::set_level_diff_config(entity_id, level_rates.to_vec())
                }
                ProposalType::FixedAmountChange { amount } => {
                    T::CommissionProvider::set_fixed_amount(entity_id, *amount)
                }
                ProposalType::FirstOrderChange {
                    amount,
                    rate,
                    use_amount,
                } => T::CommissionProvider::set_first_order_config(
                    entity_id,
                    *amount,
                    *rate,
                    *use_amount,
                ...
                // ==================== 分级提现配置类 ====================
                ProposalType::WithdrawalConfigChange {
                    tier_configs_cid: _,
                    enabled,
                } => {
                    // tier_configs_cid 需要链下解析，这里只设置基本配置
                    T::CommissionProvider::set_withdrawal_config_by_governance(entity_id, *enabled)
                }
                ...
                // ==================== 社区类 ====================
                ProposalType::CommunityEvent { .. }
                | ProposalType::RuleSuggestion { .. }
                | ProposalType::General { .. } => Ok(()),
                ...
                ProposalType::TransferRestrictionChange {
                    restriction,
                    min_receiver_kyc,
                } => T::TokenProvider::governance_set_transfer_restriction(
                    entity_id,
                    *restriction,
                    *min_receiver_kyc,
                ),
                ...
                ProposalType::OwnerRewardRateChange { new_rate } => {
                    T::CommissionProvider::set_owner_reward_rate(entity_id, *new_rate)
                }
                ...
                ProposalType::MarketConfigChange {
                    min_order_amount,
                    order_ttl,
                } => T::MarketGovernance::governance_set_market_config(
                    entity_id,
                    *min_order_amount,
                    *order_ttl,
                ),
                ...
                ProposalType::PriceProtectionChange {
                    ...
                    circuit_breaker_threshold,
                    min_trades_for_twap,
                } => T::MarketGovernance::governance_set_price_protection(
                    entity_id,
                    *max_price_deviation,
                    *max_slippage,
                    *circuit_breaker_threshold,
                ...
                ProposalType::SingleLineConfigChange {
                    ...
                    max_downline_levels,
                    reach_mode,
                } => T::SingleLineGovernance::governance_set_single_line_config(
                    entity_id,
                    *upline_rate,
                    *downline_rate,
                    *base_upline_levels,
                ...
                ProposalType::PoolRewardConfigChange {
                    level_rules,
                    round_duration,
                } => T::CommissionGovernance::governance_set_pool_reward_config(
                    entity_id,
                    level_rules.iter().cloned().collect(),
                    *round_duration,
                ),
                ProposalType::PoolRewardClear => {
                    T::CommissionGovernance::governance_clear_pool_reward_config(entity_id)
                }
                ProposalType::PoolRewardTokenPoolToggle { enabled } => {
                    T::CommissionGovernance::governance_set_pool_reward_token_enabled(
                        entity_id, *enabled,
                    )
                }
                ...
                ProposalType::TokenBlacklistManage { account_cid, add } => {
                    T::TokenGovernance::governance_manage_blacklist(
                        entity_id,
                        account_cid.as_slice(),
                        *add,
                    )
                }
                ...
                ProposalType::WithdrawalCooldownChange {
                    nex_cooldown,
                    token_cooldown,
                } => T::CommissionGovernance::governance_set_withdrawal_cooldown(
                    entity_id,
                    *nex_cooldown,
                    *token_cooldown,
                ),
                ...
                ProposalType::ReferrerGuardChange {
                    min_referrer_spent,
                    min_referrer_orders,
                } => T::CommissionGovernance::governance_set_referrer_guard(
                    entity_id,
                    *min_referrer_spent,
                    *min_referrer_orders,
                ),
                ProposalType::CommissionCapChange {
                    max_per_order,
                    max_total_earned,
                } => T::CommissionGovernance::governance_set_commission_cap(
                    entity_id,
                    *max_per_order,
                    *max_total_earned,
                ),
                ...
                // -- KYC 管理类 --
                ProposalType::KycRequirementChange {
                    min_level,
                    mandatory,
                    grace_period,
                } => T::KycGovernance::governance_set_kyc_requirement(
                    entity_id,
                    *min_level,
                    *mandatory,
                    *grace_period,
                ),
                ProposalType::KycProviderAuthorize { provider } => {
                    T::KycGovernance::governance_authorize_kyc_provider(entity_id, provider.clone())
                }
                ProposalType::KycProviderDeauthorize { provider } => {
                    T::KycGovernance::governance_deauthorize_kyc_provider(
                        entity_id,
                        provider.clone(),
                    )
                }
                ...
                ProposalType::PointsConfigChange {
                    ...
                    exchange_rate,
                    transferable,
                } => T::ShopGovernance::governance_set_points_config(
                    entity_id,
                    *reward_rate,
                    *exchange_rate,
                    *transferable,
                ...
                ProposalType::ShopPoliciesChange { policies_cid } => {
                    T::ShopGovernance::governance_set_shop_policies(
                        entity_id,
                        policies_cid.as_slice(),
                    )
                }

                // -- 披露链下类（仍需 CID 链下解析）--
                ProposalType::DisclosureInsiderManage { .. } => {
                    Self::emit_offchain_note(proposal.id, "DisclosureInsiderManage approved")
                }
                ...
                ProposalType::FundProtectionChange {
                    ...
                    FundProtectionConfigs::<T>::insert(
                        entity_id,
                        FundProtectionConfig {
        ...
        ///
        /// min_treasury_threshold：阻断——支出后余额不得低于阈值
        /// max_single_spend：不阻断执行，仅发出预警事件。
        fn validate_fund_protection(entity_id: u64, spend_amount: BalanceOf<T>) -> DispatchResult {
            if let Some(config) = FundProtectionConfigs::<T>::get(entity_id) {
                // min_treasury_threshold：阻断——支出后余额不得低于阈值
                if !config.min_treasury_threshold.is_zero() {
                    let treasury_balance: BalanceOf<T> =
                        T::TreasuryPort::treasury_balance(entity_id).saturated_into();
                    let post_spend = treasury_balance.saturating_sub(spend_amount);
                    ensure!(
                        post_spend >= config.min_treasury_threshold,
                        Error::<T>::TreasuryBelowThreshold
                    );
                }
        ...
        fn record_daily_spend(entity_id: u64, spend_amount: BalanceOf<T>) {
            if let Some(config) = FundProtectionConfigs::<T>::get(entity_id) {
        ...
        /// 计算 min_treasury_threshold 最低值（USDT → NEX 动态换算）
        ///
        /// 算法与 registry::calculate_initial_fund 对称：
        /// nex_amount = usdt_amount * 10^12 / nex_usdt_price
        ///
        /// 价格不可用或过时时返回 None，调用方使用 fallback 策略。
        fn min_treasury_threshold_floor() -> Option<BalanceOf<T>> {
            let price = T::PricingProvider::get_nex_usdt_price();
            if price == 0 || T::PricingProvider::is_price_stale() {
                return None;
            }
            let usdt = T::MinTreasuryThresholdUsdt::get() as u128;
            let nex = usdt
                .checked_mul(1_000_000_000_000)?
                .checked_div(price as u128)?;
            Some(nex.saturated_into())
        }
    }
}
...
// ============================================================================
// FundProtectionQueryPort 实现
// ============================================================================

impl<T: Config> pallet_entity_common::FundProtectionQueryPort for Pallet<T> {
    fn fund_protection_status(entity_id: u64) -> Option<(u128, u128, u128, u128)> {
        use sp_runtime::SaturatedConversion;

        let config = pallet::FundProtectionConfigs::<T>::get(entity_id)?;

        // 读取当日累计支出
        let now = <frame_system::Pallet<T>>::block_number();
        let day_number: u32 = now.saturated_into::<u32>() / 14400u32;
        let (tracked_day, tracked_amount) = pallet::DailySpendTracker::<T>::get(entity_id);

        let daily_spent: u128 = if tracked_day == day_number {
            tracked_amount.saturated_into()
        } else {
            0u128
        };

        Some((
            config.min_treasury_threshold.saturated_into(),
            config.max_single_spend.saturated_into(),
            config.max_daily_spend.saturated_into(),
            daily_spent,
        ))
    }
    ...
    fn min_treasury_threshold(entity_id: u64) -> u128 {
        use sp_runtime::SaturatedConversion;
        pallet::FundProtectionConfigs::<T>::get(entity_id)
            .map(|c| c.min_treasury_threshold.saturated_into())
            .unwrap_or(0)
    }
}

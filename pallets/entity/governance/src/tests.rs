use crate::mock::*;
use crate::pallet::*;
use frame_support::{assert_noop, assert_ok, pallet_prelude::ConstU32, traits::Hooks, BoundedVec};
use pallet_entity_common::{EntityTokenProvider, GovernanceMode};

fn proposal_type_general() -> ProposalType<u64, u128> {
...
fn proposal_type_price_change() -> ProposalType<u64, u128> {
    ProposalType::PriceChange {
        product_id: 1,
        new_usdt_price: 500_000,
    }
}
...
fn execute_proposal_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::ApprovedOffchain);
    });
}

#[test]
fn unsupported_proposals_rejected_at_creation() {
    ExtBuilder::build().execute_with(|| {
        for proposal_type in [
            ProposalType::KycProviderAuthorize { provider: 42 },
            ProposalType::KycProviderDeauthorize { provider: 42 },
        ] {
            assert_ok!(EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
...
#[test]
fn referral_governance_proposals_create_successfully() {
    ExtBuilder::build().execute_with(|| {
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::ReferrerGuardChange {
                min_referrer_spent: 1234,
                min_referrer_orders: 7,
            },
            b"Guard".to_vec(),
            None,
        ));
...
#[test]
fn shop_policies_governance_proposal_creates_successfully() {
    ExtBuilder::build().execute_with(|| {
        let cid: BoundedVec<u8, ConstU32<256>> = b"QmGovPolicy".to_vec().try_into().unwrap();
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::ShopPoliciesChange { policies_cid: cid },
            b"Policy".to_vec(),
            None,
        ));
    });
...
#[test]
fn kyc_provider_authorize_executes_onchain() {
    ExtBuilder::build().execute_with(|| {
        set_mock_kyc_provider(42, true);
        execute_proposal_flow(ProposalType::KycProviderAuthorize { provider: 42 });
        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::Executed);
        assert!(is_mock_kyc_authorized(SHOP_ID, 42));
    });
}

#[test]
fn kyc_provider_deauthorize_executes_onchain() {
    ExtBuilder::build().execute_with(|| {
        set_mock_kyc_provider(42, true);
        set_mock_kyc_authorized(SHOP_ID, 42, true);
        execute_proposal_flow(ProposalType::KycProviderDeauthorize { provider: 42 });
        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::Executed);
        assert!(!is_mock_kyc_authorized(SHOP_ID, 42));
    });
...
#[test]
fn kyc_provider_authorize_execution_fails_when_provider_missing() {
    ExtBuilder::build().execute_with(|| {
        execute_proposal_flow(ProposalType::KycProviderAuthorize { provider: 42 });
        let proposal = Proposals::<Test>::get(0).unwrap();
...
#[test]
fn treasury_spend_success_is_offchain_and_does_not_track_daily_spend() {
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
                min_treasury_threshold: 500,
                max_single_spend: 100,
                max_daily_spend: 200,
            },
        );
        set_mock_treasury_balance(1, 1000);

        execute_proposal_flow(ProposalType::TreasurySpend {
            amount: 400,
            recipient_cid: b"cid1".to_vec().try_into().unwrap(),
            reason_cid: b"cid2".to_vec().try_into().unwrap(),
        });

        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::ApprovedOffchain);
        assert_eq!(DailySpendTracker::<Test>::get(1u64), (0, 0));
    });
}

#[test]
fn treasury_allocate_success_tracks_daily_spend() {
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
                min_treasury_threshold: 0,
                max_single_spend: 100,
                max_daily_spend: 200,
            },
        );
        set_mock_treasury_balance(1, 1_000_000);

        execute_proposal_flow(ProposalType::TreasuryAllocateToShop {
            shop_id: SHOP_ID,
            amount: 150,
        });
        ...
        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::Executed);
        assert_eq!(DailySpendTracker::<Test>::get(1u64), (0, 150));
    });
}

#[test]
fn treasury_allocate_failed_does_not_track_daily_spend() {
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
        ...
        set_mock_treasury_balance(1, 800);

        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::TreasuryAllocateToShop {
                shop_id: SHOP_ID,
                amount: 400,
            },
            b"Allocate too much".to_vec(),
            None,
        ));
        assert_ok!(EntityGovernance::vote(
            RuntimeOrigin::signed(BOB),
...
fn cancel_proposal_by_proposer() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::cancel_proposal(
            RuntimeOrigin::signed(ALICE),
            0
        ));
...
fn veto_proposal_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::veto_proposal(
            RuntimeOrigin::signed(OWNER),
            0
        ));
...
fn lock_governance_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::lock_governance(
            RuntimeOrigin::signed(OWNER),
            1
        ));
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::VotingPeriodChange {
                    new_period_blocks: 5
                },
                b"Short period".to_vec(),
                None,
            ),
            Error::<Test>::VotingPeriodTooShort
        );
...
fn h1_voting_period_change_accepts_valid() {
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::create_proposal(
            ...
            SHOP_ID,
            ProposalType::VotingPeriodChange {
                new_period_blocks: 200
            },
            b"Long period".to_vec(),
            None,
        ));
    });
}
...
fn m1_execute_voting_period_change_validates_minimum() {
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::create_proposal(
            ...
            SHOP_ID,
            ProposalType::VotingPeriodChange {
                new_period_blocks: 50
            },
            b"Change period".to_vec(),
            None,
        ));
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::CommissionModesChange {
                    modes: 0b1000_0000_0000
                },
                b"Bad modes".to_vec(),
                None,
            ),
...
fn m1_commission_modes_change_accepts_valid_bits() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::create_proposal(
            ...
            SHOP_ID,
            ProposalType::CommissionModesChange {
                modes: 0b0000_0000_0001
            },
            b"Valid modes".to_vec(),
            None,
        ));
...
fn h3_vote_records_cleaned_after_finalize() {
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::cleanup_proposal(
            RuntimeOrigin::signed(ALICE),
            0
        ));
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_noop!(
            EntityGovernance::configure_governance(
                ...
                None,
                None,
                None, // voting_period=1 < MinVotingPeriod=10
            ),
            Error::<Test>::VotingPeriodTooShort
        );
    });
...
fn execute_proposal_flow(proposal_type: ProposalType<u64, u128>) {
    assert_ok!(EntityGovernance::create_proposal(
        RuntimeOrigin::signed(ALICE),
        SHOP_ID,
        proposal_type,
...
fn f1_execution_delay_change_proposal_works() {
    ExtBuilder::build().execute_with(|| {
        execute_proposal_flow(ProposalType::ExecutionDelayChange {
            new_delay_blocks: 100,
        });
    ...
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::ExecutionDelayChange {
                    new_delay_blocks: 1
                },
                b"Test".to_vec(),
                None,
    ...
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            EntityGovernance::create_proposal(
                ...
                SHOP_ID,
                ProposalType::MultiLevelChange {
                    tiers: vec![(10001, 0, 0, 0, 0)].try_into().unwrap(),
                },
                b"Test".to_vec(),
                None,
            ),
...
fn f5_delegate_vote_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        // 验证事件
        System::assert_has_event(RuntimeEvent::EntityGovernance(Event::VoteDelegated {
            entity_id: SHOP_ID,
            delegator: ALICE,
            delegate: BOB,
        }));
...
fn f5_undelegate_vote_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        // 验证事件
        System::assert_has_event(RuntimeEvent::EntityGovernance(Event::VoteUndelegated {
            entity_id: SHOP_ID,
            delegator: ALICE,
        }));
    });
...
fn c3_on_idle_respects_weight_limit() {
    ExtBuilder::build().execute_with(|| {
        ...
        let _total_still_voting = (0..3u64)
            .filter(|&i| {
...
fn f1_change_vote_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::change_vote(
            RuntimeOrigin::signed(BOB),
            0,
            VoteType::No
        ));
...
fn f6_resume_governance_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::resume_governance(
            RuntimeOrigin::signed(OWNER),
            SHOP_ID
        ));
        assert!(!GovernancePaused::<Test>::get(SHOP_ID));
...
fn f9_vote_records_cleaned_on_cleanup() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert!(matches!(
            proposal.status,
            ProposalStatus::Passed | ProposalStatus::Failed
        ));
        ...
        assert_noop!(
            EntityGovernance::create_proposal(
                ...
                ProposalType::PriceProtectionChange {
                    max_price_deviation: 10001,
                    max_slippage: 500,
                    circuit_breaker_threshold: 500,
                    min_trades_for_twap: 10,
                },
                b"Bad".to_vec(),
                None,
            ),
...
#[test]
fn r10_pool_reward_proposals_validate() {
    ExtBuilder::build().execute_with(|| {
        use pallet_entity_common::{PoolRewardCapBehavior, PoolRewardLevelClaimRule};
        ...
        assert_noop!(
            EntityGovernance::create_proposal(
                ...
                ProposalType::PoolRewardConfigChange {
                    level_rules: vec![].try_into().unwrap(),
                    round_duration: 100,
                },
                b"Bad".to_vec(),
                None,
                ...
                ProposalType::PoolRewardConfigChange {
                    level_rules: vec![(
                        ...
                        PoolRewardLevelClaimRule {
                            base_cap_percent: 1000,
                            cap_behavior: PoolRewardCapBehavior::UnlockByTeam {
                                direct_per_unlock: 0,
                                team_per_unlock: 2,
                                unlock_percent: 200,
                            },
        ...
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::PoolRewardTokenPoolToggle { enabled: true },
            b"Toggle".to_vec(),
            None,
        ));
    });
...
#[test]
fn r10_pool_reward_proposals_execute() {
    ExtBuilder::build().execute_with(|| {
        use crate::mock::{get_mock_pool_reward_config, get_mock_pool_reward_token_enabled};
        use pallet_entity_common::{PoolRewardCapBehavior, PoolRewardLevelClaimRule};

        execute_proposal_flow(ProposalType::PoolRewardConfigChange {
            level_rules: vec![(
                6,
                PoolRewardLevelClaimRule {
                    base_cap_percent: 1000,
        ...
        let (rules, duration) = get_mock_pool_reward_config(SHOP_ID).unwrap();
        assert_eq!(duration, 14400);
        assert_eq!(rules[0].0, 6);
        match &rules[0].1.cap_behavior {
            PoolRewardCapBehavior::UnlockByTeam {
                ...
                assert_eq!(
                    (*direct_per_unlock, *team_per_unlock, *unlock_percent),
                    (1, 2, 200)
                );
            }
            _ => panic!("expected UnlockByTeam"),
        }
        ...
        assert!(get_mock_pool_reward_token_enabled(SHOP_ID));
        ...
        assert!(get_mock_pool_reward_config(SHOP_ID).is_none());
    });
}
...
fn r10_token_extension_proposals_validate() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::TokenMaxSupplyChange {
                new_max_supply: 1_000_000
            },
            b"OK".to_vec(),
            None,
        ));
        ...
        assert_ok!(EntityGovernance::create_proposal(
            ...
            ProposalType::TokenBlacklistManage {
                account_cid: b"acc1".to_vec().try_into().unwrap(),
                add: true
            },
            b"Blacklist".to_vec(),
            None,
    ...
    ExtBuilder::build().execute_with(|| {
        ...
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::OwnerRewardRateChange { new_rate: 10001 },
                b"Bad".to_vec(),
                None,
            ),
            Error::<Test>::InvalidParameter
        ...
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::OwnerRewardRateChange { new_rate: 300 },
            b"OK".to_vec(),
            None,
        ));
        assert_ok!(EntityGovernance::create_proposal(
...
fn r10_member_proposals_validate() {
    ExtBuilder::build().execute_with(|| {
        // 0b0010_0000 = 32, undefined bit → reject
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::MemberPolicyChange { policy: 32 },
                b"Bad".to_vec(),
                None,
            ),
            Error::<Test>::InvalidParameter
        );
        // 0b0001_1111 = 31 = ALL_VALID (bitmask) → accept
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::MemberPolicyChange { policy: 31 },
            b"AllFlags".to_vec(),
            None,
        ));
        assert_ok!(EntityGovernance::create_proposal(
...
fn r10_shop_extension_proposals_validate() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::create_proposal(
            ...
            b"Disable points".to_vec(),
            None,
        ));
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::ShopPoliciesChange {
                policies_cid: b"QmPolicy".to_vec().try_into().unwrap()
            },
            b"Policy".to_vec(),
            None,
        ));
    ...
    ExtBuilder::build().execute_with(|| {
        assert_ok!(EntityGovernance::create_proposal(
            ...
            ProposalType::DisclosureInsiderManage {
                account_cid: b"insider".to_vec().try_into().unwrap(),
                add: true
            },
            b"Add insider".to_vec(),
            None,
        ));
    ...
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            EntityGovernance::create_proposal(
                ...
                ProposalType::PriceChange {
                    product_id: 999,
                    new_usdt_price: 100_000
                },
                b"Ghost product".to_vec(),
                None,
            ),
            Error::<Test>::InvalidParameter
        );
...
fn r11_f1_member_policy_upper_bound() {
    ExtBuilder::build().execute_with(|| {
        // ALL_VALID = 0b0001_1111 = 31 → accept
        assert_ok!(EntityGovernance::create_proposal(
            RuntimeOrigin::signed(ALICE),
            SHOP_ID,
            ProposalType::MemberPolicyChange { policy: 31 },
...
fn p2_force_unlock_governance_works() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_ok!(EntityGovernance::force_unlock_governance(
            RuntimeOrigin::root(),
            SHOP_ID
        ));
        assert!(!GovernanceLocked::<Test>::get(SHOP_ID));
...
fn p2_force_unlock_emits_events() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert!(event_names
            .iter()
            .any(|e| e.contains("GovernanceForceUnlocked")));
        assert!(event_names
            .iter()
            .any(|e| e.contains("GovernanceForceResumed")));
    });
}
...
fn audit_bug3_batch_cancel_clears_voting_power_snapshot() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert!(
            VotingPowerSnapshot::<Test>::contains_key(0, BOB),
...
fn audit_bug4_finalize_unlocks_all_voter_tokens() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(
            GovernanceLockCount::<Test>::get(SHOP_ID, BOB),
            0,
            "BUG-4: lock count should be 0 after finalize"
        );
        assert_eq!(
            GovernanceLockCount::<Test>::get(SHOP_ID, CHARLIE),
            0,
            "BUG-4: lock count should be 0 after finalize"
        );
    });
...
fn audit_r1r2_batch_cancel_clears_active_list() {
    ExtBuilder::build().execute_with(|| {
        ...
        assert_eq!(
            EntityProposals::<Test>::get(SHOP_ID).len(),
            0,
...
// ============================================================================
// TreasuryAllocateToShop proposal tests
// ============================================================================

#[test]
fn treasury_allocate_to_shop_proposal_works() {
    ExtBuilder::build().execute_with(|| {
        execute_proposal_flow(ProposalType::TreasuryAllocateToShop {
            shop_id: SHOP_ID,
            amount: 1_000,
...
#[test]
fn treasury_allocate_to_shop_validates_shop_belongs() {
    ExtBuilder::build().execute_with(|| {
        // Shop 999 does not belong to entity 1
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ...
                b"Allocate to unknown shop".to_vec(),
                None,
            ),
            Error::<Test>::ShopNotFound
        );
...
#[test]
fn treasury_allocate_to_shop_zero_rejected() {
    ExtBuilder::build().execute_with(|| {
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
...
// ==================== min_treasury_threshold 动态最低值 + 支出保护 ====================

#[test]
fn fund_protection_threshold_too_low_rejected() {
    // FundProtectionChange 提案：min_treasury_threshold < 50 USDT 等值 → ThresholdTooLow
    ExtBuilder::build().execute_with(|| {
        // price = 500_000_000_000_000_000 → floor = 50_000_000 * 10^12 / 500_000_000_000_000_000 = 100
        set_mock_nex_price(500_000_000_000_000_000);
        assert_noop!(
            EntityGovernance::create_proposal(
                RuntimeOrigin::signed(ALICE),
                SHOP_ID,
                ProposalType::FundProtectionChange {
                    min_treasury_threshold: 99,
                    max_single_spend: 0,
                    max_daily_spend: 0,
                },
                b"Too low threshold".to_vec(),
                None,
            ),
...
#[test]
fn fund_protection_threshold_at_floor_accepted() {
    // FundProtectionChange 提案：min_treasury_threshold >= 50 USDT 等值 → 成功
    ExtBuilder::build().execute_with(|| {
        set_mock_nex_price(500_000_000_000_000_000);
        // floor = 100
        execute_proposal_flow(ProposalType::FundProtectionChange {
            min_treasury_threshold: 100,
            max_single_spend: 0,
            max_daily_spend: 0,
        });
        let config = FundProtectionConfigs::<Test>::get(1u64).unwrap();
        assert_eq!(config.min_treasury_threshold, 100);
    });
}

#[test]
fn fund_protection_threshold_above_floor_accepted() {
    // min_treasury_threshold > floor → 成功
    ExtBuilder::build().execute_with(|| {
        set_mock_nex_price(500_000_000_000_000_000);
        // floor = 100, set threshold to 500
        execute_proposal_flow(ProposalType::FundProtectionChange {
            min_treasury_threshold: 500,
            max_single_spend: 0,
            max_daily_spend: 0,
        ...
        let config = FundProtectionConfigs::<Test>::get(1u64).unwrap();
        assert_eq!(config.min_treasury_threshold, 500);
    });
}

#[test]
fn fund_protection_price_unavailable_allows_proposal() {
    // 价格不可用时 → 提案校验放行（不因 oracle 故障阻塞治理）
    ExtBuilder::build().execute_with(|| {
        set_mock_nex_price(0); // 价格为 0，不可用
        execute_proposal_flow(ProposalType::FundProtectionChange {
            min_treasury_threshold: 1,
            max_single_spend: 0,
            max_daily_spend: 0,
        });
        let config = FundProtectionConfigs::<Test>::get(1u64).unwrap();
        assert_eq!(config.min_treasury_threshold, 1);
    });
}

#[test]
fn fund_protection_price_stale_allows_proposal() {
    // 价格过期时 → 提案校验放行
    ExtBuilder::build().execute_with(|| {
        set_mock_nex_price(500_000_000_000_000_000);
        set_mock_price_stale(true);
        execute_proposal_flow(ProposalType::FundProtectionChange {
            min_treasury_threshold: 1,
            max_single_spend: 0,
            max_daily_spend: 0,
...
#[test]
fn treasury_spend_blocked_when_below_threshold() {
    // TreasurySpend 执行：支出后余额 < threshold → ExecutionFailed
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
        ...
        // 提案状态应为 ExecutionFailed（TreasuryBelowThreshold 被捕获）
        let proposal = Proposals::<Test>::get(0).unwrap();
        assert_eq!(proposal.status, ProposalStatus::ExecutionFailed);
    });
}
...
#[test]
fn treasury_spend_succeeds_when_above_threshold() {
    // TreasurySpend 执行：支出后余额 >= threshold → 成功
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
...
#[test]
fn treasury_spend_no_config_not_blocked() {
    // 未配置 FundProtectionConfig → 不阻塞
    ExtBuilder::build().execute_with(|| {
        execute_proposal_flow(ProposalType::TreasurySpend {
            amount: 1_000,
            recipient_cid: b"cid1".to_vec().try_into().unwrap(),
...
#[test]
fn treasury_allocate_blocked_when_below_threshold() {
    // TreasuryAllocateToShop 执行：支出后余额 < threshold → ExecutionFailed
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {
...
#[test]
fn max_single_spend_alert_still_nonblocking() {
    // max_single_spend / max_daily_spend → 仍为非阻塞告警（回归测试）
    ExtBuilder::build().execute_with(|| {
        FundProtectionConfigs::<Test>::insert(
            1u64,
            FundProtectionConfig {

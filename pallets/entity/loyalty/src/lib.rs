//! # Loyalty pallet (pallet-entity-loyalty)
//!
//! ## Overview
//!
//! Unified management of the Entity member-loyalty system:
//! 统一管理 Entity 会员忠诚度系统：
//! - **shop points system** (moved from the shop pallet): mint, burn, transfer, redeem,
//!   and expiry handling
//! - **Shop 积分系统**（从 shop 模块搬入）：积分发放、销毁、转移、兑换、过期
//! - **NEX shopping balance** (moved from the commission pallet): spendable credit produced by
//!   commission repurchase / reward flows
//! - **NEX 购物余额**（从 commission 模块搬入）：佣金复购 / 奖励产生的消费额度
//! - **token shopping balance** (moved from `commission/core`): token-denominated spending
//!   credit from token-repurchase flows
//! - **Token 购物余额**（从 `commission/core` 搬入）：Token 返佣复购产生的 Token 消费额度
//!
//! ## Design principles
//!
//! - the points system is moved in completely; loyalty owns all related storage and logic
//! - 积分系统 100% 搬入，loyalty 拥有全部 storage + 逻辑
//! - token operations are delegated through `Config::TokenProvider` rather than moving
//!   token-internal storage
//! - Token 操作通过 `Config::TokenProvider` 委托，不搬 token 内部 storage
//! - NEX shopping-balance storage lives in loyalty, and commission talks to it through
//!   `LoyaltyWritePort`
//! - NEX 购物余额的 storage 搬入 loyalty，commission 通过 `LoyaltyWritePort` 交互
//! - token shopping-balance storage moves from `commission/core` into loyalty, and
//!   commission talks to it through `LoyaltyTokenWritePort`
//! - Token 购物余额的 storage 从 `commission/core` 搬入 loyalty，commission 通过
//!   `LoyaltyTokenWritePort` 交互
...
/// Points-cleanup phase for cursor-based batched cleanup.
/// 积分清理阶段（游标式分批清理）。
#[derive(
    codec::Encode,
    codec::Decode,
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        EntityProvider, EntityTokenProvider, LoyaltyReadPort, LoyaltyTokenReadPort,
        LoyaltyTokenWritePort, LoyaltyWritePort, ShopProvider,
    };
    use sp_runtime::{
    ...
    /// Points configuration type alias.
    /// 积分配置类型别名。
    pub type PointsConfigOf<T> =
        PointsConfig<<T as Config>::MaxPointsNameLength, <T as Config>::MaxPointsSymbolLength>;
    ...
    pub type ShopShoppingTotal<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BalanceOf<T>, ValueQuery>;
    ...
    pub type TokenShoppingTotal<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, TokenBalanceOf<T>, ValueQuery>;

    /// 积分清理游标 shop_id → CleanupPhase（分批清理进度）
    #[pallet::storage]
    pub type PointsCleanupCursor<T: Config> = StorageMap<_, Blake2_128Concat, u64, CleanupPhase>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn disable_points(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn transfer_points(
            ...
            let config =
                ShopPointsConfigs::<T>::get(shop_id).ok_or(Error::<T>::PointsNotEnabled)?;
            ...
            let from_balance = ShopPointsBalances::<T>::get(shop_id, &who);
                ...
                from_balance >= amount,
                Error::<T>::InsufficientPointsBalance
            );
        ...
        pub fn redeem_points(
            ...
            // Shop 运营资金充足性检查（佣金资金在 Entity 账户，不在 Shop 账户）
        ...
        pub fn set_points_ttl(
            ...
            Self::deposit_event(Event::PointsTtlSet {
                shop_id,
                ttl_blocks,
            });
            Ok(())
        ...
        pub fn set_points_max_supply(
            ...
            Self::deposit_event(Event::PointsMaxSupplySet {
                shop_id,
                max_supply,
            });
            Ok(())
        ...
        #[pallet::weight(T::WeightInfo::continue_cleanup())]
        pub fn continue_cleanup(origin: OriginFor<T>, shop_id: u64) -> DispatchResult {
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn ensure_shop_not_closed(shop_id: u64) -> DispatchResult {
            ...
            if let Some(status) = T::ShopProvider::shop_own_status(shop_id) {
                    ...
                    status != ShopOperatingStatus::Closed,
                    Error::<T>::ShopAlreadyClosed
                );
            }
            Ok(())
        ...
        pub fn issue_points(
            ...
            if let Some(status) = T::ShopProvider::shop_own_status(shop_id) {
                    ...
                    status != ShopOperatingStatus::Closing,
                    Error::<T>::ShopAlreadyClosed
                );
            }
        ...
        pub fn cleanup_shop_points(shop_id: u64) {
            ...
            // 分批清理 balances
            let bal_result =
                ShopPointsBalances::<T>::clear_prefix(shop_id, POINTS_CLEANUP_LIMIT, None);
            ...
            // 分批清理 expiry
            let exp_result =
                ShopPointsExpiresAt::<T>::clear_prefix(shop_id, POINTS_CLEANUP_LIMIT, None);
        ...
        pub fn do_consume_shopping_balance(
            ...
            // MEDIUM-3 审计修复: Entity 非 Active 状态下禁止消费购物余额
            ensure!(
                T::EntityProvider::is_entity_active(entity_id),
                Error::<T>::EntityNotActive
            );
        ...
        pub fn do_forfeit_shopping_balance(
            entity_id: u64,
            account: &T::AccountId,
        ) -> DispatchResult {
            let amount = MemberShoppingBalance::<T>::take(entity_id, account);
            if !amount.is_zero() {
                ShopShoppingTotal::<T>::mutate(entity_id, |total| {
                    *total = total.saturating_sub(amount);
                });
        ...
        pub fn do_consume_token_shopping_balance(
            ...
            // MEDIUM-3 审计修复: Entity 非 Active 状态下禁止消费 Token 购物余额
            ensure!(
                T::EntityProvider::is_entity_active(entity_id),
                Error::<T>::EntityNotActive
            );
            ...
            MemberTokenShoppingBalance::<T>::try_mutate(
                ...
                |balance| -> DispatchResult {
                    ...
                    *balance = balance.saturating_sub(amount);
                    ...
                    // Transfer token from entity account to member wallet
                    ...
                    // Only update the total after transfer succeeds, to avoid
                    // permanent divergence when try_mutate rolls back the
                    // member balance but this inner mutate does not.
                    // 仅在 transfer 成功后才扣减总额，避免 try_mutate 回滚
                    // MemberTokenShoppingBalance 但 TokenShoppingTotal 已被
                    // 不可逆扣减导致的永久性偏差。
                    TokenShoppingTotal::<T>::mutate(entity_id, |total| {
                        *total = total.saturating_sub(amount);
                    });
    ...
    impl<T: Config> LoyaltyWritePort<T::AccountId, BalanceOf<T>> for Pallet<T> {
        ...
        fn forfeit_all_shopping_balances(entity_id: u64) {
            // HIGH-2 审计修复: 清零 Entity 所有会员的 NEX 购物余额
            // drain prefix 会删除该 entity_id 下所有 (entity_id, account) 条目
            let _ = MemberShoppingBalance::<T>::clear_prefix(entity_id, u32::MAX, None);
            ShopShoppingTotal::<T>::remove(entity_id);
        }
        ...
        fn forfeit_shopping_balance(
            ...
            Self::do_forfeit_shopping_balance(entity_id, who)
        }
    }
    ...
    impl<T: Config> LoyaltyTokenWritePort<T::AccountId, TokenBalanceOf<T>> for Pallet<T> {
        ...
        fn forfeit_all_token_shopping_balances(entity_id: u64) {
            // HIGH-2 审计修复: 清零 Entity 所有会员的 Token 购物余额
            let _ = MemberTokenShoppingBalance::<T>::clear_prefix(entity_id, u32::MAX, None);
            TokenShoppingTotal::<T>::remove(entity_id);
        }
    }
    ...
    impl<T: Config> pallet_entity_common::ShopGovernancePort for Pallet<T> {
        fn governance_set_points_config(
            ...
            frame_support::ensure!(
                updated > 0,
                sp_runtime::DispatchError::Other("NoPointsConfigFound")
            );
            Ok(())
        }

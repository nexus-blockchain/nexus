...
#[test]
fn rollback_shopping_balance_restores_balance_only() {
        ...
        assert!(
            System::events().is_empty()
                || !System::events().iter().any(|e| {
                    matches!(
                        e.event,
                        RuntimeEvent::Loyalty(Event::ShoppingBalanceRolledBack { .. })
                    )
                })
        );
    });
...
fn rollback_multiple_times_accumulates() {
    new_test_ext().execute_with(|| {
        ...
        // rollback 120
        assert_ok!(Loyalty::do_rollback_shopping_balance(
            ENTITY_ID, &BUYER, 120
        ));
...
fn disable_points_sets_cleanup_cursor() {
    new_test_ext().execute_with(|| {
        ...
        // 禁用 — config 应立即删除
        assert_ok!(Loyalty::disable_points(
            RuntimeOrigin::signed(OWNER),
            SHOP_ID
        ));
        assert!(!ShopPointsConfigs::<Test>::contains_key(SHOP_ID));
...
fn continue_cleanup_works_when_cursor_exists() {
    new_test_ext().execute_with(|| {
        ...
        // 无实际数据 → 一次完成
        assert_ok!(Loyalty::continue_cleanup(
...
fn consume_then_rollback_is_identity() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(balance_of(ENTITY_ACCOUNT), entity_bal_before);
        assert_eq!(
            MemberShoppingBalance::<Test>::get(ENTITY_ID, BUYER),
            member_bal_before
        );

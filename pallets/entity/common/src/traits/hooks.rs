...
/// 等级删除回调（member 删除等级时通知 commission-single-line 清理 override）
pub trait OnLevelRemoved {
    /// 清理指定实体中指定等级的关联数据
    fn on_level_removed(entity_id: u64, level_id: u8);
}

impl OnLevelRemoved for () {
    fn on_level_removed(_entity_id: u64, _level_id: u8) {}
}
...
// ============================================================================
// Member Hook — 会员等级/团队变更通知
// ============================================================================

pub trait OnMemberLevelChanged<AccountId> {
    fn on_level_changed(entity_id: u64, account: &AccountId, old_level: u8, new_level: u8);
}

impl<AccountId> OnMemberLevelChanged<AccountId> for () {
    fn on_level_changed(_entity_id: u64, _account: &AccountId, _old_level: u8, _new_level: u8) {}
}

pub trait OnMemberTeamChanged<AccountId> {
    fn on_team_changed(
        entity_id: u64,
        account: &AccountId,
        old_direct: u32,
...
impl<AccountId> OnMemberTeamChanged<AccountId> for () {
    fn on_team_changed(
        _entity_id: u64,
        _account: &AccountId,
        _old_direct: u32,

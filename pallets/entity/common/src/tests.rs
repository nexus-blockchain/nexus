...
fn entity_type_default_governance() {
    assert_eq!(
        EntityType::Merchant.default_governance(),
        GovernanceMode::None
    );
    assert_eq!(
        EntityType::Enterprise.default_governance(),
        GovernanceMode::MultiSig
    );
    assert_eq!(
        EntityType::DAO.default_governance(),
        GovernanceMode::FullDAO
    );
    assert_eq!(
        EntityType::Community.default_governance(),
        GovernanceMode::Council
    );
    assert_eq!(
        EntityType::Project.default_governance(),
        GovernanceMode::FullDAO
    );
    assert_eq!(
        EntityType::Fund.default_governance(),
        GovernanceMode::Council
    );
    assert_eq!(
        EntityType::ServiceProvider.default_governance(),
        GovernanceMode::None
    );
    assert_eq!(
        EntityType::Custom(99).default_governance(),
        GovernanceMode::None
    );
}
...
fn entity_type_default_token_type() {
    ...
    assert_eq!(
        EntityType::Enterprise.default_token_type(),
        TokenType::Equity
    );
    assert_eq!(
        EntityType::Community.default_token_type(),
...
fn entity_type_default_transfer_restriction() {
    assert_eq!(
        EntityType::Merchant.default_transfer_restriction(),
        TransferRestrictionMode::None
    );
    assert_eq!(
        EntityType::Enterprise.default_transfer_restriction(),
        TransferRestrictionMode::Whitelist
    );
    assert_eq!(
        EntityType::DAO.default_transfer_restriction(),
        TransferRestrictionMode::None
    );
    assert_eq!(
        EntityType::Project.default_transfer_restriction(),
...
fn token_type_default_transfer_restriction_returns_enum() {
    assert_eq!(
        TokenType::Points.default_transfer_restriction(),
        TransferRestrictionMode::None
    );
    assert_eq!(
        TokenType::Membership.default_transfer_restriction(),
        TransferRestrictionMode::MembersOnly
    );
    assert_eq!(
        TokenType::Governance.default_transfer_restriction(),
        TransferRestrictionMode::KycRequired
    );
    assert_eq!(
        TokenType::Equity.default_transfer_restriction(),
        TransferRestrictionMode::Whitelist
    );
    assert_eq!(
        TokenType::Hybrid.default_transfer_restriction(),
        TransferRestrictionMode::None
    );
}
...
fn m2_admin_permission_is_valid_rejects_undefined_bits() {
    ...
    assert!(!AdminPermission::is_valid(
        AdminPermission::SHOP_MANAGE | 0x8000_0000
    ));
}
...
// ============================================================================
// M3: MemberRegistrationPolicy is_valid
// ============================================================================
...
fn l2_try_from_u8_valid_values() {
    assert_eq!(
        TransferRestrictionMode::try_from_u8(0),
        Some(TransferRestrictionMode::None)
    );
    assert_eq!(
        TransferRestrictionMode::try_from_u8(1),
        Some(TransferRestrictionMode::Whitelist)
    );
    assert_eq!(
        TransferRestrictionMode::try_from_u8(2),
        Some(TransferRestrictionMode::Blacklist)
    );
    assert_eq!(
        TransferRestrictionMode::try_from_u8(3),
        Some(TransferRestrictionMode::KycRequired)
    );
    assert_eq!(
        TransferRestrictionMode::try_from_u8(4),
        Some(TransferRestrictionMode::MembersOnly)
    );
}
...
fn admin_permission_new_bits_are_valid() {
    assert!(AdminPermission::is_valid(
        AdminPermission::GOVERNANCE_MANAGE
    ));
...
fn null_entity_provider_returns_defaults() {
    assert!(!<NullEntityProvider as EntityProvider<u64>>::entity_exists(
        1
    ));
    assert!(!<NullEntityProvider as EntityProvider<u64>>::is_entity_active(1));
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::entity_status(1),
        None
    );
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::entity_owner(1),
        None
    );
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::entity_account(1),
        0u64
    );
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::update_entity_stats(1, 100, 1),
        Ok(())
    );
    // default trait methods
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::entity_type(1),
        None
    );
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::register_shop(1, 1),
        Ok(())
    );
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::unregister_shop(1, 1),
        Ok(())
    );
    ...
    assert_eq!(
        <NullEntityProvider as EntityProvider<u64>>::entity_metadata_cid(1),
        None
    );
...
fn null_vesting_provider_returns_defaults() {
    assert_eq!(
        <NullVestingProvider as VestingProvider<u64>>::vesting_balance(1, &1u64),
        0
    );
    assert_eq!(
        <NullVestingProvider as VestingProvider<u64>>::releasable_balance(1, &1u64),
        0
    );
    assert_eq!(
        <NullVestingProvider as VestingProvider<u64>>::release(1, &1u64),
        Ok(0)
    );
    assert_eq!(
        <NullVestingProvider as VestingProvider<u64>>::vesting_schedule(1, &1u64),
        None
    );
    assert!(!<NullVestingProvider as VestingProvider<u64>>::has_vesting(
        1, &1u64
    ));
}
...
fn null_dividend_provider_returns_defaults() {
    assert_eq!(
        <NullDividendProvider as DividendProvider<u64, u128>>::pending_dividend(1, &1u64),
        0
    );
    assert_eq!(
        <NullDividendProvider as DividendProvider<u64, u128>>::claim_dividend(1, &1u64),
        Ok(0)
    );
    assert!(!<NullDividendProvider as DividendProvider<u64, u128>>::is_dividend_active(1));
    assert_eq!(
        <NullDividendProvider as DividendProvider<u64, u128>>::next_distribution_at(1),
        None
    );
    assert_eq!(
        <NullDividendProvider as DividendProvider<u64, u128>>::total_distributed(1),
        0
    );
}
...
fn unit_entity_token_price_provider_returns_defaults() {
    ...
    assert_eq!(
        <() as EntityTokenPriceProvider>::get_token_price_usdt(1),
        None
    );
    assert_eq!(
        <() as EntityTokenPriceProvider>::token_price_confidence(1),
        0
    );
    assert!(<() as EntityTokenPriceProvider>::is_token_price_stale(
        1, 100
    ));
    assert!(!<() as EntityTokenPriceProvider>::is_token_price_reliable(
        1
    ));
}
...
fn null_disclosure_provider_returns_defaults() {
    ...
    assert_eq!(
        <NullDisclosureProvider as DisclosureProvider<u64>>::get_violation_count(1),
        0
    );
    assert_eq!(
        <NullDisclosureProvider as DisclosureProvider<u64>>::get_insider_role(1, &1u64),
        None
    );
...
fn null_product_provider_returns_defaults() {
    ...
    assert_eq!(
        <NullProductProvider as ProductProvider<u64>>::product_shop_id(1),
        None
    );
    assert_eq!(
        <NullProductProvider as ProductProvider<u64>>::product_stock(1),
        None
    );
    assert_eq!(
        <NullProductProvider as ProductProvider<u64>>::product_category(1),
        None
    );
    // trait default methods
    assert_eq!(
        <NullProductProvider as ProductProvider<u64>>::product_status(1),
        None
    );
    assert_eq!(
        <NullProductProvider as ProductProvider<u64>>::product_owner(1),
        None
    );
...
fn null_order_provider_extended_methods() {
    assert_eq!(
        <NullOrderProvider as OrderProvider<u64, u128>>::order_status(1),
        None::<OrderStatus>
    );
    assert_eq!(
        <NullOrderProvider as OrderProvider<u64, u128>>::order_entity_id(1),
        None::<u64>
    );
    assert_eq!(
        <NullOrderProvider as OrderProvider<u64, u128>>::order_product_id(1),
        None::<u64>
    );
    assert_eq!(
        <NullOrderProvider as OrderProvider<u64, u128>>::order_quantity(1),
        None::<u32>
    );
}
...
fn null_shop_provider_ban_unban() {
    ...
    assert_eq!(
        <NullShopProvider as ShopProvider<u64>>::unban_shop(1),
        Ok(())
    );
}
...
fn null_token_sale_provider_returns_defaults() {
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::active_sale_round(1),
        None
    );
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::sale_round_status(1),
        None
    );
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::sold_amount(1),
        None
    );
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::remaining_amount(1),
        None
    );
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::participants_count(1),
        None
    );
    assert!(!<NullTokenSaleProvider as TokenSaleProvider<u128>>::has_active_sale(1));
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::sale_total_supply(1),
        None
    );
    assert_eq!(
        <NullTokenSaleProvider as TokenSaleProvider<u128>>::sale_entity_id(1),
        None
    );
}
...
fn null_entity_token_provider_metadata() {
    assert!(<NullEntityTokenProvider as EntityTokenProvider<u64, u128>>::token_name(1).is_empty());
    assert!(
    ...
    assert!(!<NullEntityTokenProvider as EntityTokenProvider<
        u64,
        u128,
    >>::is_token_transferable(1));
    assert_eq!(
...
fn dispute_resolution_partial_settlement() {
    let ps = DisputeResolution::PartialSettlement {
        complainant_share_bps: 6000,
    };
    assert_ne!(ps, DisputeResolution::Settlement);
    if let DisputeResolution::PartialSettlement {
...
fn dispute_resolution_partial_settlement_full_range() {
    let zero = DisputeResolution::PartialSettlement {
        complainant_share_bps: 0,
    };
    let full = DisputeResolution::PartialSettlement {
...
fn dispute_resolution_is_valid() {
    ...
    assert!(DisputeResolution::PartialSettlement {
        complainant_share_bps: 0
    }
    .is_valid());
    assert!(DisputeResolution::PartialSettlement {
        complainant_share_bps: 5000
    }
    .is_valid());
    assert!(DisputeResolution::PartialSettlement {
        complainant_share_bps: 10000
    ...
    .is_valid());
    assert!(!DisputeResolution::PartialSettlement {
        complainant_share_bps: 10001
    }
    .is_valid());
    assert!(!DisputeResolution::PartialSettlement {
        complainant_share_bps: u16::MAX
    }
    .is_valid());
}
...
fn dispute_resolution_complainant_share_bps() {
    assert_eq!(
        DisputeResolution::ComplainantWin.complainant_share_bps(),
        10000
    );
...
fn null_review_provider_returns_defaults() {
    assert_eq!(
        <NullReviewProvider as ReviewProvider<u64>>::shop_average_rating(1),
        0
    );
    assert_eq!(
        <NullReviewProvider as ReviewProvider<u64>>::shop_review_count(1),
        0
    );
    assert_eq!(
        <NullReviewProvider as ReviewProvider<u64>>::product_average_rating(1),
        0
    );
    assert_eq!(
        <NullReviewProvider as ReviewProvider<u64>>::product_review_count(1),
        0
    );
    ...
    assert_eq!(
        <NullReviewProvider as ReviewProvider<u64>>::user_review_count(1, &1u64),
        0
    );
}
...
fn null_market_provider_returns_defaults() {
    ...
    assert_eq!(
        <NullMarketProvider as MarketProvider<u64, u128>>::trading_volume_24h(1),
        0
    );
    assert_eq!(
        <NullMarketProvider as MarketProvider<u64, u128>>::best_bid(1),
        None
    );
    assert_eq!(
        <NullMarketProvider as MarketProvider<u64, u128>>::best_ask(1),
        None
    );
    assert_eq!(
        <NullMarketProvider as MarketProvider<u64, u128>>::user_active_order_count(1, &1u64),
        0
    );
...
fn null_disclosure_read_provider_returns_defaults() {
    assert!(!<NullDisclosureReadProvider as DisclosureReadProvider<
        u64,
    >>::is_in_blackout(1));
    assert!(!<NullDisclosureReadProvider as DisclosureReadProvider<
        u64,
...
fn null_disclosure_write_provider_defaults() {
    ...
    assert!(
        <NullDisclosureWriteProvider as DisclosureWriteProvider<u64>>::governance_reset_violations(
            1
        )
        .is_err()
    );
    assert_eq!(
        <NullDisclosureWriteProvider as DisclosureWriteProvider<u64>>::register_major_holder(
            1, &1u64
        ),
        Ok(())
    );
    assert_eq!(
        <NullDisclosureWriteProvider as DisclosureWriteProvider<u64>>::governance_set_penalty_level(
            1, 2
        ),
        Ok(())
    );
...
fn disclosure_provider_bridges_to_read_provider() {
    ...
    assert_eq!(
        <NullDisclosureProvider as DisclosureReadProvider<u64>>::get_violation_count(1),
        0
    );
...
fn disclosure_provider_bridges_to_write_provider() {
    ...
    assert_eq!(
        <NullDisclosureProvider as DisclosureWriteProvider<u64>>::governance_set_penalty_level(
            1, 0
        ),
        Ok(())
    );
...
fn null_member_query_provider_returns_defaults() {
    ...
    assert_eq!(
        <NullMemberQueryProvider as MemberQueryProvider<u64>>::get_referrer(1, &1u64),
        None
    );
    assert_eq!(
        <NullMemberQueryProvider as MemberQueryProvider<u64>>::custom_level_id(1, &1u64),
        0
    );
    assert_eq!(
        <NullMemberQueryProvider as MemberQueryProvider<u64>>::get_level_commission_bonus(1, 0),
        0
    );
    assert!(!<NullMemberQueryProvider as MemberQueryProvider<u64>>::uses_custom_levels(1));
    assert_eq!(
        <NullMemberQueryProvider as MemberQueryProvider<u64>>::get_member_stats(1, &1u64),
        MemberStats {
            direct_referrals: 0,
            team_size: 0,
...
fn member_provider_bridges_to_query_provider() {
    ...
    assert_eq!(
        <NullMemberProvider as MemberQueryProvider<u64>>::get_referrer(1, &1u64),
        None
    );
    assert_eq!(
        <NullMemberProvider as MemberQueryProvider<u64>>::member_count(1),
        0
    );
...
fn null_entity_provider_ownership_transfer_defaults() {
    ...
    assert!(<NullEntityProvider as EntityProvider<u64>>::cancel_ownership_transfer(1).is_err());
}

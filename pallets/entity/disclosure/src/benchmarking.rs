...
fn seed_published_disclosure<T: Config>(entity_id: u64, who: &T::AccountId) -> u64 {
    ...
    EntityDisclosures::<T>::mutate(entity_id, |h| {
        let _ = h.try_push(id);
    });
    id
}
...
fn seed_announcement<T: Config>(entity_id: u64, who: &T::AccountId) -> u64 {
    ...
    EntityAnnouncements::<T>::mutate(entity_id, |h| {
        let _ = h.try_push(id);
    });
    id
}
...
fn seed_expiring_announcement<T: Config>(
    entity_id: u64,
    who: &T::AccountId,
...
mod benchmarks {
    ...
    fn cleanup_disclosure_history() {
        ...
        let did = seed_published_disclosure::<T>(ENTITY_1, &caller);
        // 标记为 Withdrawn 使其可清理
        Disclosures::<T>::mutate(did, |r| {
            if let Some(rec) = r {
                rec.status = DisclosureStatus::Withdrawn;
            }
        });
        #[extrinsic_call]
    ...
    fn cleanup_announcement_history() {
        ...
        let aid = seed_announcement::<T>(ENTITY_1, &caller);
        // 标记为 Withdrawn 使其可清理
        Announcements::<T>::mutate(aid, |r| {
            if let Some(rec) = r {
                rec.status = AnnouncementStatus::Withdrawn;
            }
        });
        #[extrinsic_call]
        _(RawOrigin::Signed(caller), ENTITY_1, aid);
    ...
    fn unpin_announcement() {
        ...
        let aid = seed_announcement::<T>(ENTITY_1, &caller);
        // 先置顶
        PinnedAnnouncements::<T>::mutate(ENTITY_1, |p| {
            let _ = p.try_push(aid);
        });
        Announcements::<T>::mutate(aid, |r| {
            if let Some(rec) = r {
                rec.is_pinned = true;
    ...
    fn batch_add_insiders() {
        ...
        let list: BoundedVec<(T::AccountId, InsiderRole), T::MaxInsiders> = vec![
            (insider1, InsiderRole::Admin),
            (insider2, InsiderRole::Auditor),
        ]
        .try_into()
        .unwrap();
        #[extrinsic_call]
    ...
    fn set_disclosure_metadata() {
        ...
        _(
            ...
            did,
            Some(now),
            Some(now.saturating_add(100u32.into())),
            true,
        );
    }

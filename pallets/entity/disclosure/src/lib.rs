//! # Entity disclosure pallet (pallet-entity-disclosure)
//!
//! ## Overview
//!
//! This pallet implements financial-disclosure workflows for entities:
//! 本模块实现实体财务信息披露功能：
//! - multi-level disclosure requirements (basic / standard / enhanced / full)
//! - 多级别披露要求（基础 / 标准 / 增强 / 完全）
//! - multiple disclosure types (financial reports, material events, related-party trades, etc.)
//! - 多种披露类型（财务报告、重大事件、关联交易等）
//! - insider-trading control (blackout windows and trading restrictions)
//! - 内幕交易控制（黑窗口期、交易限制）
//! - disclosure verification and audit tracking
//! - 披露验证和审计追踪
//!
//! ## Disclosure levels
//!
//! - **Basic**: annual summary reporting
//! - **Basic**：基础披露（年度简报）
//! - **Standard**: quarterly reporting
//! - **Standard**：标准披露（季度报告）
//! - **Enhanced**: monthly reporting plus material events
//! - **Enhanced**：增强披露（月度报告 + 重大事件）
//! - **Full**: real-time and detailed financial disclosure
//! - **Full**：完全披露（实时 + 详细财务）
//!
//! ## Version history
//!
//! - v0.1.0 (2026-02-03): initial Phase 6 release
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        AdminPermission, DisclosureProvider, EntityProvider, EntityStatus, OnDisclosureViolation,
    ...
    pub struct DisclosureRecord<AccountId, BlockNumber, MaxCidLen: Get<u32>> {
        ...
        /// Disclosure content CID stored in IPFS.
        /// 披露内容 CID（IPFS）
        pub content_cid: BoundedVec<u8, MaxCidLen>,
        /// Optional disclosure summary CID.
        /// 披露摘要 CID
        ...
        /// Previous disclosure ID linked to this record for corrections.
        /// 关联的前一个披露 ID（用于更正）
    ...
    /// Entity disclosure configuration.
    /// 实体披露配置。
    #[derive(
        Encode,
        Decode,
    ...
    pub struct DisclosureConfig<BlockNumber> {
        ...
        /// Whether insider-trading controls are enabled.
        /// 是否启用内幕交易控制
        pub insider_trading_control: bool,
        /// Post-disclosure blackout window in blocks.
        /// 披露后黑窗口期（区块数）
        pub blackout_period_after: BlockNumber,
        /// Next required disclosure deadline.
        /// 下次必须披露时间
        pub next_required_disclosure: BlockNumber,
        /// Last disclosure time.
        /// 上次披露时间
        pub last_disclosure: BlockNumber,
        /// Consecutive violation count, incremented by `report_disclosure_violation`.
    ...
    pub type InsiderTransactionReportOf<T> =
        InsiderTransactionReport<<T as frame_system::Config>::AccountId, BlockNumberFor<T>>;
    ...
    pub type ViolationReportOf<T> =
        ViolationReport<<T as frame_system::Config>::AccountId, BlockNumberFor<T>>;

    /// 内幕人员记录类型别名
    pub type InsiderRecordOf<T> =
        InsiderRecord<<T as frame_system::Config>::AccountId, BlockNumberFor<T>>;
    ...
    pub type BlackoutPeriods<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, (BlockNumberFor<T>, BlockNumberFor<T>)>;
    ...
    pub enum Event<T: Config> {
        ...
        /// P2-a2: 草稿已更新
        DraftUpdated { disclosure_id: u64 },
        /// P2-a2: 草稿已删除
        DraftDeleted { disclosure_id: u64, entity_id: u64 },
        /// 披露已发布
        DisclosurePublished {
            disclosure_id: u64,
            entity_id: u64,
        ...
        /// 披露已撤回
        DisclosureWithdrawn { disclosure_id: u64, entity_id: u64 },
        /// 披露已更正
        DisclosureCorrected {
        ...
        /// P1-a20: 实体披露存储已清理
        EntityDisclosureCleaned { entity_id: u64 },
        ...
        /// 公告已取消置顶
        AnnouncementUnpinned { entity_id: u64 },
        /// 公告已标记为过期
        ...
        /// M1-R3: 披露历史索引已清理
        DisclosureHistoryCleaned { entity_id: u64, disclosure_id: u64 },
        /// M1-R3: 公告历史索引已清理
        ...
        /// F1: 批量添加内幕人员
        InsidersBatchAdded { entity_id: u64, count: u32 },
        /// F1: 批量移除内幕人员
        InsidersBatchRemoved { entity_id: u64, count: u32 },
        /// F4: 内幕人员冷静期开始
        ...
        /// F8: 违规次数已重置
        ViolationCountReset { entity_id: u64 },
        /// F9: 黑窗口期已过期清理
        BlackoutExpired { entity_id: u64 },
        ...
        /// 处罚已重置
        PenaltyReset { entity_id: u64 },
        /// 财务年度已配置
        FiscalYearConfigured { entity_id: u64 },
        /// 过期冷静期已清理
        CooldownsCleaned { entity_id: u64, count: u32 },
        /// 披露截止时间已暂停（实体被暂停/封禁时）
        DeadlinePaused { entity_id: u64 },
        /// 披露截止时间已恢复（实体恢复运营时）
        DeadlineResumed {
            entity_id: u64,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn configure_disclosure(
            ...
            T::EntityProvider::entity_owner(entity_id).ok_or(Error::<T>::EntityNotFound)?;
                ...
                T::EntityProvider::is_entity_admin(
                    entity_id,
                    &who,
                    AdminPermission::DISCLOSURE_MANAGE
                ),
                Error::<T>::NotAdmin
            );
            ensure!(
            ...
            let existing_last = existing
                .as_ref()
                .map(|c| c.last_disclosure)
                .unwrap_or_else(Zero::zero);

            DisclosureConfigs::<T>::insert(
                entity_id,
                DisclosureConfig {
                    level,
            ...
            Self::deposit_event(Event::DisclosureConfigUpdated { entity_id, level });
            Ok(())
        }
        ...
        pub fn publish_disclosure(
            ...
            NextDisclosureId::<T>::put(disclosure_id.checked_add(1).ok_or(Error::<T>::IdOverflow)?);

            // 更新实体披露历史
            EntityDisclosures::<T>::try_mutate(entity_id, |history| -> DispatchResult {
                history
                    .try_push(disclosure_id)
                    .map_err(|_| Error::<T>::HistoryFull)?;
                Ok(())
            })?;
            ...
            DisclosureConfigs::<T>::mutate(entity_id, |maybe_config| {
                if let Some(config) = maybe_config {
                    ...
                    config.next_required_disclosure =
                        Self::calculate_next_disclosure(config.level, now);
        ...
        pub fn update_draft(
            ...
            Disclosures::<T>::try_mutate(disclosure_id, |maybe_record| -> DispatchResult {
                let record = maybe_record
                    ...
                    record.status == DisclosureStatus::Draft,
                    Error::<T>::DisclosureNotDraft
                );

                T::EntityProvider::entity_owner(record.entity_id)
                    .ok_or(Error::<T>::EntityNotFound)?;
                ensure!(
                    T::EntityProvider::is_entity_active(record.entity_id),
                    Error::<T>::EntityNotActive
                );
                ensure!(
                    T::EntityProvider::is_entity_admin(
                    ...
                    !T::EntityProvider::is_entity_locked(record.entity_id),
                    Error::<T>::EntityLocked
                );
        ...
        pub fn delete_draft(origin: OriginFor<T>, disclosure_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let record =
                Disclosures::<T>::get(disclosure_id).ok_or(Error::<T>::DisclosureNotFound)?;
            ensure!(
                record.status == DisclosureStatus::Draft,
                Error::<T>::DisclosureNotDraft
            );
            ...
            Self::deposit_event(Event::DraftDeleted {
                disclosure_id,
                entity_id,
            });
            Ok(())
        ...
        pub fn publish_draft(origin: OriginFor<T>, disclosure_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            let entity_id = Disclosures::<T>::try_mutate(
                disclosure_id,
                |maybe_record| -> Result<u64, DispatchError> {
                    let record = maybe_record
                        .as_mut()
                        .ok_or(Error::<T>::DisclosureNotFound)?;
                    ensure!(
                    ...
                    if let Some(approval_config) = ApprovalConfigs::<T>::get(record.entity_id) {
                        if approval_config.required_approvals > 0 {
                            let count = DisclosureApprovalCounts::<T>::get(disclosure_id);
                                ...
                                count >= approval_config.required_approvals,
        ...
        pub fn withdraw_disclosure(origin: OriginFor<T>, disclosure_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Disclosures::<T>::try_mutate(disclosure_id, |maybe_record| -> DispatchResult {
                ...
                // 验证状态
                    ...
                    record.status == DisclosureStatus::Published,
                    Error::<T>::InvalidDisclosureStatus
                );
                ...
                DisclosureConfigs::<T>::mutate(entity_id, |maybe_config| {
                    if let Some(config) = maybe_config {
                        if config.last_disclosure == disclosed_at {
                            ...
                            let grace = (T::BasicDisclosureInterval::get() / 4u32.into())
                                .min(100u32.into());
        ...
        pub fn correct_disclosure(
            ...
            let old_record =
                Disclosures::<T>::get(old_disclosure_id).ok_or(Error::<T>::DisclosureNotFound)?;

            // H2: 只能更正 Published 状态的记录
            ensure!(
                old_record.status == DisclosureStatus::Published,
                Error::<T>::InvalidDisclosureStatus
            );
            ...
            T::EntityProvider::entity_owner(old_record.entity_id)
                ...
                T::EntityProvider::is_entity_active(old_record.entity_id),
                Error::<T>::EntityNotActive
            );
            ensure!(
                T::EntityProvider::is_entity_admin(
                    old_record.entity_id,
                    &who,
                    AdminPermission::DISCLOSURE_MANAGE
                ),
                Error::<T>::NotAdmin
                ...
                !T::EntityProvider::is_entity_locked(old_record.entity_id),
                Error::<T>::EntityLocked
            );
            ...
            NextDisclosureId::<T>::put(
                new_disclosure_id
                    .checked_add(1)
                    .ok_or(Error::<T>::IdOverflow)?,
            );

            // 更新历史
            EntityDisclosures::<T>::try_mutate(
                old_record.entity_id,
                |history| -> DispatchResult {
                    history
                        .try_push(new_disclosure_id)
                        .map_err(|_| Error::<T>::HistoryFull)?;
                    Ok(())
                },
            )?;
        ...
        pub fn add_insider(
            ...
            Insiders::<T>::try_mutate(entity_id, |insiders| -> DispatchResult {
                ...
                insiders
                    .try_push(record)
                    .map_err(|_| Error::<T>::InsidersFull)?;
                Ok(())
            })?;

            // P2-a23: 记录初始角色历史
            InsiderRoleHistory::<T>::try_mutate(
                entity_id,
                &account,
                |history| -> DispatchResult {
                    ...
                    history
                        .try_push(record)
                        .map_err(|_| Error::<T>::RoleHistoryFull)?;
                    Ok(())
                },
            )?;
        ...
        pub fn update_insider_role(
            ...
            let old_role = Insiders::<T>::try_mutate(
                entity_id,
                |insiders| -> Result<InsiderRole, DispatchError> {
                    let insider = insiders
                        .iter_mut()
        ...
        pub fn remove_insider(
            ...
            Self::deposit_event(Event::InsiderRemoved { entity_id, account });
            Ok(())
        }
        ...
        pub fn start_blackout(
            ...
            let max_blackout = T::MaxBlackoutDuration::get();
                ...
                !duration.is_zero() && duration <= max_blackout,
                Error::<T>::InvalidBlackoutDuration
            );
        ...
        pub fn end_blackout(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            // L1: 检查黑窗口期是否存在
                ...
                BlackoutPeriods::<T>::contains_key(entity_id),
                Error::<T>::BlackoutNotFound
            );
        ...
        pub fn update_announcement(
            ...
            Announcements::<T>::try_mutate(announcement_id, |maybe_record| -> DispatchResult {
                ...
                T::EntityProvider::entity_owner(record.entity_id)
                    ...
                    record.status == AnnouncementStatus::Active,
                    Error::<T>::AnnouncementNotActive
                );
        ...
        pub fn withdraw_announcement(origin: OriginFor<T>, announcement_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn pin_announcement(
            ...
            let mut record =
                Announcements::<T>::get(announcement_id).ok_or(Error::<T>::AnnouncementNotFound)?;
            ensure!(
                record.entity_id == entity_id,
                Error::<T>::AnnouncementNotFound
            );
            ensure!(
                record.status == AnnouncementStatus::Active,
                Error::<T>::AnnouncementNotActive
            ...
            PinnedAnnouncements::<T>::try_mutate(entity_id, |pinned| -> DispatchResult {
                if !pinned.contains(&announcement_id) {
                    pinned
                        .try_push(announcement_id)
                        .map_err(|_| Error::<T>::PinnedAnnouncementsFull)?;
                }
                Ok(())
            })?;
        ...
        pub fn unpin_announcement(
            ...
            PinnedAnnouncements::<T>::try_mutate(entity_id, |pinned| -> DispatchResult {
                let pos = pinned
                    .iter()
                    .position(|&id| id == announcement_id)
        ...
        pub fn expire_announcement(origin: OriginFor<T>, announcement_id: u64) -> DispatchResult {
            ...
            Announcements::<T>::try_mutate(announcement_id, |maybe_record| -> DispatchResult {
                ...
                let exp = record
                    .expires_at
                    .ok_or(Error::<T>::AnnouncementNotExpired)?;
        ...
        pub fn force_configure_disclosure(
            ...
            Self::deposit_event(Event::DisclosureForceConfigured { entity_id, level });
            Ok(())
        }
            ...
            match violation_type {
                ViolationType::LateDisclosure => {
                    // A-2: 暂停截止时间的实体不可报告逾期违规
                        ...
                        !PausedDeadlines::<T>::contains_key(entity_id),
                        Error::<T>::DeadlineAlreadyPaused
                    );
                    ensure!(
                        now > config.next_required_disclosure,
                        Error::<T>::DisclosureNotOverdue
                    );
        ...
        pub fn cleanup_entity_disclosure(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_signed(origin)?;
        ...
        pub fn batch_add_insiders(
            ...
            Self::deposit_event(Event::InsidersBatchAdded {
                entity_id,
                count: added,
            });
            Ok(())
        ...
        pub fn batch_remove_insiders(
            ...
            Self::deposit_event(Event::InsidersBatchRemoved {
                entity_id,
                count: removed_count,
            });
            Ok(())
        ...
        pub fn reset_violation_count(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn expire_blackout(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_signed(origin)?;

            let (_, end) =
                BlackoutPeriods::<T>::get(entity_id).ok_or(Error::<T>::BlackoutNotFound)?;
        ...
        pub fn approve_disclosure(origin: OriginFor<T>, disclosure_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            let approval_config =
                ApprovalConfigs::<T>::get(entity_id).ok_or(Error::<T>::ApprovalNotConfigured)?;
            ...
            let insider_role = Insiders::<T>::get(entity_id)
                ...
                approval_config.role_allowed(insider_role),
                Error::<T>::NotApprover
            );
        ...
        pub fn reject_disclosure(origin: OriginFor<T>, disclosure_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn publish_emergency_disclosure(
            ...
            DisclosureMetadataStore::<T>::insert(
                disclosure_id,
                DisclosureMetadata {
                    period_start: None,
                    period_end: None,
            ...
            DisclosureConfigs::<T>::mutate(entity_id, |maybe_config| {
                if let Some(config) = maybe_config {
                    ...
                    if !config.blackout_period_after.is_zero() {
                        let multiplier: BlockNumberFor<T> =
                            T::EmergencyBlackoutMultiplier::get().into();
        ...
        pub fn report_insider_transaction(
            ...
            InsiderTransactionReports::<T>::try_mutate(
                entity_id,
                &who,
                |reports| -> DispatchResult {
                    reports
                        .try_push(report)
                        .map_err(|_| Error::<T>::TransactionHistoryFull)?;
                    Ok(())
                },
            )?;
        ...
        pub fn reset_penalty(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn set_disclosure_metadata(
            ...
            let merged_start =
                period_start.or_else(|| existing_meta.as_ref().and_then(|m| m.period_start));
            let merged_end =
                period_end.or_else(|| existing_meta.as_ref().and_then(|m| m.period_end));
        ...
        pub fn audit_disclosure(
            ...
            let insider_role = Insiders::<T>::get(entity_id)
                ...
                insider_role == InsiderRole::Auditor,
                Error::<T>::NotApprover
            );

            DisclosureMetadataStore::<T>::try_mutate(
                disclosure_id,
                |maybe_meta| -> DispatchResult {
                    let meta = maybe_meta.as_mut().ok_or(Error::<T>::DisclosureNotFound)?;
                    ensure!(
                        meta.audit_status == AuditStatus::Pending,
                        Error::<T>::InvalidDisclosureStatus
                    );

                    meta.audit_status = if approved {
        ...
        pub fn confirm_violation_report(origin: OriginFor<T>, report_id: u64) -> DispatchResult {
            ensure_root(origin)?;

            ViolationReports::<T>::try_mutate(report_id, |maybe_report| -> DispatchResult {
                let report = maybe_report
                    .as_mut()
                    .ok_or(Error::<T>::ViolationReportNotFound)?;
                ensure!(
                    report.status == ViolationReportStatus::Pending,
                    Error::<T>::ViolationReportNotPending
                );
        ...
        pub fn reject_violation_report(origin: OriginFor<T>, report_id: u64) -> DispatchResult {
            ensure_root(origin)?;
    ...
    impl<T: Config> Pallet<T> {
        /// 计算下次必须披露时间
        pub fn calculate_next_disclosure(
            level: DisclosureLevel,
        ...
        fn do_register_major_holder(
            ...
            Insiders::<T>::try_mutate(entity_id, |insiders| -> sp_runtime::DispatchResult {
                ...
                InsiderRoleHistory::<T>::try_mutate(
                    entity_id,
                    account,
                    |history| -> sp_runtime::DispatchResult {
        ...
        fn do_deregister_major_holder(
            ...
            Insiders::<T>::try_mutate(entity_id, |insiders| -> sp_runtime::DispatchResult {
                let pos = insiders
                    .iter()
                    .position(|i| &i.account == account && i.role == InsiderRole::MajorHolder);
                if let Some(idx) = pos {
    ...
    impl<T: Config> DisclosureProvider<T::AccountId> for Pallet<T> {
        ...
        fn governance_configure_disclosure(
            ...
            Pallet::<T>::deposit_event(Event::DisclosureConfigUpdated { entity_id, level });
            Ok(())
        }

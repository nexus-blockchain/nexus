...
fn remove_insider_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityDisclosure::remove_insider(RuntimeOrigin::signed(OWNER), ENTITY_ID, ALICE,),
...
fn calculate_next_disclosure_intervals() {
    new_test_ext().execute_with(|| {
        let now = 100u64;
        assert_eq!(
            Pallet::<Test>::calculate_next_disclosure(DisclosureLevel::Basic, now),
            1100
        );
        assert_eq!(
            Pallet::<Test>::calculate_next_disclosure(DisclosureLevel::Standard, now),
            600
        );
        assert_eq!(
            Pallet::<Test>::calculate_next_disclosure(DisclosureLevel::Enhanced, now),
            200
        );
        assert_eq!(
            Pallet::<Test>::calculate_next_disclosure(DisclosureLevel::Full, now),
            u64::MAX
        ); // max_value, never overdue
    });
}
...
fn get_disclosure_level_default() {
    new_test_ext().execute_with(|| {
        assert_eq!(
            Pallet::<Test>::get_disclosure_level(ENTITY_ID),
            DisclosureLevel::Basic
        );
    });
}
...
fn h2_correct_disclosure_rejects_empty_cid() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityDisclosure::correct_disclosure(RuntimeOrigin::signed(OWNER), 0, vec![], None,),
            Error::<Test>::EmptyCid
        );
    });
}
...
fn m3_start_blackout_respects_config_max() {
    new_test_ext().execute_with(|| {
        // MaxBlackoutDuration = 200
        assert_noop!(
            EntityDisclosure::start_blackout(RuntimeOrigin::signed(OWNER), ENTITY_ID, 201u64,),
            Error::<Test>::InvalidBlackoutDuration
        );
        // 零时长也不允许
        assert_noop!(
            EntityDisclosure::start_blackout(RuntimeOrigin::signed(OWNER), ENTITY_ID, 0u64,),
            Error::<Test>::InvalidBlackoutDuration
        );
        // 恰好 200 应成功
        assert_ok!(EntityDisclosure::start_blackout(
...
fn pin_announcement_fails_not_owner() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityDisclosure::pin_announcement(RuntimeOrigin::signed(ALICE), ENTITY_ID, 0,),
            Error::<Test>::NotAdmin
        );
    });
}
...
fn pin_announcement_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityDisclosure::pin_announcement(RuntimeOrigin::signed(OWNER), ENTITY_ID, 999,),
            Error::<Test>::AnnouncementNotFound
        );
    });
}
...
fn pin_announcement_fails_wrong_entity() {
    new_test_ext().execute_with(|| {
        ...
        // 尝试在 ENTITY_ID_2 下置顶
        assert_noop!(
            EntityDisclosure::pin_announcement(RuntimeOrigin::signed(OWNER_2), ENTITY_ID_2, 0,),
            Error::<Test>::AnnouncementNotFound
        );
    });
}
...
fn pin_announcement_fails_withdrawn() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityDisclosure::pin_announcement(RuntimeOrigin::signed(OWNER), ENTITY_ID, 0,),
            Error::<Test>::AnnouncementNotActive
        );
    });
}
...
fn get_active_pinned_announcements_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Pallet::<Test>::get_active_pinned_announcements(ENTITY_ID),
            vec![0]
        );
    });
}
...
fn m1_deep_update_announcement_works_before_expiry() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityDisclosure::update_announcement(
            RuntimeOrigin::signed(OWNER),
            0,
            Some(b"Updated Sale".to_vec()),
            None,
            None,
            None,
        ));
        assert_eq!(
            Announcements::<Test>::get(0).unwrap().title.to_vec(),
            b"Updated Sale".to_vec()
        );
    });
}
...
fn m2_deep_expire_announcement_clears_pin() {
    new_test_ext().execute_with(|| {
        ...
        assert!(!PinnedAnnouncements::<Test>::get(ENTITY_ID).contains(&0));
        assert_eq!(
            Announcements::<Test>::get(0).unwrap().status,
...
fn m1r2_start_blackout_cannot_shorten_active() {
    new_test_ext().execute_with(|| {
        ...
        // 尝试用更短的 50 区块覆盖 — 应被拒绝
        assert_noop!(
            EntityDisclosure::start_blackout(RuntimeOrigin::signed(OWNER), ENTITY_ID, 50u64,),
...
fn m3r2_get_pinned_announcements_filters_expired() {
    new_test_ext().execute_with(|| {
        ...
        // 过期前 — 应返回
        assert_eq!(
            EntityDisclosure::get_active_pinned_announcements(ENTITY_ID),
            vec![0]
        );
...
fn m1_deep_auto_blackout_does_not_shorten_existing() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            ...
            "auto-blackout should NOT shorten existing longer blackout"
        );
    });
}
...
fn m1_deep_correct_disclosure_blackout_does_not_shorten() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            actual_end, manual_end,
            "correct_disclosure auto-blackout should NOT shorten existing"
        );
    });
}
...
fn m1r3_cleanup_disclosure_history_accepts_corrected() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Disclosures::<Test>::get(0).unwrap().status,
...
fn p0a7_report_late_disclosure_works() {
    new_test_ext().execute_with(|| {
        ...
        // 事件验证
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::DisclosureViolation {
            entity_id: ENTITY_ID,
...
fn p1a20_cleanup_works_for_closed_entity() {
    new_test_ext().execute_with(|| {
        ...
        // 活跃实体不能清理
        assert_noop!(
            EntityDisclosure::cleanup_entity_disclosure(RuntimeOrigin::signed(ALICE), ENTITY_ID,),
...
fn p1a16_on_idle_no_duplicate_in_same_period() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            DisclosureConfigs::<Test>::get(ENTITY_ID)
                .unwrap()
                .violation_count,
            1
        );
...
fn p2a2_create_draft_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::DraftCreated {
            disclosure_id: 0,
            entity_id: ENTITY_ID,
...
fn p2a2_delete_draft_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::DraftDeleted {
            disclosure_id: 0,
            entity_id: ENTITY_ID,
        }));
    });
...
fn p2a2_publish_draft_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::DisclosurePublished {
            disclosure_id: 0,
            entity_id: ENTITY_ID,
            disclosure_type: DisclosureType::AnnualReport,
            discloser: OWNER,
...
fn p2a3_update_insider_role_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::InsiderRoleUpdated {
            entity_id: ENTITY_ID,
            account: ALICE,
...
fn f1_batch_add_insiders_works() {
    new_test_ext().execute_with(|| {
        let batch: BoundedVec<_, <Test as crate::Config>::MaxInsiders> =
        ...
        assert!(insiders
            .iter()
            .any(|i| i.account == ALICE && i.role == InsiderRole::Admin));
        assert!(insiders
            .iter()
            .any(|i| i.account == BOB && i.role == InsiderRole::Auditor));

        // 角色历史也应记录
        assert_eq!(InsiderRoleHistory::<Test>::get(ENTITY_ID, ALICE).len(), 1);
        assert_eq!(InsiderRoleHistory::<Test>::get(ENTITY_ID, BOB).len(), 1);

        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::InsidersBatchAdded {
            entity_id: ENTITY_ID,
            count: 2,
        }));
    });
...
fn f1_batch_add_insiders_rejects_empty() {
    new_test_ext().execute_with(|| {
        let empty: BoundedVec<(u64, InsiderRole), <Test as crate::Config>::MaxInsiders> =
            BoundedVec::default();
        assert_noop!(
            EntityDisclosure::batch_add_insiders(RuntimeOrigin::signed(OWNER), ENTITY_ID, empty,),
...
fn f6_high_risk_queryable_via_provider() {
    new_test_ext().execute_with(|| {
        ...
        assert!(<EntityDisclosure as DisclosureProvider<u64>>::is_high_risk(
            ENTITY_ID
        ));
    });
}
...
fn f7_get_violation_count_works() {
    new_test_ext().execute_with(|| {
        use pallet_entity_common::DisclosureProvider;
        assert_eq!(
            <EntityDisclosure as DisclosureProvider<u64>>::get_violation_count(ENTITY_ID),
            0
        );
...
fn f7_get_insider_role_works() {
    new_test_ext().execute_with(|| {
        use pallet_entity_common::DisclosureProvider;
        assert_eq!(
            <EntityDisclosure as DisclosureProvider<u64>>::get_insider_role(ENTITY_ID, &ALICE),
            None
        );
...
fn f7_is_disclosure_configured_works() {
    new_test_ext().execute_with(|| {
        use pallet_entity_common::DisclosureProvider;
        assert!(
            !<EntityDisclosure as DisclosureProvider<u64>>::is_disclosure_configured(ENTITY_ID)
        );
...
fn f8_reset_violation_count_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::ViolationCountReset {
            entity_id: ENTITY_ID,
        }));
    });
}
...
fn f8_reset_violation_count_rejects_non_root() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityDisclosure::reset_violation_count(RuntimeOrigin::signed(OWNER), ENTITY_ID,),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn f8_reset_violation_count_rejects_entity_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityDisclosure::reset_violation_count(RuntimeOrigin::root(), 999,),
            Error::<Test>::EntityNotFound
        );
    });
}
...
fn f9_expire_blackout_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::EntityDisclosure(Event::BlackoutExpired {
            entity_id: ENTITY_ID,
        }));
    });
}
...
fn f9_expire_blackout_rejects_still_active() {
    new_test_ext().execute_with(|| {
        ...
        // 黑窗口期仍活跃
        assert_noop!(
            EntityDisclosure::expire_blackout(RuntimeOrigin::signed(ALICE), ENTITY_ID,),
...
fn h1r2_on_idle_scans_all_entities() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            DisclosureConfigs::<Test>::get(ENTITY_ID_2)
                .unwrap()
                .violation_count,
            1
        );
    ...
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityDisclosure::configure_approval_requirements(
                ...
                ENTITY_ID,
                2,
                0, // roles=0 invalid when required>0
            ),
...
fn v06_reject_disclosure_resets_approvals() {
    new_test_ext().execute_with(|| {
        ...
        // Auditor 拒绝 → 重置
        assert_ok!(EntityDisclosure::reject_disclosure(
            RuntimeOrigin::signed(ALICE),
            0
        ));
        assert_eq!(DisclosureApprovalCounts::<Test>::get(0), 0);
...
fn v06_register_major_holder_works() {
    new_test_ext().execute_with(|| {
        ...
        // 通过 trait 注册大股东
        assert_ok!(
            <EntityDisclosure as DisclosureProvider<u64>>::register_major_holder(ENTITY_ID, &ALICE,)
        );
...
fn v06_deregister_major_holder_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            <EntityDisclosure as DisclosureProvider<u64>>::deregister_major_holder(
                ENTITY_ID, &ALICE,
            )
        );
        assert!(!EntityDisclosure::is_insider(ENTITY_ID, &ALICE));
...
fn v06_auto_penalty_escalation() {
    new_test_ext().execute_with(|| {
        ...
        let count = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .violation_count;
        assert_eq!(count, 3);
        ...
        assert_eq!(
            EntityPenalties::<Test>::get(ENTITY_ID),
        ...
        assert_eq!(
            <EntityDisclosure as DisclosureProvider<u64>>::get_penalty_level(ENTITY_ID),
            2
        );
    });
}
...
fn v06_reset_penalty_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityDisclosure::reset_penalty(
            RuntimeOrigin::root(),
            ENTITY_ID
        ));
...
fn v06_audit_disclosure_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            DisclosureMetadataStore::<Test>::get(0)
                .unwrap()
                .audit_status,
            AuditStatus::Pending,
        );
...
fn a3_reconfigure_same_level_no_deadline_push() {
    new_test_ext().execute_with(|| {
        ...
        let first_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
        ...
        let second_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .next_required_disclosure;
...
fn a3_upgrade_picks_earlier_deadline() {
    new_test_ext().execute_with(|| {
        ...
        let basic_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .next_required_disclosure;
        ...
        let enhanced_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .next_required_disclosure;
...
fn a3_root_rebase_works() {
    new_test_ext().execute_with(|| {
        ...
        let old_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .next_required_disclosure;
        ...
        let new_deadline = DisclosureConfigs::<Test>::get(ENTITY_ID)
            .unwrap()
            .next_required_disclosure;
...
fn a5_governance_set_penalty_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(
            <EntityDisclosure as DisclosureProvider<u64>>::governance_set_penalty_level(
                ENTITY_ID, 2
            )
        );
        assert_eq!(
...
fn c1_confirm_non_pending_fails() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityDisclosure::confirm_violation_report(RuntimeOrigin::root(), 0,),
...
fn e1_withdraw_restores_obligation() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            config_after_withdraw.next_required_disclosure
                < config_after_publish.next_required_disclosure
        );
    });
}

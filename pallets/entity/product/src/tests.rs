...
fn update_product_soldout_to_onsale_on_restock() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityProduct as ProductProvider<u64>>::deduct_stock(
            0, 100
        ));
...
fn publish_product_from_offshelf_works() {
    new_test_ext().execute_with(|| {
        create_default_product();
        assert_ok!(EntityProduct::publish_product(RuntimeOrigin::signed(1), 0));
        assert_ok!(EntityProduct::unpublish_product(
            RuntimeOrigin::signed(1),
            0
        ));
...
fn unpublish_soldout_product_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Products::<Test>::get(0).unwrap().status,
...
fn product_provider_basic_queries() {
    new_test_ext().execute_with(|| {
        ...
        assert!(!<EntityProduct as ProductProvider<u64>>::product_exists(
            999
        ));

        assert!(<EntityProduct as ProductProvider<u64>>::is_product_on_sale(
            0
        ));
...
fn restore_stock_works() {
    new_test_ext().execute_with(|| {
        ...
        // SoldOut 后恢复库存
        assert_ok!(<EntityProduct as ProductProvider<u64>>::restore_stock(
            0, 50
        ));
...
fn add_sold_count_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityProduct as ProductProvider<u64>>::add_sold_count(
            0, 5
        ));
        assert_ok!(<EntityProduct as ProductProvider<u64>>::add_sold_count(
            0, 3
...
fn m2_deduct_stock_emits_stock_updated_event() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "StockUpdated event should be emitted by deduct_stock"
        );
    });
}
...
fn m2_restore_stock_emits_stock_updated_event() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "StockUpdated event should be emitted by restore_stock"
        );
    });
}
...
fn h2_add_sold_count_emits_sold_count_updated_event() {
    new_test_ext().execute_with(|| {
        ...
        let found_sold = events.iter().any(|record| {
            matches!(
                record.event,
                RuntimeEvent::EntityProduct(Event::SoldCountUpdated {
                    product_id: 0,
                    sold_count: 5
                })
            )
        ...
        assert!(
            !found_stock,
            "StockUpdated should NOT be emitted by add_sold_count"
        );
    });
}
...
fn ipfs_update_product_unchanged_fields_no_pin() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            get_pinned_cids().is_empty(),
            "no CID changed, no pin expected"
        );
        assert!(
            get_unpinned_cids().is_empty(),
            "no CID changed, no unpin expected"
        );
    });
}
...
fn h1_delete_product_succeeds_when_pallet_insolvent() {
    new_test_ext().execute_with(|| {
        ...
        let pallet_account: u64 =
            sp_runtime::traits::AccountIdConversion::<u64>::into_account_truncating(
                &frame_support::PalletId(*b"et/prod/"),
            );
        let pallet_balance = Balances::free_balance(pallet_account);
        if pallet_balance > 0 {
            let _ = <Balances as frame_support::traits::Currency<u64>>::slash(
                &pallet_account,
                pallet_balance,
            );
        }
...
fn m2_create_product_fails_insufficient_balance_with_ed() {
    new_test_ext().execute_with(|| {
        ...
        if current > 0 {
            let _ =
                <Balances as frame_support::traits::Currency<u64>>::slash(&shop_account, current);
        }
        let _ = <Balances as frame_support::traits::Currency<u64>>::deposit_creating(
            &shop_account,
            deposit + ed - 1,
        );
...
fn batch_publish_products_works() {
    new_test_ext().execute_with(|| {
        ...
        for i in 0..3 {
            assert_eq!(
                Products::<Test>::get(i).unwrap().status,
                ProductStatus::OnSale
            );
        }
        assert_eq!(ProductStats::<Test>::get().on_sale_products, 3);
...
fn batch_publish_fails_too_large() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityProduct::batch_publish_products(RuntimeOrigin::signed(1), ids,),
...
fn governance_update_price_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(<EntityProduct as ProductProvider<u64>>::update_usdt_price(
            0, 200_000
        ));
...
fn governance_set_inventory_works() {
    new_test_ext().execute_with(|| {
        ...
        // 设置库存为 200
        assert_ok!(<EntityProduct as ProductProvider<u64>>::set_inventory(
            0, 200
        ));
...
fn batch_publish_best_effort_partial_success() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Products::<Test>::get(1).unwrap().status,
            ProductStatus::OnSale
        );
...
fn m1_force_unpublish_rejects_reason_too_long_before_mutation() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityProduct::force_unpublish_product(RuntimeOrigin::root(), 0, Some(long_reason),),
...
fn m1_batch_publish_empty_list_noop() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            ...
            "empty batch should not emit BatchCompleted"
        );
    });
}
...
fn create_product_empty_tags_sku_no_extra_pin() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            pinned.len(),
            3,
            "empty tags/sku should not trigger extra pins"
        );
    });
}
...
fn update_product_tags_cid_pins_and_unpins() {
    new_test_ext().execute_with(|| {
        ...
        let pinned = get_pinned_cids();
        assert!(
            unpinned.contains(&b"QmOldTags".to_vec()),
            "old tags should be unpinned"
        );
        assert!(
            pinned.iter().any(|(_, c)| c == &b"QmNewTags".to_vec()),
            "new tags should be pinned"
        );
    });
}
...
fn update_product_sku_cid_pins_and_unpins() {
    new_test_ext().execute_with(|| {
        ...
        let pinned = get_pinned_cids();
        assert!(
            unpinned.contains(&b"QmOldSku".to_vec()),
            "old sku should be unpinned"
        );
        assert!(
            pinned.iter().any(|(_, c)| c == &b"QmNewSku".to_vec()),
            "new sku should be pinned"
        );
    });
}
...
fn update_product_clear_tags_unpins_only() {
    new_test_ext().execute_with(|| {
        ...
        let pinned = get_pinned_cids();
        assert!(
            unpinned.contains(&b"QmTags".to_vec()),
            "old tags should be unpinned"
        );
...
fn force_delete_product_fails_not_root() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityProduct::force_delete_product(RuntimeOrigin::signed(1), 0, None,),
            sp_runtime::DispatchError::BadOrigin
        );
    });
}
...
fn force_delete_product_fails_not_found() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            EntityProduct::force_delete_product(RuntimeOrigin::root(), 999, None,),
            Error::<Test>::ProductNotFound
        );
    });
}
...
fn force_delete_product_reason_too_long() {
    new_test_ext().execute_with(|| {
        ...
        let long_reason = vec![0u8; 257];
        assert_noop!(
            EntityProduct::force_delete_product(RuntimeOrigin::root(), 0, Some(long_reason),),
            Error::<Test>::ReasonTooLong
        );
        assert!(Products::<Test>::get(0).is_some());
    });
...
fn update_product_rejects_category_change_on_sale() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Products::<Test>::get(0).unwrap().category,
            ProductCategory::Physical
        );
    });
}
...
fn force_delete_product_offshelf() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityProduct::force_delete_product(
            RuntimeOrigin::root(),
            0,
            Some(b"policy violation".to_vec()),
        ));
...
fn delist_product_offshelf_is_noop() {
    new_test_ext().execute_with(|| {
        ...
        let status_events: Vec<_> = System::events()
            .into_iter()
            .filter(|e| {
                matches!(
                    e.event,
        ...
        assert!(
            ...
            "OffShelf delist should not emit status event"
        );
    });
}
...
fn m1_delist_product_onsale_emits_status_event() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            found,
            "delist OnSale product should emit ProductStatusChanged"
        );
    });
}
...
fn l2_update_product_same_cid_no_pin_unpin() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            get_pinned_cids().is_empty(),
            "same-value CID should not trigger pin"
        );
        assert!(
            get_unpinned_cids().is_empty(),
            "same-value CID should not trigger unpin"
        );
    });
}
...
fn h2_update_product_cid_failure_no_orphan_unpin() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            get_pinned_cids().is_empty(),
            "rollback should not cause orphan pins"
        );
        assert!(
            get_unpinned_cids().is_empty(),
            "rollback should not cause orphan unpins"
        );
...
fn v12_force_delist_all_shop_products_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Products::<Test>::get(2).unwrap().status,
            ProductStatus::SoldOut
        );
        assert_eq!(ProductStats::<Test>::get().on_sale_products, 1);

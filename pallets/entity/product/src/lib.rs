//! # Entity product pallet (pallet-entity-product)
//!
//! ## Overview
//!
//! This pallet manages the product lifecycle, including:
//! 本模块负责商品的生命周期管理，包括：
//! - product creation (deducts a deposit from the shop-derived account)
//! - 商品创建（从店铺派生账户扣取押金）
//! - product information updates
//! - 商品信息更新
//! - listing / delisting
//! - 商品上架 / 下架
//! - product deletion (refunds the deposit back to the shop-derived account)
//! - 商品删除（退还押金到店铺派生账户）
...
//! ## Deposit model
//!
//! - creating a product deducts 1 USDT worth of NEX from the shop-derived account
//! - 创建商品时从店铺派生账户扣取 1 USDT 等值 NEX
//! - the deposit is held in the pallet account
//! - 押金转入 Pallet 账户托管
//! - deleting the product refunds the deposit back to the shop-derived account
...
//! - v0.2.0 (2026-02-01): implemented the shop-derived-account deposit model
...
pub mod pallet {
    ...
    use pallet_entity_common::{
        AdminPermission, EntityProvider, PricingProvider, ProductCategory, ProductProvider,
        ProductStatus, ProductVisibility, ShopProvider,
    };
    ...
    /// Product type alias.
    /// 商品类型别名。
    pub type ProductOf<T> = Product<BlockNumberFor<T>, <T as Config>::MaxCidLength>;
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        ...
        fn integrity_test() {
            ...
            assert!(T::MaxReasonLength::get() > 0, "MaxReasonLength must be > 0");
            assert!(
    ...
    pub type ShopProducts<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, BoundedVec<u64, T::MaxProductsPerShop>, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 店铺全部在售商品已下架（店铺封禁时触发）
        ShopProductsDelisted { shop_id: u64, count: u32 },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_product(
            ...
            if stock > 0 && min_order_quantity > 0 {
                ...
                !matches!(
                    category,
                    ProductCategory::Subscription | ProductCategory::Bundle
                ),
            ...
            let ed = T::Currency::minimum_balance();
                ...
                shop_balance >= deposit.saturating_add(ed),
                Error::<T>::InsufficientShopFund
            );
            ...
            let next_id = product_id
                .checked_add(1)
                .ok_or(Error::<T>::ArithmeticOverflow)?;
            NextProductId::<T>::put(next_id);

            // 记录押金信息
            ProductDeposits::<T>::insert(
                product_id,
                ProductDepositInfo {
                    shop_id,
                    amount: deposit,
        ...
        pub fn update_product(
            ...
            let has_any_change = name_cid.is_some()
                || images_cid.is_some()
                || detail_cid.is_some()
                || usdt_price.is_some()
                || stock.is_some()
                ...
                || tags_cid.is_some()
                || sku_cid.is_some()
                || min_order_quantity.is_some()
                || max_order_quantity.is_some()
                || visibility.is_some();
        ...
        pub fn delete_product(origin: OriginFor<T>, product_id: u64) -> DispatchResult {
            ...
            let deposit_refunded =
                if let Some(deposit_info) = ProductDeposits::<T>::take(product_id) {
                    let pallet_account: T::AccountId = PRODUCT_PALLET_ID.into_account_truncating();
    ...
    impl<T: Config> Pallet<T> {
        ...
        /// 计算商品押金（ProductDepositUsdt 等值 NEX）
        ///
        /// # 算法
        /// 1. 获取 NEX/USDT 价格（精度 10^6）
        /// 2. 计算所需 NEX = USDT 金额 * 10^12 / 价格
        /// 3. 价格不可用 → PriceUnavailable 错误（拒绝操作）
        pub fn calculate_product_deposit() -> Result<BalanceOf<T>, sp_runtime::DispatchError> {
            ...
            Ok(nex_amount_u128.saturated_into())
        }

        /// IPFS Pin 商品 CID（best-effort：失败仅记录日志，不阻断业务流程）
        /// H1-fix: 统一使用 shop_owner 作为 pin owner，确保 unpin 时 owner 一致
        fn pin_product_cid(shop_id: u64, product_id: u64, cid: &BoundedVec<u8, T::MaxCidLength>) {
        ...
        fn unpin_product_cid(shop_id: u64, cid: &BoundedVec<u8, T::MaxCidLength>) {
            let owner = T::ShopProvider::shop_owner(shop_id)
                .unwrap_or_else(|| T::ShopProvider::shop_account(shop_id));
    ...
    impl<T: Config> ProductProvider<T::AccountId> for Pallet<T> {
        ...
        fn deduct_stock(product_id: u64, quantity: u32) -> Result<(), sp_runtime::DispatchError> {
            Products::<T>::try_mutate(
                product_id,
                |maybe_product| -> Result<(), sp_runtime::DispatchError> {
                    let product = maybe_product.as_mut().ok_or(Error::<T>::ProductNotFound)?;
                    ensure!(
                        product.status == ProductStatus::OnSale,
                        Error::<T>::InvalidProductStatus
        ...
        fn product_owner(product_id: u64) -> Option<T::AccountId> {
            Products::<T>::get(product_id).and_then(|p| T::ShopProvider::shop_owner(p.shop_id))
        }
            ...
            if removed_count > 0 {
                ProductStats::<T>::mutate(|stats| {
                    stats.total_products =
                        stats.total_products.saturating_sub(removed_count as u64);
        ...
        fn governance_set_visibility(
            product_id: u64,
            visibility: pallet_entity_common::ProductVisibility,
        ) -> Result<(), sp_runtime::DispatchError> {
            Products::<T>::try_mutate(
                product_id,
                |maybe_product| -> Result<(), sp_runtime::DispatchError> {

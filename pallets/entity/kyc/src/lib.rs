//! # Entity KYC / AML pallet (pallet-entity-kyc)
//!
//! ## Overview
//!
//! This pallet implements KYC for regular users inside an Entity.
//! 本模块实现普通用户在 Entity 内的 KYC（了解你的客户）认证。
//! The same account keeps separate KYC records and states across different entities.
//! 同一账户在不同 Entity 拥有独立的 KYC 记录和状态。
//!
//! ## Per-Entity verification model
//!
//! - users submit KYC requests to a specific Entity
//! - 用户向特定 Entity 提交 KYC 申请
//! - authorized providers or the Entity owner / admin review the request
//! - Entity 授权的 Provider 或 Entity Owner / Admin 审核
//! - each `(Entity, Account)` pair has an independent KYC record
//! - 每个 `(Entity, Account)` 对拥有独立的 KYC 记录
//! - KYC state does not propagate between entities
//! - Entity 之间的 KYC 状态互不影响
//! - upgrade requests are stored separately in `UpgradeRequests` and do not overwrite
...
//! - **Basic**：基础认证（邮箱 / 手机验证）
//! - **Standard**: standard verification (identity documents)
//! - **Standard**：标准认证（身份证件）
//! - **Enhanced**: enhanced verification (address proof + source of funds)
//! - **Enhanced**：增强认证（地址证明 + 资金来源）
//! - **Institutional**: institutional verification (corporate docs + beneficial owners)
...
pub mod pallet {
    ...
    pub type KycProviderOf<T> = KycProvider<<T as Config>::MaxProviderNameLength>;
    ...
    pub type KycUpgradeRequestOf<T> =
        KycUpgradeRequest<BlockNumberFor<T>, <T as Config>::MaxCidLength>;
    ...
    pub type KycRecords<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, u64, Blake2_128Concat, T::AccountId, KycRecordOf<T>>;

    #[pallet::storage]
    #[pallet::getter(fn providers)]
    pub type Providers<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, KycProviderOf<T>>;
    ...
    #[pallet::storage]
    pub type EntityAuthorizedProviders<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, u64, Blake2_128Concat, T::AccountId, ()>;

    #[pallet::storage]
    #[pallet::getter(fn entity_requirements)]
    pub type EntityRequirements<T: Config> =
        StorageMap<_, Blake2_128Concat, u64, EntityKycRequirement>;

    #[pallet::storage]
    #[pallet::getter(fn high_risk_countries)]
    pub type HighRiskCountries<T: Config> =
        StorageValue<_, BoundedVec<[u8; 2], ConstU32<50>>, ValueQuery>;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn submit_kyc(
            ...
            let (is_upgrade, current_level) =
                if let Some(record) = KycRecords::<T>::get(entity_id, &who) {
                    ensure!(
        ...
        pub fn approve_kyc(
            ...
            if let Some(upgrade) = UpgradeRequests::<T>::get(entity_id, &account) {
                if let Some(max_level) = provider_max_level {
                        ...
                        upgrade.target_level <= max_level,
                        Error::<T>::ProviderLevelNotSupported
                    );
                }
        ...
        pub fn register_provider(
            ...
            T::AdminOrigin::ensure_origin(origin)?;
                ...
                !Providers::<T>::contains_key(&provider_account),
        ...
        pub fn update_high_risk_countries(
            ...
            let bounded: BoundedVec<[u8; 2], ConstU32<50>> = deduped
                .try_into()
                .map_err(|_| Error::<T>::TooManyCountries)?;
        ...
        pub fn cancel_kyc(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn update_provider(
            ...
            T::AdminOrigin::ensure_origin(origin)?;
                ...
                name.is_some() || max_level.is_some(),
        ...
        pub fn update_kyc_data(
            ...
            let data_bounded: BoundedVec<u8, T::MaxCidLength> = new_data_cid
                .try_into()
                .map_err(|_| Error::<T>::CidTooLong)?;

            if UpgradeRequests::<T>::contains_key(entity_id, &who) {
                UpgradeRequests::<T>::try_mutate(
                    entity_id,
                    &who,
                    |maybe_upgrade| -> DispatchResult {
        ...
        pub fn purge_kyc_data(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn remove_entity_requirement(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;

            Self::ensure_entity_operable(entity_id)?;
            Self::ensure_entity_owner_or_admin(entity_id, &who)?;
            ensure!(
                EntityRequirements::<T>::contains_key(entity_id),
        ...
        pub fn timeout_pending_kyc(
            ...
            if let Some(upgrade) = UpgradeRequests::<T>::get(entity_id, &account) {
                ...
                let timeout = T::PendingKycTimeout::get();
                    ...
                    now > upgrade.submitted_at.saturating_add(timeout),
                    Error::<T>::PendingNotTimedOut
                );
            ...
            KycRecords::<T>::try_mutate(entity_id, &account, |maybe_record| -> DispatchResult {
                ...
                let timeout = T::PendingKycTimeout::get();
                    ...
                    now > submitted_at.saturating_add(timeout),
                    Error::<T>::PendingNotTimedOut
                );
        ...
        pub fn authorize_provider(
            ...
            Self::ensure_entity_operable(entity_id)?;
            Self::ensure_entity_owner_or_admin(entity_id, &who)?;
            Self::do_authorize_provider(entity_id, provider_account)
        }
        ...
        pub fn deauthorize_provider(
            ...
            Self::ensure_entity_operable(entity_id)?;
            Self::ensure_entity_owner_or_admin(entity_id, &who)?;
            Self::do_deauthorize_provider(entity_id, provider_account)
        }
        ...
        pub fn entity_revoke_kyc(
            ...
            Self::ensure_entity_operable(entity_id)?;
            // 审计修复: 使用统一 helper（含 EntityLocked 检查），替代内联权限检查
            Self::ensure_entity_owner_or_admin(entity_id, &who)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn ensure_can_review(
            entity_id: u64,
            who: &T::AccountId,
        ) -> Result<Option<KycLevel>, DispatchError> {
        ...
        fn do_authorize_provider(entity_id: u64, provider_account: T::AccountId) -> DispatchResult {
            ensure!(
                Providers::<T>::contains_key(&provider_account),
                Error::<T>::ProviderNotFound
            );
            ...
            ProviderAuthorizedEntities::<T>::try_mutate(
                &provider_account,
                |entities| -> DispatchResult {
                    entities
                        .try_push(entity_id)
        ...
        pub(crate) fn update_status_counts(
            entity_id: u64,
            ...
            match old_status {
                ...
                KycStatus::Approved => {
                    ApprovedKycCount::<T>::mutate(entity_id, |c| *c = c.saturating_sub(1))
                }
                _ => {}
            }
            match new_status {
        ...
        pub fn get_kyc_stats(entity_id: u64) -> (u32, u32) {
            (
                PendingKycCount::<T>::get(entity_id),
                ApprovedKycCount::<T>::get(entity_id),
            )
        }
    ...
    impl<T: Config> pallet_entity_common::KycGovernancePort<T::AccountId> for Pallet<T> {
        ...
        fn governance_authorize_kyc_provider(
            ...
            Self::ensure_entity_operable(entity_id)?;
            Self::do_authorize_provider(entity_id, provider)
        }
        ...
        fn governance_deauthorize_kyc_provider(
            ...
            Self::ensure_entity_operable(entity_id)?;
            Self::do_deauthorize_provider(entity_id, provider)
        }
    }
}

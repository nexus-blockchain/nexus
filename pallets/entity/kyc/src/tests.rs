...
fn remove_provider_cleans_up_authorizations() {
    new_test_ext().execute_with(|| {
        ...
        assert!(EntityAuthorizedProviders::<Test>::contains_key(
            ENTITY_1, PROVIDER
        ));
        assert!(EntityAuthorizedProviders::<Test>::contains_key(
            ENTITY_2, PROVIDER
...
fn deauthorize_provider_fails_not_authorized() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            EntityKyc::deauthorize_provider(
                RuntimeOrigin::signed(ENTITY_OWNER),
                ENTITY_1,
                PROVIDER
            ),
...
fn submit_kyc_allowed_after_rejection() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::reject_kyc(
            ...
            ENTITY_1,
            USER,
            RejectionReason::UnclearDocument,
            None,
        ));
        assert_ok!(EntityKyc::submit_kyc(
            RuntimeOrigin::signed(USER),
...
fn upgrade_preserves_permissions_during_review() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Pallet::<Test>::get_kyc_level(ENTITY_1, &USER),
            KycLevel::Basic
        );
        assert!(Pallet::<Test>::meets_kyc_requirement(
            ENTITY_1,
            &USER,
            KycLevel::Basic
        ));
...
fn upgrade_timeout_keeps_original_level() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::timeout_pending_kyc(
            RuntimeOrigin::signed(USER),
            ENTITY_1,
            USER
        ));
...
fn upgrade_update_data_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            upgrade.submitted_at, 10,
            "submitted_at must not be reset by update_kyc_data"
        );
    });
}
...
fn different_entities_have_independent_kyc() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Pallet::<Test>::get_kyc_level(ENTITY_2, &USER),
            KycLevel::None
        );
...
fn revoke_in_one_entity_does_not_affect_another() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::revoke_kyc(
            ...
            ENTITY_1,
            USER,
            RejectionReason::SuspiciousActivity
        ));

        assert_eq!(
            KycRecords::<Test>::get(ENTITY_1, USER).unwrap().status,
            KycStatus::Revoked
        );
        assert_eq!(
            KycRecords::<Test>::get(ENTITY_2, USER).unwrap().status,
            KycStatus::Approved
        );
    });
}
...
fn approve_kyc_by_entity_admin_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::approve_kyc(
            RuntimeOrigin::signed(ENTITY_ADMIN_USER),
            ENTITY_1,
            USER,
            10,
        ));
...
fn entity_revoke_kyc_by_admin_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::entity_revoke_kyc(
            ...
            ENTITY_1,
            USER,
            RejectionReason::ForgedDocument,
        ));
...
fn update_high_risk_countries_deduplicates() {
    new_test_ext().execute_with(|| {
        let countries: BoundedVec<[u8; 2], ConstU32<50>> =
            vec![*b"IR", *b"IR", *b"KP"].try_into().unwrap();
        assert_ok!(EntityKyc::update_high_risk_countries(
            RuntimeOrigin::root(),
            countries
        ));
...
fn renew_kyc_works_for_approved() {
    new_test_ext().execute_with(|| {
        ...
        let old_expires = KycRecords::<Test>::get(ENTITY_1, USER)
            .unwrap()
            .expires_at
            .unwrap();

        System::set_block_number(100);
        assert_ok!(EntityKyc::renew_kyc(
            RuntimeOrigin::signed(PROVIDER),
            ENTITY_1,
            USER
        ));
...
fn purge_kyc_data_works_for_rejected() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(EntityKyc::purge_kyc_data(
            RuntimeOrigin::signed(USER),
            ENTITY_1
        ));
...
fn batch_revoke_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            KycRecords::<Test>::get(ENTITY_1, USER2).unwrap().status,
            KycStatus::Revoked
        );
    });
}
...
fn update_risk_score_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            KycRecords::<Test>::get(ENTITY_1, USER).unwrap().risk_score,
            50
        );
    });
}
...
fn update_provider_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Providers::<Test>::get(PROVIDER).unwrap().name.to_vec(),
            b"New Name".to_vec()
        );
    });
}
...
fn kyc_provider_trait_uses_entity_id() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            <Pallet<Test> as KycProvider<u64>>::kyc_level(ENTITY_1, &USER),
            3
        );
        assert_eq!(
            <Pallet<Test> as KycProvider<u64>>::kyc_level(ENTITY_2, &USER),
            0
        );
        assert!(<Pallet<Test> as KycProvider<u64>>::is_kyc_approved(
            ENTITY_1, &USER
        ));
        assert!(!<Pallet<Test> as KycProvider<u64>>::is_kyc_approved(
            ENTITY_2, &USER
...
fn kyc_provider_is_kyc_expired_works() {
    new_test_ext().execute_with(|| {
        ...
        use pallet_entity_common::KycProvider;
        assert!(!<Pallet<Test> as KycProvider<u64>>::is_kyc_expired(
            ENTITY_1, &USER
        ));
...
fn kyc_provider_can_participate_uses_entity_logic() {
    new_test_ext().execute_with(|| {
        ...
        use pallet_entity_common::KycProvider;
        assert!(!<Pallet<Test> as KycProvider<u64>>::can_participate(
            ENTITY_1, &USER
        ));
...
fn kyc_provider_expires_at_works() {
    new_test_ext().execute_with(|| {
        ...
        use pallet_entity_common::KycProvider;
        assert_eq!(
            <Pallet<Test> as KycProvider<u64>>::kyc_expires_at(ENTITY_1, &USER),
            0
        );
...
fn compliance_non_mandatory_always_passes() {
    new_test_ext().execute_with(|| {
        ...
        assert!(Pallet::<Test>::check_account_compliance(
            ENTITY_1, &USER, &req
        ));
    });
}

...
impl<T: Config> Pallet<T> {
    ...
    pub(crate) fn do_request_refund(
        ...
        let payment_asset = Orders::<T>::try_mutate(
            order_id,
            |maybe_order| -> Result<PaymentAsset, DispatchError> {
    ...
    /// 同意退款核心逻辑（卖家）
    pub(crate) fn do_approve_refund(who: T::AccountId, order_id: u64) -> DispatchResult {
    ...
    pub(crate) fn do_reject_refund(
        ...
        Self::deposit_event(Event::RefundRejected {
            order_id,
    ...
    /// 管理员强制退款核心逻辑（Root / 治理）
    pub(crate) fn do_force_refund(order_id: u64, reason_cid: Option<Vec<u8>>) -> DispatchResult {
        ...
        Self::deposit_event(Event::OrderForceRefunded {
            order_id,
            reason_cid: reason,
        });
        Self::deposit_event(Event::OrderRefunded {
    ...
    pub(crate) fn do_force_complete(order_id: u64, reason_cid: Option<Vec<u8>>) -> DispatchResult {
        let reason = Self::validate_optional_reason_cid(&reason_cid)?;
        ...
        Self::deposit_event(Event::OrderForceCompleted {
            order_id,
            reason_cid: reason,
        });
        Ok(())
    ...
    pub(crate) fn do_force_partial_refund(
        ...
        reason_cid: Option<Vec<u8>>,
            ...
            refund_bps >= 1 && refund_bps <= 9999,
            Error::<T>::InvalidRefundBps
        );
        let reason = Self::validate_optional_reason_cid(&reason_cid)?;
        ...
        // 部分退款仍全额回滚忠诚度资产（购物余额/Token折扣不可部分恢复）
        ...
        Self::deposit_event(Event::OrderPartialRefunded {
            order_id,
            refund_bps,
            reason_cid: reason,
        });
    ...
    pub(crate) fn do_withdraw_dispute(who: T::AccountId, order_id: u64) -> DispatchResult {
        let order = Orders::<T>::get(order_id).ok_or(Error::<T>::OrderNotFound)?;
        ensure!(
            Self::is_order_participant(&order, &who),
            Error::<T>::NotOrderParticipant

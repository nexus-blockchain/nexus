...
use pallet_grouprobot_primitives::{
    BotRegistryProvider, CommunityIdHash, NodeConsensusProvider, NodeId, RewardAccruer,
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type StakeTiers<T: Config> = StorageValue<_, BoundedVec<(u128, u32), ConstU32<10>>>;
    ...
    pub enum Event<T: Config> {
        ...
        /// TEE 节点广告分成百分比已更新
        TeeAdPercentUpdated { tee_pct: u32 },
        /// 社区广告分成百分比已更新
        CommunityAdPercentUpdated { community_pct: u32 },
        /// 社区管理员已变更
        ...
        /// audience 突增暂停已恢复
        AudienceSurgeResumed { community_id_hash: CommunityIdHash },
        /// L5: 多节点偏差过大, 拒结
        ...
        /// 社区管理员暂停广告
        AdsPausedByAdmin { community_id_hash: CommunityIdHash },
        /// 社区管理员恢复广告
        AdsResumedByAdmin { community_id_hash: CommunityIdHash },
        /// 质押阶梯已更新
        StakeTiersUpdated { tier_count: u32 },
        /// 社区管理员已辞职
        ...
        /// 全局广告暂停状态变更
        GlobalAdsPauseToggled { paused: bool },
        /// Bot 广告开关变更
        BotAdsToggled {
            community_id_hash: CommunityIdHash,
            disabled: bool,
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn stake_for_ads(
            ...
            if existing.is_zero() {
                let count = CommunityStakerCount::<T>::get(&community_id_hash);
                    ...
                    count < T::MaxStakersPerCommunity::get(),
        ...
        pub fn set_tee_ad_pct(origin: OriginFor<T>, tee_pct: Option<u32>) -> DispatchResult {
            ensure_root(origin)?;
            ...
            Self::deposit_event(Event::TeeAdPercentUpdated {
                tee_pct: Self::effective_tee_pct(),
            });
            Ok(())
        }
        ...
        pub fn set_community_ad_pct(
            ...
            Self::deposit_event(Event::CommunityAdPercentUpdated {
                community_pct: Self::effective_community_pct(),
            });
            Ok(())
        }
        ...
        pub fn report_node_audience(
            ...
            let who = ensure_signed(origin)?;
                ...
                T::NodeConsensus::is_node_active(&node_id),
        ...
        pub fn cross_validate_nodes(
            ...
            if let Err((min_audience, max_audience)) =
                Self::validate_node_reports(&community_id_hash)
            {
        ...
        pub fn admin_pause_ads(
            ...
            Self::ensure_admin_or_bot_owner(&who, &community_id_hash)?;
                ...
                !AdminPausedAds::<T>::get(&community_id_hash),
        ...
        pub fn force_set_community_admin(
            ...
            ensure_root(origin)?;
            CommunityAdmin::<T>::insert(&community_id_hash, new_admin.clone());
            Self::deposit_event(Event::ForceAdminSet {
                community_id_hash,
                new_admin,
            });
            Ok(())
        ...
        pub fn set_global_ads_pause(origin: OriginFor<T>, paused: bool) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn set_bot_ads_enabled(
            ...
            Self::deposit_event(Event::BotAdsToggled {
                community_id_hash,
                disabled,
            });
            Ok(())
    ...
    impl<T: Config> RevenueDistributor<T::AccountId, BalanceOf<T>> for Pallet<T> {
        fn distribute(
            ...
            frame_support::ensure!(!GlobalAdsPaused::<T>::get(), Error::<T>::GlobalAdsPausedErr);
            ...
            if !staker_pool.is_zero() {
                ...
                if !total_stake.is_zero() {
                    ...
                    for (staker, staker_balance) in stakers {
                        ...
                        let staker_reward =
                            staker_pool.saturating_mul(staker_balance) / total_stake;

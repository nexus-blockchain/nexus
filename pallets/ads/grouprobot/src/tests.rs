...
fn set_tee_ad_pct_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(AdsGroupRobot::set_tee_ad_pct(
            RuntimeOrigin::root(),
            Some(20)
        ));
...
fn set_community_ad_pct_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(AdsGroupRobot::set_community_ad_pct(
            RuntimeOrigin::root(),
            Some(70)
        ));
...
fn set_community_admin_fails_not_authorized() {
    new_test_ext().execute_with(|| {
        let ch = community_hash(1);
        assert_noop!(
            AdsGroupRobot::set_community_admin(RuntimeOrigin::signed(STAKER2), ch, STAKER,),
...
fn report_node_audience_fails_not_operator() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            AdsGroupRobot::report_node_audience(RuntimeOrigin::signed(STAKER), ch, nid, 100,),
...
fn validate_node_reports_passes_within_threshold() {
    new_test_ext().execute_with(|| {
        let ch = community_hash(1);
        NodeAudienceReports::<Test>::insert(
            &ch,
            BoundedVec::<(u32, u32), ConstU32<10>>::try_from(vec![(1, 100), (2, 115)]).unwrap(),
        );
...
fn validate_node_reports_fails_high_deviation() {
    new_test_ext().execute_with(|| {
        ...
        NodeAudienceReports::<Test>::insert(
            &ch,
            BoundedVec::<(u32, u32), ConstU32<10>>::try_from(vec![(1, 100), (2, 150)]).unwrap(),
        );
...
fn delivery_verifier_caps_audience() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <Pallet<Test> as pallet_ads_primitives::DeliveryVerifier<u64>>::verify_and_cap_audience(
                &TEE_NODE_OPERATOR,
                &ch,
                500,
                None,
...
fn placement_admin_from_storage() {
    new_test_ext().execute_with(|| {
        ...
        let admin =
            <Pallet<Test> as pallet_ads_primitives::PlacementAdminProvider<u64>>::placement_admin(
                &ch,
            );
...
fn placement_status_returns_paused_for_admin_paused() {
    new_test_ext().execute_with(|| {
        ...
        let status =
            <Pallet<Test> as pallet_ads_primitives::PlacementAdminProvider<u64>>::placement_status(
                &ch,
            );
...
fn revenue_distributor_default_80pct() {
    new_test_ext().execute_with(|| {
        let ch = community_hash(1);
        let result =
            <Pallet<Test> as pallet_ads_primitives::RevenueDistributor<u64, u128>>::distribute(
                &ch,
                100 * UNIT,
                &STAKER,
            );
...
fn h1_distribute_emits_node_ad_reward_event() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::NodeAdRewardAccrued {
            node_id: ch,
            amount: 15 * UNIT,
        }));
    });
...
fn h3_resume_audience_surge_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(AdsGroupRobot::resume_audience_surge(
            RuntimeOrigin::root(),
            ch
        ));
        assert_eq!(AudienceSurgePaused::<Test>::get(&ch), 0);
...
fn m3_cross_validate_nodes_emits_event_on_deviation() {
    new_test_ext().execute_with(|| {
        ...
        System::reset_events();
        assert_ok!(AdsGroupRobot::cross_validate_nodes(
            RuntimeOrigin::root(),
            ch
        ));

        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::NodeDeviationRejected {
...
fn m3_slash_community_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::CommunitySlashed {
            community_id_hash: ch,
...
fn unstake_emits_unbonding_started_event() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::UnbondingStarted {
            community_id_hash: ch,
            who: STAKER,
            amount: 20 * UNIT,
            unlock_at: 11,
...
fn set_stake_tiers_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Pallet::<Test>::compute_audience_cap((500 * UNIT).try_into().unwrap()),
            20_000
        );
        assert_eq!(
            Pallet::<Test>::compute_audience_cap((50 * UNIT).try_into().unwrap()),
            3_000
        );
        assert_eq!(
            Pallet::<Test>::compute_audience_cap((5 * UNIT).try_into().unwrap()),
            500
        );
        assert_eq!(
            Pallet::<Test>::compute_audience_cap((4 * UNIT).try_into().unwrap()),
            0
        );
    });
}
...
fn set_stake_tiers_requires_root() {
    new_test_ext().execute_with(|| {
        use frame_support::BoundedVec;
        let tiers =
            BoundedVec::<(u128, u32), ConstU32<10>>::try_from(vec![(100 * UNIT, 5_000)]).unwrap();
        assert_noop!(
...
fn global_ads_pause_works() {
    new_test_ext().execute_with(|| {
        assert_ok!(AdsGroupRobot::set_global_ads_pause(
            RuntimeOrigin::root(),
            true
        ));
...
fn force_unstake_works() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::ForceUnstaked {
            community_id_hash: ch,
...
fn report_node_audience_emits_event() {
    new_test_ext().execute_with(|| {
        ...
        System::assert_has_event(RuntimeEvent::AdsGroupRobot(Event::NodeAudienceReported {
            community_id_hash: ch,
            node_id: nid,
            audience_size: 100,
        }));

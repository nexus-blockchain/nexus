...
fn delivery_verifier_routes_to_entity_for_registered_placement() {
    new_test_ext().execute_with(|| {
        ...
        let result = <AdsRouter<Test> as DeliveryVerifier<u64>>::verify_and_cap_audience(
            &ALICE, &pid, 500, None,
        );
        assert!(result.is_ok());
...
fn click_verifier_routes_to_entity_for_registered_placement() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsRouter<Test> as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 50, 50);
        assert!(result.is_ok());
...
fn click_verifier_rejects_grouprobot_placement_with_structured_error() {
    new_test_ext().execute_with(|| {
        ...
        let result = <AdsRouter<Test> as ClickVerifier<u64>>::verify_and_cap_clicks(
            &TEE_OPERATOR,
            &pid,
            10,
            10,
...
fn click_verifier_rejects_unknown_placement_as_grouprobot_path() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsRouter<Test> as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 10, 10);
        assert!(result.is_err());
        assert_eq!(
            result.unwrap_err(),
            AdsRouterError::CpcNotSupportedForPath.into(),
...
fn placement_admin_routes_to_entity_returns_owner() {
    new_test_ext().execute_with(|| {
        ...
        let admin = <AdsRouter<Test> as PlacementAdminProvider<u64>>::placement_admin(&pid);
...
fn placement_status_entity_active() {
    new_test_ext().execute_with(|| {
        ...
        let status = <AdsRouter<Test> as PlacementAdminProvider<u64>>::placement_status(&pid);
...
fn placement_banned_entity_false_by_default() {
    new_test_ext().execute_with(|| {
        ...
        let banned = <AdsRouter<Test> as PlacementAdminProvider<u64>>::is_placement_banned(&pid);
        assert!(!banned);
    });
}
...
fn revenue_distributor_entity_path_two_way_split() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsRouter<Test> as RevenueDistributor<u64, u128>>::distribute(&pid, 10_000, &BOB);
        assert!(result.is_ok());
...
fn revenue_distributor_entity_zero_cost() {
    new_test_ext().execute_with(|| {
        ...
        let result = <AdsRouter<Test> as RevenueDistributor<u64, u128>>::distribute(&pid, 0, &BOB);
        assert!(result.is_ok());
        let breakdown = result.unwrap();

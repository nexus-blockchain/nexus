    ...
    T: pallet_ads_entity::Config + pallet_ads_grouprobot::Config,
{
    fn placement_admin(placement_id: &PlacementId) -> Option<T::AccountId> {
        if is_entity_placement::<T>(placement_id) {
            <pallet_ads_entity::Pallet<T> as PlacementAdminProvider<T::AccountId>>::placement_admin(
                placement_id,
            )
        } else {
    ...
    fn placement_status(placement_id: &PlacementId) -> PlacementStatus {
        if is_entity_placement::<T>(placement_id) {
            <pallet_ads_entity::Pallet<T> as PlacementAdminProvider<T::AccountId>>::placement_status(
                placement_id,
            )
        } else {
...
where
    T: pallet_ads_entity::Config
        + pallet_ads_grouprobot::Config<Currency = <T as pallet_ads_entity::Config>::Currency>,
    BalanceOf<T>: Default,
{
    fn distribute(
        placement_id: &PlacementId,

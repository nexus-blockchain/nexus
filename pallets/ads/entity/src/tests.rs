...
fn deregister_placement_fails_not_registered() {
    new_test_ext().execute_with(|| {
        assert_noop!(
            AdsEntity::deregister_placement(RuntimeOrigin::signed(ALICE), [0u8; 32],),
...
fn delivery_verifier_works() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsEntity as pallet_ads_primitives::DeliveryVerifier<u64>>::verify_and_cap_audience(
                &ALICE, &pid, 500, None,
            );
        assert_ok!(&result);
...
fn placement_admin_entity_level() {
    new_test_ext().execute_with(|| {
        ...
        let admin =
            <AdsEntity as pallet_ads_primitives::PlacementAdminProvider<u64>>::placement_admin(
                &pid,
            );
...
fn placement_banned_check() {
    new_test_ext().execute_with(|| {
        ...
        assert!(
            !<AdsEntity as pallet_ads_primitives::PlacementAdminProvider<u64>>::is_placement_banned(
                &pid
            )
        );
...
fn revenue_distributor_default_split() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsEntity as pallet_ads_primitives::RevenueDistributor<u64, u128>>::distribute(
                &pid, 10_000, &99,
            );
...
fn set_click_cap_works() {
    new_test_ext().execute_with(|| {
        ...
        // 设置为 500
        assert_ok!(AdsEntity::set_click_cap(
            RuntimeOrigin::signed(ALICE),
            pid,
            500
        ));
        ...
        // 事件验证
        System::assert_last_event(RuntimeEvent::AdsEntity(crate::Event::ClickCapUpdated {
            placement_id: pid,
            daily_cap: 500,
        }));
    });
...
fn click_verifier_no_cap_passes_through() {
    new_test_ext().execute_with(|| {
        ...
        let result =
            <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 1000, 1000);
        assert_ok!(&result);
        assert_eq!(result.unwrap(), 1000);
...
fn click_verifier_caps_at_daily_limit() {
    new_test_ext().execute_with(|| {
        ...
        // 第一次: 请求 150, 通过 150
        let r1 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 150, 150);
        assert_eq!(r1.unwrap(), 150);
        assert_eq!(DailyClicks::<Test>::get(&pid), 150);

        // 第二次: 请求 100, 只剩 50, 裁切到 50
        let r2 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 100, 100);
        assert_eq!(r2.unwrap(), 50);
        assert_eq!(DailyClicks::<Test>::get(&pid), 200);

        // 第三次: 已达上限, 拒绝
        let r3 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 10, 10);
...
fn click_verifier_fails_inactive_placement() {
    new_test_ext().execute_with(|| {
        ...
        use pallet_ads_primitives::ClickVerifier;
        let result = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 10, 10);
        assert_noop!(result, Error::<Test>::PlacementNotActive);
    });
}
...
fn click_verifier_daily_reset_works() {
    new_test_ext().execute_with(|| {
        ...
        // 消耗全部上限
        let r1 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 100, 100);
        assert_eq!(r1.unwrap(), 100);
        assert_eq!(DailyClicks::<Test>::get(&pid), 100);

        // 上限已达, 拒绝
        let r2 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 10, 10);
        ...
        // 重置后应该可以再次提交
        let r3 = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 50, 50);
...
fn deregister_placement_clears_click_storage() {
    new_test_ext().execute_with(|| {
        ...
        // 产生一些点击数据
        use pallet_ads_primitives::ClickVerifier;
        let _ = <AdsEntity as ClickVerifier<u64>>::verify_and_cap_clicks(&ALICE, &pid, 100, 100);
        assert_eq!(DailyClicks::<Test>::get(&pid), 100);

...
//! # Pallet Ads Entity — Entity DApp advertising adapter.
//! # Pallet Ads Entity — Entity DApp 广告适配层。
//!
//! ## Features
//! ## 功能
//! - Entity impression verification (implements `DeliveryVerifier`)
//! - Entity 展示量验证（实现 `DeliveryVerifier`）
//! - Entity / Shop admin resolution (implements `PlacementAdminProvider`)
//! - Entity / Shop 管理员映射（实现 `PlacementAdminProvider`）
//! - Two-way revenue split: Entity Owner / platform (implements `RevenueDistributor`)
//! - 二方收入分配：Entity Owner / 平台（实现 `RevenueDistributor`）
//! - Entity placement registration (opt-in) and shop-level placement management
//! - Entity 广告注册（opt-in）+ Shop 级广告位管理
//! - Daily impression caps based on registered advertising plans
//! - 每日展示量上限（按注册的广告计划配额）
//! - Governance-controlled Entity advertising share percentages
//! - Entity 广告分成百分比治理
//!
//! ## Design
//! ## 设计
//! This pallet does not implement core campaign CRUD logic, which lives in `pallet-ads-core`.
//! 本 pallet 不包含 Campaign CRUD 等核心广告逻辑（由 `pallet-ads-core` 提供）。
//! It only implements Entity-specific adapter traits and exposes extra Entity-only
//! extrinsics such as placement registration, share configuration, impression caps,
//! and shop-level placement management.
//! 仅实现 Entity DApp 领域特定的适配 trait，并提供额外的 Entity 专属 extrinsic
//! （注册广告位、设置分成、设置展示量上限、Shop 级广告位管理等）。
//!
//! ## PlacementId mapping
//! ## PlacementId 映射
//! Entity placements use `blake2_256(b"entity-ad:" ++ entity_id.to_le_bytes())`.
//! Entity 使用 `blake2_256(b"entity-ad:" ++ entity_id.to_le_bytes())` 作为 PlacementId。
//! Shop-level placements use `blake2_256(b"shop-ad:" ++ shop_id.to_le_bytes())`.
...
/// Entity-level `PlacementId`: `blake2_256(b"entity-ad:" ++ entity_id.to_le_bytes())`.
...
/// Shop-level `PlacementId`: `blake2_256(b"shop-ad:" ++ shop_id.to_le_bytes())`.
...
/// Placement info for a registered Entity or Shop advertising slot.
/// 广告位注册信息。
#[derive(
    Encode,
    Decode,
...
pub struct AdPlacementInfo<T: Config> {
    /// Entity ID that owns this placement.
    /// 广告位所属 entity_id
    pub entity_id: u64,
    /// Shop ID for this placement. `0` means an Entity-level placement.
    /// 广告位所属 shop_id（`0` = Entity 级）
    pub shop_id: u64,
    /// Placement level.
    /// 广告位级别。
    pub level: PlacementLevel,
    /// Daily impression cap. `0` means unlimited.
    /// 每日展示量上限（`0` = 无限制）
    pub daily_impression_cap: u32,
    /// Daily click cap. `0` means unlimited.
    /// 每日点击量上限（`0` = 无限制）
    pub daily_click_cap: u32,
    /// Account that registered this placement (Entity owner or Shop manager).
...
pub mod pallet {
    ...
    #[pallet::storage]
    pub type EntityAdShareBps<T: Config> = StorageMap<_, Blake2_128Concat, u64, u16, ValueQuery>;

    /// 被禁止投放广告的 Entity
    #[pallet::storage]
    pub type BannedEntities<T: Config> = StorageMap<_, Blake2_128Concat, u64, bool, ValueQuery>;
    ...
    /// 广告位今日点击量计数 (CPC)
    #[pallet::storage]
    pub type DailyClicks<T: Config> = StorageMap<_, Blake2_128Concat, PlacementId, u32, ValueQuery>;

    /// 广告位累计点击量 (CPC)
    #[pallet::storage]
    pub type TotalClicks<T: Config> = StorageMap<_, Blake2_128Concat, PlacementId, u64, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// Entity 广告分成比例已更新
        EntityShareUpdated { entity_id: u64, share_bps: u16 },
        /// Entity 被禁止广告
        EntityBanned { entity_id: u64 },
        /// Entity 禁令解除
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn register_entity_placement(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ...
            // 验证 Entity 状态
                ...
                T::EntityProvider::entity_owner(entity_id) == Some(who.clone())
                    || T::EntityProvider::is_entity_admin(
                        entity_id,
                        &who,
                        pallet_entity_common::AdminPermission::ADS_MANAGE
                    ),
                Error::<T>::NotEntityAdmin
            );
            ensure!(
                !BannedEntities::<T>::get(entity_id),
            ...
            ids.try_push(placement_id)
                .map_err(|_| Error::<T>::MaxPlacementsReached)?;
        ...
        pub fn deregister_placement(
            ...
            // 权限: Entity owner/admin 或注册者
                ...
                !T::EntityProvider::is_entity_locked(info.entity_id),
                Error::<T>::EntityLocked
            );
            ensure!(
        ...
        pub fn set_impression_cap(
            ...
            RegisteredPlacements::<T>::try_mutate(&placement_id, |maybe_info| {
                ...
                // L1-R2: 值未变更时跳过，避免冗余事件
                    ...
                    Error::<T>::ImpressionCapUnchanged
                );
        ...
        pub fn ban_entity(origin: OriginFor<T>, entity_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(
                T::EntityProvider::entity_exists(entity_id),
                Error::<T>::EntityNotFound
    ...
    impl<T: Config> DeliveryVerifier<T::AccountId> for Pallet<T> {
        fn verify_and_cap_audience(
            ...
            // 2. Entity 必须活跃且未被禁止
                ...
                !BannedEntities::<T>::get(info.entity_id),
                Error::<T>::EntityBanned
            );

            // 3. 调用者权限: Entity owner/admin 或 shop manager
            let is_authorized = T::EntityProvider::entity_owner(info.entity_id)
                == Some(who.clone())
                || T::EntityProvider::is_entity_admin(
                    info.entity_id,
                    who,

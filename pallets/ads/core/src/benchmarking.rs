...
fn seed_campaign<T: Config>(advertiser: &T::AccountId) -> u64 {
    ...
    let url: BoundedVec<u8, T::MaxAdUrlLength> = b"https://bench.example.com"
        .to_vec()
        .try_into()
        .expect("url fits");
    ...
    let _ = CampaignsByAdvertiser::<T>::try_mutate(advertiser, |list| list.try_push(id));
...
fn seed_cpc_campaign<T: Config>(advertiser: &T::AccountId) -> u64 {
    ...
    let url: BoundedVec<u8, T::MaxAdUrlLength> = b"https://cpc-bench.example.com"
        .to_vec()
        .try_into()
        .expect("url fits");
...
fn seed_receipt<T: Config>(campaign_id: u64, pid: PlacementId, submitter: &T::AccountId) -> u32 {
    ...
    let _ = DeliveryReceipts::<T>::try_mutate(&pid, |receipts| receipts.try_push(receipt));
    ReceiptConfirmation::<T>::insert((campaign_id, pid, receipt_index), ReceiptStatus::Pending);
    ReceiptSubmittedAt::<T>::insert((campaign_id, pid, receipt_index), now);
...
mod benches {
    ...
    fn create_campaign() {
        ...
        let text: BoundedVec<u8, T::MaxAdTextLength> = b"Benchmark Ad".to_vec().try_into().unwrap();
    ...
    fn pause_campaign() {
        ...
        assert_eq!(
            Campaigns::<T>::get(id).unwrap().status,
    ...
    fn review_campaign() {
        ...
        assert_eq!(
            Campaigns::<T>::get(id).unwrap().review_status,
    ...
    fn advertiser_unblock_placement() {
        let caller = funded_account::<T>("caller", 0);
        let pid = placement_id(1);
        let _ = AdvertiserBlacklist::<T>::try_mutate(&caller, |list| list.try_push(pid));
    ...
    fn advertiser_unprefer_placement() {
        let caller = funded_account::<T>("caller", 0);
        let pid = placement_id(1);
        let _ = AdvertiserWhitelist::<T>::try_mutate(&caller, |list| list.try_push(pid));
    ...
    fn placement_unblock_advertiser() {
        ...
        let pid = placement_id(1);
        let advertiser = funded_account::<T>("advertiser", 1);
        let _ = PlacementBlacklist::<T>::try_mutate(&pid, |list| list.try_push(advertiser.clone()));
    ...
    fn placement_unprefer_advertiser() {
        ...
        let pid = placement_id(1);
        let advertiser = funded_account::<T>("advertiser", 1);
        let _ = PlacementWhitelist::<T>::try_mutate(&pid, |list| list.try_push(advertiser.clone()));
    ...
    fn resume_campaign() {
        ...
        Campaigns::<T>::mutate(id, |c| {
            c.as_mut().unwrap().status = CampaignStatus::Paused;
        });
    ...
    fn update_campaign() {
        ...
        register_seed_advertiser::<T>(&caller);
        let id = seed_campaign::<T>(&caller);
        let new_text: BoundedVec<u8, T::MaxAdTextLength> = b"Updated".to_vec().try_into().unwrap();
    ...
    fn extend_campaign_expiry() {
        ...
        register_seed_advertiser::<T>(&caller);
        let id = seed_campaign::<T>(&caller);
        let new_expiry = Campaigns::<T>::get(id)
            .unwrap()
            .expires_at
            .saturating_add(1000u32.into());
    ...
    fn unsuspend_campaign() {
        ...
        let id = seed_campaign::<T>(&caller);
        Campaigns::<T>::mutate(id, |c| {
            c.as_mut().unwrap().status = CampaignStatus::Suspended;
        });
    ...
    fn resubmit_campaign() {
        ...
        let text: BoundedVec<u8, T::MaxAdTextLength> = b"Resubmitted".to_vec().try_into().unwrap();
        let url: BoundedVec<u8, T::MaxAdUrlLength> =
    ...
    fn cleanup_campaign() {
        ...
        T::Currency::unreserve(&caller, escrow);
        Campaigns::<T>::mutate(id, |c| {
            c.as_mut().unwrap().status = CampaignStatus::Cancelled;
        });
    ...
    fn confirm_receipt() {
        ...
        assert_eq!(
            ReceiptConfirmation::<T>::get((id, pid, idx)),
    ...
    fn auto_confirm_receipt() {
        ...
        frame_system::Pallet::<T>::set_block_number(
            submitted.saturating_add(window).saturating_add(1u32.into()),
        );
        let anyone = funded_account::<T>("anyone", 1);

...
//! # Pallet Ads Core — Generic advertising engine.
//! # Pallet Ads Core — 通用广告引擎。
//!
//! ## Features
//! ## 功能
//! - Campaign CRUD (create / top up budget / pause / cancel / review / report)
//! - Campaign CRUD（创建 / 追加预算 / 暂停 / 取消 / 审核 / 举报）
//! - Escrow-backed budget custody
//! - Escrow 预算托管
//! - Delivery-receipt submission and Era settlement
//! - 投放收据提交 + Era 结算
//! - Bidirectional advertiser ⇄ placement blacklist / whitelist preferences
//! - 双向偏好：广告主 ⇄ 广告位 黑名单 / 白名单
//! - Placement revenue withdrawal
//! - 广告位收入提取
//! - Slash / penalty mechanism
//! - Slash / 惩罚机制
//! - Private-campaign registration
//! - 私域广告登记
//!
//! ## Design
//! ## 设计
//! The core engine contains no domain-specific logic such as TEE nodes or bot subscriptions.
//! 核心引擎不包含任何领域特定逻辑（TEE 节点、Bot 订阅等）。
//! Domain behavior is injected through adapter traits in `Config`:
//! 通过 `Config` trait 中的适配层 trait 注入领域行为：
//! - `DeliveryVerifier`: delivery validation (TEE signatures / Entity activity checks)
//! - `DeliveryVerifier`：投放验证（TEE 签名 / Entity 活跃检查）
//! - `PlacementAdminProvider`: placement admin resolution (Bot Owner / Entity Owner)
//! - `PlacementAdminProvider`：广告位管理员（Bot Owner / Entity Owner）
//! - `RevenueDistributor`: revenue split strategy (three-way / two-way split)
...
pub mod pallet {
    ...
    pub trait Config: frame_system::Config<RuntimeEvent: From<Event<Self>>> {
        ...
        /// Maximum ad-text length.
        /// 广告文本最大长度
        #[pallet::constant]
        type MaxAdTextLength: Get<u32>;

        /// Maximum ad URL length.
        /// 广告 URL 最大长度
        #[pallet::constant]
        type MaxAdUrlLength: Get<u32>;

        /// Maximum number of receipts per placement.
        /// 每广告位最大收据数
        #[pallet::constant]
        ...
        /// Delivery verifier implemented by the adapter layer.
        /// 投放验证器（适配层实现）
        type DeliveryVerifier: DeliveryVerifier<Self::AccountId>;

        /// Click verifier implemented by the adapter layer using the C2b proxy-account flow.
        ...
        /// Placement-admin provider implemented by the adapter layer.
        /// 广告位管理员（适配层实现）
        type PlacementAdmin: PlacementAdminProvider<Self::AccountId>;

        /// Revenue-distribution strategy implemented by the adapter layer.
        /// 收入分配策略（适配层实现）
    ...
    #[pallet::storage]
    pub type CampaignEscrow<T: Config> =
    ...
    #[pallet::storage]
    pub type SlashCount<T: Config> = StorageMap<_, Blake2_128Concat, PlacementId, u32, ValueQuery>;
    ...
    #[pallet::storage]
    pub type CampaignDailySpent<T: Config> =
        StorageDoubleMap<_, Blake2_128Concat, u64, Blake2_128Concat, u32, BalanceOf<T>, ValueQuery>;
    ...
    #[pallet::storage]
    pub type CampaignMultiplier<T: Config> = StorageMap<_, Blake2_128Concat, u64, u32>;

    /// 广告位级 CPM 倍率 (Entity Owner 设置, 基点 100 = 1x)
    #[pallet::storage]
    pub type PlacementMultiplier<T: Config> = StorageMap<_, Blake2_128Concat, PlacementId, u32>;
    ...
    #[pallet::storage]
    pub type ActiveApprovedCampaigns<T: Config> =
        StorageValue<_, BoundedVec<u64, T::MaxActiveApprovedCampaigns>, ValueQuery>;
    ...
    pub type CampaignsByDeliveryType<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        u8,
        BoundedVec<u64, T::MaxCampaignsByDeliveryType>,
        ValueQuery,
    >;
    ...
    pub type CampaignsForPlacement<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        PlacementId,
        BoundedVec<u64, T::MaxCampaignsForPlacement>,
        ValueQuery,
    >;
    ...
    #[pallet::storage]
    pub type CampaignReportCount<T: Config> = StorageMap<_, Blake2_128Concat, u64, u32, ValueQuery>;
    ...
    #[pallet::storage]
    pub type AdvertiserReferrer<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, T::AccountId>;

    /// 广告主注册区块
    #[pallet::storage]
    pub type AdvertiserRegisteredAt<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, BlockNumberFor<T>>;

    /// 推荐人可提取佣金
    #[pallet::storage]
    pub type ReferrerClaimable<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, BalanceOf<T>, ValueQuery>;

    /// 推荐人累计佣金收入 (统计用)
    #[pallet::storage]
    pub type ReferrerTotalEarnings<T: Config> =
        StorageMap<_, Blake2_128Concat, T::AccountId, BalanceOf<T>, ValueQuery>;
    ...
    pub type ReferrerAdvertisers<T: Config> = StorageMap<
        ...
        Blake2_128Concat,
        T::AccountId,
        BoundedVec<T::AccountId, T::MaxReferredAdvertisers>,
        ValueQuery,
    >;

    /// 平台 (Treasury) 累计净留存份额 (统计/审计用)
    #[pallet::storage]
    pub type TreasuryShareAccumulator<T: Config> = StorageValue<_, BalanceOf<T>, ValueQuery>;
    ...
    pub enum Event<T: Config> {
        ...
        /// 审核结果
        CampaignReviewed { campaign_id: u64, approved: bool },
        /// 投放收据已提交
        ...
        /// 广告位被永久禁止
        PlacementBannedFromAds { placement_id: PlacementId },
        /// 广告主拉黑广告位
        ...
        /// 广告活动过期时间已延长
        CampaignExpiryExtended {
            campaign_id: u64,
            new_expires_at: BlockNumberFor<T>,
        },
        /// Root 强制取消 Campaign
        CampaignForceCancelled {
            campaign_id: u64,
            refunded: BalanceOf<T>,
        },
        /// 广告位已解除封禁
        ...
        /// 广告位投放类型已设置
        PlacementDeliveryTypesSet {
            placement_id: PlacementId,
            delivery_types: u8,
        },
        /// 私域广告已注销
        PrivateAdUnregistered {
            placement_id: PlacementId,
            count: u32,
        },
        /// 已终结 Campaign 存储已清理
        ...
        /// 结算激励已发放
        SettlementIncentivePaid {
            settler: T::AccountId,
            amount: BalanceOf<T>,
        },
        /// 平台份额已记账 (treasury 净留存)
        PlatformShareCredited {
            campaign_id: u64,
            amount: BalanceOf<T>,
        },
        /// Campaign 定向广告位已设置
        ...
        /// Campaign CPM 倍率已设置
        CampaignMultiplierSet {
            campaign_id: u64,
            multiplier_bps: u32,
        },
        /// 广告位 CPM 倍率已设置
        PlacementMultiplierSet {
            placement_id: PlacementId,
            multiplier_bps: u32,
        },
        /// 广告位审核要求已设置
        PlacementApprovalRequirementSet {
            placement_id: PlacementId,
            required: bool,
        },
        /// Campaign 已被广告位批准
        CampaignApprovedForPlacement {
            placement_id: PlacementId,
            campaign_id: u64,
        },
        /// Campaign 已被广告位拒绝
        CampaignRejectedForPlacement {
            placement_id: PlacementId,
            campaign_id: u64,
        },
        /// 收据已被广告主确认
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn create_campaign(
            ...
            // Phase 6: 必须是注册广告主
                ...
                AdvertiserRegisteredAt::<T>::contains_key(&who),
                Error::<T>::NotRegisteredAdvertiser
            );

            ensure!(!text.is_empty(), Error::<T>::EmptyAdText);
            ensure!(
                delivery_types > 0 && delivery_types <= 0b111,
                Error::<T>::InvalidDeliveryTypes
            );
            ...
            match campaign_type {
                CampaignType::Cpc => {
                        ...
                        bid_per_click >= T::MinBidPerClick::get(),
                        Error::<T>::ClickBidTooLow
                    );
                }
                _ => {
                    ensure!(
                        bid_per_mille >= T::MinBidPerMille::get(),
                        Error::<T>::BidTooLow
                    );
                }
            }
        ...
        pub fn fund_campaign(
            ...
            Self::deposit_event(Event::CampaignFunded {
                campaign_id,
                amount,
            });
            Ok(())
        ...
        pub fn pause_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn cancel_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn review_campaign(
            ...
            Self::deposit_event(Event::CampaignReviewed {
                campaign_id,
                approved,
            });
            Ok(())
        ...
        pub fn submit_delivery_receipt(
            ...
            let campaign = Campaigns::<T>::get(campaign_id).ok_or(Error::<T>::CampaignNotFound)?;
                ...
                campaign.review_status == AdReviewStatus::Approved,
                Error::<T>::CampaignNotApproved
            );

            // CPM 收据仅允许 CPM/Fixed/Private 类型 Campaign
            ensure!(
                campaign.campaign_type != CampaignType::Cpc,
                Error::<T>::CampaignTypeMismatch
            );
            ...
            let now = frame_system::Pallet::<T>::block_number();
                ...
                !BannedPlacements::<T>::get(&placement_id),
                Error::<T>::PlacementBanned
            );

            // Campaign 定向检查: 如果 Campaign 指定了目标广告位, 仅目标可投放
            if let Some(targets) = CampaignTargets::<T>::get(campaign_id) {
                ensure!(
                    targets.contains(&placement_id),
                    Error::<T>::PlacementNotTargeted
                );
            }
            ...
            DeliveryReceipts::<T>::try_mutate(&placement_id, |receipts| {
                receipts
                    .try_push(receipt)
                    .map_err(|_| Error::<T>::ReceiptsFull)
            })?;
            ...
            ReceiptSubmittedAt::<T>::insert((campaign_id, placement_id, receipt_index), now);
        ...
        pub fn submit_click_receipt(
            ...
            let who = ensure_signed(origin)?;
                ...
                verified_clicks <= click_count,
                Error::<T>::VerifiedExceedsTotal
            );
            ...
            let effective_verified = if click_count > 0 && effective_clicks < click_count {
                (verified_clicks as u64).saturating_mul(effective_clicks as u64)
                    / (click_count as u64)
            } else {
        ...
        pub fn settle_era_ads(origin: OriginFor<T>, placement_id: PlacementId) -> DispatchResult {
        ...
        pub fn flag_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            Self::deposit_event(Event::CampaignFlagged {
                campaign_id,
                reporter: who,
            });
            Ok(())
        ...
        pub fn advertiser_block_placement(
            ...
            AdvertiserBlacklist::<T>::try_mutate(&who, |list| {
                    ...
                    !list.contains(&placement_id),
                    Error::<T>::AlreadyBlacklisted
                );
                list.try_push(placement_id)
                    .map_err(|_| Error::<T>::BlacklistFull)?;
                Ok::<(), DispatchError>(())
            })?;
        ...
        pub fn flag_placement(origin: OriginFor<T>, placement_id: PlacementId) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn resume_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn expire_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            ...
            Self::deposit_event(Event::CampaignMarkedExpired {
                campaign_id,
                refunded,
            });
            Ok(())
        ...
        pub fn register_private_ad(
            ...
            if !fee.is_zero() {
                let treasury = T::TreasuryAccount::get();
                T::Currency::transfer(&who, &treasury, fee, ExistenceRequirement::AllowDeath)?;
            }
        ...
        pub fn update_campaign(
            ...
            Campaigns::<T>::try_mutate(campaign_id, |maybe| {
                ...
                if let Some(b) = bid_per_mille {
                        ...
                        c.campaign_type != CampaignType::Cpc,
                        Error::<T>::CampaignTypeMismatch
                    );
        ...
        pub fn extend_campaign_expiry(
            ...
            Self::deposit_event(Event::CampaignExpiryExtended {
                campaign_id,
                new_expires_at,
            });
            Ok(())
        ...
        pub fn force_cancel_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            Self::index_remove_active(campaign_id, dt);
            Self::deposit_event(Event::CampaignForceCancelled {
                campaign_id,
                refunded,
            });
            Ok(())
        ...
        pub fn unban_placement(origin: OriginFor<T>, placement_id: PlacementId) -> DispatchResult {
            ensure_root(origin)?;

            ensure!(
                BannedPlacements::<T>::get(&placement_id),
                Error::<T>::PlacementNotBanned
            );
        ...
        pub fn suspend_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            ensure_root(origin)?;
        ...
        pub fn unsuspend_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            ensure_root(origin)?;
            ...
            let delivery_types = Campaigns::<T>::try_mutate(campaign_id, |maybe| {
                let c = maybe.as_mut().ok_or(Error::<T>::CampaignNotFound)?;
                    ...
                    c.status == CampaignStatus::Suspended,
                    Error::<T>::CampaignNotSuspended
                );
                c.status = CampaignStatus::Active;
                Ok::<u8, DispatchError>(c.delivery_types)
            })?;

            if Campaigns::<T>::get(campaign_id)
                .map_or(false, |c| c.review_status == AdReviewStatus::Approved)
            {
                Self::index_add_active(campaign_id, delivery_types);
            }
        ...
        pub fn report_approved_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn set_placement_delivery_types(
            ...
            Self::deposit_event(Event::PlacementDeliveryTypesSet {
                placement_id,
                delivery_types,
            });
            Ok(())
        ...
        pub fn unregister_private_ad(
            ...
            Self::deposit_event(Event::PrivateAdUnregistered {
                placement_id,
                count,
            });
            Ok(())
        ...
        pub fn cleanup_campaign(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            ensure_signed(origin)?;
        ...
        pub fn clear_campaign_targets(origin: OriginFor<T>, campaign_id: u64) -> DispatchResult {
            let who = ensure_signed(origin)?;
        ...
        pub fn set_campaign_multiplier(
            ...
            if multiplier_bps == 0 {
                CampaignMultiplier::<T>::remove(campaign_id);
                    ...
                    multiplier_bps >= 10 && multiplier_bps <= 10_000,
                    Error::<T>::InvalidMultiplier
                );
                CampaignMultiplier::<T>::insert(campaign_id, multiplier_bps);
            }

            Self::deposit_event(Event::CampaignMultiplierSet {
                campaign_id,
                multiplier_bps,
            });
            Ok(())
        ...
        pub fn set_placement_multiplier(
            ...
            Self::deposit_event(Event::PlacementMultiplierSet {
                placement_id,
                multiplier_bps,
            });
            Ok(())
        ...
        pub fn set_placement_approval_required(
            ...
            Self::deposit_event(Event::PlacementApprovalRequirementSet {
                placement_id,
                required,
            });
            Ok(())
        ...
        pub fn approve_campaign_for_placement(
            ...
            Self::deposit_event(Event::CampaignApprovedForPlacement {
                placement_id,
                campaign_id,
            });
            Ok(())
        ...
        pub fn reject_campaign_for_placement(
            ...
            Self::deposit_event(Event::CampaignRejectedForPlacement {
                placement_id,
                campaign_id,
            });
            Ok(())
        ...
        pub fn confirm_receipt(
            ...
            Self::deposit_event(Event::ReceiptConfirmed {
                campaign_id,
                placement_id,
                receipt_index,
            });
        ...
        pub fn dispute_receipt(
            ...
            Self::deposit_event(Event::ReceiptDisputed {
                campaign_id,
                placement_id,
                receipt_index,
            });
        ...
        pub fn auto_confirm_receipt(
            ...
            let submitted_at =
                ReceiptSubmittedAt::<T>::get((campaign_id, placement_id, receipt_index))
                    .ok_or(Error::<T>::ReceiptNotFound)?;
            let now = frame_system::Pallet::<T>::block_number();
            let window = T::ReceiptConfirmationWindow::get();
            ensure!(
                now > submitted_at.saturating_add(window),
                Error::<T>::ConfirmationWindowNotExpired
            );
            ...
            Self::deposit_event(Event::ReceiptAutoConfirmed {
                campaign_id,
                placement_id,
                receipt_index,
            });
        ...
        pub fn register_advertiser(origin: OriginFor<T>, referrer: T::AccountId) -> DispatchResult {
            let who = ensure_signed(origin)?;

            ensure!(
                !AdvertiserRegisteredAt::<T>::contains_key(&who),
                Error::<T>::AlreadyRegisteredAdvertiser
            );
            ensure!(who != referrer, Error::<T>::SelfReferral);
            ensure!(
                AdvertiserRegisteredAt::<T>::contains_key(&referrer),
                Error::<T>::ReferrerNotAdvertiser
            );

            ReferrerAdvertisers::<T>::try_mutate(&referrer, |list| {
                list.try_push(who.clone())
                    .map_err(|_| Error::<T>::ReferralListFull)
            })?;
        ...
        pub fn force_register_advertiser(
            ...
            ensure_root(origin)?;
                ...
                !AdvertiserRegisteredAt::<T>::contains_key(&advertiser),
                Error::<T>::AlreadyRegisteredAdvertiser
            );
        ...
        pub fn claim_referral_earnings(origin: OriginFor<T>) -> DispatchResult {
            let who = ensure_signed(origin)?;
            ...
            let treasury = T::TreasuryAccount::get();
            T::Currency::transfer(&treasury, &who, claimable, ExistenceRequirement::AllowDeath)?;

            ReferrerClaimable::<T>::remove(&who);

            Self::deposit_event(Event::ReferralEarningsClaimed {
                referrer: who,
                amount: claimable,
            });
            Ok(())
    ...
    impl<T: Config> Pallet<T> {
        ...
        fn do_settle_era_ads(
            ...
            for (idx, receipt) in receipts_snapshot.iter().enumerate() {
                ...
                if let Some(campaign) = Campaigns::<T>::get(receipt.campaign_id) {
                    ...
                    let daily_remaining = if !campaign.daily_budget.is_zero() {
                        ...
                        let day_spent =
                            CampaignDailySpent::<T>::get(receipt.campaign_id, day_index);
            ...
            for (campaign_id, snapshot_cost, advertiser) in settle_items.iter() {
                ...
                let breakdown =
                    T::RevenueDistributor::distribute(&placement_id, adjusted_cost, advertiser)
                ...
                if referral_rate > 0 {
                    if let Some(ref referrer) = AdvertiserReferrer::<T>::get(advertiser) {
                        ...
                        let referral_commission =
                            breakdown.platform_share.saturating_mul(rate_bal) / bps_divisor;
                        if !referral_commission.is_zero() {
                            net_platform_share =
                                net_platform_share.saturating_sub(referral_commission);
                ...
                placement_share_total =
                    placement_share_total.saturating_add(breakdown.placement_share);
            }
            ...
            if !total_cost.is_zero() {
                ...
                PlacementTotalRevenue::<T>::mutate(&placement_id, |r| {
                    *r = r.saturating_add(total_cost)
                });
        ...
        pub fn available_campaigns_for_placement(
            ...
            let mut results: alloc::vec::Vec<
                runtime_api::CampaignSummary<T::AccountId, BalanceOf<T>>,
            > = alloc::vec::Vec::new();
            ...
            for cid in candidate_ids {
                if let Some(c) = Campaigns::<T>::get(cid) {
                    ...
                    let effective_bid =
                        base_bid.saturating_mul(multiplier_bps.into()) / 100u32.into();
        ...
        pub fn campaign_details(
            ...
            let targets = CampaignTargets::<T>::get(campaign_id).map(|t| t.into_inner());
    ...
    impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
        fn on_idle(_n: BlockNumberFor<T>, remaining_weight: Weight) -> Weight {
            ...
            while removed < MAX_GC_PER_BLOCK {
                ...
                let Some((campaign_id, day_index, _amount)) = iter.next() else {
                    break;
                };

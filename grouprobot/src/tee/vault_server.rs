            ...
            match listener.accept().await {
                Ok((stream, _addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = handle_connection(vault, stream, ipc_key).await {
...
async fn handle_connection(
    ...
    loop {
        ...
        // 处理请求
        let response = process_request(&vault, request).await;
        ...
        // Shutdown 请求后退出
        if matches!(VaultResponse::from_bytes(&resp_payload[4..]), Ok(VaultResponse::ShutdownAck)) {
            return Ok(());
        }
    }
}

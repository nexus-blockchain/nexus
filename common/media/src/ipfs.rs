...
impl IpfsHelper {
    ...
    pub fn extract_hash_from_cid(cid: &str) -> Result<[u8; 32], MediaError> {
        ...
        for (i, chunk) in hex_part.as_bytes().chunks(2).enumerate() {
            ...
            let hex_str =
                core::str::from_utf8(chunk).map_err(|_| MediaError::InvalidCidEncoding)?;
            hash[i] =
                u8::from_str_radix(hex_str, 16).map_err(|_| MediaError::InvalidCidEncoding)?;
        }
...
mod tests {
    ...
    fn test_cid_too_long() {
        ...
        assert_eq!(
            IpfsHelper::validate_cid(&long_cid),
            Err(MediaError::CidTooLong)
        );
    }
    ...
    fn test_is_valid_base58() {
        assert!(IpfsHelper::is_valid_base58(

...
// 重新导出常用类型和结构
pub use types::{
    AudioFormat, ContentType, DocumentFormat, ImageFormat, MediaKind, MediaMetadata, VideoFormat,
};

pub use error::MediaError;

pub use validation::{AudioValidator, ImageValidator, VideoValidator};

pub use hash::HashHelper;

pub use ipfs::{CidInfo, IpfsHelper};
...
mod tests {
    ...
    fn test_verify_media_integrity() {
        ...
        assert!(!utils::verify_media_integrity(
            b"different content",
            &hash,
            &cid
        ));
...
macro_rules! verify_evidence {
    ($ns:expr, $subject:expr, $cid:expr, $salt:expr, $version:expr, $expected:expr) => {{
        let actual = $crate::HashHelper::evidence_commitment($ns, $subject, $cid, $salt, $version);
        actual == $expected
    }};
}

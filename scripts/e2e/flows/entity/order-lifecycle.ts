...
async function orderLifecycle(ctx: FlowContext): Promise<void> {
  ...
  await ctx.check('商品已上架', 'eve', async () => {
    await assertStorageExists(api, 'entityService', 'products', [productId], '商品应存在');
  });
  ...
  const placeOrderTx = (api.tx as any).entityOrder.placeOrder(
    shopId,
    productId,
    1,                    // quantity
    null,                 // referrer
    null,                 // shopping_balance_amount
    null,                 // memo
  );
  ...
  const shipTx = (api.tx as any).entityOrder.shipOrder(orderId, null /* tracking_cid */);
  ...
  await ctx.check('验证 OrderCompleted 事件', 'system', () => {
    assertEventEmitted(confirmResult, 'entityOrder', 'OrderCompleted', '应有 OrderCompleted');
  });
  ...
  // 创建新订单用于错误路径测试
  const placeOrder2Tx = (api.tx as any).entityOrder.placeOrder(shopId, productId, 1, null, null, null);
  ...
  // 发货
  const ship2Tx = (api.tx as any).entityOrder.shipOrder(order2Id, null);
  await ctx.send(ship2Tx, eve, '发货(错误路径用)', 'eve');

  // Charlie 尝试确认收货
  const fakeConfirmTx = (api.tx as any).entityOrder.confirmReceipt(order2Id);
  ...
  const unpublishTx = (api.tx as any).entityService.unpublishProduct(shopId, productId);
  const unpubResult = await ctx.send(unpublishTx, eve, '下架商品', 'eve');
  assertTxSuccess(unpubResult, '下架');

  const failOrderTx = (api.tx as any).entityOrder.placeOrder(shopId, productId, 1, null, null, null);
  ...
  // 重新上架供后续测试
  const republishTx = (api.tx as any).entityService.publishProduct(shopId, productId);
  ...
  const placeOrder3Tx = (api.tx as any).entityOrder.placeOrder(shopId, productId, 1, null, null, null);
  ...
  const cancelTx = (api.tx as any).entityOrder.cancelOrder(order3Id);
  ...
  await ctx.check('验证 OrderCancelled 事件', 'bob', () => {
    assertEventEmitted(cancelResult, 'entityOrder', 'OrderCancelled', '应有 OrderCancelled');
  });

  // ─── Step 11: 退款流程 ────────────────────────────────────

  // 对 order2 申请退款 (之前 Charlie 确认失败, Bob 还没确认)
  const refundTx = (api.tx as any).entityOrder.requestRefund(order2Id, null /* reason_cid */);
  const refundResult = await ctx.send(refundTx, bob, 'Bob 申请退款', 'bob');
  assertTxSuccess(refundResult, '申请退款');

  const approveRefundTx = (api.tx as any).entityOrder.approveRefund(order2Id);
  ...
  await ctx.check('验证退款事件', 'system', () => {
    assertEventEmitted(approveRefundResult, 'entityOrder', 'RefundApproved', '应有 RefundApproved');
  });

...
async function tokenGovernance(ctx: FlowContext): Promise<void> {
  ...
  await ctx.check('验证 Bob 收到代币', 'bob', async () => {
    const balance = await (api.query as any).entityToken.tokenBalances(entityId, bob.address);
  ...
  const overMintTx = (api.tx as any).entityToken.mintTokens(
    entityId, eve.address, 999_999_999,
  );
  ...
  const configGovTx = (api.tx as any).entityGovernance.configureGovernance(
    ...
    null,        // votingPeriod
    null,        // executionDelay
    null,        // quorumThreshold
    null,        // passThreshold
    null,        // proposalThreshold
    null,        // adminVetoEnabled
  );
  ...
  const createProposalTx = (api.tx as any).entityGovernance.createProposal(
    entityId,
    'General',                     // proposalType
    'E2E Test Proposal',           // title
    'QmProposalDescCid001',        // descriptionCid
  );
  ...
  const charlieProposalTx = (api.tx as any).entityGovernance.createProposal(
    entityId, 'General', 'Charlie Proposal', 'QmDesc',
  );
  ...
  if (proposalId !== null) {
    ...
    // 结束投票
    const finalizeTx = (api.tx as any).entityGovernance.finalizeVoting(proposalId);
    ...
    if (finalizeResult.success) {
      ...
      // 执行提案
      const executeTx = (api.tx as any).entityGovernance.executeProposal(proposalId);
  ...
  const setRestrictionTx = (api.tx as any).entityToken.setTransferRestriction(entityId, 'Whitelist', 0);
  const restrictResult = await ctx.send(setRestrictionTx, eve, '设置白名单转账限制', 'eve');

  if (restrictResult.success) {
    // 添加 Bob 到白名单
    const addWhitelistTx = (api.tx as any).entityToken.addToWhitelist(entityId, [bob.address]);
    const whitelistResult = await ctx.send(addWhitelistTx, eve, '添加 Bob 到白名单', 'eve');
    assertTxSuccess(whitelistResult, '添加白名单');

    // Bob 转账应成功
    const bobTransferTx = (api.tx as any).entityToken.transferTokens(entityId, eve.address, 100);
    const bobTxResult = await ctx.send(bobTransferTx, bob, 'Bob 转账(白名单内)', 'bob');
    assertTxSuccess(bobTxResult, 'Bob 转账');

    // Charlie 不在白名单, 转账应失败
    const charlieTransferTx = (api.tx as any).entityToken.transferTokens(entityId, eve.address, 100);
    ...
    // 恢复无限制模式
    const resetTx = (api.tx as any).entityToken.setTransferRestriction(entityId, 'None', 0);
  ...
  const lockTx = (api.tx as any).entityToken.lockTokens(
    entityId,
    5_000,
    5,               // lockDuration: 5 blocks
  );
  const lockResult = await ctx.send(lockTx, eve, '锁仓 Bob 的代币', 'eve');
  if (lockResult.success) {
    // 立即解锁应失败 (未到期)
    const earlyUnlockTx = (api.tx as any).entityToken.unlockTokens(entityId);
    ...
    const unlockTx = (api.tx as any).entityToken.unlockTokens(entityId);
